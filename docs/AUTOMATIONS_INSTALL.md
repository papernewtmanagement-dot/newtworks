...
|---|---|---|---|---|---|
| 1 | **SF Daily Comp Processor** | 10:00 AM CDT daily | GMAIL_FETCH_EMAILS + Groq | income | Pulls SF daily comp emails, parses line items via Groq, writes to `comp_recap`. Primary daily income feed. |
| 2 | **Deduction Statement Processor** | every 6 hours | GMAIL_FETCH_EMAILS + Groq | Documents | Parses SF deduction statements, writes deductions (negative amounts) to `comp_recap` and `journal_entries`. |
| 3 | **Bank Statement Processor** | every 6 hours | GMAIL_FETCH_EMAILS + Groq | Documents | Parses bank statement emails, posts to `journal_entries`. Dedups via reference_number. |
| 4 | **Credit Card Statement Processor** | every 6 hours | GMAIL_FETCH_EMAILS + Groq | Documents | Parses credit card statements, writes to `credit_transactions`. Pairs with Bank Statement Processor for full cash-basis reconciliation. |
| 5 | **Payroll Processor** | every 6 hours | GMAIL_FETCH_EMAILS + Groq | Documents | Parses payroll provider notifications (Gusto, ADP), writes `payroll_runs` + `payroll_detail`. |
| 6 | **Producer Production Report Processor** | Monthly (1st @ 4 AM CDT / 9 UTC) | GMAIL_FETCH_EMAILS + Groq | hr_people | **Parses agent's monthly producer reports → writes to `producer_production`. THIS feeds the HR & People → Performance tab ROI projection.** |
...
| 9 | **Daily Briefing Email** | 7:00 AM CDT daily | GMAIL_SEND_EMAIL + Groq | Communication | Composes morning briefing via Groq from real data — revenue YTD, AIPP, top tasks, alerts, today's posts. Sends to agent's PERSONAL email (never @statefarm.com). |
