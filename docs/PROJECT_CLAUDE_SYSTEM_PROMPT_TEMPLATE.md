...
> **Note on "+ Groq" below:** LLM parsing runs through `COMPOSIO_SEARCH_GROQ_CHAT`, the Composio-hosted Groq endpoint. It authenticates with your existing `composio_api_key`. You do NOT need a separate Groq API key — never ask the agent for one.

| Recipe | Schedule | Composio Tool | What It Does |
|---|---|---|---|
| SF Daily Comp Processor | 10:00 AM CDT daily | GMAIL_FETCH_EMAILS + Groq | Parses SF daily comp emails → comp_recap |
| Deduction Statement Processor | every 6 hours | GMAIL_FETCH_EMAILS + Groq | Parses SF deduction statements → comp_recap (negative) + journal_entries |
| Bank Statement Processor | every 6 hours | GMAIL_FETCH_EMAILS + Groq | Parses bank statements → journal_entries |
| Credit Card Statement Processor | every 6 hours | GMAIL_FETCH_EMAILS + Groq | Parses credit card statements → credit_transactions |
| Payroll Processor | every 6 hours | GMAIL_FETCH_EMAILS + Groq | Parses payroll provider notifications → payroll_runs + payroll_detail |
| Producer Production Report Processor | Monthly (1st @ 9 UTC) | GMAIL_FETCH_EMAILS + Groq | Parses monthly producer reports → producer_production. **Feeds the Performance tab.** |
...
| Daily Briefing Email | 7:00 AM CDT daily | GMAIL_SEND_EMAIL + Groq | Composes morning briefing from real data, sends to [AGENT_PERSONAL_EMAIL] |
