// =========================================================================
// automation-runner  (Newtworks Master Template)
// =========================================================================
...
//     (b) manual call from the Automations module in the Newtworks web app via
...
async function composeDailyBriefing(agencyId: string, recipientEmail: string): Promise<any> {
  ...
  // --- Subject ---
  const subject = `Newtworks Daily Briefing — ${weekdayName.slice(0,3)}, ${dateLabel} — MTD ${money(mtdRevenue)}, pacing ${money(eomProjection)}`;
...
— Claude`;
  ...
  html += `<p style="color:#aaa; font-size: 11px;">Generated by Paper Newt Newtworks — ${recipientEmail}</p>`;
  html += `</body></html>`;
    ...
    if (recipe.internal_handler === 'daily_briefing_composer') {
      try {
        ...
        await sb.from('briefings').upsert({
          ...
          subject: recipe.input_config?.subject || 'Newtworks Daily Briefing (fallback)',
