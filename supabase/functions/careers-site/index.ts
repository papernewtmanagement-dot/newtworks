...
function formatSalary(min: number | null, max: number | null, period: string | null): string {
  if (!min && !max) return "";
  const fmt = (n: number) => period === "hour" ? `$${n}` : `$${n.toLocaleString()}`;
...
function locationLabel(mode: string, city: string | null, state: string | null): string {
...
async function renderListing(): Promise<Response> {
...
async function renderJobDetail(slug: string): Promise<Response> {
  ...
  const orderedQuestions = posting.screener_codes
    .map((c: string) => questionMap.get(c))
    .filter(Boolean);

  const screenerFields = orderedQuestions.map((q: any) => {
  ...
  if (!knockoutReason && email) {
    const { data: cand } = await supabase
      ...
      .insert({
        ...
        job_posting_id: posting.id,
        ingestion_metadata: { source: "careers_page", job_application_id: appRow.id, screener_answers: screenerAnswers },
      })
      .select("id")
      .single();
...
Deno.serve(async (req) => {
  ...
  // The incoming path can arrive in three shapes depending on how we were called:
  //   /functions/v1/careers-site/...  (full Supabase path)
  //   /careers-site/...              (Supabase edge runtime, slug only)
  //   /careers/...                   (Vercel rewrite passthrough)
  // Each prefix is stripped only at a path boundary. Without the boundary check,
  // the "/careers" rule eats the front of "/careers-site" and leaves "-site",
  // which matches no route and 404s every request.
