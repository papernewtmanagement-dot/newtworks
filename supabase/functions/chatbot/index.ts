...
//     instead of the legacy blanket is_excluded (which now governs pjsagencybot only).
//     Allows admin_backoffice folks (Marie) excluded from pjsagencybot to still
//     use paper_newt_bot in the Paper Newt Management group.
//
// v3 (2026-06-17):
  ...
  try {
    ...
    if (name === "search_knowledge") {
      ...
      const [pm, cp, hb, pb] = await Promise.all([
        ...
        sb.from("manuals").select("title, content").eq("agency_id", AGENCY_ID)
          .eq("manual_type", "handbook").eq("is_active", true)
          .or(`title.ilike.${like},content.ilike.${like}`).limit(maxPer),
        sb.from("manuals").select("title, content").eq("agency_id", AGENCY_ID)
          .eq("manual_type", "processes").eq("is_active", true)
          .or(`title.ilike.${like},content.ilike.${like}`).limit(maxPer),
      ]);
