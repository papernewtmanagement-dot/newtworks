...
import { retryLLMCall } from "./lib/llm.ts";
import {
  classifyDocument,
  classifyBankTxn,
...
// ---- retry_queued mode -----------------------------------------------------
// Drains llm_parse_queue rows whose inline Groq call previously failed and
// left the corresponding documents row parked at processing_status='queued_for_llm'.
// Only the parse_bank_statement purpose is wired to a writer in this build;
// other purposes get their last_error stamped so the failure surfaces, but
// stay pending. Every attempt (success or failure) advances attempts++ and
// last_attempt_at so we can track progress without silent orphans.

interface RetryQueuedResult {
  queue_id: string;
  document_id: string | null;
  purpose: string;
  status: "processed" | "groq_failed" | "bad_json_shape"
        | "doc_missing" | "missing_source_account"
        | "unsupported_purpose" | "would_retry";
  file_name?: string;
  je_count?: number;
...
async function processRetryQueuedMode(
  ctx: RunCtx,
  ...
  processed: number;
  errored: number;
  results: RetryQueuedResult[];
  note?: string;
}> {
  const { data: queueRows, error: qErr } = await sb
    .from("llm_parse_queue")
    .select("id, document_id, purpose, system_prompt, user_content, model, attempts")
    .eq("agency_id", ctx.agencyId)
    .eq("status", "pending")
  ...
  if (qErr) {
    return { evaluated: 0, processed: 0, errored: 0, results: [], note: `queue fetch failed: ${qErr.message}` };
  }
  if (!queueRows || queueRows.length === 0) {
    return { evaluated: 0, processed: 0, errored: 0, results: [], note: "no pending llm_parse_queue rows with document_id" };
  }

  const nowIso = () => new Date().toISOString();
  const results: RetryQueuedResult[] = [];

  for (const row of queueRows) {
    // Dry-run: report what we WOULD do without touching Groq or the DB.
    if (opts.dryRun) {
      results.push({
        queue_id: row.id,
    ...
    // Only bank statements have a wired-up writer path here.
    if (row.purpose !== "parse_bank_statement") {
      await sb.from("llm_parse_queue")
        .update({
          attempts: (row.attempts ?? 0) + 1,
          last_attempt_at: nowIso(),
          last_error: `purpose "${row.purpose}" not yet supported by retry_queued mode`,
        })
        .eq("id", row.id);
      results.push({
        queue_id: row.id, document_id: row.document_id, purpose: row.purpose,
        status: "unsupported_purpose",
      });
      continue;
    }

    // Load the document so we know where to post (source_account_code) and
    // can update its processing_status when done.
    const { data: doc, error: docErr } = await sb.from("documents")
      .select("id, file_name, source_account_code")
      .eq("agency_id", ctx.agencyId)
    ...
    if (docErr || !doc) {
      await sb.from("llm_parse_queue")
        .update({
          attempts: (row.attempts ?? 0) + 1,
          last_attempt_at: nowIso(),
          last_error: `document lookup failed: ${docErr?.message ?? "not found"}`,
        })
        .eq("id", row.id);
      results.push({
        queue_id: row.id, document_id: row.document_id, purpose: row.purpose,
        status: "doc_missing",
        error: docErr?.message ?? "not found",
      });
      continue;
    }
    if (!doc.source_account_code) {
      await sb.from("llm_parse_queue")
        .update({
          attempts: (row.attempts ?? 0) + 1,
          last_attempt_at: nowIso(),
          last_error: "document.source_account_code is null — cannot post JEs",
        })
        .eq("id", row.id);
      results.push({
        queue_id: row.id, document_id: row.document_id, purpose: row.purpose,
        file_name: doc.file_name, status: "missing_source_account",
      });
      continue;
    }

    // Re-call Groq. retryLLMCall does NOT re-insert into the queue on
    // failure — the row already exists; we just stamp last_error.
    const llm = await retryLLMCall({
      agencyId: ctx.agencyId,
    ...
    if (!llm.ok) {
      await sb.from("llm_parse_queue")
        ...
        .eq("id", row.id);
      results.push({
        queue_id: row.id, document_id: doc.id, purpose: row.purpose,
        file_name: doc.file_name, status: "groq_failed", error: llm.error,
      });
      continue;
    }

    // Shape-check the JSON payload before touching the GL.
    const json = llm.json;
    ...
    if (!period?.start || !period?.end) {
      await sb.from("llm_parse_queue")
        .update({
          attempts: (row.attempts ?? 0) + 1,
          last_attempt_at: nowIso(),
          last_error: "LLM response missing statement_period.start or .end",
        })
        .eq("id", row.id);
      results.push({
        queue_id: row.id, document_id: doc.id, purpose: row.purpose,
        file_name: doc.file_name, status: "bad_json_shape",
      });
      continue;
    }

    // Statement balance write (idempotent-by-natural-key).
    const balWrite = await writeStatementBalance({
      agencyId: ctx.agencyId,
      documentId: doc.id,
    ...
    if (!balWrite.ok) {
      console.warn(`[retry_queued] statement_balance write failed doc=${doc.id}: ${balWrite.error}`);
    }

    // Post the transactions. Same shape as handleBankStatement's inline loop.
    resetReferenceCounters();
    let jeCount = 0;
    let suspenseCount = 0;
    ...
    for (const t of rawTxns) {
      ...
      const txn = {
        ...
        sourceAccountCode: doc.source_account_code,
      };
      const classification = await classifyBankTxn(ctx.agencyId, txn);
      const post = await postJournalEntry({
        agencyId: ctx.agencyId,
        txn,
        txnDate: String(t.date),
        classification,
    ...
    // Mark queue completed with the parsed payload retained for audit.
    await sb.from("llm_parse_queue")
      .update({
        status: "completed",
        completed_at: nowIso(),
    ...
    await markDocument(doc.id, "processed", jeCount,
      ["journal_entries", "journal_lines", "statement_balances"],
      `retry_queued: ${jeCount} JEs posted, ${suspenseCount} in suspense`);

    results.push({
      queue_id: row.id, document_id: doc.id, purpose: row.purpose,
      file_name: doc.file_name, status: "processed",
      je_count: jeCount, suspense_count: suspenseCount,
    });
  }

  return {
    evaluated: queueRows.length,
    processed: results.filter((r) => r.status === "processed").length,
    errored: results.filter((r) => r.status !== "processed" && r.status !== "would_retry").length,
    results,
  };
}
...
async function run(req: Request): Promise<Response> {
  ...
  if (mode === "retry_queued") {
    // retry_queued (2026-07-28). Drains llm_parse_queue rows that have stalled
    // because the inline Groq call inside parseWithLLM failed (typically 429
    // rate-limit) and the row was inserted with attempts=0, last_error=null.
    // The default attachments path never re-visits existing documents rows, so
    // without this mode `queued_for_llm` is a terminal state.
    //
    // Body: { dry_run?: boolean, limit?: number (default 5) }
    // Only supports purpose="parse_bank_statement" in this build; other
    // purposes are marked with last_error so they surface but stay pending.
    const rqCtx = { agencyId, composioApiKey, composioUserId, gmailAccountId, driveAccountId };
    const startedAt = new Date().toISOString();
    const dryRun = body?.dry_run === true;
    const limit = typeof body?.limit === "number" && body.limit > 0 ? body.limit : 5;
    const result = await processRetryQueuedMode(rqCtx, { dryRun, limit });
    return jsonResponse({ ok: true, mode: "retry_queued", started_at: startedAt, finished_at: new Date().toISOString(), ...result });
  }
