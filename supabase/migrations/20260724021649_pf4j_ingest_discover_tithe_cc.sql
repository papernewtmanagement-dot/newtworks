...
-- 3. Post JEs:
--    - Charges (all tithe/charitable per Peter): DR Tithe & Charitable, CR Discover CC
--    - Payments received (INTERNET PAYMENT - THANK YOU): DR Discover CC, CR Internal Transfers
-- 4. Reclassify existing 3 bank Discover withdrawals from *Unclassified to Internal Transfers
-- 5. Seed classification rules for tithe patterns + Amazon-on-Discover -> Tithe
-- =====================================================================
...
BEGIN
  ...
  -- Cache other COAs needed for JEs
  ...
  VALUES
    ...
    -- Statement 26-05 (Mar 28 - Apr 27, 2026)
    ...
    -- Statement 26-06 (May 28 - Jun 27, 2026)
  ...
  -- 4. Create JEs for each Discover txn
  FOR v_txn IN
  ...
  LOOP
    ...
    IF v_txn.amount < 0 THEN
      -- Payment received: DR CC (reduces liability), CR Internal Transfers (mirrors bank withdrawal)
    ...
    ELSE
      -- Charge (all Tithe & Charitable per Peter — including Amazon which = Vault donations)
      INSERT INTO public.journal_entries
        (agency_id, business_entity_id, entry_date, entry_type, description, source, classification_status, created_by, memo)
      VALUES
        (v_agency_id, v_pers_id, v_txn.transaction_date, 'personal_credit',
  ...
  -- 5. Reclassify existing 3 bank Discover withdrawals: DR *Unclassified -> DR Internal Transfers
  UPDATE public.journal_lines
   ...
   WHERE journal_entry_id IN (
     ...
     WHERE bt.business_entity_id = v_pers_id
       AND bt.description ILIKE '%DISCOVER%'
   )
...
-- 6. Seed classification rules (tithe patterns + Discover-scoped Amazon rule)
...
VALUES
  -- Tithe recipients (any source, but practically only on Discover)
  ...
  -- Amazon on Discover ONLY -> Tithe & Charitable (Vault donation items). Higher priority than global Amazon rule.
