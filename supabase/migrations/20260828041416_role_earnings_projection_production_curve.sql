-- Add a production curve to the earning-potential projection.
-- The chart on Team > Earning Potential now plots pay against production
-- (weekly sales points; annual life premium for the Life Specialist seat)
-- instead of years of employment. The function keeps every existing output
-- key untouched (the year grid still feeds the year-by-year table) and adds
-- one 'curve' object per role:
--   curve: { x_kind, x_label, x_max, entry_base, bands[], points[] }
--   bands:  one per tier — from_x is that tier's steady production level
--           (role weekly target x tier multiplier; for Life, steady annual
--           premium x multiplier), base_annual is the year-five ladder row
--           (the pay band a sustained performer at that tier reaches),
--           goals_weekly is the tier's weekly goals bonus.
--   points: sampled (x, base, total) pairs, dense enough to draw both lines
--           with crisp steps at each band boundary. Total = base + commission
--           + bonus-pool share + goals bonus, at today's pool rates, steady
--           production pace (year-three ramp).
-- Runtime computation only, per core_principles compensation_data_freshness
-- (650) — nothing stored, everything recomputed on read.
...
-- production curve per role (see migration header above).
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- ── Production curve for this role ─────────────────────────────
      -- Steady-state pace (year-three ramp, 1.00). Same stored inputs as
      -- the year grid: tier multipliers set the band boundaries, the
      -- year-five ladder row sets the base a sustained performer at that
      -- tier reaches, and today's pool rates price the bonus share.
      IF r_role.role_key = 'life_specialist' THEN
      ...
      -- Sample grid: 120 uniform points plus a pair hugging each band
      -- boundary so both lines step crisply where a raise lands.
      v_xs := ARRAY[]::numeric[];
      FOR i IN 0..120 LOOP
        v_xs := v_xs || round(v_x_max * i / 120.0, 2);
      END LOOP;
      FOR v_b IN SELECT (b->>'from_x')::numeric AS fx FROM jsonb_array_elements(v_bands) b LOOP
        IF v_b.fx > 0 AND v_b.fx <= v_x_max THEN
          v_xs := v_xs || GREATEST(round(v_b.fx - v_x_max * 0.0005, 3), 0) || v_b.fx;
        END IF;
      END LOOP;
      SELECT array_agg(DISTINCT u ORDER BY u) INTO v_xs FROM unnest(v_xs) u;
  ...
  RETURN jsonb_build_object(
    ...
    'assumptions', jsonb_build_object(
      ...
      'note', 'One sales point equals one dollar of commission. Bonus pool share '
           ...
           || '2012), not an even spread. The chart holds a steady production pace '
           || '(the year-three ramp); years one and two typically run lower — see '
