-- Health Bonus: $25 if team hits 5 health days any time Sun-Sat (agency week).
-- Replaces prior Mon-Fri-only logic. Counts either:
--   (a) hit_today=true days anywhere in the week, OR
--   (b) max week_total_override reported during the week (running total)
-- whichever is larger.
...
BEGIN
  ...
  WITH base AS (
    SELECT wctd.team_member_id,
      ...
      -- HEALTH DAYS: any 5 days Sun-Sat. Max(daily-hit count, max running total reported).
      GREATEST(
