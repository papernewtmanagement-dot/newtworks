...
LANGUAGE sql STABLE AS $function$ SELECT public._newtworks_role_fit_core(p_candidate, 'sales_outbound'); $function$;

CREATE OR REPLACE FUNCTION public.newtworks_role_fit_sales_inbound(p_candidate hiring_candidates) RETURNS jsonb
LANGUAGE sql STABLE AS $function$ SELECT public._newtworks_role_fit_core(p_candidate, 'sales_inbound'); $function$;

CREATE OR REPLACE FUNCTION public.newtworks_role_fit_sales_in_book(p_candidate hiring_candidates) RETURNS jsonb
LANGUAGE sql STABLE AS $function$ SELECT public._newtworks_role_fit_core(p_candidate, 'sales_in_book'); $function$;

CREATE OR REPLACE FUNCTION public.newtworks_role_fit_retention_escalation(p_candidate hiring_candidates) RETURNS jsonb
LANGUAGE sql STABLE AS $function$ SELECT public._newtworks_role_fit_core(p_candidate, 'retention_escalation'); $function$;

CREATE OR REPLACE FUNCTION public.newtworks_role_fit_retention_reception(p_candidate hiring_candidates) RETURNS jsonb
LANGUAGE sql STABLE AS $function$ SELECT public._newtworks_role_fit_core(p_candidate, 'retention_reception'); $function$;

CREATE OR REPLACE FUNCTION public.newtworks_role_fit_retention_support(p_candidate hiring_candidates) RETURNS jsonb
LANGUAGE sql STABLE AS $function$ SELECT public._newtworks_role_fit_core(p_candidate, 'retention_support'); $function$;

CREATE OR REPLACE FUNCTION public.newtworks_role_fit_aspirant(p_candidate hiring_candidates) RETURNS jsonb
LANGUAGE sql STABLE AS $function$ SELECT public._newtworks_role_fit_core(p_candidate, 'aspirant'); $function$;
