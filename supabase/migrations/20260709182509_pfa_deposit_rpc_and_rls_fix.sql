-- ============================================================================
-- PFA team-facing deposit RPC + RLS auth pattern fix
-- ============================================================================
-- Fix (auth pattern): u.id = auth.uid() → u.auth_user_id = auth.uid()
--   The earlier pfa_admin_only_* policies used the older/incorrect pattern.
--   users.id and auth.uid() are different uuids; the correct link is
--   users.auth_user_id = auth.uid(). Confirmed against the marketing_ideas,
--   tasks, and sales_points_band_config policies which use this pattern.

-- Fix admin-only RLS policies to use the correct auth pattern
...
-- ============================================================================
-- pfa_record_customer_deposit RPC — team-facing deposit entry
-- SECURITY DEFINER: bypasses RLS so team members (who don't have direct
-- table access) can insert their own deposit rows without exposing the ledger.
-- Enforces:
--   * Masked customer_name format: "First L." only
--   * policy_type from fixed list (auto | fire | life | health | billing)
--   * Amount > 0, sane upper bound
--   * Caller must be an active, non-backoffice team member
-- Fires Telegram DM to Peter via paper_newt_bot on success (fire-and-forget).
-- ============================================================================
...
BEGIN
  ...
  -- Resolve caller to an active team member
  ...
  -- Validate last_initial — exactly one letter (accept lowercase, uppercase it)
  ...
  -- Build masked customer name — "First L."
  v_customer_name := v_first_normalized || ' ' || v_last_initial_normalized || '.';

  -- Look up PFA account for this agency
  ...
  -- Insert paired ledger rows (Deposit + matching State Farm EFT).
  -- Both dated today, both uncleared (bank statement will clear them later).
  ...
  -- Fire Telegram DM to Peter (paper_newt_bot). Fire-and-forget — don't fail
  -- the deposit if the DM errors out.
  ...
  IF v_chat_id IS NOT NULL THEN
    ...
    BEGIN
      PERFORM public.telegram_send_message_v2(
        p_chat_id => v_chat_id,
        p_text    => v_msg,
        p_bot     => 'paper_newt'
      );
...
-- Lock down execution: authenticated users only. RLS is bypassed by SECURITY DEFINER;
-- the function does its own team-member check.
...
COMMENT ON FUNCTION public.pfa_record_customer_deposit IS
  'Team-facing PFA deposit entry. Inserts paired Deposit + State Farm EFT rows with masked customer name (First L.) and policy_type. Sends Telegram DM to Peter via paper_newt_bot on success. SECURITY DEFINER — team members have no direct table access.';
