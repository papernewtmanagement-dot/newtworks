-- Patch payroll_gl_writer to catch "unrecognized items" gap for legacy raw_earnings
-- key schemas (2026-07-04 had REGULAR/2Serve/3True/5Goals/etc keys pre-normalization).
-- Any gross_pay dollars not matched to SALARY/HOURLY/OT/BONUS/COMMISSION/OTHER/REIMBURSEMENTS
-- get bucketed into team_share as unclassified — conservative default (doesn't dilute Growth Budget).
  ...
  LOOP
    ...
    LOOP
      ...
      IF v_route_papernewt THEN
        -- Full paycheck (gross + ER taxes) to PN, regardless of item breakdown
      ...
      ELSE
        ...
        -- Gap = unrecognized items (legacy key schemas). Book to team_share; don't dilute Growth Budget.
