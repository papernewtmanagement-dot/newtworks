...
-- This migration was applied live on 2026-07-29 16:56:38 UTC (ledger name: reclassify_pending_je_v3_skip_unusable_rules) but was never
-- committed to this repo at the time. Content below is copied verbatim
-- from supabase_migrations.schema_migrations.statements for version 20260729165638.
-- No live change is made by adding this file — it only brings the repo
-- mirror into sync with what is already running in production, so a
-- fresh `supabase db reset` from this repo reproduces live state.
-- Patch: skip rules whose target for THIS direction is __SOURCE__ or COA-SUSP,
-- so lower-priority rules with real targets get a chance.
...
BEGIN
  ...
  LOOP
    ...
    -- Only consider rules whose target for THIS direction is a REAL account code
    -- (not __SOURCE__, not COA-SUSP, not NULL)
    SELECT r.id,
    ...
    WHERE r.agency_id = p_agency_id AND r.is_active = TRUE
      ...
      -- Must have usable target on this direction's side
