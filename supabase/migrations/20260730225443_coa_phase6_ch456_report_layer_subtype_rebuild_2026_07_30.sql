...
WITH leaf_balances AS (
  SELECT
    v.agency_id, v.account_id, v.account_code, v.account_name, v.account_type,
    v.account_subtype, v.section_label AS rollup_section,
    v.source_bucket, v.month_start, SUM(v.net_balance) AS period_balance
...
SELECT
  agency_id,
  rollup_section AS parent_account,
  account_type, source_bucket, month_start,
...
AS $function$
  WITH src AS (
    SELECT v.year::int AS year, v.month::int AS month, v.account_name,
      v.account_type::text AS account_type,
      INITCAP(REPLACE(COALESCE(v.account_subtype, v.account_type), '_', ' ')) AS section,
      v.amount
    FROM public.v_income_statement v WHERE v.account_id IS NOT NULL
    UNION ALL
    SELECT v.year::int, v.month::int, v.account_name, v.account_type::text,
      COALESCE(v.account_subtype, 'Uncategorized') AS section, v.amount
    FROM public.v_income_statement v WHERE v.account_id IS NULL
  )
...
AS $function$
  WITH journal_side AS (
    SELECT 'journal'::text AS source, je.id AS je_id, jl.id AS line_id, NULL::uuid AS pyp_id, je.entry_date,
      ...
      coa.id AS account_id, coa.account_code, coa.account_name, je.document_id, je.created_at
    FROM public.journal_lines jl
    JOIN public.journal_entries je   ON je.id  = jl.journal_entry_id
    JOIN public.chart_of_accounts coa ON coa.id = jl.account_id
...
BEGIN
  ...
  SELECT COALESCE(SUM(jl.debit - jl.credit), 0) INTO v_spend_qtd
