-- Amendment 3: opening_balance falls back to account_starting_balances anchor when
-- no prior-week running balance exists. Only an account with NO anchor row at all
-- gets skipped (all its weeks), loudly disclosed in both the email and the jsonb
-- return -- everything else still writes and sends. All other Amendment 1-2 stop
-- rules (registry resolution, send failure) are unchanged hard-aborts.
...
BEGIN
  ...
  -- (unchanged hard-abort per Amendment 1)
  ...
  -- pre-flight B: identify accounts with NO usable opening_balance basis at all
  -- (no prior-week running balance for ANY of their weeks AND no anchor row).
  -- Amendment 3: this is now the ONLY per-account skip condition; everything else writes.
  FOR v_agg IN
  ...
  -- resolve display names for skipped accounts + build disclosure
