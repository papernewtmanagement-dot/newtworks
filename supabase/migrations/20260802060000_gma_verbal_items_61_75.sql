...
--
-- Format: classic verbal analogy (A is to B as C is to ?), a long-
-- established verbal-reasoning marker distinct from raw vocabulary recall --
-- candidate must infer the RELATIONSHIP in the first pair and apply it to
-- the second (Sternberg 1977, Intelligence, Information Processing, and
-- Analogical Reasoning: componential analysis of analogical reasoning).
--
-- Unlike the other 3 GMA subtests, this content can't be verified by a
-- formal checker (word meaning isn't provable the way arithmetic or logic
-- is). Compensating discipline: every item is built so exactly one option
-- matches the stated relationship, and each was reviewed to rule out a
-- second defensible answer -- e.g. a wheel:car::wing:? item was rejected
-- during drafting because both "bird" and "airplane" genuinely have wings,
-- creating real ambiguity; swapped to sail:boat instead, where no second
-- vehicle-type plausibly carries a sail as standard equipment.
--
-- Tier distribution: tier1=4 (61-64, synonym/antonym -- most familiar
-- relationship types), tier2=4 (65-68, category/part-to-whole), tier3=3
-- (69-71, function/worker-to-workplace), tier4=4 (72-75, cause-effect/
-- degree -- relationship must be inferred, not just recalled).
--
