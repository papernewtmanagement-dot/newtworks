-- Add time_off_hours to get_weekly_cpr_hours so the CPR Hours section can show two
-- separate totals: hours worked and hours of time off.
--
-- DESIGN RECORD (2026-09-08, Peter directive): "the CPR hours totals should show
-- total hours worked and total hours time off. and make sure that the paid time off
-- is not included in the retention split formula."
--
-- TWO TIME-OFF COLUMNS, on purpose:
--   time_off_hours      — ALL approved time off for the day, paid or unpaid. Drives
--                         the new "Time Off" total column on the CPR.
--   paid_time_off_hours — the paid subset. Drives the paid/unpaid marker on the cell
--                         and answers pay questions. Added 2026-09-08 earlier.
-- Without the all-time-off figure an UNPAID absence would silently shrink the worked
-- number with nothing on the row explaining it — the same visibility hole that
-- started this whole thread, one category over.
--
-- RETENTION SPLIT IS UNAFFECTED, and must stay that way. The split runs off
-- compute_weekly_retention_points, which reads ONLY the `hours` column and only where
-- location = 'in_office'. Neither time_off_hours nor paid_time_off_hours is read by
-- any other function in the database (verified by scanning every function body for
-- both names). Verified live for week ending 2026-09-12: Stephanie 32.00 in-office
-- hours, not 40, with Monday's paid closure excluded. NEVER add either column to that
-- function or to any pay-split input.
--
-- OVERTIME NOTE (unchanged): paid time off is not hours worked and never counts
-- toward the 40-hour overtime line (29 CFR 778.218).
...
CREATE OR REPLACE FUNCTION public.get_weekly_cpr_hours(p_agency_id uuid, p_week_ending_date date)
 RETURNS TABLE(team_member_id uuid, day_idx integer, day_label text, work_date date, hours numeric, time_off_hours numeric, paid_time_off_hours numeric, location text)
 LANGUAGE sql
 SECURITY DEFINER
...
SELECT
  ...
  END AS hours,
  CASE
    -- ALL approved time off for the day, paid or not. This is what the CPR "Time Off"
    -- total column reads. Kept separate from paid_time_off_hours so an unpaid day is
    -- still visible: without it an unpaid absence would just shrink the worked figure
    -- with nothing on the row to explain the gap.
    WHEN at.start_date IS NOT NULL AND wd.work_date < at.start_date THEN 0
    WHEN at.end_date   IS NOT NULL AND wd.work_date > at.end_date   THEN 0
    ELSE COALESCE(toff.hours_off, 0)
  END::numeric AS time_off_hours,
  CASE
