...
BEGIN
  ...
  SELECT c.team_member_id, c.c_weekly_pay, c.c_base_advance, c.c_health_bonus, c.c_service_surge_share,
    -- TPB = sales-pool reconciliation. Subtract only sales-pool components this week
    -- (WP, BA, AP). H/S/M come from other pools (retention budget, scorecard, etc.)
    -- and are not drained from the sales pool. HSM_prior compensates for the H+S+M
    -- dollars sitting inside qtd_paid that didn't come from the sales pool.
    CASE WHEN c.this_week_payroll_ytd IS NULL OR c.prior_qtr_end_payroll_ytd IS NULL THEN NULL
         ELSE GREATEST(0,
           ...
           - c.qtd_paid
           - c.c_weekly_pay - c.c_base_advance - c.c_agency_profit_share)
    ...
    jsonb_build_object(
      ...
      'this_week_sales_pool_components',
        c.c_weekly_pay + c.c_base_advance + c.c_agency_profit_share,
