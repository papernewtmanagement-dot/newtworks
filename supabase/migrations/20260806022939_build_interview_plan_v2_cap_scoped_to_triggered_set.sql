-- v2: subconstruct cap (max 2 per construct::subconstruct) applies to the TRIGGERED
-- set only, per spec. v1 counted universal_core rows into the cap counter, which
-- pre-capped commitment::motivation_level (2 core rows) and made all 4 triggered
-- questions on that subconstruct permanently unplaceable — killing coverage for
-- T_CHURN_RISK, T_LOW_ACHIEVEMENT, T_RES_FOLLOW_THROUGH_LOW, T_GAP_COMMITMENT.
...
BEGIN
  ...
  -- Step 5: triggered questions (cap: max 2 per construct::subconstruct within this set)
    ...
    FOREACH v_code IN ARRAY v_triggers LOOP
      ...
      LOOP
        ...
        IF v_current_count >= 2 THEN
          CONTINUE; -- subconstruct cap already hit within the triggered set
        END IF;
      ...
      IF NOT v_any_placed THEN
        -- either no matching row, or budget/cap ran out for every candidate row on this code
        IF NOT EXISTS (
