-- Change Manager Base Advance divisor from constant 13 to weeks ELAPSED in the current cycle.
-- Surgical patch on compute_weekly_pay — only the manager_baseline computation changes.
...
BEGIN
  ...
  -- Weeks elapsed in the current cycle, inclusive of this week. Floor-divide days by 7, +1.
  ...
  -- Manager baseline = team_total_sp / required_AM_count / weeks_ELAPSED
  v_manager_baseline := CASE WHEN v_required_count > 0
    THEN v_team_total_sp / v_required_count::numeric / v_weeks_elapsed::numeric
    ELSE 0
  END;
