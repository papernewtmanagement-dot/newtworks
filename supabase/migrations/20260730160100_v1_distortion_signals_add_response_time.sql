...
CREATE OR REPLACE FUNCTION public.compute_newtworks_v1_distortion_signals(p_candidate_id uuid, p_stint integer DEFAULT NULL::integer, p_sitting integer DEFAULT NULL::integer)
 RETURNS TABLE(n_likert_items integer, max_consecutive_run integer, overall_sd numeric, straight_line_flag boolean, acquiescence_mean numeric, acquiescence_bias numeric, acquiescence_flag boolean, n_timed_items integer, mean_response_ms numeric, min_response_ms numeric, straight_through_flag boolean)
 LANGUAGE sql
 STABLE
AS $function$
  WITH likert AS (
...
CREATE OR REPLACE FUNCTION public.compute_newtworks_v1_traits_as_row(p_candidate_id uuid, p_stint integer DEFAULT NULL::integer, p_sitting integer DEFAULT NULL::integer)
 RETURNS TABLE(candidate_id uuid, assertiveness integer, independent_spirit integer, compassion integer, belief_in_others integer, optimism integer, analytical integer, deadline_motivation integer, self_promotion integer, recognition_drive integer, overall_score integer, n_items_scored integer, cognitive_score integer, cognitive_n integer, impression_mgmt_score integer, impression_mgmt_n integer, nonsense_inflation integer, nonsense_n integer, retest_divergence numeric, retest_n_pairs integer, expansion_triggers jsonb, reliability_by_trait jsonb, distortion_n_likert_items integer, distortion_max_consecutive_run integer, distortion_overall_sd numeric, distortion_straight_line_flag boolean, distortion_acquiescence_mean numeric, distortion_acquiescence_bias numeric, distortion_acquiescence_flag boolean, distortion_n_timed_items integer, distortion_mean_response_ms numeric, distortion_min_response_ms numeric, distortion_straight_through_flag boolean)
 LANGUAGE plpgsql
 STABLE
AS $function$
DECLARE
