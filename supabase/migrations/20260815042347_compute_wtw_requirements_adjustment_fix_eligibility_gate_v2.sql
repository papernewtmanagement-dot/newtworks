...
BEGIN
  ...
  -- Individual buy-back (step 1) is computed canonically inside get_weekly_cpr_requirements
  ...
  -- here; just read it, sum it, and charge each person $10/quote.
  WITH reqs AS (
  ...
  -- Eligibility gate for step 2 (team pool): same team-level condition
  -- get_weekly_cpr_requirements used to decide whether to fold in step 1 at all — enough raw
  -- quotes AND enough sales points to have been on track to win before requirements touched
  -- anything. v_team_net above is already net of step 1, so this is exactly what's still owed.
  v_raw_pass := v_team_raw >= v_team_requirement;
  v_sp_pass  := v_sp_qtd   >= v_sp_target;
  v_eligible := v_raw_pass AND v_sp_pass;

  v_step2_restored := CASE WHEN v_eligible THEN GREATEST(0, v_team_requirement - v_team_net) ELSE 0 END;
