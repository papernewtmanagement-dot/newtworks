-- Split team-checkin inclusion into separate work + health flags so Peter can
-- be excluded from work checkins (morning/midday/eod) while staying in the
-- evening health checkin.

-- 1) New column. Same default semantics as the existing flag:
--    NULL  -> include if category='agency' AND role!='Owner'
--    true  -> include explicitly
--    false -> exclude explicitly
ALTER TABLE public.team
  ADD COLUMN IF NOT EXISTS include_in_health_checkins boolean;

COMMENT ON COLUMN public.team.include_in_health_checkins IS
  'Inclusion in the evening health checkin (separate from include_in_team_checkins which governs work checkins). NULL defaults to: category=''agency'' AND role!=''Owner''.';

-- 2) Patch team_health_checkin_compile to use the new column instead of include_in_team_checkins
...
BEGIN
  ...
  -- HEALTH inclusion: uses include_in_health_checkins (separate from work flag)
  ...
  WHERE t.agency_id = p_agency_id
    ...
    AND (
      t.include_in_health_checkins = true OR
      (t.include_in_health_checkins IS NULL AND t.category = 'agency' AND t.role != 'Owner')
    );
  ...
  v_text := v_text || E'\nTeam: ' || v_total_hits || '/' || v_total_target || ' workouts';
...
-- 3) Set Peter: out of work checkins, in for health
UPDATE public.team
SET include_in_team_checkins = false,
    include_in_health_checkins = true,
    updated_at = now()
