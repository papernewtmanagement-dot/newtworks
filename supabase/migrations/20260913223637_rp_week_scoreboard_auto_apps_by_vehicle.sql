...
BEGIN
  ...
  prod AS (
    -- Peter 2026-09-13: auto apps count per VEHICLE, not per household. State Farm
    -- writes one auto app per vehicle and the auto tier steps every 6 apps, so
    -- counting households undercharges the P&C rate on ALL premium. auto_units is
    -- what feeds the formula; policy_count still drives fire.
    SELECT s.team_member_id AS tm, p.id, s.id AS sale_id, p.line_of_business AS lob, p.product_type, COALESCE(p.issued_premium, p.premium) AS premium,
           ...
           p.vehicle_count, p.issued_date, s.customer_label, pt.label AS type_label, s.on_file_answer, s.phone_last4
    FROM public.sales_log s
    JOIN public.sales_log_products p ON p.sales_log_id = s.id
  ...
  sp AS (
    SELECT r.id AS tm,
      (SELECT public.compute_sp_from_production(
          COALESCE(SUM(CASE WHEN x.lob = 'auto' THEN x.auto_units END), 0), COALESCE(SUM(CASE WHEN x.lob = 'fire' THEN x.policy_count END), 0),
