-- 2026-08-11, Peter-approved ("Ship it as specced"): EARLY-CAREER ASSESSMENT LANE.
-- Rationale: the resume layer measures accumulated evidence; early-career
-- candidates structurally have almost none, so the 50-gate declined them at
-- the only layer blind to them, before the assessment (the instrument built
-- to measure traits without a track record) could run. Natural experiment:
-- of 7 below-50 candidates who completed the assessment pre-gate, 4 scored
-- role-fit 70+ (one at 83 aspirant fit with a perfect SJT).
-- Lane: a below-threshold resume still receives the attempt-1 invite when
--   (a) resume_analysis->>'early_career' = 'true' (structured flag written
--       by the resume scorer going forward; judgment: total assessable work
--       history under ~36 months and/or education completed within ~4 years),
--   (b) the Character-signal average (honesty, concern_for_others,
--       hard_work_ethic, personal_responsibility, presentation) is >= 40, and
--   (c) integrity_flag is not true.
-- Screen-signal flags have no structured storage today, so (c) is the
-- checkable disqualifier; extend if screen signals gain columns.
-- Measurement stays honest: the resume verdict label is unchanged. This is
-- routing only. COALESCE-to-0 on missing signals fails safe (lane won't fire
-- on malformed scoring records). Character floor 40 hardcoded per Peter
-- approval 2026-08-11; documented in hiregauge_rules
-- "Config: Early-career assessment lane".
...
BEGIN
  ...
  -- attempt 1 but blocked by the resume layer: below the consider threshold
  ...
  WHERE hc.agency_id = p_agency_id
    ...
    AND (
      ...
      OR (
        (hc.resume_analysis->>'avg')::numeric < v_resume_consider_threshold
        AND NOT (
  ...
  -- Eligible when resume score is at consider or better (gate added
  -- 2026-08-06), OR via the early-career lane (added 2026-08-11):
  -- early_career flag + Character-signal average >= 40 + no integrity flag.
  FOR v_cand IN
  ...
  LOOP
    BEGIN
      ...
      -- Bug fix (2026-08-11, Peter-reported): this function never advanced
      -- hiring_candidates.status on send. Nothing else in the codebase does
      -- either — the 130 pre-existing assessment_sent rows were set by
      -- manual staff action, not by this pipeline. Confirmed live: several
      -- candidates scored this same session (Bursh, West, S.Martinez, King)
      -- got real invitations sent minutes ago and were still sitting at
      -- status=applied. Advance status only on the FIRST send (attempt 1);
      -- reminders (attempt 2/3, below) do not touch status since the
      -- candidate is already past 'applied' by then.
  ...
  -- Fix (2026-08-11, Peter-approved): eligibility accepts 'assessment_sent'
  -- in addition to 'applied'. The prior 'applied'-only filter meant (a) the
  -- ~130 manually-advanced candidates never got reminders, and (b) once the
  -- same-day status-advance fix landed, attempt 1 instantly removed every
  -- candidate from reminder eligibility — reminders were unreachable.
  FOR v_cand IN
    SELECT hc.id, hc.first_name, hc.last_name, hc.email, hc.position,
  ...
  LOOP
    BEGIN
      ...
      -- Same status-advance safety net as attempt 1, in case a candidate
      -- somehow reaches a reminder while still sitting at 'applied'
      -- (shouldn't happen post-fix, but cheap insurance against drift).
      UPDATE public.hiring_candidates
      SET status = 'assessment_sent'
      WHERE id = v_cand.id AND status = 'applied';
