-- v2: split hourly hours and locations into separate CTEs to avoid the
-- "subquery uses ungrouped column" error in v1.
...
GRANT EXECUTE ON FUNCTION public.get_weekly_cpr_hours(uuid, date) TO authenticated, anon;

COMMENT ON FUNCTION public.get_weekly_cpr_hours(uuid, date) IS
'Runtime hours/location computation for CPR Hours Worked section. HOURLY: sum time_clock_entries durations per day, location from latest clock-in. SALARIED: 8h/weekday baseline minus approved time-off impact, location from team.work_location. Returns 5 rows per active non-Owner agency teammate (Mon-Fri).';
