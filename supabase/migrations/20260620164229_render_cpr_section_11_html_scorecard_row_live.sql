...
BEGIN
  ...
  v_sc_current := NULLIF(v_sc->>'current', '')::numeric;
  ...
  v_html := v_html
    ...
    || '<td style="padding:6px 10px;text-align:right;border-bottom:1px solid #e5e7eb;color:#94a3b8">'
       ...
       || '</td>'
    || '<td style="padding:6px 10px;text-align:right;border-bottom:1px solid #e5e7eb;color:#334155;font-weight:700">'
       || CASE WHEN v_sc_current IS NULL THEN '—'
               ELSE '$' || to_char(ROUND(v_sc_current), 'FM999,999,999') END
       || '</td>'
    || '<td style="padding:6px 10px;text-align:right;border-bottom:1px solid #e5e7eb;font-weight:700;background:#eff6ff;color:'
