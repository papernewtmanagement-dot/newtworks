-- Migration: 20260727220032_ingest_jun26_us_bank_3977_and_chase_cc_statements.sql
-- Applied via Supabase MCP 2026-07-27 by Claude session (Peter directive).
-- Mirror of the applied SQL for repo parity per op-rule "Newtworks commits — canonical path" (migration_mirror).
--
-- What: Ingest June 2026 statement transactions for two accounts whose
-- documents were catalogued 2026-07-16 but never had their PDFs parsed.
--
--   - US Bank Silver Business Checking 3977 (COA-007, agency b2222222).
--     8 deposits ($54,058.13) + 13 withdrawals ($53,378.59), ending $30,034.83.
--
--   - Chase Ink Business (consolidated statement, cycle 05/23/26 - 06/22/26).
--     Card 7762 (Peter, COA-011): 1 payment -$4,986.04.
--     Card 7770 (Marie, COA-012): 11 charges +$3,923.55.
--
-- Also updates the two documents rows to processing_status='processed',
-- populates records_created, tables_updated, processed_at, and appends
-- a note trail — same fields the document-processor's markDocument() sets.
--
-- FK gotcha for future reference: bank_transactions.bank_account_id points
-- to chart_of_accounts.id (COA-007 UUID), NOT bank_accounts.id. Column name
-- is misleading. credit_transactions.credit_account_id does point to
-- credit_accounts.id as expected.
...
VALUES
-- 7762 (Peter, COA-011 Chase Marketing 1): the payment
...
-- 7770 (Marie, COA-012 Chase Marketing 2): all EverQuote / Butler-Till / Agent Tag marketing spend
...
-- =========================================================================
-- Mark source documents processed
-- =========================================================================
