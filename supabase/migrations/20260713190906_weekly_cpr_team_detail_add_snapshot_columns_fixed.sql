...
-- Backfill from team_weekly_snapshot via a WITH
WITH src AS (
...
-- License flags from live team (stable, not previously snapshotted)
UPDATE public.weekly_cpr_team_detail d
...
-- BEFORE INSERT trigger: on new detail rows, snapshot live team state.
