...
BEGIN
  ...
  -- Team Total / Goal / Result rows. All values from weekly_cpr_reports
  -- columns so the email mirrors what CPRDetail.jsx renders.
  ...
  -- Sum per-person Net Quotes at runtime (= Σ quotes_discussed − paid).
  ...
  -- Team-level Win-the-Week pass/fail + shortfall, computed at view time so
  -- the email matches CPRDetail.jsx exactly (single formula, single source).
  ...
  v_activity_summary_rows :=
    ...
    || '</tr>'
    -- Result — per-column shortfall, centered under Net Quotes and Q Sales Pts
    || '<tr>'
    || '<td style="padding:6px 10px;font-weight:700;color:#334155">Result</td>'
    || '<td style="padding:6px 10px;text-align:right;color:#334155"></td>'
    -- Net Quotes column
    || '<td style="padding:6px 10px;text-align:center;font-weight:700;color:'
       || CASE WHEN v_quotes_pass THEN '#15803d' ELSE '#b91c1c' END || '">'
       || CASE WHEN v_quotes_pass THEN '✓ Met'
               ELSE 'Carryover: ' || v_quote_short::text || ' quote' || CASE WHEN v_quote_short = 1 THEN '' ELSE 's' END
          END
       || '</td>'
    -- Q Sales Pts column
    || '<td style="padding:6px 10px;text-align:center;font-weight:700;color:'
       || CASE WHEN v_sp_pass THEN '#15803d' ELSE '#b91c1c' END || '">'
       || CASE WHEN v_sp_pass THEN '✓ Met'
               ELSE 'Carryover: ' || to_char(v_sp_short, 'FM999,999') || ' pts'
          END
       || '</td>'
    || '</tr>';
  ...
  -- Team Activity table — per-person rows + Team Total / Goal / Result summary
  v_html := v_html
    || '<div style="margin:14px 0">'
