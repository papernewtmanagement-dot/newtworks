...
--
-- (a) Seed the analytical floor row.
-- (b) Rewrite the fn to consume hiregauge_lss_delta_v2 with 2c monotonic floor-only curve.
--
-- Semantic change vs v1: LSS is now a multiplier (0.0-1.0) on base, not an additive delta.
-- Above floor → mult=1.0 → base unchanged. Below floor → mult<1.0 → base dragged down.
-- No upside from exceptional cognitive capacity (comp-side is monotonic floor-only per Step 2c).
-- This matches Step 2 locked spec 2026-07-31.
...
BEGIN
  ...
  IF v_composite IS NULL OR v_floor IS NULL THEN
    v_mult := 1.0;   -- No LSS data or no floor configured → neutral (no penalty)
  ...
  ELSE
    ...
    v_delta := v_pre_rel - v_base;   -- reported for backward-compat with v1 return shape
