...
-- Any cash-register (email-alert) activity on this account dated after the
-- last statement's close has not been captured in any statement's closing
-- balance yet -- ls is by construction the most recent statement, so a row
-- dated after its close cannot already be baked into ls.closing_balance.
-- Bank/investment accounts are asset accounts: a credit (money in) raises
-- the balance, a debit (money out) lowers it.
pending AS (
  SELECT
    b.account_id,
...
SELECT
  ...
  -- current_balance_derived is the TRUE current balance: last statement's
  -- closing balance plus any cash-register activity since that close that
  -- hasn't shown up on a statement yet.
