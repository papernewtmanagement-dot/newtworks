-- v5. Merging "H E B" into the literal HEB meant the pattern no longer matched
-- the row it came from, because the statement writes it "H-E-B". Merged initials
-- now keep a gap between them in the pattern, so H-E-B, H E B and HEB all match.
...
BEGIN
  ...
  -- rejoin runs of single letters ("H E B") with a marker, so they survive the
  -- two-letter minimum but still allow whatever punctuation the statement used
  ...
  FOREACH t IN ARRAY v_tokens LOOP
    IF length(replace(t, '~', '')) >= 2 AND NOT (t = ANY(c_noise)) THEN
      v_keep := v_keep || t;
      v_letters := v_letters + length(replace(t, '~', ''));
    END IF;
  ...
  RETURN '(?i)' || replace(array_to_string(v_keep, '.{0,24}?'), '~', '[^A-Za-z]*');
END;
$$;
