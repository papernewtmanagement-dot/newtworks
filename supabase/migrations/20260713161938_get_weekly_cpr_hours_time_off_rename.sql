-- Migration: get_weekly_cpr_hours — rename pto_* references to time_off_*
-- Caught during sweep: this function computes salaried employee daily hours,
-- subtracting time-off day-values. Previous body still matched 'pto_full_day' / 'pto_half_day'
-- after the enum rename, so it would have silently returned 8 (full day, not reduced) for
-- any approved time_off row.
