-- get_weekly_cpr_hours: honor approved remote_day / remote_half_day when picking
-- the location shown in the CPR day-grid. Before this change, salaried teammates
-- always displayed their team.work_location default for every worked day, even
-- when an approved remote request covered that day (e.g. Stephanie's Mon+Wed
-- morning remote days via standing_time_off_preferences).
--
-- Behavior after this migration:
--   Salaried (non-HOURLY): remote_per_day flag → at.work_location
--   HOURLY: hourly_locations (actual clock-in) → remote_per_day flag → at.work_location
--
-- Rationale: a day with any approved remote coverage is a deviation from the
-- default location and should render as 🟣 (remote) in the CPR grid, matching
-- how Story Agency — Location calendar treats deviations.
