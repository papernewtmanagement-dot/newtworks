...
DECLARE
  v_ta record;
  v_team_id uuid; v_is_active boolean; v_is_archived boolean;
  ...
  v_best_fit_role text; v_best_fit_os numeric; v_best_role_category text; v_display_label text;
  v_lss_acc numeric;
  v_dims_scored int := 0;
  v_char_floors_failed text[] := ARRAY[]::text[];
  v_confidence text; v_calibration text; v_retro_verdict text;
  v_t_resume record; v_t_others record; v_t_frame record;
  v_lss_autopass jsonb;
  v_lss_status text; v_lss_tier text;
  v_confirmed_exc jsonb;
  v_lss_override_applied boolean := false;
  v_lss_override_reason text := NULL;
  v_confirmed_count int;
  v_has_degree boolean; v_has_prior_job boolean;
BEGIN
  SELECT * INTO v_ta FROM public.v_hiring_candidates WHERE id = p_assessment_id;
  IF NOT FOUND THEN RETURN; END IF;
  SELECT pass_threshold AS p, consider_threshold AS c INTO v_t_resume FROM public.hiregauge_verdict_thresholds WHERE layer='resume';
  SELECT pass_threshold AS p, consider_threshold AS c INTO v_t_others FROM public.hiregauge_verdict_thresholds WHERE layer='assessment';
  SELECT pass_threshold AS p, consider_threshold AS c INTO v_t_frame  FROM public.hiregauge_verdict_thresholds WHERE layer='framework';
  SELECT
  ...
  INTO v_row_r_nat, v_row_r_nur, v_row_r_dr, v_row_a_nat, v_row_a_nur, v_row_a_dr,
       v_row_i_nat, v_row_i_nur, v_row_i_dr, v_row_ref_nat, v_row_ref_nur, v_row_ref_dr
  ...
  SELECT t.id, (t.archived_at IS NULL AND COALESCE(t.is_active, false)), (t.archived_at IS NOT NULL)
    INTO v_team_id, v_is_active, v_is_archived FROM public.team t WHERE t.id = v_ta.team_member_id;
  retrospective_context := CASE WHEN v_is_active THEN 'hired_and_performing' WHEN v_is_archived THEN 'former_team' ELSE NULL END;
  ...
  IF v_nr IS NULL AND v_ta.resume_quality IS NOT NULL THEN
    v_nr := 10 * v_ta.resume_quality::numeric; v_nur := 10 * v_ta.resume_quality::numeric; v_dr := 10 * v_ta.resume_quality::numeric;
    v_dims_scored := v_dims_scored + 1;
  END IF;
  IF v_ta.assessment_nature IS NOT NULL THEN v_na := v_ta.assessment_nature::numeric; END IF;
  IF v_na IS NOT NULL THEN v_dims_scored := v_dims_scored + 1; END IF;
  IF v_ta.assessment_nurture IS NOT NULL THEN v_nua := v_ta.assessment_nurture::numeric; v_dims_scored := v_dims_scored + 1; END IF;
  IF v_ta.assessment_drivers IS NOT NULL THEN v_da := v_ta.assessment_drivers::numeric; v_dims_scored := v_dims_scored + 1; END IF;
  ...
  END;
  resume_verdict := public._hiregauge_layer_verdict('resume', resume_score);
  assessment_verdict := public._hiregauge_layer_verdict('assessment', assessment_score);
  interview_verdict := CASE
    ...
    ELSE public._hiregauge_layer_verdict('interview', interview_score)
  END;
  reference_verdict := public._hiregauge_layer_verdict('reference', reference_score);
  verdict := CASE
    WHEN character_floor_status = 'floor_failed' THEN 'decline_character'
    ...
    ELSE (CASE public._hiregauge_layer_verdict('framework', score_0_10)
          ...
          END)
  END;
  -- LSS auto-pass rule enforcement
  v_lss_autopass := public._hiregauge_lss_autopass(
    v_ta.lss_total_accuracy::numeric, v_ta.reliability, v_ta.analytical::numeric,
    v_ta.assessment_target_role, v_best_fit_role
  );
  v_lss_status := v_lss_autopass->>'status';
  v_lss_tier := v_lss_autopass->>'tier';
  v_confirmed_exc := COALESCE(v_ta.lss_exceptions_confirmed, '[]'::jsonb);
  IF v_lss_status IN ('auto_pass_no_exceptions', 'strong_pass_heavy_evidence_required', 'soft_pass_check_manual_exceptions')
     AND character_floor_status <> 'floor_failed' AND verdict <> 'insufficient_data' THEN
    v_has_degree := v_confirmed_exc ? 'reputable_degree';
    v_has_prior_job := v_confirmed_exc ? 'documented_prior_job_success';
    v_confirmed_count := jsonb_array_length(v_confirmed_exc);
    IF v_lss_tier = 'tier2' THEN
      IF v_has_degree AND v_has_prior_job THEN
        v_lss_override_applied := false;
      ELSE
        verdict := 'decline';
        v_lss_override_applied := true;
        v_lss_override_reason := 'LSS Tier 2 requires reputable degree AND documented prior job success both confirmed';
      END IF;
    ELSIF v_lss_tier IN ('tier1', 'tier3') THEN
      verdict := 'decline';
      v_lss_override_applied := true;
      v_lss_override_reason := 'LSS ' || v_lss_tier || ' has no exception path — auto-decline';
    ELSIF v_lss_tier = 'tier4' AND v_lss_status = 'soft_pass_check_manual_exceptions' THEN
      IF v_confirmed_count > 0 THEN
        v_lss_override_applied := false;
      ELSE
        verdict := 'decline';
        v_lss_override_applied := true;
        v_lss_override_reason := 'LSS Tier 4 with no auto-detected exceptions — confirm at least one manual exception (reputable_degree, documented_prior_job_success, or relevant_licensure) to clear';
      END IF;
    END IF;
  END IF;
  v_lss_autopass := v_lss_autopass || jsonb_build_object(
    'override_applied', v_lss_override_applied,
    'override_reason', v_lss_override_reason,
    'confirmed_exceptions', v_confirmed_exc
  );
  ...
  meta := jsonb_build_object(
    ...
    'thresholds_used', jsonb_build_object(
      'framework_verdict', jsonb_build_object('hire', v_t_frame.p,  'consider', v_t_frame.c),
      'resume_layer',      jsonb_build_object('pass', v_t_resume.p, 'consider', v_t_resume.c),
      'other_layers',      jsonb_build_object('pass', v_t_others.p, 'consider', v_t_others.c)),
    'assessment_target_role', v_ta.assessment_target_role,
    'best_fit_role',          v_best_fit_role, 'best_fit_os', v_best_fit_os,
    'best_role_category',     v_best_role_category, 'display_label', v_display_label,
    'lss_accuracy', v_lss_acc, 'reliability', v_ta.reliability, 'response_distortion', v_ta.response_distortion,
    'lss_autopass', v_lss_autopass);
  RETURN NEXT;
END;
$function$;

-- Smoke test — Allan should now come back verdict=decline (was consider), Stephanie stays fine, Marie decline
SELECT candidate_name, verdict, ROUND(score_0_10, 2) AS composite,
       (meta->'lss_autopass'->>'override_applied')::boolean AS overrode,
       meta->'lss_autopass'->>'override_reason' AS why
FROM public.hiring_candidates hc
CROSS JOIN LATERAL public.hiregauge_three_construct_verdict(hc.id) v
WHERE hc.agency_id='126794dd-25ff-47d2-a436-724499733365'
  AND hc.candidate_name IN ('Allan Piedrabuena','Stephanie Rogers','Marie Story (test)','Katie Barraco');
