-- Switch lookback to calendar intervals: WoW = 1 week, MoM = 1 month, YoY = 1 year.
-- Postgres handles month-end and leap years correctly; fixes the Feb-March MoM bug.
...
COMMENT ON VIEW public.v_book_snapshot_with_changes IS 'book_snapshot with calendar-interval lookback: WoW=1 week, MoM=1 month, YoY=1 year. Calendar-aware (handles Feb and leap years). WoW only computed for cadence=weekly.';
