-- HireGauge Step 7: cts_best_fit_role → 7-way max + display metadata
-- Also updates hiregauge_three_construct_verdict CASE (sole consumer)
-- to route 7 new role slugs to their _competencies_adjusted variants.
--
-- Non-additive: OLD signature returned (best_role, best_os, sales_os, service_os, service_sales_os, aspirant_os).
--               NEW signature returns (best_role, best_role_category, display_label, best_os, 7 OS columns).
--               service_os / service_sales_os columns removed (unblocks Step 3 retirement).
-- Consumer audit: only hiregauge_three_construct_verdict reads this fn. Updated in same migration.
--                 No frontend/edge/script callers (repo grep clean).
--
-- Tie-break priority (deterministic when two roles tie on best_os):
--   sales_outbound > sales_inbound > sales_in_book
--   > retention_reception > retention_escalation > retention_support
--   > aspirant
-- Matches sprint slate order.

------------------------------------------------------------
-- 1. Drop old cts_best_fit_role (signature change forces DROP)
------------------------------------------------------------
DROP FUNCTION IF EXISTS public.cts_best_fit_role(uuid);

------------------------------------------------------------
-- 2. Create new cts_best_fit_role (7-way)
------------------------------------------------------------
...
BEGIN
  ...
  -- If any primary trait is NULL, cannot compute — return NULLs
  ...
  os_so  := public.cts_sales_outbound_os      (t.deadline_motivation, t.recognition_drive, t.assertiveness, t.independent_spirit, t.analytical, t.compassion, t.self_promotion, t.belief_in_others, t.optimism, t.lss_total_accuracy, t.avg_speed);
  os_si  := public.cts_sales_inbound_os       (t.deadline_motivation, t.recognition_drive, t.assertiveness, t.independent_spirit, t.analytical, t.compassion, t.self_promotion, t.belief_in_others, t.optimism, t.lss_total_accuracy, t.avg_speed);
  os_sib := public.cts_sales_in_book_os       (t.deadline_motivation, t.recognition_drive, t.assertiveness, t.independent_spirit, t.analytical, t.compassion, t.self_promotion, t.belief_in_others, t.optimism, t.lss_total_accuracy, t.avg_speed);
  os_rr  := public.cts_retention_reception_os (t.deadline_motivation, t.recognition_drive, t.assertiveness, t.independent_spirit, t.analytical, t.compassion, t.self_promotion, t.belief_in_others, t.optimism, t.lss_total_accuracy, t.avg_speed);
  os_re  := public.cts_retention_escalation_os(t.deadline_motivation, t.recognition_drive, t.assertiveness, t.independent_spirit, t.analytical, t.compassion, t.self_promotion, t.belief_in_others, t.optimism, t.lss_total_accuracy, t.avg_speed);
  os_rs  := public.cts_retention_support_os   (t.deadline_motivation, t.recognition_drive, t.assertiveness, t.independent_spirit, t.analytical, t.compassion, t.self_promotion, t.belief_in_others, t.optimism, t.lss_total_accuracy, t.avg_speed);
  os_asp := public.cts_aspirant_os            (t.deadline_motivation, t.recognition_drive, t.assertiveness, t.independent_spirit, t.analytical, t.compassion, t.self_promotion, t.belief_in_others, t.optimism, t.lss_total_accuracy, t.avg_speed);

  best_o := GREATEST(os_so, os_si, os_sib, os_rr, os_re, os_rs, os_asp);

  -- Tie-break priority: sprint slate order
  best_r := CASE
...
------------------------------------------------------------
-- 3. Update hiregauge_three_construct_verdict CASE (sole consumer)
--    Route 7 new slugs to their _competencies_adjusted variants.
--    Everything else in the fn body byte-for-byte identical to pre-migration.
------------------------------------------------------------
...
DECLARE
  ...
  v_drivers_r_w  numeric := 0.10; v_drivers_a_w  numeric := 0.25; v_drivers_i_w  numeric := 0.45; v_drivers_ref_w  numeric := 0.20;
...
BEGIN
  ...
  IF v_ta.deadline_motivation IS NOT NULL THEN
    ...
    -- Step 7: 7-way slug routing to matching _competencies_adjusted variants
