...
-- Rewire the roster CTE to pull pay_type, pay_rate, work_location, licenses,
-- and weekly_health_benefit_agency_paid from weekly_cpr_team_detail snapshot
-- columns (frozen at first-write). Live team is fallback only.
--
-- Also make base_by_week use PER-WEEK snapshot pay via LEFT JOIN to that 
-- week's detail row — historical weeks now consistent with their own frozen 
-- state, not current-roster state.
...
BEGIN
  ...
  RETURN QUERY
  WITH roster AS (
    -- SNAPSHOT-AWARE: prefer weekly_cpr_team_detail snapshot cols for the 
    -- current week (frozen at first-write). Live team is fallback for 
    -- weeks that haven't been written yet.
  ...
  -- PER-WEEK pay state (snapshot from that week's detail row; falls back to 
  -- current roster if no detail row exists for that week yet)
  per_week_pay AS (
  ...
  SELECT
    ...
    jsonb_build_object(
      ...
      'design_note',
        'Snapshot-aware (2026-07-13 pm5): roster reads pay_type/pay_rate/'
        || 'work_location/licenses/weekly_health from weekly_cpr_team_detail '
        || 'snapshot cols; base_by_week uses per-week snapshot pay via LEFT JOIN. '
        || 'Live team is fallback only for weeks without detail rows.',
      ...
      'qtd_subtractions', jsonb_build_object(
        ...
        'qtd_burden',                   ROUND(
          (s.qtd_base_in_pool_total + s.qtd_comm_total
           ...
           + s.qtd_prize_cart + s.qtd_wtq_trip
           + s.qtd_bonus_pool
          ) * v_burden_mult, 2)
      ),
