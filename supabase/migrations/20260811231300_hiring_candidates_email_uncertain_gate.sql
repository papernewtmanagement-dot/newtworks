-- =========================================================================
-- Email-extraction cross-check gate
-- =========================================================================
-- Why: the actual root cause of the 2026-08-11 bounces wasn't a typo by
-- either applicant -- it was a PDF text-layer misread (a subsetted/embedded
-- font decoding v as y and 1 as l) that both the deterministic regex pass
-- and the LLM identity pass faithfully copied, because both only ever see
-- the extracted text, never the rendered page. Nothing caught it before an
-- invite fired.
--
-- Fix (code side, resume_manual_batch.ts): when a resume's contact info
-- comes from the direct PDF text layer, also read the same file through
-- Drive's OCR (which reads rendered glyphs, not the font's cmap, so it
-- doesn't share this failure mode) and compare emails. A mismatch sets
-- email_uncertain=true instead of picking one silently.
--
-- This migration adds the flag and gates the auto-invite send on it -- an
-- uncertain email must never reach an automatic assessment invitation.
-- =========================================================================

ALTER TABLE public.hiring_candidates ADD COLUMN IF NOT EXISTS email_uncertain boolean NOT NULL DEFAULT false;
ALTER TABLE public.hiring_candidates ADD COLUMN IF NOT EXISTS email_uncertain_detail text;
...
BEGIN
  ...
  -- Step 1d: Resume-score gate sweep.
  ...
  -- Step 1e: Email-uncertain gate sweep (new). Candidates otherwise eligible
  -- for attempt 1 but whose extracted email failed the OCR cross-check --
  -- skipped and counted, no invitation row created, so they don't silently
  -- fall through to attempt 1 the moment the resume score clears.
  SELECT COUNT(*) INTO v_email_uncertain_gated
  FROM public.hiring_candidates hc
  WHERE hc.agency_id = p_agency_id
    AND hc.status = 'applied'
    AND hc.email IS NOT NULL
  ...
  -- Step 2b: Reminder sends — attempts 2 & 3. No email_uncertain re-check
  -- needed: a bounced invitation already got moved off outcome='sent' by
  -- the bounce-detection trigger, so it won't show up here regardless.
  FOR v_cand IN
    SELECT hc.id, hc.first_name, hc.last_name, hc.email, hc.position,
  ...
  RETURN jsonb_build_object(
    ...
    'email_uncertain_gated', v_email_uncertain_gated,
