...
BEGIN
  ...
  -- Normalize first name: trim edges, then collapse any run of internal whitespace to a single space.
  ...
  -- PFA masking rule: max 2 space- or hyphen-separated words in the first name
  -- (e.g. "Mary Jo" or "Jean-Pierre"). Third word breaks the "Firstname L." format.
  v_word_count := array_length(regexp_split_to_array(v_first_normalized, '[ -]'), 1);
  IF v_word_count > 2 THEN
    RAISE EXCEPTION
      'First name "%" has % parts, but PFA records only keep the first 1-2 word first name (e.g. "Mary Jo" or "Jean-Pierre"). Shorten it before submitting.',
      v_first_normalized, v_word_count;
  END IF;
