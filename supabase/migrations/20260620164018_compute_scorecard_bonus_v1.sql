...
BEGIN
  -- Days elapsed in calendar year; pace factor for annualization
  v_days_elapsed := LEAST(365, EXTRACT(DOY FROM p_as_of_date)::int);
  v_pace_factor := 365.0 / GREATEST(1, v_days_elapsed);

  -- Honor Club career tier → points lookup
  SELECT honor_club_career_tier INTO v_career_tier
  ...
  -- Latest agency_snapshot with YTD data
  SELECT * INTO v_snap
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object(
      'error', 'No agency_snapshot row with YTD data found on or before as_of_date',
  ...
  -- FS Credits YTD = Life Premium Credits + IPS new money + 0 (Health excluded)
  ...
  -- Score each sub-bucket on PROJECTED values using universal linear formula
  -- Points = ((Result − Min) / (Max − Min)) × Bucket Max — floor 0, cap at bucket max
  ...
  -- Honor Club qualification (on projected pace) + tier-based points
  ...
  -- Modifiers (applied BEFORE bonus rate)
  ...
  -- Pull YTD eligible new commissions from comp_recap
  SELECT
  ...
  -- Apply modifiers and FS eligibility multiplier
