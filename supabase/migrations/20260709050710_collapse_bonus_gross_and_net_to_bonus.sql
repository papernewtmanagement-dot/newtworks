...
-- This migration was applied live on 2026-07-09 05:07:10 UTC (ledger name: collapse_bonus_gross_and_net_to_bonus) but was never
-- committed to this repo at the time. Content below is copied verbatim
-- from supabase_migrations.schema_migrations.statements for version 20260709050710.
-- No live change is made by adding this file — it only brings the repo
-- mirror into sync with what is already running in production, so a
-- fresh `supabase db reset` from this repo reproduces live state.
-- 1) Add unified `bonus` column, backfill from bonus_net, then drop the two legacy columns
ALTER TABLE public.weekly_cpr_team_detail ADD COLUMN IF NOT EXISTS bonus numeric;

UPDATE public.weekly_cpr_team_detail
SET bonus = COALESCE(bonus_net, bonus_gross)
WHERE bonus IS NULL AND (bonus_net IS NOT NULL OR bonus_gross IS NOT NULL);

ALTER TABLE public.weekly_cpr_team_detail DROP COLUMN IF EXISTS bonus_gross;
ALTER TABLE public.weekly_cpr_team_detail DROP COLUMN IF EXISTS bonus_net;

COMMENT ON COLUMN public.weekly_cpr_team_detail.bonus IS
  'Residual-pool bonus for the week. Collapsed from bonus_gross/bonus_net 2026-07-08 after health subtraction moved to envelope level (no per-person deduction remains).';

-- 2) Drop + recreate compute function: collapse annual_bonus_gross + annual_bonus_net -> annual_bonus
...
CREATE FUNCTION public.compute_weekly_comp_residual_pool(p_agency_id uuid, p_week_end_date date)
 RETURNS TABLE(team_member_id uuid, full_name text, role text, role_category text, role_level text, annual_base_salary numeric, weekly_base_salary numeric, annual_commission_projected numeric, weekly_commission_projected numeric, ytd_sales_points numeric, sales_points_share_pct numeric, weighted_hours_at_40 numeric, retention_hours_share_pct numeric, person_share_pct numeric, annual_bonus numeric, weekly_bonus numeric, annual_total_comp numeric, weekly_total_comp numeric, diagnostics jsonb)
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
  ...
  final AS (
    SELECT d.*,
      ...
      (0.65 * d.sp_share + 0.35 * d.wh_share) * d.annual_bonus_pool AS annual_bonus
    FROM distributed d
  )
  SELECT
    f.tm_id,
    ...
    jsonb_build_object(
      ...
      'burden_note',          'burden = 8% of (base_in_envelope + comm + bonus_pool + carveouts). Pool net of pre-pool carveouts + team health at envelope (2026-07-08). Remote location_w = 0.50 (2026-07-08). bonus_gross/bonus_net collapsed to single `bonus` (2026-07-08).',
...
-- 3) Update writer to use `bonus` column + `annual_bonus` field
...
BEGIN
  ...
  FROM (
    SELECT
      jsonb_build_object(
        ...
        'weekly_bonus',             ROUND(weekly_bonus, 2),
  ...
  SELECT
    SUM(annual_base_salary), SUM(annual_commission_projected), SUM(annual_bonus),
    SUM(sales_points_share_pct), SUM(retention_hours_share_pct), SUM(person_share_pct)
  INTO
    v_sum_base, v_sum_comm, v_sum_bonus, v_sum_sp_pct, v_sum_rh_pct, v_sum_person_pct
  FROM compute_weekly_comp_residual_pool(v_agency_id, p_week_end_date);

  v_implied_burden_wc := v_envelope_annual - (v_sum_base + v_sum_comm + v_sum_bonus);

  RETURN jsonb_build_object(
    'week_end_date', p_week_end_date,
    'envelope_summary', jsonb_build_object(
      'phase', v_phase, 'pool_pct', v_pool_pct,
    ...
    'reconciliation', jsonb_build_object(
      ...
      'sum_bonus_annual',      ROUND(v_sum_bonus, 0),
      'sum_total_paid_annual', ROUND(v_sum_base + v_sum_comm + v_sum_bonus, 0),
      'implied_burden_and_wc', ROUND(v_implied_burden_wc, 0),
      'expected_burden_and_wc', ROUND((v_sum_base + v_sum_comm + v_sum_bonus) * 0.08 + 500, 0),
      ...
      'note', 'share sums should each land at 100.00. Health at envelope (no per-person subtract). Remote location_w = 0.50. bonus_gross/bonus_net collapsed to single `bonus` (2026-07-08).'
    ),
    'computed_at', now()
  );
END;
