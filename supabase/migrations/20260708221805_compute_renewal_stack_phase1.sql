...
DECLARE
  ...
  v_lapse      numeric;
BEGIN
  -- Resolve agency + L&H rate for this team member
  ...
  IF v_agency_id IS NULL THEN
    -- Team member not found; return empty
    RETURN;
  END IF;
  ...
  -- Pull blended lapse for this agency at as-of
  SELECT annualized_rate
  INTO v_lapse
  FROM public.compute_lapse_rate(v_agency_id, p_as_of_date)
  WHERE line = 'blended';

  -- If no snapshot available, fall back to conservative industry benchmark 0.10
