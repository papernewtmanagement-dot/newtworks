...
BEGIN
  ...
  UPDATE public.bank_register_preliminary
     SET suggested_debit_account  = v_rule.debit_account,
         ...
         coding_status            = CASE
                                      WHEN v_rule.confidence = 'high'   THEN 'auto_coded'
                                      ELSE 'needs_review'
                                    END,
         updated_at               = NOW()
   WHERE id = p_row_id;
...
RETURNS TABLE (rows_scanned int, rows_matched int, rows_auto_applied int)
LANGUAGE plpgsql AS $$
DECLARE
  v_row_id uuid;
  v_scanned int := 0;
...
SELECT id, txn_date, account_label, direction, amount, merchant,
 ...
 WHERE (coding_status IS NULL
        OR coding_status = ANY (ARRAY['needs_peter','unclassified','needs_review']))
   AND (status IS NULL
