...
-- ---------------------------------------------------------------------
-- Internal helper — builds a 3-5 column board at a given per-row point
-- multiplier (10 for round one, 20 for round two). Not exposed over REST:
-- no grants to anon/authenticated, callable only from the SECURITY DEFINER
-- functions below, which run as the owning role.
...
-- ---------------------------------------------------------------------
-- Round one now calls the shared helper instead of repeating the board
-- build inline. Behavior is unchanged (multiplier 10).
...
-- ---------------------------------------------------------------------
-- Round three: one wager finale question. Category shows first (the way a
-- live host would say "here's the category — lock in your wagers"), then
-- the host collects each player's wager, locks them, and only then the
-- question and options appear. Reuses quiz_shared_grid_reveal /
-- quiz_shared_grid_state's existing active_item_id handling for the
-- question/answer part — final_item_id just becomes the active item once
-- wagers are locked.
...
-- ---------------------------------------------------------------------
-- quiz_shared_grid_reveal already works unmodified for round 3 — it only
-- checks active_item_id / active_revealed on the session row, which
-- quiz_shared_grid_lock_wagers populates the same way pick() does for
-- rounds one and two.
...
-- ---------------------------------------------------------------------
-- quiz_shared_grid_state gains round, final_wagers, and (in 'wagering'
-- status) the finale's category so the host can announce it before
-- collecting wagers, without exposing the question itself early.
