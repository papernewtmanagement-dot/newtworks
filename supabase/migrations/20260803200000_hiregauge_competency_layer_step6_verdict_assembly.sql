-- Step 6: verdict assembly. Consumer audit found the plan's named targets
-- (hiregauge_composite_recommendation, hiregauge_evaluate_candidate) are an
-- UNRELATED system -- a character-floor rules-narrative engine driven by
-- hiregauge_rules trait_signature matching, with zero connection to
-- role_fit/competency scores. The REAL downstream chain is:
-- verdict_overall -> verdict_assessment -> assessment_nature ->
-- assessment_best_fit_role -> (was) assessment_role_fit_<role> (retired,
-- wired to deadline_motivation/optimism, always NULL for v2 candidates).
-- Rewiring assessment_best_fit_role to the new gated role_fit functions
-- fixes the entire chain above it with zero changes needed to
-- assessment_nature/verdict_assessment/verdict_overall -- same output
-- shape preserved. Confirmed via frontend grep: CandidateDetail.jsx reads
-- bf.<role>_fit_score by name (JS property access), so additive new
-- columns (gate/cap/decline/churn info) are safe to add.
...
BEGIN
  ...
  -- Entry guard rewritten per Step 4 spec: gate on the 21-trait v2 block
  -- being populated, not the retired deadline_motivation/optimism columns.
  ...
  -- fit_score is numeric with one decimal (e.g. 76.1) -- round through
  -- numeric before casting to the integer output column.
