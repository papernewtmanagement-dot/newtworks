-- 2026-07-21: REVERT — undo compute_residual_pool_pay_date_boundary + fallback_prior_only.
--
-- Restores compute_weekly_comp_residual_pool to its pre-2026-07-21 state:
--   1. Payroll joins match on pr.pay_period_end = cw.week_end_date only (no pay_date filter).
--   2. wctd.commission / wctd.manager_bonus fallback fires for ALL cycle weeks (no gating).
--
-- Peter diagnostic: my prior pay_date-boundary migration caused commission to not be
-- subtracted from the pool when the compute week's payroll had a future check_date.
-- Concretely: week ending 07-11 had qtd_actual_commission = $0 (07-11 payroll pay_date
-- was 07-17, filtered out) and week ending 07-18 had qtd_actual_commission = $262
-- (only 07-11 payroll counted; 07-18 payroll filtered out because pay_date 07-24 > 07-18).
--
-- Correct semantic (restored): commission earned during the pay period counts as
-- subtracted from that period's pool regardless of check_date. A payroll whose
-- pay_period_end falls in Q3 is a Q3 obligation, period.
--
-- Root cause of the original 07-18 Monday drift was NEVER the compute logic. It was
-- write_weekly_comp_v2 re-executing on a sent week. The freeze guard (migration 210000)
-- is the actual fix. The pay_date filter I added was an over-fix that broke the math.
