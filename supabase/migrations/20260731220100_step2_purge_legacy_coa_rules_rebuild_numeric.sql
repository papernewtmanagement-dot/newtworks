-- Migration: step2_purge_legacy_coa_rules_rebuild_numeric
-- Applied: 2026-07-31 via Supabase MCP
-- Session note: 2026-07-31 — Pipeline repair Step 2: 9 decisions locked, Leslie blocked on cross-entity architecture
--
-- Purpose: purge 100 legacy COA-* rules in gl_classification_rules, rebuild to numeric COA codes.
-- Blast radius: 100 rules touched. Post-flight verified 0 active rules reference legacy COA-* codes.
--
-- 10 decisions from session note:
--   1. Fuel COA-SUB-007: strip COA-024 source filter, remap to 6810
--   2. SF commission COA-SUB-058: deactivate (comp_recap is source of truth)
--   3. Life stipends: Leslie (COA-SUB-042) & John/Tommy (COA-SUB-074) → 6110 Employee Benefits
--   4. Auto loan COA-SUB-084 → 2540 Vehicle Loan Payable (balance sheet, not expense)
...
--
-- PaperNewt 6110 Employee Benefits COA row added in prior migration papernewt_add_6110_employee_benefits (2026-07-31).
...
-- =====================================
-- MAP+STRIP: COA-SUB-007 fuel — remap to 6810, strip COA-024 source filter (2 rules)
-- =====================================
UPDATE public.gl_classification_rules
