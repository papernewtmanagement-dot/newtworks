-- Amendment to v_hiring_candidates_composite_uses_validity_shrinkage (same session).
-- The renormalized composite was rendering a Character/Commitment-only badge for 37
-- legacy-facet candidates who never took the Newtworks assessment (21 declined, 9
-- assessment_sent, 7 former -- facet columns populated by the old pre-v2 imports, no
-- GMA/SJT, assessment never started). The pre-change view correctly showed NULL for
-- them. Restore that gate: assessment_composite renders only when assessment_capability
-- is present (true for all 31 completed assessments), keeping the validity-shrinkage
-- math from the prior migration. verdict_assessment still renormalizes for its own
-- callers (verdict_overall handles partial constructs deliberately); this gate is a
-- display-surface rule, and for every capability-present candidate the two remain
-- penny-identical.
...
WITH (security_invoker = true) AS
 ...
 SELECT hc.id,
    ...
    (SELECT CASE WHEN ac.a_cap IS NULL THEN NULL
                 WHEN w.wsum > 0 THEN round(w.tot / w.wsum, 2) END
       FROM ( SELECT
