-- NOTE: this file consolidates three migrations applied live in the database
-- during the 2026-08-11 session (20260811231424 reminder_loop_include_
-- assessment_sent_status, 20260811232239 early_career_assessment_lane,
-- 20260811232400 auto_decline_trigger_early_career_lane_exception) that were
-- never mirrored to the repo at the time. Mirrored 2026-08-12 using the exact
-- function bodies read live from pg_proc before this session's own edits
-- (see 20260812035528/035619 for what changed after that). The three
-- original migrations are not separately reconstructable byte-for-byte;
-- this file and its sibling for auto_decline_on_resume_score together
-- represent the correct end state after all three, which is what matters
-- for db reset / fresh-clone purposes since later CREATE OR REPLACE
-- migrations are idempotent.
--
-- Reminder-loop fix (2026-08-11, Peter-approved): eligibility accepts
-- 'assessment_sent' in addition to 'applied'. The prior 'applied'-only
-- filter meant (a) the ~130 manually-advanced candidates never got
-- reminders, and (b) once the same-day status-advance fix landed, attempt 1
-- instantly removed every candidate from reminder eligibility — reminders
-- were unreachable for anyone.
--
-- Early-career lane (2026-08-11, Peter-approved): a below-threshold resume
-- still routes to the assessment invite when early_career=true, Character-
-- signal average >= 40, and no integrity flag. Enforced here (Steps 1d/2a)
-- and in the auto_decline_on_resume_score trigger (sibling file).
