-- Migration: role_fit_v3_5_realism_tune_aspirant
-- 2026-07-24
--
-- Broaden the aspirant profile from an outbound-hunter shape to a full
-- agent-in-training shape. An aspirant is someone building toward running
-- their own agency, which requires far more than a rockstar hunter profile:
-- staying composed in chaos, making quick decisions, warm rapport, follow-
-- through on cadences, paperwork discipline, and the ability to interview
-- and hire.
--
-- Changes vs v3.4:
--
-- - attention_to_detail FLIPPED -0.02 -> +0.04 (agents live and die by
--   paperwork; the small negative was likely a "primarily-detail-oriented
--   people don't tend to become agents" archetype signal from vendor
--   regression, but the causal reading is that agents need this).
--
-- - Added six competencies that live in the day-to-day agent role but were
--   missing entirely: composure_under_load (+0.06), makes_decisions_quickly
--   (+0.05), rapid_rapport_warm (+0.04), pivots_to_customer_need (+0.04),
--   cadence_compliance (+0.03), proactive_touch_discipline (+0.03).
--
-- - balances_logic_and_emotion_when_hiring bumped 0.03 -> 0.04 (agents make
--   many hiring calls; this competency is closer to the core than the
--   original v3.4 weight reflected).
--
-- - Trimmed the top drives (has_entrepreneurial_spirit, maintains_high_
--   activity) by 0.02-0.03 each and reduced tail weights (retention_
--   watchfulness, is_fast_start_oriented, prospects_in_community,
--   dials_cold_calls) to make room without inflating positive_weight_sum.
--
-- - Deepened queue_throughput_discipline negative -0.03 -> -0.05 (pure
--   "clear the queue and go home" mindset is antithetical to building
--   a business from scratch).
--
-- positive_weight_sum = 1.05, negative_weight_sum = -0.05, net = 1.00.
-- Model tag: role_fit_v3_5_realism_tune_2026_07_24.
