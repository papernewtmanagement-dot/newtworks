-- Two guards tightened now that a team row can exist before anyone starts.
--
-- 1. work_checkin and work_display treated a missing start date as "already
--    started", so a row created at offer acceptance with no date yet would be
--    nagged in the daily check-ins from the moment it existed. A row with no
--    start date has not started. Nobody on the roster or in any weekly report
--    snapshot has a missing start date, so this changes nothing today.
--
-- 2. The Daily Kickoff invite only held someone from a week before their start
--    date up to the day itself. If the active flag was not switched on by hand
--    that morning, the next sync would take them back off the invite and email
--    them to say so. The window now runs from a week out with no upper edge;
--    leaving is handled by archiving, which is filtered above.
...
BEGIN
  ...
  WHERE r.is_test_user IS NOT TRUE
    ...
    -- Archived-teammate cutoff. Default is the exact as-of date. For the team
    -- status DISPLAY only, someone archived mid-week stays on the roster for the
    -- rest of that week (Peter 2026-09-04). Reminder rosters are unaffected --
    -- a teammate who has left is never pinged.
    AND (r.archived_at IS NULL
         OR (p_as_of_date IS NOT NULL
             AND (
               r.archived_at > p_as_of_date::timestamptz
               OR (p_purpose = 'work_display'
                   AND (r.archived_at AT TIME ZONE 'America/Chicago')::date >= v_week_start)
             )))
    AND (
      -- work_checkin: nag list. Excludes teammates whose approved time off covers
      -- the specific reminder window (full day always; half-day mapped by
      -- p_checkin_type -- morning-off covers morning+midday, afternoon-off covers eod).
      -- No license requirement: every active agency teammate flagged into the
      -- check-ins has to react, licensed or not (Peter 2026-09-14).
      -- A missing start date means they have not started, so they are not nagged.
      (p_purpose = 'work_checkin'
        ...
        AND r.start_date IS NOT NULL AND r.start_date <= v_today_ct
      ...
      OR
      -- No license requirement: the status block carries marketing and retention
      ...
      (p_purpose = 'wtw_am_sales'
        AND r.role_level = ANY(public.manager_tier_levels())
      ...
      (p_purpose = 'agency_active_all'
        ...
        AND r.is_active = true)
      OR
      -- Calendar invites only. Everyone on agency_active_all, plus anyone with a
      -- start date a week out or nearer. No upper edge: once they are on the
      -- invite they stay on it until they are archived, so the sync never takes
      -- a new teammate back off on their first morning.
      (p_purpose = 'agency_calendar_invite'
        AND r.category = 'agency'
        AND (r.is_active = true
             OR (r.start_date IS NOT NULL
