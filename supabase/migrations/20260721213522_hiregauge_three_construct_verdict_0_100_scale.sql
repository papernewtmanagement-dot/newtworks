-- Migration 20260721213522 (mirror of applied migration)
-- Normalize hiregauge_three_construct_verdict to 0-100 scale end-to-end.
-- Before: resume 0-100, assessment/interview computed on 0-10 from raw rp_/char_/mot_/OS+comp math,
--         reference 0-10. Mixed scales made score_0_10 nonsense (e.g. Priscilla=22.46 "hire").
-- After: every construct cell + every layer + every construct-average + score_0_10 all on 0-100.
--        Cell sourcing switches to view fields (assessment_nature/nurture/drivers, iv_*) which are
--        already the authoritative 0-100 values the frontend reads. Reference cells (smallint 0-10)
--        multiplied by 10. All thresholds bumped x10 (7.5/6.0 -> 75/60, 7.0/5.0 -> 70/50, etc).
-- Character floor logic unchanged (raw char_* on 0-10 with < 7 threshold).
-- score_0_10 column name kept for frontend compatibility; the VALUE is now on the 0-100 scale.
...
BEGIN
  ...
  IF v_nr IS NULL AND v_ta.resume_quality IS NOT NULL THEN
    v_nr  := (v_ta.resume_quality::numeric) * 10;
    v_nur := (v_ta.resume_quality::numeric) * 10;
    v_dr  := (v_ta.resume_quality::numeric) * 10;
    v_dims_scored := v_dims_scored + 1;
  END IF;

  -- === ASSESSMENT (view 0-100) ===
  ...
  IF v_ta.assessment_nature IS NOT NULL THEN v_na := v_ta.assessment_nature::numeric; v_dims_scored := v_dims_scored + 1; END IF;
  ...
  -- === REFERENCE (view smallint 0-10 -> * 10) ===
  IF v_ta.ref_nature  IS NOT NULL THEN v_nref  := (v_ta.ref_nature ::numeric) * 10; v_dims_scored := v_dims_scored + 1; END IF;
  IF v_ta.ref_nurture IS NOT NULL THEN v_nuref := (v_ta.ref_nurture::numeric) * 10; v_dims_scored := v_dims_scored + 1; END IF;
  IF v_ta.ref_drivers IS NOT NULL THEN v_dref  := (v_ta.ref_drivers::numeric) * 10; v_dims_scored := v_dims_scored + 1; END IF;
