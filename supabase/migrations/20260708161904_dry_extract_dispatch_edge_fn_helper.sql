...
-- This migration was applied live on 2026-07-08 16:19:04 UTC (ledger name: dry_extract_dispatch_edge_fn_helper) but was never
-- committed to this repo at the time. Content below is copied verbatim
-- from supabase_migrations.schema_migrations.statements for version 20260708161904.
-- No live change is made by adding this file — it only brings the repo
-- mirror into sync with what is already running in production, so a
-- fresh `supabase db reset` from this repo reproduces live state.
-- Pair 1 (Tier 2): extract shared _dispatch_edge_fn helper.
--
...
-- payload to an edge function via pg_net, returned a jsonb summary. The only
-- differences were the edge-fn name, the pg_net timeout, and an optional trailing
-- sentence in output_summary.
--
-- automation_recipes.internal_handler pins the callable name, so both public
-- wrappers stay (rows `Payroll Summary Ingest` + `Daily License Reminder
-- Dispatcher` continue routing unchanged). Wrappers become 3-line delegates.
...
-- Thin wrappers preserving automation_recipes.internal_handler contract
