-- Revision v2 of the SJT pilot bank, per Peter's review 2026-08-02:
-- - Item 8 rewritten: correct answer is now a constructive action (offer to
--   review coverage/discounts to genuinely lower cost) instead of a flat
--   decline. Peter's point: declining outright wasn't the best available
--   move -- looking for a legitimate way to help is.
-- - Items 4 and 7 rewritten to stay at peer level (no assumed authority over
--   a teammate or a manager's decision -- these are front-line hire
--   scenarios, and the originals quietly broke that assumption).
-- - Item 10 rewritten: original only offered two extremes (infinite patience
--   vs. dismissive rushing). Peter correctly flagged that unlimited patience
--   isn't actually good time management. New correct answer balances
--   attentiveness with efficiency.
-- - ALL 10 items: distractors reworded to remove obvious tell words
--   ("definitely", "quietly", "hope they forget", "push"). Original
--   distractors were too easy to spot by tone alone rather than judgment --
--   a low-face-validity design flaw. New distractors are each plausible on
--   first read and wrong on a specific, less obvious dimension.
