-- Option A: compute_person_commissions_quarterly rebased to base 0.5% P&C / 1.5% L&H.
-- Step size 0.05% / 0.15% unchanged. Caps 6% / 18% unchanged.
-- Restructures function to separate BASE from STEP (previously merged at 0.06%/0.18% each).
-- Design principle: 1 Sales Point = $1 team commission.
...
DECLARE
  ...
  -- Option A rate structure (SQL applied 2026-07-06).
  -- Base + tiered step. Each policy earns EITHER P&C OR L&H rate by LOB, never both.
  c_pc_base_pct   CONSTANT numeric := 0.005;    -- 0.5% base
  c_pc_step_pct   CONSTANT numeric := 0.0005;   -- 0.05% per tier
  c_lh_base_pct   CONSTANT numeric := 0.015;    -- 1.5% base
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'plan_version',     'option_a_2026_07_06',
    'computed_at', now()
  );
END;
$function$;
