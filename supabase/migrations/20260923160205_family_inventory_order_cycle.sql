-- Household inventory: count "before next week's order" from the item's last order,
-- stock check or add when that was under half a week ago. Fixes weekly items coming
-- straight back onto Likely out right after they were ordered or marked plenty.
...
--    expected to run out before next week's order. suggested_qty = enough to last to the
--    next weekly order, never less than the usual amount, rounded up to a whole unit.
--    The week is counted from the item's last order, stock check or add if that was
--    under half a week ago (it belongs to the current order: nearest-order rule),
--    otherwise from now. So an item that lasts exactly a week does not come straight back
--    after it is ordered, and "plenty left" on a weekly item keeps it off this week's list.
...
BEGIN
  ...
  LOOP
    ...
    v_low_at := NULL; v_dancer := NULL; v_ordered := NULL; v_handled := NULL;
    ...
    LOOP
      ...
      IF ev.kind IN ('start', 'ordered', 'left') THEN
        v_handled := ev.at;
      END IF;
    ...
    v_week_end := CASE WHEN v_handled > now() - interval '1 day' * (c_horizon / 2)::float8 THEN v_handled ELSE now() END
                  + interval '1 day' * c_horizon::float8;
    likely_out := (NOT waiting) AND v_left IS NOT NULL
                  AND now() + interval '1 day' * (v_left / v_rate)::float8 < v_week_end - interval '1 hour';
