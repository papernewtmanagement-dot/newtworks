...
-- This migration was applied live on 2026-07-23 22:15:23 UTC (ledger name: step10prep_inline_cell_math_and_rewire_view) but was never
-- committed to this repo at the time. Content below is copied verbatim
-- from supabase_migrations.schema_migrations.statements for version 20260723221523.
-- No live change is made by adding this file — it only brings the repo
-- mirror into sync with what is already running in production, so a
-- fresh `supabase db reset` from this repo reproduces live state.
-- Step 10 PREP of HireGauge function architecture refactor
-- Rewrite cell functions to do the math directly (no more cts_* wrappers),
-- then rewire v_hiring_candidates composite calc to call the cell functions.
-- Byte-identical output required against the pre-migration baseline.
