-- Closing a task now takes its booking off the calendar too. A booking for work that is
-- already done is noise on a calendar whose whole job is "when am I doing this".
-- complete_at is left alone, so the record of when it was planned survives.
-- Only passes 3 and 4 change; everything else is as shipped earlier today.
...
BEGIN
  ...
  -- 3. Time moved on a task that is still open: edit the same event, never a new one.
  FOR v_t IN
  ...
  -- 4. Time cleared, or the task is done: take the event off the calendar.
  FOR v_t IN
    SELECT id, calendar_event_id
    FROM public.tasks
    WHERE agency_id = p_agency_id
      AND calendar_event_id IS NOT NULL
      AND (complete_at IS NULL OR COALESCE(status, 'open') = 'closed')
  LOOP
    PERFORM public.composio_post(
