-- 1. Calls block no longer ends with a newline. Every other block builder
--    (render_daily_commits_block) already rtrims, and the kickoff appends
--    E'\n\n' after the calls block, so the trailing newline produced two blank
--    lines instead of one (Peter 2026-09-15). Only caller is
...
-- 2. The kickoff reports the PRIOR WORKDAY, not the newest team_checkins EOD
--    row. That row stopped being written when the compile step was removed on
--    2026-09-14, so the date froze at the last day anyone replied and the
--    Monday branch (prior week + won/missed marker) fired every day after it
--    (Peter 2026-09-15). checklist_prev_workday is the same day the commits
--    block and the checklist bridge already report on.
...
BEGIN
  ...
  ELSE
    ...
    v_last_eod_date := public.checklist_prev_workday(p_agency_id, v_today);
    ...
    -- Prior workday's commits. The marker is what gets stored; the live block
    -- is dropped in at send time and again on every refresh, so the quote above
    -- never changes when someone marks a commit.
    v_text := v_text || E'\n\n{{commits}}';
