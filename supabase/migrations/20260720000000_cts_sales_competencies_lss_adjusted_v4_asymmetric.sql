-- v4 asymmetric fix for cts_sales_competencies_lss_adjusted.
-- Fixes v1-v3 design flaw where BETWEEN in flag counting penalized
-- above-range accuracy the same as below-range, and slower speed
-- the same as faster.
--
-- v4 change (6 CASE WHEN clauses inside IF v_has_lss block):
--   acc flags credit in-range OR ABOVE  (score >= low_bound)
--   spd flags credit in-range OR FASTER (spd   <= high_bound)
--
-- All other logic unchanged from v3: distortion dampening on CO/OP/BO,
-- per-competency LSS additive delta with unchanged weights, reliability
-- regression toward 50. Same signature as v3 → CREATE OR REPLACE, no DROP.
--
-- Model tag: sensitivity_weighted_v4_asymmetric.
-- Base fn cts_sales_competencies remains untouched.

CREATE OR REPLACE FUNCTION public.cts_sales_competencies_lss_adjusted(p_dm integer, p_rd integer, p_as integer, p_is integer, p_an integer, p_co integer, p_sp integer, p_bo integer, p_op integer, p_m_acc integer DEFAULT NULL::integer, p_v_acc integer DEFAULT NULL::integer, p_ps_acc integer DEFAULT NULL::integer, p_m_spd integer DEFAULT NULL::integer, p_v_spd integer DEFAULT NULL::integer, p_ps_spd integer DEFAULT NULL::integer, p_reliability text DEFAULT NULL::text, p_distortion text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 IMMUTABLE
AS $function$
...
BEGIN
  ...
  IF v_has_lss THEN
    -- v4 asymmetric: acc flags credit in-range OR ABOVE (>= low_bound);
    --                spd flags credit in-range OR FASTER (<= high_bound).
    -- Fixes v1-v3 flaw where above-range accuracy + faster-than-range speed were penalized like below.
    v_acc_flags :=
      (CASE WHEN p_m_acc  >= 10 THEN 1 ELSE 0 END) +
      (CASE WHEN p_v_acc  >= 8  THEN 1 ELSE 0 END) +
      (CASE WHEN p_ps_acc >= 7  THEN 1 ELSE 0 END);
    v_spd_flags :=
      (CASE WHEN p_m_spd  <= 50 THEN 1 ELSE 0 END) +
      (CASE WHEN p_v_spd  <= 52 THEN 1 ELSE 0 END) +
      (CASE WHEN p_ps_spd <= 77 THEN 1 ELSE 0 END);

    v_acc_signal := (v_acc_flags - 1.5) / 1.5;
    v_spd_signal := (v_spd_flags - 1.5) / 1.5;

    -- v3 weights carried forward unchanged: every competency has some LSS sensitivity
