-- Peter's correction: an Amazon order always precedes its credit card charge.
-- The matcher should prefer the MOST RECENT order on or before the charge date,
-- not just whichever order is closest by raw day-count (which could wrongly pick
-- a later order over a valid earlier one). Only fall back to a later-dated order
-- if no prior-or-same-date candidate exists at all (e.g. bank posts before
-- Amazon's order-confirmation timestamp due to timezone rounding).
...
paired AS (
  SELECT al.ledger_id, al.agency_id, al.entry_date, al.description, al.charge_amount, al.source,
    ...
    CASE WHEN os.order_date <= al.entry_date THEN 0 ELSE 1 END AS date_priority
  FROM amz_ledger al
...
with_pref AS (
  SELECT *, min(date_priority) OVER (PARTITION BY ledger_id) AS best_priority
  FROM paired
),
filtered AS (
  SELECT * FROM with_pref WHERE date_priority = best_priority
),
with_min AS (
