-- =====================================================================
-- send_v1_assessment_invitations: completion detection + exit terminal
-- 2026-08-03
-- =====================================================================
-- Two live breaks fixed:
--
-- (1) Completion was never detected. Every completion check matched only
--     alert_type = 'v1_assessment_complete'. The live delivery function
--     writes 'v2_assessment_complete'. Consequence: a candidate who
--     finished the full assessment stayed at outcome='sent', received
--     reminder attempts 2 and 3 asking them to start, and was eventually
--     swept to 'no_response'. Both names are now matched -- 10 historical
--     'v1_assessment_complete' rows still exist, so this is additive.
--     Named constant list, not a rename.
--
-- (2) Silently-exited candidates were still nagged. The stint-1 exit gate
--     deliberately leaves status='applied' and tells the candidate
--     nothing, so status alone cannot distinguish them. They now become a
--     terminal 'exited' outcome and are excluded from first sends and
--     reminders by assessment_exit_gate IS NULL.
--
-- Nothing else about the send logic, cadence, rate cap, or email copy is
-- changed.
-- =====================================================================
