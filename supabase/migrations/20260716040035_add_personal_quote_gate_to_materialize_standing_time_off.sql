...
BEGIN
  ...
  -- LOOKAHEAD GUARD (unchanged from prior version)
  ...
  LOOP
    ...
    -- Gate 2 (added 2026-07-15): individual personal-quote minimum for prior week.
    -- Per canonical "Win the Week — official team-level definition" personal_minimum rule:
    --   AM-Sales:     15 net quotes (Life attribute deferred until schema ships)
    --   AM-Retention: 8 net quotes  (Life attribute deferred until schema ships)
    --   AA:           tenure-adjusted target not modeled here — AAs skip this gate.
    -- Net = quotes_discussed - paid (from get_weekly_cpr_requirements). Matches the same
    -- metric team-level Condition 1 uses.
    IF v_pref.trigger_type = 'wtw_won_prior_week' THEN
      ...
      IF v_role_level IN ('Account Manager','Unit Manager') THEN
        ...
        END IF;
      END IF;
      -- AA / non-AM: skip personal gate (falls through to fire)
    END IF;
