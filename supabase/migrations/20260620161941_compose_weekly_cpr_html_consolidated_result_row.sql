-- Email parity with CPRDetail.jsx (commit 35b9349): consolidate the Team Activity
-- Result row from two cells (per-column "✓ Met" / "Carryover: N quotes" / "Carryover: N pts")
-- into ONE cell spanning Net Quotes + Q Sales Pts.
...
BEGIN
  ...
  v_activity_summary_rows :=
    ...
    || '</tr>'
    -- Result — one consolidated label spanning Net Quotes + Q Sales Pts.
    -- Win  (both conditions cleared): ✓ Win the Week!  (green)
    -- Loss (either missed):           Carryover: N quotes / M pts  (each piece shown only if > 0, red)
    || '<tr>'
    || '<td style="padding:6px 10px;font-weight:700;color:#334155">Result</td>'
    || '<td style="padding:6px 10px;text-align:right;color:#334155"></td>'
    || '<td colspan="2" style="padding:6px 10px;text-align:center;font-weight:700;color:'
