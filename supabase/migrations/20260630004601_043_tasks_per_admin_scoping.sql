...
-- 2) FK to public.users(id), nullable, ON DELETE SET NULL (don't orphan-cascade tasks)
DO $$ BEGIN
...
-- Manager: read/write only tasks assigned to themselves (and cannot reassign away)
...
-- Note: anon role + staff role get no policy = no access (RLS default deny).
