-- create_onboarding_plan now copies auto_source from the template onto the
-- new step, and runs the reference sync once so a self-filling step shows
-- its summary the moment the plan is created.
...
BEGIN
  ...
  INSERT INTO public.team_onboarding_steps (
    ...
    substeps, substeps_done, owner_kind, assigned_to, track, blocked_by, auto_source
  )
  SELECT
    ...
    t.owner_kind, t.assigned_to, t.track, t.blocked_by, t.auto_source
...
GRANT EXECUTE ON FUNCTION public.create_onboarding_plan(uuid, uuid, text, text, text, date, text)
