-- Step C: add On-Time Annual column, convert all payroll cells to cpr_fmt_money.
-- Flattens via CTE (payroll_calc → payroll_with_totals) to avoid the 42601
-- "INTO specified more than once" plpgsql parser confusion from earlier attempts.
...
BEGIN
  ...
  SELECT string_agg(
    ...
    || '<td style="padding:6px 10px;text-align:right;border-bottom:1px solid #e5e7eb;color:#334155;font-weight:700">' || to_char((COALESCE(hp.mon_h,0) + COALESCE(hp.tue_h,0) + COALESCE(hp.wed_h,0) + COALESCE(hp.thu_h,0) + COALESCE(hp.fri_h,0)), 'FM999.00') || '</td>'
    || '</tr>',
    '' ORDER BY t.hire_date, t.last_name
  )
  INTO v_hours_rows
  ...
  -- PAYROLL rows (CTE-flattened to avoid 42601 parser confusion).
  -- All money cells use cpr_fmt_money (zero/null → "—").
  -- Columns: Member | Weekly Pay | Base Adv | Health | Benefits | Retention | True Pay | Mgr | Agcy Profit | Week Total | On-Time Annual
  -- On-Time Annual = (payroll_ytd_paid + Σ components excl Benefits) × 365/days_employed_year + annual_benefits_value (flat add, no compounding).
  WITH payroll_calc AS (
    SELECT
      ...
      COALESCE(d.weekly_pay, 0)           AS weekly_pay,
      COALESCE(d.base_advance, 0)         AS base_advance,
      COALESCE(d.health_bonus, 0)         AS health_bonus,
      COALESCE(d.service_surge_share, 0)  AS retention,
      COALESCE(d.true_pay_bonus, 0)       AS true_pay,
      COALESCE(d.manager_bonus, 0)        AS manager_bonus,
      COALESCE(d.agency_profit_share, 0)  AS agency_profit,
      d.payroll_ytd_paid                  AS payroll_ytd_paid,
  ...
  payroll_with_ota AS (
    SELECT
      *,
      CASE
        WHEN payroll_ytd_paid IS NULL THEN NULL::numeric
        ELSE (payroll_ytd_paid + week_excl_benefits) * 365.0 / days_employed_year + annual_benefits
      END AS on_time_annual
    FROM payroll_with_totals
  )
  SELECT string_agg(
    '<tr>'
    || '<td style="padding:6px 10px;border-bottom:1px solid #e5e7eb;font-weight:600;color:#1e293b">' || COALESCE(NULLIF(nickname,''), first_name) || '</td>'
    ...
    || '<td style="padding:6px 10px;text-align:right;border-bottom:1px solid #e5e7eb;color:#1e293b;font-weight:700;background:#eff6ff">' || public.cpr_fmt_money(on_time_annual, 0) || '</td>'
    || '</tr>',
