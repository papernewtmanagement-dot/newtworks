-- Update team_checkin_compile_results to append a Win the Week comparison block
-- after the team total, with rotating encouragement based on which conditions pass.
...
BEGIN
  ...
  v_text := v_text || '  SP pace: ' || v_total_sales::text || ' of ' || v_wtw.sp_target::text;
  IF v_sp_pass THEN
    v_text := v_text || '  ✅ cleared';
  ELSE
    v_text := v_text || '  —  ' || v_sp_short::text || ' to clear';
  END IF;
  v_text := v_text || E'\n';

  -- Encouragement (rotating, scenario-dependent)
  ...
  RETURN jsonb_build_object(
    ...
    'output_summary', format('%s compile: %s of %s reporting; totals %s/%s; WtW Q:%s/%s SP:%s/%s',
      v_checkin_type, v_fresh_count, v_expected_count, v_total_quotes, v_total_sales,
      v_total_quotes, v_wtw.quotes_target_total, v_total_sales, v_wtw.sp_target)
  );
END;
$function$;
