...
BEGIN
  ...
  -- Quotes: single shared calculation (get_team_checkin_totals) — was previously a
  -- duplicated inline query filtered to checkin_type IN ('midday','eod'), which silently
  -- dropped anyone whose latest submission that week was a health_eve check-in. Fixed
  -- 2026-08-12. This stays a live calculation — there is no stored record of the
  -- self-reported weekly quotes pace number to read back (weekly_cpr_reports.quotes_total_net
  -- is a different metric, from the CPR debt/penalty system — see get_team_checkin_totals
  -- comment. Do not read that column here.)
  ...
  -- Sales points: read from the weekly_cpr_reports record instead of recomputing.
  -- weekly_cpr_upsert_in_progress now runs before this function (team_checkin_compile_results,
  -- 2026-08-12) and writes quarterly_sales_points_qtd using this exact same window
  -- (cycle_start .. week_ending_saturday) via get_team_checkin_totals. One calculation,
  -- saved once, read here rather than run a second time.
  SELECT COALESCE((
