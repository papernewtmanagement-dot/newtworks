...
BEGIN
  ...
  -- HDB: per-person $25/wk x 52 x own tenure_ramp
  SELECT
  ...
  -- $10 x team_tenure_ramp_sum x 13 x 4 per bucket, scales with each teammate's 0.0->1.0 tenure ramp.
  -- Prior asymmetry (leaderboard by slots x cats, all-star by cats x tenure, trailblazer by cats x 1) retired.
  ...
  RETURN jsonb_build_object(
    ...
    'design_note', 'Carve-and-forget. All 5 goals buckets (WtW, Gain, Leaderboard, All-Star, Trailblazer) now share the WtW formula: $10 x team_tenure_ramp_sum x 13 x 4. Each scales with per-person tenure ramp (0.0->1.0 over first 52 weeks). Rev 2026-07-18.',
    ...
    'manager_bonus', jsonb_build_object('annual_dollars', ROUND(v_annual_manager_bonus, 2), 'weekly_dollars', ROUND(v_annual_manager_bonus / 52.0, 2), 'formula', 'sum(role_level_pct x on-time Scorecard annual)', 'detail', v_manager_detail),
    'life_insurance_stipend', jsonb_build_object('annual_dollars', ROUND(v_annual_life_ins, 2), 'weekly_dollars', ROUND(v_annual_life_ins / 52.0, 2), 'formula', 'sum(monthly_cap_by_year_of_employment x 12)', 'detail', v_life_ins_detail),
    ...
    'health_development_bonus', jsonb_build_object('annual_dollars', ROUND(v_annual_hdb, 2), 'weekly_dollars', ROUND(v_annual_hdb / 52.0, 2), 'formula', 'per-person $25/wk x 52 x tenure_ramp (structural max, carve-and-forget)', 'detail', v_hdb_detail),
    'champions_circle', jsonb_build_object('annual_dollars', ROUND(v_annual_cc, 2), 'weekly_dollars', ROUND(v_annual_cc / 52.0, 2), 'pct_of_basis', v_cc_pct, 'formula', '3% x on-time (SMVC + Scorecard) annual'),
    'mvp_prize_cart', jsonb_build_object('quarterly_dollars', ROUND(v_quarterly_mvp, 2), 'annual_dollars', ROUND(v_quarterly_mvp * 4.0, 2), 'weekly_dollars', ROUND(v_quarterly_mvp / 13.0, 2), 'formula', '1% x on-time (SMVC + Scorecard) x pace, QUARTERLY pot', 'rate_pct', v_rate, 'pace', ROUND(v_pace, 4), 'projected_wins', v_projected_wins),
    'wtq_trip', jsonb_build_object('quarterly_dollars', ROUND(v_quarterly_wtq, 2), 'annual_dollars', ROUND(v_quarterly_wtq * 4.0, 2), 'weekly_dollars', ROUND(v_quarterly_wtq / 13.0, 2), 'formula', '1% x on-time (SMVC + Scorecard) x pace, QUARTERLY pot', 'rate_pct', v_rate, 'pace', ROUND(v_pace, 4), 'projected_wins', v_projected_wins, 'floor_wins', 9, 'halted', v_wtq_halted, 'halt_reason', v_wtq_halt_reason, 'mvp_share_pct', v_mvp_share_pct, 'rest_share_pct', v_rest_share_pct, 'team_count', v_team_count, 'rest_of_team_count', v_rest_count, 'mvp_dollars', ROUND(v_mvp_dollars, 2), 'rest_pool_dollars', ROUND(v_rest_pool_dollars, 2), 'rest_per_person_dollars', ROUND(v_rest_per_person, 2)),
    'wtw_bonus', jsonb_build_object('annual_dollars', ROUND(v_annual_wtw_bonus, 2), 'quarterly_dollars', ROUND(v_annual_wtw_bonus / 4.0, 2), 'weekly_dollars', ROUND(v_annual_wtw_bonus / 52.0, 2), 'per_hit_dollars', 10, 'formula', '$10 x team_tenure_ramp_sum x 13 x 4 (carve-and-forget)'),
    'gain_bonus', jsonb_build_object('annual_dollars', ROUND(v_annual_gain_bonus, 2), 'quarterly_dollars', ROUND(v_annual_gain_bonus / 4.0, 2), 'weekly_dollars', ROUND(v_annual_gain_bonus / 52.0, 2), 'per_hit_dollars', 10, 'formula', '$10 x team_tenure_ramp_sum x 13 x 4 (carve-and-forget)'),
    'leaderboard_bonus', jsonb_build_object('annual_dollars', ROUND(v_annual_leaderboard_bonus, 2), 'quarterly_dollars', ROUND(v_annual_leaderboard_bonus / 4.0, 2), 'weekly_dollars', ROUND(v_annual_leaderboard_bonus / 52.0, 2), 'per_hit_dollars', 10, 'formula', '$10 x team_tenure_ramp_sum x 13 x 4 (carve-and-forget, uniform WtW shape)'),
    'all_star_bonus', jsonb_build_object('annual_dollars', ROUND(v_annual_all_star_bonus, 2), 'quarterly_dollars', ROUND(v_annual_all_star_bonus / 4.0, 2), 'weekly_dollars', ROUND(v_annual_all_star_bonus / 52.0, 2), 'per_hit_dollars', 10, 'formula', '$10 x team_tenure_ramp_sum x 13 x 4 (carve-and-forget, uniform WtW shape)'),
    'trailblazer_bonus', jsonb_build_object('annual_dollars', ROUND(v_annual_trailblazer_bonus, 2), 'quarterly_dollars', ROUND(v_annual_trailblazer_bonus / 4.0, 2), 'weekly_dollars', ROUND(v_annual_trailblazer_bonus / 52.0, 2), 'per_hit_dollars', 10, 'formula', '$10 x team_tenure_ramp_sum x 13 x 4 (carve-and-forget, uniform WtW shape)'),
