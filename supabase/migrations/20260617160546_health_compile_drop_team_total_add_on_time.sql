-- Health compile footer changes:
--   - Drop "Team: X/Y workouts" line entirely
--   - Change "X/Y at goal" → "X/Y at goal or on time"
--     where "on time" = on pace to hit 5 by week's end:
--       threshold = max(0, dow - 1)
--       (Sun dow=0 → 0, Mon dow=1 → 0, Tue → 1, Wed → 2, Thu → 3, Fri → 4, Sat → 5)
--     So Saturday "on time" collapses to "at goal" (must be at 5).
...
BEGIN
  ...
  LOOP
    ...
    -- Count "at goal or on time" (at goal counts always; on time means on pace given the day)
  ...
  -- New footer: drop Team workouts total, switch "at goal" to "at goal or on time"
  ...
  RETURN jsonb_build_object('records_processed', v_responded_count,
    'output_summary', format('health_eve compile (dow=%s): %s/%s on pace, %s/%s reporting',
      v_dow, v_on_pace, v_expected_count, v_responded_count, v_expected_count));
END;
$function$;
