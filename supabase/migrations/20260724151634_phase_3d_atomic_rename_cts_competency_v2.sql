...
BEGIN
  ...
  -- (while _v2 names still resolve; the regex replaces the text, doesn't need resolution)
  FOR r IN
    SELECT proname,
  ...
  IF jsonb_object_keys(transformed_defs) IS NULL OR (SELECT count(*) FROM jsonb_object_keys(transformed_defs)) <> 9 THEN
    RAISE EXCEPTION 'Phase 3D: expected 9 consumer defs, captured %', (SELECT count(*) FROM jsonb_object_keys(transformed_defs));
  END IF;
