-- Migration: payroll_gl_writer_plan_a_routing_expanded
-- Applied 2026-07-18 via Supabase MCP.
-- Purpose: Rewrite payroll_gl_writer with Plan A routing (Owner OR is_admin_backoffice OR
-- business_entity_id=PaperNewt → PN direct; else agency-split with growth/team ramp).
-- Replaces prior narrow routing that only checked business_entity_id.
