-- Dial rate 5% → 3% per Peter (2026-07-12 pm4). Single-constant change in both functions.
...
DECLARE
  ...
  -- Rate + pace shared by both trip pot and prize cart (locked 2026-07-12 pm4).
  v_rate                CONSTANT numeric := 0.03;
  v_pace                numeric := 0;
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'mvp_prize_cart', jsonb_build_object(
      ...
      'formula',        '3% × on-time (SMVC + Scorecard) × on-time pace (curr wins / week of cycle, capped 1.0)',
      'rate_pct',       v_rate,
      'pace',           ROUND(v_pace, 4),
...
-- quarter_close constants also dial to 3%
...
BEGIN
  ...
  VALUES (p_agency_id, v_next_q_end, v_next_budget,
          format('3%% × on-time (SMVC $%s + Scorecard $%s) × pace %s/13 = $%s',
  ...
  VALUES (
    ...
    format('Quarter closed %s. %s prizes carried ($%s total). Budget $%s (3%% × OT basis × %s wins/13 = pace %s). Available for new prizes: $%s.',
...
-- Now REFRESH the stored residual_pool_diag for week 2026-07-11 so CPR page sees the new values.
SELECT public.write_weekly_comp_v2('126794dd-25ff-47d2-a436-724499733365', '2026-07-11');
