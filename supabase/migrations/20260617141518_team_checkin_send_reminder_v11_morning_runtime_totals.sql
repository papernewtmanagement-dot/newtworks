-- v11: morning recap now reads team_checkins directly and computes the team
-- total at runtime. No dependency on stored total_quotes_week. Uses the most
-- recent EOD checkin_date < today regardless of whether the compile row was
-- written or has a (now-deprecated) stored total.
...
BEGIN
  ...
  IF v_checkin_type = 'morning' THEN
    ...
    -- Find most recent EOD checkin_date with data, from team_checkins directly
    ...
    IF v_last_eod_date IS NOT NULL THEN
      -- Build per-person breakdown and totals in one pass at runtime
      SELECT
        string_agg(
          format('• %s: %s/%s',
      ...
      v_text := v_text || format('📊 Last EOD (%s):',
        to_char(v_last_eod_date, 'Mon DD')) || E'\n';

      IF v_per_person IS NOT NULL THEN
        v_text := v_text || v_per_person || E'\n';
      END IF;

      v_text := v_text || format('Team total: %s/%s',
        v_total_q::text, v_total_s::text);
    ELSE
  ...
  IF v_checkin_type = 'eod' AND v_dow = 5 THEN
    v_text := v_text || E'\n\n━━━━━━━━━━━━━━━━━━━\n'
      ...
      || E'2. One goal for next week — 1% gain in sales points?\n'
      || E'3. One way to improve office efficiency?\n'
      || E'4. Brags for each teammate.\n\n'
      || E'━━━━━━━━━━━━━━━━━━━\n'
