-- time_off_eligibility_retention_half_requirement
-- Peter ruling 2026-08-25: retention Account/Unit Managers carry HALF the Sales Points
-- requirement (same shape as the long-standing Win-the-Week personal minimums: Sales 15
-- quotes / Retention 8). agency_sales_points_avg_13wk was made requirement-weighted in
-- migration agency_sales_points_avg_requirement_weighted; the INDIVIDUAL side of the same
-- gate was still rating every manager against the full bands. Now the rating is taken on
-- the requirement-relative figure (weekly average divided by the seat's requirement
-- weight), so a retention seat clears Good at half a sales seat's number. Bands, weights
-- and eligibility logic are otherwise untouched.
    ...
    ELSE
      ...
      ELSE
        ...
        IF NOT v_agency_passes THEN
          v_reasons := array_append(v_reasons,
            format('Agency Sales Points 13-wk avg is %s (%s points/wk per full requirement) — Good (%s+) or better required',
