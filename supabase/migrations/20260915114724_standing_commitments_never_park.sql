-- A standing commitment is meant to roll every week. Rolling is not neglect,
-- so the rollover and staleness parking rules must skip it, regardless of
-- importance. Without this, four of the six content groups would have parked
-- themselves after four weeks.
CREATE OR REPLACE FUNCTION public.is_standing_commitment(p_task_id uuid)
RETURNS boolean LANGUAGE sql STABLE AS $fn$
  SELECT EXISTS (SELECT 1 FROM public.tasks c
                  WHERE c.parent_task_id = p_task_id AND c.backlog_state = 'checklist');
$fn$;
  ...
  IF NOT p_dry_run THEN
    ...
    UPDATE tasks SET stuck_since=COALESCE(stuck_since,v_week)
      WHERE agency_id=p_agency_id AND status='open' AND backlog_state='active'
        ...
        AND NOT public.is_standing_commitment(id);
  ...
  WITH cand AS (
    SELECT t.id, t.assigned_to, t.title, t.priority, t.estimated_hours, u.full_name,
     ...
     WHERE t.agency_id=p_agency_id AND t.status='open' AND t.backlog_state='active'
       ...
       AND (t.task_type<>'epic' OR public.is_standing_commitment(t.id))
