...
BEGIN
  ...
  -- Normalize first name: trim edges, collapse internal whitespace runs to a single space.
  v_first_normalized := btrim(COALESCE(p_first_name, ''));
  v_first_normalized := regexp_replace(v_first_normalized, '\s+', ' ', 'g');

  IF v_first_normalized = '' THEN
    RAISE EXCEPTION 'First name is required.';
  END IF;
  IF v_first_normalized ~ '\.' THEN
    RAISE EXCEPTION 'First name cannot contain a period.';
  END IF;
  IF length(v_first_normalized) > 40 THEN
    RAISE EXCEPTION 'First name is too long (max 40 characters).';
  END IF;

  -- PFA masking rule: at most ONE internal space in the first name (so "Mary Jo" is fine,
  -- "Mary Jo Ann" is not). Hyphens inside a single word are allowed ("Mary-Jo-Ann" is fine).
  v_space_count := (length(v_first_normalized) - length(replace(v_first_normalized, ' ', '')));
  IF v_space_count > 1 THEN
    RAISE EXCEPTION
      'First name "%" has too many space-separated words. PFA records keep at most a 2-word first name (e.g. "Mary Jo"). Combine with a hyphen or shorten before submitting.',
      v_first_normalized;
  END IF;

  v_last_initial_normalized := btrim(COALESCE(p_last_initial, ''));
  IF v_last_initial_normalized !~ '^[A-Za-z]$' THEN
    RAISE EXCEPTION 'Last initial must be a single letter A-Z.';
  END IF;
  ...
  IF p_policy_type IS NULL OR p_policy_type NOT IN ('auto','fire','life','health','billing') THEN
    RAISE EXCEPTION 'Policy type must be one of: auto, fire, life, health, billing.';
  END IF;
  ...
  -- Final belt-and-suspenders: mirror the DB CHECK constraint here so any residual
  -- edge case gives a clean human message instead of a raw constraint violation.
  IF v_customer_name !~ '^[^\s]+([- ][^\s]+)?\s[A-Za-z]\.$' THEN
    RAISE EXCEPTION
      'Customer name "%" does not match the required masked format "Firstname L." — check the first name for unusual characters or spacing.',
      v_customer_name;
  END IF;
