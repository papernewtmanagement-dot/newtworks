-- HireGauge competency ceiling normalization (v6)
-- Applies universal invariant: sum(|coefficients|) = 1.0 for every competency.
-- Guarantees: max_base = exactly 100, min_base = 0, all-50-average-candidate = 50.
-- Preserves relative trait weights exactly (rescales all coefficients by 1/sum(|old|)).
-- LSS delta, reliability haircut, and distortion dampening adjust from this normalized base.

-- attention_to_detail: old sum_abs=0.7000, new intercept=14.285714, max=100, min=0, all-50=50
...
-- balances_logic_and_emotion_when_hiring: old sum_abs=0.8309, new intercept=0.164883, max=100, min=0, all-50=50
...
BEGIN
  ...
  ELSE v_base := GREATEST(0, LEAST(100, ROUND(
    (0.164883) + (+0.001658)*dm + (-0.001649)*rd + (+0.396563)*ass + (+0.199582)*is_val + (+0.195562)*an + (+0.096282)*com + (+0.007988)*sp + (+0.096282)*bo + (+0.004433)*op
  )::int)); END IF;
...
-- cadence_compliance: old sum_abs=0.8000, new intercept=18.750000, max=100, min=0, all-50=50
...
BEGIN
  IF dm IS NULL THEN v_base := NULL;
  ELSE v_base := GREATEST(0, LEAST(100, ROUND(
    (18.750000) + (+0.312500)*dm + (+0.187500)*an + (+0.187500)*rd + (+0.062500)*bo + (+0.062500)*op + (-0.125000)*is_val + (-0.062500)*sp
  )::int)); END IF;
...
-- composure_under_load: old sum_abs=0.6500, new intercept=7.692308, max=100, min=0, all-50=50
...
-- cross_sell_instinct: old sum_abs=0.9500, new intercept=5.263158, max=100, min=0, all-50=50
...
-- dials_cold_calls: old sum_abs=0.9500, new intercept=10.526316, max=100, min=0, all-50=50
...
-- handles_objections: old sum_abs=0.9211, new intercept=15.340336, max=100, min=0, all-50=50
...
BEGIN
  IF dm IS NULL THEN v_base := NULL;
  ELSE v_base := GREATEST(0, LEAST(100, ROUND(
    (15.340336) + (+0.001626)*dm + (+0.241711)*rd + (+0.230160)*ass + (+0.010265)*is_val + (+0.115970)*an + (-0.120833)*com + (+0.122745)*sp + (-0.032571)*bo + (+0.124119)*op
  )::int)); END IF;
...
-- handles_rejection: old sum_abs=0.9000, new intercept=11.111111, max=100, min=0, all-50=50
...
BEGIN
  IF dm IS NULL THEN v_base := NULL;
  ELSE v_base := GREATEST(0, LEAST(100, ROUND(
    (11.111111) + (+0.333333)*op + (+0.222222)*is_val + (+0.166667)*dm + (+0.111111)*ass + (+0.055556)*sp + (-0.055556)*rd + (-0.055556)*an
  )::int)); END IF;
...
-- is_fast_start_oriented: old sum_abs=1.0122, new intercept=0.618962, max=100, min=0, all-50=50
...
BEGIN
  IF dm IS NULL THEN v_base := NULL;
  ELSE v_base := GREATEST(0, LEAST(100, ROUND(
    (0.618962) + (+0.397550)*dm + (+0.198939)*rd + (+0.200105)*ass + (+0.196542)*is_val + (+0.000118)*an + (-0.003132)*com + (-0.001366)*sp + (-0.001691)*bo + (+0.000556)*op
  )::int)); END IF;
...
-- listens_discovers_needs: old sum_abs=1.0000, new intercept=25.000000, max=100, min=0, all-50=50
...
BEGIN
  ...
  ELSE v_base := GREATEST(0, LEAST(100, ROUND(
    (25.000000) + (+0.150000)*rd + (+0.150000)*ass + (+0.250000)*com + (+0.200000)*bo + (-0.150000)*an + (-0.100000)*sp
  )::int)); END IF;
...
-- maintains_high_activity: old sum_abs=0.8230, new intercept=13.113561, max=100, min=0, all-50=50
...
BEGIN
  IF dm IS NULL THEN v_base := NULL;
  ELSE v_base := GREATEST(0, LEAST(100, ROUND(
    (13.113561) + (+0.346507)*dm + (+0.175233)*rd + (+0.169687)*ass + (+0.173621)*is_val + (-0.060753)*an + (-0.060753)*com + (-0.005219)*sp + (-0.004411)*bo + (+0.003817)*op
  )::int)); END IF;
...
-- makes_decisions_quickly: old sum_abs=0.8112, new intercept=12.566941, max=100, min=0, all-50=50
...
BEGIN
  IF dm IS NULL THEN v_base := NULL;
  ELSE v_base := GREATEST(0, LEAST(100, ROUND(
    (12.566941) + (+0.177999)*dm + (+0.001995)*rd + (+0.172868)*ass + (+0.169064)*is_val + (-0.061640)*an + (-0.061640)*com + (+0.181403)*sp + (-0.002390)*bo + (+0.171003)*op
  )::int)); END IF;
...
-- manages_time_effectively: old sum_abs=0.7902, new intercept=7.883367, max=100, min=0, all-50=50
...
BEGIN
  IF dm IS NULL THEN v_base := NULL;
  ELSE v_base := GREATEST(0, LEAST(100, ROUND(
    (7.883367) + (+0.212534)*dm + (+0.215730)*rd + (+0.219491)*ass + (+0.207672)*is_val + (+0.063278)*an + (-0.063278)*com + (+0.002463)*sp + (-0.008749)*bo + (-0.006807)*op
  )::int)); END IF;
...
-- pivots_to_customer_need: old sum_abs=0.9500, new intercept=10.526316, max=100, min=0, all-50=50
...
-- positively_influences_team: old sum_abs=0.9500, new intercept=10.526316, max=100, min=0, all-50=50
...
BEGIN
  ...
  ELSE v_base := GREATEST(0, LEAST(100, ROUND(
    (10.526316) + (+0.315789)*op + (+0.263158)*com + (+0.210526)*bo + (+0.105263)*ass + (-0.105263)*is_val
  )::int)); END IF;
...
-- presents_solutions: old sum_abs=0.9500, new intercept=0.000000, max=100, min=0, all-50=50
...
BEGIN
  IF rd IS NULL THEN v_base := NULL;
  ELSE v_base := GREATEST(0, LEAST(100, ROUND(
    (0.000000) + (+0.263158)*rd + (+0.263158)*ass + (+0.157895)*sp + (+0.210526)*an + (+0.105263)*com
  )::int)); END IF;
...
-- proactive_touch_discipline: old sum_abs=0.8500, new intercept=5.882353, max=100, min=0, all-50=50
...
BEGIN
  IF dm IS NULL THEN v_base := NULL;
  ELSE v_base := GREATEST(0, LEAST(100, ROUND(
    (5.882353) + (+0.294118)*dm + (+0.176471)*an + (+0.176471)*com + (+0.117647)*rd + (+0.117647)*ass + (+0.058824)*op + (-0.058824)*sp
  )::int)); END IF;
...
-- prospects_in_community: old sum_abs=1.0500, new intercept=4.761905, max=100, min=0, all-50=50
...
BEGIN
  IF dm IS NULL THEN v_base := NULL;
  ELSE v_base := GREATEST(0, LEAST(100, ROUND(
    (4.761905) + (+0.190476)*ass + (+0.190476)*op + (+0.142857)*com + (+0.142857)*bo + (+0.095238)*rd + (+0.095238)*is_val + (+0.047619)*dm + (+0.047619)*sp + (-0.047619)*an
  )::int)); END IF;
...
-- queue_throughput_discipline: old sum_abs=0.8500, new intercept=17.647059, max=100, min=0, all-50=50
...
BEGIN
  IF dm IS NULL THEN v_base := NULL;
  ELSE v_base := GREATEST(0, LEAST(100, ROUND(
    (17.647059) + (+0.294118)*dm + (+0.176471)*an + (+0.176471)*is_val + (+0.117647)*rd + (+0.058824)*op + (-0.058824)*com + (-0.117647)*sp
  )::int)); END IF;
...
-- rapid_rapport_warm: old sum_abs=0.9000, new intercept=16.666667, max=100, min=0, all-50=50
...
BEGIN
  IF com IS NULL THEN v_base := NULL;
  ELSE v_base := GREATEST(0, LEAST(100, ROUND(
    (16.666667) + (+0.333333)*com + (+0.222222)*op + (+0.222222)*bo + (+0.055556)*ass + (-0.111111)*an + (-0.055556)*sp
  )::int)); END IF;
...
-- receives_coaching: old sum_abs=0.9229, new intercept=28.014043, max=100, min=0, all-50=50
...
BEGIN
  IF com IS NULL THEN v_base := NULL;
  ELSE v_base := GREATEST(0, LEAST(100, ROUND(
    (28.014043) + (-0.005820)*dm + (+0.118889)*rd + (+0.123047)*ass + (-0.119075)*is_val + (-0.032507)*an + (+0.235150)*com + (-0.122739)*sp + (+0.122602)*bo + (+0.120172)*op
  )::int)); END IF;
...
-- retention_watchfulness: old sum_abs=0.7000, new intercept=14.285714, max=100, min=0, all-50=50
...
-- routing_judgment: old sum_abs=1.0000, new intercept=25.000000, max=100, min=0, all-50=50
