-- Two changes, one rebuild (a new output column forces a drop):
--  1. The debit/credit sign rule was written out inline here and would have been
--     repeated in the tithe pool. It now lives only in ledger_pnl_amount, which
--     this calls. Checked against all 2,439 ledger rows before the swap:
--     zero differences.
--  2. Returns tithe_draw_amount so the P&L drill can show and set how much of a
--     transaction comes out of the tithe pool.
-- Callers checked before dropping: no database function calls this
-- (pg_get_functiondef sweep, zero hits) and exactly one app file does,
-- src/modules/Financials.jsx, which reads fields by name.

DROP FUNCTION IF EXISTS public.pnl_drill_transactions(uuid, text, text, text, date, date);

CREATE FUNCTION public.pnl_drill_transactions(
...
RETURNS TABLE(source text, je_id uuid, line_id uuid, pyp_id uuid, entry_date date,
              amount numeric, description text, memo text, reference_number text,
              je_source text, classification_status text, account_id uuid,
              account_code text, account_name text, document_id uuid,
              created_at timestamp with time zone, confirmation text,
              cash_register_id uuid, business_entity_id uuid,
              tithe_draw_amount numeric)
LANGUAGE sql
STABLE
AS $function$
  WITH journal_side AS (
    SELECT
      ...
      NULL::uuid AS pyp_id,
      l.entry_date,
      public.ledger_pnl_amount(l.debit, l.credit, coa.account_type, l.source) AS amount,
      ...
      l.tithe_draw_amount AS tithe_draw_amount
  ...
  pyp_side AS (
    SELECT
      ...
      NULL::numeric AS tithe_draw_amount
  ...
  SELECT source, je_id, line_id, pyp_id, entry_date, amount, description, memo,
         ...
         account_code, account_name, document_id, created_at, confirmation,
         cash_register_id, business_entity_id, tithe_draw_amount
  FROM combined
