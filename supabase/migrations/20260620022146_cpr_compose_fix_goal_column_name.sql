-- Surgical fix: only the column reference. Full function body is otherwise unchanged.
-- Replace 'goal_value' → 'target_value' inside the Agency Performance lines CTE.
-- We rebuild the function via a wrapper that calls the original body with a replaced source.
-- Simpler: just redeploy with the fix.
...
BEGIN
  ...
  -- AGENCY PERFORMANCE: target_value (not goal_value) — fixed
  SELECT * INTO v_snap
