-- Kill the single-date GL cutover concept in bank + card views.
-- Each account is now anchored to its own latest statement (statement_balances row).
-- Activity since anchor = journal_lines dated AFTER that account's statement close.
-- Register = anchor + activity_since_anchor.
-- next_statement_expected_date + statement_overdue exposed for cycle indicator.
