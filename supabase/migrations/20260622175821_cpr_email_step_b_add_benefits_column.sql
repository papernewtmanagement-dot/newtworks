-- Step B: add Benefits column to Payroll table.
-- Benefits weekly = team.annual_benefits_value / 52. Uses cpr_fmt_money for zero→dash.
-- All other payroll cells unchanged (those convert in Step C).
-- Week Total math updated to include the benefits weekly slice.
...
BEGIN
  ...
  -- PAYROLL rows: Benefits column added between Health and Retention (cpr_fmt_money).
  -- Week Total math now includes benefits weekly slice.
  SELECT string_agg(
    '<tr>'
    || '<td style="padding:6px 10px;border-bottom:1px solid #e5e7eb;font-weight:600;color:#1e293b">' || COALESCE(NULLIF(t.nickname,''), t.first_name) || '</td>'
    ...
    || '<td style="padding:6px 10px;text-align:right;border-bottom:1px solid #e5e7eb;color:#334155">' || public.cpr_fmt_money(COALESCE(t.annual_benefits_value, 0) / 52.0) || '</td>'
