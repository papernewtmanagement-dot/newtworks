-- leslie_monthly_goals_send is sending VIA paper_newt_bot, so it must consult
-- the paper_newt_bot exclusion flag, not the legacy is_excluded (which now governs pjsagencybot).
...
BEGIN
  ...
  -- Paper_newt_bot channel: consult the paper_newt_bot flag, not legacy is_excluded.
