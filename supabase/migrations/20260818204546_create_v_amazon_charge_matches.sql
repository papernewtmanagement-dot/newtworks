-- Links an Amazon charge sitting in the ledger to the Amazon order that produced it,
-- and works out the right category from what was actually in the box.
-- Read-only. Nothing here changes the ledger.
...
WITH amz_ledger AS (
  SELECT l.id AS ledger_id, l.agency_id, l.entry_date, l.description,
         ...
         l.source, l.statement_id, l.cash_register_id,
...
order_majority AS (
  SELECT DISTINCT ON (i.order_id)
         i.order_id, i.gl_account_code, i.category_label
...
order_shape AS (
  SELECT o.order_id, o.agency_id, o.order_date::date AS order_date, o.grand_total,
         ...
         string_agg(
           left(i.product_name, 50) || ' [' || COALESCE(i.gl_account_code,'?') || ' $' || i.total_amount || ']',
           '; ' ORDER BY i.total_amount DESC
         ) AS item_breakdown
  FROM amazon_orders o
  JOIN amazon_order_items i USING (order_id)
  GROUP BY o.order_id, o.agency_id, o.order_date, o.grand_total,
           o.payment_card_last4, o.target_business_entity_id, o.matched_ledger_id
),
paired AS (
  SELECT al.*, os.order_id, os.order_date, os.grand_total, os.target_business_entity_id,
         os.item_count, os.distinct_categories, os.uncoded_items, os.item_breakdown,
         os.matched_ledger_id AS order_already_linked_to,
  ...
  FROM amz_ledger al
  JOIN order_shape os
    ON os.agency_id = al.agency_id
   AND os.grand_total = al.charge_amount
   AND os.payment_card_last4 IN (
         SELECT unnest(array_append(COALESCE(al.alternate_last4s, '{}'::text[]), al.card_last4))
       )
   AND os.order_date BETWEEN al.entry_date - INTERVAL '10 days'
                         AND al.entry_date + INTERVAL '2 days'
  LEFT JOIN order_majority om USING (order_id)
)
SELECT ledger_id, entry_date, description, charge_amount, source,
       current_code, current_name, card_last4,
       order_id, order_date, target_business_entity_id,
       proposed_code, proposed_label,
       item_count, distinct_categories, uncoded_items, item_breakdown,
       candidate_orders, order_already_linked_to,
       CASE
         WHEN candidate_orders > 1         THEN 'ambiguous: more than one order fits'
