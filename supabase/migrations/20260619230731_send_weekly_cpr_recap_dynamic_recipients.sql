-- Per Peter 2026-06-19: recipients pulled dynamically from team table.
-- Any team row that is (a) scoped to this agency, (b) category='agency',
-- (c) is_active=true, (d) archived_at IS NULL, (e) has a non-null email_sf
-- gets the recap. Order: team by hire_date ascending, then Owner last
-- (so Peter is on the email as a recipient but the team is the primary audience).
...
BEGIN
  ...
  -- DYNAMIC RECIPIENT LOOKUP: pull every active agency team member with an SF email.
  -- Order: Owner LAST (Peter on the email but team is the audience), then earliest hire first.
