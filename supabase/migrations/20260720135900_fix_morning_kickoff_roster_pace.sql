-- Migration: three fixes to the morning kickoff Telegram message
--
-- 1. render_team_status_block: roster looked up via p_as_of_date. On Monday
--    morning that's Friday (last EOD), which excludes anyone who was on
--    approved full-day PTO Friday. Effect: on Mon 2026-07-20 kickoff, John was
--    dropped from the "EOD Jul 17" block because he was on PTO Friday, even
--    though he's back on the roster today. Fix: use v_wtw_date (viewer's "now")
--    for roster lookup; keep p_as_of_date for check-in data selection.
--
-- 2. render_team_status_block: encouragement pool boundary bug at pace=0.
--    When pace=0 (Mon-morning, Sun): target * pace = 0, so `actual >= 0` is
--    trivially true. On Mon 2026-07-20 08:25 CT: quotes 0/38 with pace=0 gave
--    v_q_pace_pass=true → pool picked "Ahead on both. Guard the lead." which
--    is misleading at the start of the week. Fix: add a dedicated pre-work
--    pool for v_pace<=0 (Sunday all day + Mon-Fri before noon on day 1),
--    branched before the pace-pass tree.
--
-- 3. team_checkin_send_reminder: call-log fallback only tried today-1 and
--    today-2. On Monday that misses Friday. Fri 2026-07-17's block was
--    populated (Tommy/Peter/Cassie/Stephanie), but Sun+Sat returned empty so
--    the block was dropped from the message. Fix: walk back up to 4 days from
--    today-1 until a populated block is found.
--
-- Signature stability: both replacements preserve arg lists exactly, so
-- CREATE OR REPLACE with same arity — no overload risk.
...
DECLARE
  ...
  -- Pre-work pool used when pace <= 0 (Sun all day; Mon-Fri before noon on day 1)
...
BEGIN
  ...
  -- Roster uses viewer's "now" (v_wtw_date), not the check-in target date.
  -- Rationale: someone off Friday but back Monday still belongs in the Monday-morning display,
  -- with their carried numbers surfaced through the carried CTE below.
  SELECT count(*) INTO v_expected
  FROM public.get_expected_teammates(p_agency_id, 'work_checkin', v_wtw_date);
  ...
  -- Encouragement pool: pre-work branch first (pace<=0 makes downstream trivially pass),
  -- then pace-adjusted branches.
  IF v_pace <= 0 THEN
