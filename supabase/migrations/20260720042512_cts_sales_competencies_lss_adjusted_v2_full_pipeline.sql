-- v2: fold distortion dampening + reliability regression into the mirror function.
-- Pipeline: dampen traits (CO/OP/BO if >65 and distortion moderate+) → base competencies → per-competency LSS delta → reliability regression.
-- Also: Handles Rejection weights dropped to 0.0/0.0 (truly immune to LSS per reasoning refinement).
-- Existing cts_sales_competencies base function still untouched.

-- Drop v1 (15-arg) so single overload lives on the 17-arg signature.
DROP FUNCTION IF EXISTS public.cts_sales_competencies_lss_adjusted(int,int,int,int,int,int,int,int,int,int,int,int,int,int,int);
...
DECLARE
  ...
  -- Helper: compute one competency's full pipeline result
  -- Returns jsonb {base, lss_delta, pre_reliability, final}
BEGIN
  -- ── Step 1: Distortion dampening on the 3 social-desirable traits
  ...
  -- ── Step 2: Base competencies from dampened traits
  ...
  v_pit := d_op::numeric;  -- Trait-copy uses dampened optimism

  -- ── Step 3: LSS in-range flags + per-competency additive deltas
  ...
  IF v_has_lss THEN
    -- Sales role vendor Compare Report ideal ranges
    v_acc_flags :=
    ...
    -- Per-competency: 15 * (w_acc*acc_signal + w_spd*spd_signal) / 2
    ...
    -- Handles Rejection: truly immune (0.0/0.0). Emotional resilience, not cognitive.
    v_adj_hr  := 0.0;
  END IF;

  -- ── Step 4: Reliability + distortion metadata for regression
  v_rel_factor := public._cts_reliability_confidence(p_reliability);
  v_dist_sev   := public._cts_distortion_severity(p_distortion);

  -- ── Assemble return jsonb.
  -- Per competency exposes:
  --   base:            competency from dampened traits (no LSS, no reliability regression)
  --   lss_delta:       raw LSS additive (no reliability regression)
  --   pre_reliability: clipped(base + lss_delta) — what the score would be if reliability=high
  --   final:           50 + (pre_reliability - 50) * reliability_factor, clipped 0-100
  RETURN jsonb_build_object(
    ...
    '_meta', jsonb_build_object(
      ...
      'role',                'sales',
      'model',               'sensitivity_weighted_v2_full_pipeline'
    )
  );
END;
$function$;
...
'Full-pipeline mirror: distortion dampening → base competencies → per-competency LSS delta → reliability regression. Returns per-competency {base, lss_delta, pre_reliability, final} + _meta. Existing cts_sales_competencies base function stays untouched. Peter approved 2026-07-19.';

-- Verify single overload
SELECT pg_get_function_identity_arguments(oid) AS sig
FROM pg_proc
WHERE pronamespace = 'public'::regnamespace
  AND proname = 'cts_sales_competencies_lss_adjusted';
