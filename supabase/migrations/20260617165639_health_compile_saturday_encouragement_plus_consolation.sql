-- Saturday health compile: rotating encouragement line when not everyone hit
-- goal (gentle humor, validates the effort without poaching the consolation
-- tail's "fresh slate" framing), and the "Week's in the books" tail returns
-- on every Saturday regardless of outcome.
...
BEGIN
  ...
  IF v_at_or_above_target = v_expected_count AND v_expected_count > 0 THEN
    -- Whole team at goal → celebration
    IF v_is_saturday THEN
  ...
  ELSIF v_is_saturday THEN
    -- Some missed on Saturday → rotating encouragement line
  ...
  -- Saturday consolation tail — always fires regardless of outcome
  IF v_is_saturday THEN
