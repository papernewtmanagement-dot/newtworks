-- Code Reds feed the per-person requirements count.
-- Each non-blank line in weekly_cpr_team_detail.code_reds adds +1 to that person's
-- "This Wk" requirements count (the `missed` column), at the same $1/quote cost as
-- team checklist misses and personal checklist misses.
--
-- Additive change: new returned column code_red_misses exposes the count on its own
-- so the composition of `missed` stays auditable. Existing columns keep their meaning.
-- All existing callers (weekly_cpr_compute_outcome, compose_weekly_cpr_html,
-- recompute_cpr_outcome, weekly_cpr_upsert_in_progress, audit_weekly_leaderboard_crossings,
-- compute_wtw_requirements_adjustment, materialize_standing_time_off) reference columns
-- by name, never positionally -- verified before the signature change.
...
CREATE OR REPLACE FUNCTION public.get_weekly_cpr_requirements(p_agency_id uuid, p_week_ending_date date)
 RETURNS TABLE(team_member_id uuid, carryover integer, personal_misses integer, team_misses integer, code_red_misses integer, missed integer, cost integer, total integer, modified integer, quotes_discussed integer, paid integer, owed integer, net_quotes integer)
 LANGUAGE plpgsql
AS $function$
...
BEGIN
  ...
  WHILE v_loop_week <= p_week_ending_date LOOP
    ...
    per_person AS (
      SELECT
        ...
        -- One non-blank line in code_reds = one extra quote owed.
        CASE WHEN d.id IS NULL OR d.code_reds IS NULL OR btrim(d.code_reds) = '' THEN 0
             ELSE (SELECT COUNT(*)
                     FROM regexp_split_to_table(d.code_reds, E'\r?\n') AS ln
                    WHERE btrim(ln) <> '')
        END::integer AS code_red_misses,
    ...
    per_person_derived AS (
      SELECT
        tm_id, carryover, personal_misses, team_misses, code_red_misses, modified, quotes_discussed,
        (team_misses + personal_misses + code_red_misses)::integer                                  AS missed,
        1::integer                                                                                   AS cost,
        ((carryover + team_misses + personal_misses + code_red_misses + modified) * 1)::integer       AS total
      FROM per_person
    ),
    team_totals AS (
      SELECT
    ...
    allocated AS (
      SELECT
        ppd.tm_id,
        ppd.carryover, ppd.personal_misses, ppd.team_misses, ppd.code_red_misses, ppd.missed,
    ...
    SELECT COALESCE(
      jsonb_object_agg(
        ...
        jsonb_build_object(
          ...
          'code_red_misses',  a.code_red_misses,
          'missed',           a.missed,
          'cost',             a.cost,
          'total',            a.total,
          'modified',         a.modified,
  ...
  SELECT
    ...
    (value->>'code_red_misses')::integer         AS code_red_misses,
...
-- Correct the whiteboard_errors comment: it does NOT feed the requirements count.
COMMENT ON COLUMN public.weekly_cpr_reports.whiteboard_errors IS
  'Free-form weekly whiteboard errors log, mirrors the eur column. Displayed to the right of EUR on the CPR page. Page-only, not in the digest email. Has NO effect on the requirements count (that is Code Reds, per weekly_cpr_team_detail.code_reds).';
