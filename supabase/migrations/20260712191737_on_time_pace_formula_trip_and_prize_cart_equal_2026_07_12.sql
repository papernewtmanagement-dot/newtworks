-- Both WtQ Trip pot and MVP Prize Cart pool use SAME formula: 5% × OT basis × on-time pace.
-- On-time pace = LEAST(1.0, curr_qtr_wins / week_of_cycle). If team is winning every week, pace = 100%, pool maxed.
-- Prize cart NO LONGER uses prior quarter wins/13. It uses current-quarter pace, same as trip pot.
-- Trip pot keeps its 9-win floor halt condition; prize cart has no floor (per handbook design).
...
DECLARE
  ...
  -- Rate + pace shared by both trip pot and prize cart (locked 2026-07-12).
...
BEGIN
  ...
  -- ON-TIME PACE for the current quarter.
  -- pace = curr_qtr_wins / weeks_elapsed, capped at 1.0.
  -- If team wins every week so far, pace = 1.0 (maxed pool). Falling behind pace shrinks pool proportionally.
  v_pace := LEAST(1.0, v_curr_qtr_wins::numeric / GREATEST(1, v_week_of_cycle)::numeric);
  v_pool_pace_dollars := v_rate * v_annual_ot_basis * v_pace;

  -- WtQ Trip: 5% × OT basis × on-time pace. Trip halts if math cannot reach 9-wins floor.
  ...
  -- MVP Prize Cart: 5% × OT basis × on-time pace. Same formula as WtQ Trip, no floor.
  -- Note: even if trip halts, prize cart still accrues (per handbook: cart restocks regardless of trip threshold).
  v_annual_mvp := v_pool_pace_dollars;

  -- WtQ split: 30% Quarter MVP, 70% pooled among REST OF THE TEAM (non-MVP teammates only), split evenly per head.
  ...
  RETURN jsonb_build_object(
    ...
    'inputs', jsonb_build_object(
      ...
      'pace_formula',                'LEAST(1.0, curr_qtr_wins / week_of_cycle)'
    ),
    ...
    'mvp_prize_cart', jsonb_build_object(
      ...
      'formula',        '5% × on-time (SMVC + Scorecard) × on-time pace (curr wins / week of cycle, capped 1.0)',
      'rate_pct',       v_rate,
      'pace',           ROUND(v_pace, 4),
    ...
    'wtq_trip', jsonb_build_object(
      ...
      'note',           'Same formula as MVP Prize Cart. Halts to $0 if team mathematically cannot hit 9-wins floor.',
...
-- quarter_close_prize_cart_and_leaderboards: same formula (at close, pace = closing_wins/13).
...
BEGIN
  ...
  -- Snapshot pool basis + closing quarter wins.
  ...
  -- On-time pace at close = closing_wins / 13 (weeks_elapsed = 13 at quarter end).
  ...
  VALUES (p_agency_id, v_next_q_end, v_next_budget,
          format('5%% × on-time (SMVC $%s + Scorecard $%s) × pace %s/13 = $%s',
  ...
  VALUES (
    ...
    format('Quarter closed %s. %s prizes carried ($%s total). Budget $%s (5%% × OT basis × %s wins/13 = pace %s). Available for new prizes: $%s.',
           p_quarter_ending_date::text, v_carried, v_carried_value_total::text,
           v_next_budget::text, v_closing_qtr_wins::text, ROUND(v_pace, 4)::text, v_available_budget::text)
  )
  ON CONFLICT (agency_id, quarter_ending_date) DO UPDATE
  ...
  VALUES (
    ...
    format('Quarter closed. %s prizes carried ($%s). Budget $%s (pace %s). Available for new prizes: $%s. '
           'Run Claude session with op-rule "Newtworks quarter-end prize cart research" to research + verify links + propose new items.',
           v_carried, v_carried_value_total::text, v_next_budget::text, ROUND(v_pace, 4)::text, v_available_budget::text),
    false
  );
...
-- Delete the stale $124 row (was computed under the wrong 7/11 formula).
-- Under the new design, CPR page displays live-computed values from compute_pool_carveouts,
-- not the stored quarter_prize_budgets row (which is now only used at quarter-close as a historical record).
DELETE FROM public.quarter_prize_budgets
WHERE agency_id = '126794dd-25ff-47d2-a436-724499733365'
  AND quarter_ending_date = '2026-10-03';
