-- CPR digest email composer — final state of this session.
-- Adds ⭐ All-Star Crossings + 🔥 Trailblazer Crossings block below the MVP block.
-- Renders on any week that has crossings (independent of Win-the-Week outcome).
-- Fixes prize-draw pluralization ("1 prize draw" vs "2 prize draws").
-- Corrects column names on trailblazer_crossings (crossing_value / threshold_at_crossing,
-- NOT value_at_crossing / floor_at_crossing which is the all_star_crossings shape).
...
BEGIN
  ...
  -- All-Star crossings (columns: value_at_crossing / floor_at_crossing).
  ...
  -- Trailblazer crossings (columns: crossing_value / threshold_at_crossing — different from all_star).
  ...
  FROM (
    SELECT
      ...
      '<div style="padding:4px 0;font-size:13px;color:#334155">' ||
        '<span style="font-weight:700;color:#1e293b">' || COALESCE(NULLIF(t.nickname,''), t.first_name) || '</span> — ' ||
        CASE tc.category
          WHEN 'week_quotes' THEN round(tc.crossing_value)::text || ' Weekly Quotes (record ' || round(tc.threshold_at_crossing)::text || ')'
          WHEN 'week_sp'      THEN '$' || to_char(tc.crossing_value, 'FM999,999,990') || ' Weekly Sales (record $' || to_char(tc.threshold_at_crossing, 'FM999,999,990') || ')'
          WHEN 'four_week_sp' THEN '$' || to_char(tc.crossing_value, 'FM999,999,990') || ' 4-Week Sales (record $' || to_char(tc.threshold_at_crossing, 'FM999,999,990') || ')'
          WHEN 'quarter_sp'   THEN '$' || to_char(tc.crossing_value, 'FM999,999,990') || ' Quarterly Sales (record $' || to_char(tc.threshold_at_crossing, 'FM999,999,990') || ')'
          ELSE tc.category || ' ' || round(tc.crossing_value)::text
        END ||
      '</div>' AS row_html
