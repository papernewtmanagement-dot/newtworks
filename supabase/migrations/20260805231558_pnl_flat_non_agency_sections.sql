...
-- This migration was applied live on 2026-08-05 23:15:58 UTC (ledger name: pnl_flat_non_agency_sections) but was never
-- committed to this repo at the time. Content below is copied verbatim
-- from supabase_migrations.schema_migrations.statements for version 20260805231558.
...
-- Income/Expense. Fix: subtype-based fallback now only fires for PSS;
-- everywhere else falls back straight to INITCAP(account_type) = 'Income'/'Expense'.
  ...
  journal_side AS (
    SELECT
      ...
      CASE
        ...
        WHEN coa.account_type = 'expense' THEN COALESCE(jl.debit,0) - COALESCE(jl.debit,0) - COALESCE(jl.credit,0) + COALESCE(jl.debit,0)
        ELSE 0
      END AS amount,
