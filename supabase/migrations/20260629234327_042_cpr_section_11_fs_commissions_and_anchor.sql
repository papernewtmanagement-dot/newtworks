-- Two fixes in get_cpr_section_11:
--   1) FS Commissions input wired to compute_fs_commissions_ytd (Life new + renewal
--      from comp_recap). Previously fed life_paid_for_premium_ytd which is PREMIUM,
--      not commissions. SMVC bucket spec is commissions.
--   2) p_as_of_date passed to compute_on_time_smvc_with_better_of now anchored to
--      LEAST(week_ending_date, snapshot_date) so annualization doesn't stretch stale
--      YTD across extra calendar days and understate pace.
--
...
BEGIN
  -- Latest snapshot row with YTD inputs
  ...
  -- Manual overrides for the target week, if any
  SELECT
  ...
  IF v_snap.snapshot_date IS NOT NULL OR v_has_overrides THEN
    -- Resolved values: manual override > snapshot > 0
