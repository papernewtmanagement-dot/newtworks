-- Peter lock 2026-08-25 (bands thread):
--  1. Unlimited paid time off is gated on the requester's OWN 13-week Sales
--     Points band only. The agency-average condition is removed: the team's
--     number already lives in Win the Week's team leg (compute_wtw_week_targets),
--     so gating an individual benefit on it was double-gating, and with two
--     sales seats one teammate's slump stripped the other's benefit.
--     Agency figures stay in the JSON output for display, informational only.
--  2. agency_sales_points_avg_13wk excludes the Owner (owner-role principle:
--     zero forward-looking production by design), unlicensed seats
--     (license_pc = false: structurally zero, cannot quote or bind), and seats
--     inside their first 13 weeks (ramp — same probation window the gate uses).
--     Retention seats stay in at face value: retention bands are designed at
--     half the sales numbers but NOT implemented yet.
-- Bands themselves live in sales_points_band_config (Danger <50, Caution 50-100,
-- Good 100-300, Great 300-500, Elite 500+ as of 2026-08-25).
...
AS $function$
  ...
  WHERE t.agency_id = p_agency_id
    ...
    AND (t.hire_date IS NULL OR t.hire_date <= p_end_date - 91);
$function$;
...
BEGIN
  ...
  -- Retention manager seats carry half the requirement; sales seats carry the whole one.
