-- v13: Morning recap appends a Win the Week comparison block tied to the
-- date of the last EOD shown. Midday/EOD reminders unchanged (they're just prompts).
-- Friday EOD wrapup tail unchanged.
...
BEGIN
  ...
  IF v_checkin_type = 'morning' THEN
    ...
    IF v_last_eod_date IS NOT NULL THEN
      ...
      -- Win the Week block (uses last EOD's date for target context)
      ...
      v_text := v_text || '  SP pace: ' || v_total_s::text || ' of ' || v_wtw.sp_target::text;
      IF v_sp_pass THEN v_text := v_text || '  ✅ cleared';
      ELSE v_text := v_text || '  —  ' || v_sp_short::text || ' to clear';
      END IF;
