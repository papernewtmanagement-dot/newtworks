-- Add a two-way framing paragraph to the initial invitation email HTML.
-- Peter directive 2026-07-29: candidates should understand from the outset that
-- the assessment isn't just Peter evaluating them — it's also a chance for them
-- to decide whether the role fits how they like to work. Reminder emails are
-- kept lean (they're nudges, not first impressions).
...
BEGIN
  ...
  -- Step 2b: Reminder sends — attempts 2 & 3
  FOR v_cand IN
    SELECT hc.id, hc.first_name, hc.last_name, hc.email, hc.position,
