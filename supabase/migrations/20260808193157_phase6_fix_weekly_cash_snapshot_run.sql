...
BEGIN
  ...
  FOR v_rec IN SELECT DISTINCT account_last4, account_type FROM public.v_weekly_cash_position LOOP
    IF v_rec.account_type = 'credit_card' THEN
      SELECT count(*) INTO v_match_count FROM public.accounts a
        WHERE a.account_kind = 'credit' AND a.is_active AND (a.account_number_last4 = v_rec.account_last4
              OR a.alternate_last4s::text ~ ('\m' || v_rec.account_last4 || '\M'));
    ELSE
      SELECT count(*) INTO v_match_count FROM public.accounts a
        WHERE a.account_kind = 'bank' AND a.is_active AND a.account_number_last4 = v_rec.account_last4;
    END IF;
  ...
  IF array_length(v_skip_accounts, 1) > 0 THEN
    ...
    LOOP
      IF v_rec.account_type = 'credit_card' THEN
        SELECT a.account_name INTO v_account_name FROM public.accounts a
          WHERE a.account_kind = 'credit' AND a.is_active AND (a.account_number_last4 = v_rec.account_last4
                OR a.alternate_last4s::text ~ ('\m' || v_rec.account_last4 || '\M')) LIMIT 1;
      ELSE
        SELECT a.account_name INTO v_account_name FROM public.accounts a
          WHERE a.account_kind = 'bank' AND a.is_active AND a.account_number_last4 = v_rec.account_last4 LIMIT 1;
      END IF;
  ...
  LOOP
    IF v_agg.account_type = 'credit_card' THEN
      SELECT a.business_entity_id, a.account_name INTO v_entity_id, v_account_name
        FROM public.accounts a WHERE a.account_kind = 'credit' AND a.is_active
        AND (a.account_number_last4 = v_agg.account_last4 OR a.alternate_last4s::text ~ ('\m' || v_agg.account_last4 || '\M'))
        LIMIT 1;
    ELSE
      SELECT a.business_entity_id, a.account_name INTO v_entity_id, v_account_name
        FROM public.accounts a WHERE a.account_kind = 'bank' AND a.is_active AND a.account_number_last4 = v_agg.account_last4
        LIMIT 1;
    END IF;
