...
-- 1) New helper: agency renewal TTM
...
-- 2) Column adds on weekly_cpr_team_detail
...
-- 3) Rewrite compute_warning_trigger (semantics expanded; name kept for backward compat)
...
DECLARE
  ...
  v_retention_split     CONSTANT numeric := 0.35;    -- core_principles #700 joint attribution
  v_stack_producer_pct  CONSTANT numeric := 0.65;    -- 1 - v_retention_split
...
-- 4) Update write_weekly_comp_v2 to write new columns
...
BEGIN
  ...
  wt_upd AS (
    ...
    SET
      -- new columns
      fully_loaded_annual         = w.fully_loaded_annual,
      ...
      -- legacy columns (populated from new for backward compat)
