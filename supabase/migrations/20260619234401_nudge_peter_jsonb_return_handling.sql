...
DECLARE
  ...
  v_message       text;
  v_state         text;
  v_send_resp     jsonb;                 -- *** FIX: jsonb not bigint ***
  v_result        jsonb;
BEGIN
  ...
  v_send_resp := public.telegram_send_message(v_peter_chat_id, v_message, 'Markdown');

  -- Handle Telegram API failure (e.g. user hasn't /start-ed the bot → 403 Forbidden)
  IF v_send_resp IS NULL OR (v_send_resp->>'ok')::boolean IS NOT TRUE THEN
    ...
    VALUES (
      ...
      'Telegram send failed: ' || coalesce(v_send_resp->>'description', 'unknown'),
