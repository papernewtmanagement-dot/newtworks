-- Tier-3 DRY (comp engine): compute_pool_carveouts now uses get_expected_teammates.
--
-- 4 roster scans consolidated:
--   Scan 1 (Manager Bonus)      -> time_off_participant + WHERE role_level IN (managers)
--   Scan 2 (Life Insurance)     -> time_off_participant + WHERE start_date IS NOT NULL
--   Scan 3 (Apparel)            -> time_off_participant + WHERE start_date IS NOT NULL
--   Scan 4 (Health Dev Bonus)   -> time_off_participant
--
-- All 4 scans previously hand-rolled: category='agency' + role_level<>'Owner' +
-- is_active=true + is_admin_backoffice=false + archived_at IS NULL. Canonical
-- adds is_test_user IS NOT TRUE (baseline) and switches archived-check to
-- as_of=p_week_end_date. Behavioral parity verified 2026-07-08 against live agency:
-- all 3 scans (manager/lifeins/hdb) return identical row counts (1/4/4) under
-- hand-rolled vs canonical filters. Zero numerical drift.
--
-- Parity baseline captured in public._tier3_comp_parity BEFORE this migration;
-- post-migration output will be diffed row-by-row.
...
BEGIN
  ...
  -- LIFE INSURANCE STIPEND (roster via canonical: time_off_participant, start_date required)
  SELECT
  ...
  -- APPAREL (roster via canonical: time_off_participant, start_date required)
  SELECT
  ...
  -- HEALTH DEVELOPMENT BONUS (roster via canonical: time_off_participant)
  -- $25/week x 52 weeks per active non-owner team member (structural max).
  SELECT
