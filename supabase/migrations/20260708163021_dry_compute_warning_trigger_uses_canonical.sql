-- Tier-3 DRY: compute_warning_trigger now uses get_expected_teammates('time_off_participant').
--
...
--
-- Behavioral note: original had NO archived_at filter (only is_active=true).
-- get_expected_teammates always applies the archived_at check via as_of_date.
-- Net effect: someone archived-but-somehow-still-active would previously slip
-- through and now gets excluded. Consistent-improvement, not regression.
