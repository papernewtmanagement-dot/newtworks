-- Peter 2026-09-18: History should filter appointments the same way it filters
-- activities and sales. Appointments were being pulled separately inside the
-- customer account popup, which meant History could not see them at all. The
-- appointment select moves into the shared row function, so History, its
-- filter, and the account popup all read the same six kinds from one place,
-- and the popup stops carrying its own copy.
...
-- Appointments now arrive with everything else, so the popup's own copy goes.
...
BEGIN
  ...
  tl2 AS (
    SELECT e.kind, e.id, e.occurred_on, btrim(COALESCE(e.summary, '')) AS summary,
           e.amount, e.issued_amount, e.created_at,
           e.entry_source, e.note, e.ecrm_url, e.meta, e.customer_label, e.phone_last4,
           COALESCE(t.nickname, t.first_name, 'Unknown') AS who,
           (a.is_admin OR (e.team_member_id = a.actor_id
                           AND (e.week_end_date >= v_week OR e.created_at >= v_today))) AS can_change
      FROM ent e LEFT JOIN public.team t ON t.id = e.team_member_id
  ),
  pol AS (
  ...
  SELECT jsonb_build_object(
    ...
    'customer', jsonb_build_object(
      ...
      'phone_last4', v_phone,
      'first_seen', (SELECT min(x.occurred_on) FROM ent x),
      'last_seen', (SELECT max(x.occurred_on) FROM ent x),
      ...
      'marketing_source', (SELECT e.meta->>'marketing_source' FROM ent e
                            ...
                            ORDER BY e.occurred_on DESC LIMIT 1),
      'phones', COALESCE((SELECT jsonb_agg(DISTINCT x.phone_last4) FROM ent x WHERE x.phone_last4 IS NOT NULL), '[]'::jsonb)
    ),
    'totals', jsonb_build_object(
      ...
      'appointments', (SELECT count(*) FROM ent e WHERE e.kind = 'appointment'),
