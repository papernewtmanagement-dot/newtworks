...
-- payroll_gl_writer: remove all writes to journal_entries.business_entity_id
-- and journal_lines.business_entity_id. Entity flows through the account_id in
-- each line (chart_of_accounts.business_entity_id is the source of truth).
-- Reads from team.business_entity_id are unchanged — that's the routing decision
-- for which entity's cash-pool pays which team member and is a legitimate
-- team-table concern, unrelated to the JE/JL entity redundancy.
