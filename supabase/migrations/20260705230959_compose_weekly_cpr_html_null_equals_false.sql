-- Fix: compose_weekly_cpr_html team-checklist hit count used COALESCE(x, true) → NULL counted as ✓.
-- This produced wrong "Hit: 11 of 11" header, wrong quote-owed penalty, wrong ✓ mark on team checklist.
-- Personal hit count used CASE WHEN x THEN — semantically correct (NULL→0=miss) but standardized to IS TRUE for consistency.
...
BEGIN
  ...
  -- 2026-07-05: NULL = false. Only IS TRUE counts as hit.
  ...
  IF v_snap.id IS NOT NULL THEN
    WITH lines AS (
      SELECT 1 AS ord, 'Auto Gain' AS label,
             ...
             false AS is_money
      UNION ALL SELECT 2, 'Fire Gain', COALESCE(v_snap.fire_new_ytd,0) - COALESCE(v_snap.fire_lost_ytd,0),
      ...
      UNION ALL SELECT 3, 'Life Gain', COALESCE(v_snap.life_new_ytd,0) - COALESCE(v_snap.life_lost_ytd,0),
      ...
      UNION ALL SELECT 4, 'Life Paid #', COALESCE(v_snap.life_paid_for_count_ytd,0),
      ...
      UNION ALL SELECT 5, 'Life Premium', COALESCE(v_snap.life_paid_for_premium_ytd,0)::numeric,
