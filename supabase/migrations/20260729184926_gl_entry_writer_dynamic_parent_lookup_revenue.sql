-- Patch gl_entry_writer revenue-path JOIN: use dynamic parent lookup so pet/IPS/US Bank
-- categories can resolve to their real accounts under Alliances/IPS parents (not just State Farm).
CREATE OR REPLACE FUNCTION public.gl_entry_writer(p_agency_id uuid, p_dry_run boolean DEFAULT false)
 RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
DECLARE
  v_cash_acct_name text; v_sf_parent_name text;
  v_cash_acct_id uuid; v_sf_parent_id uuid;
  ...
  LOOP
    ...
    IF v_is_deduction THEN
      ...
      JOIN chart_of_accounts coa
        ON coa.agency_id = m.agency_id AND coa.account_name = m.source_account_name
        AND coa.parent_account_id = (
          SELECT p.id FROM chart_of_accounts p
          WHERE p.agency_id = m.agency_id AND p.account_name = m.source_parent_account_name
    ...
    ELSE
      -- REVENUE path: dynamic parent lookup (matches deduction pattern).
      -- Allows pet/IPS/Alliance accounts to resolve to their actual parents,
      -- not just accounts under "State Farm".
      SELECT coa.id, coa.account_name INTO v_target_account_id, v_target_account_name
...
GRANT EXECUTE ON FUNCTION public.gl_entry_writer(uuid, boolean) TO authenticated;
