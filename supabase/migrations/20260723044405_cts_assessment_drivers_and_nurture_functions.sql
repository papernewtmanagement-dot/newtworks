-- Wrap the view's current inline math for assessment drivers + nurture into two functions.
-- Behavior unchanged; math moved from view derivations into named functions.

-- Function 1: assessment drivers cell.
-- Arithmetic mean of the three drivers-related CTS traits, rounded to 2dp.
-- Any NULL input propagates to NULL (matches current view behavior).
...
-- Function 2: assessment nurture cell.
-- Averages non-null values of three sub-components:
--   honesty    <- response_distortion mapping (low=85, moderate=50, high=15, else NULL)
--   concern    <- compassion*0.7 + belief_in_others*0.3 (or whichever is non-null; else NULL)
--   work_ethic <- reliability mapping (high=85, moderate=50, low=15, else NULL)
-- Combined = sum of non-null / count of non-null, rounded to 2dp.
...
-- Rebuild v_hiring_candidates: replace the inline drivers arithmetic and the
-- inline nurture CTE with calls to the two functions. Sub-part columns
-- (assessment_nurture_honesty/concern/work_ethic) still exposed via the
-- CASE expressions in the lateral join (single source of the raw signals).
