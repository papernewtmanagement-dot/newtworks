-- telegram_recover_checkins(date,text) was dropped in 20260914183211, but three
-- functions still called it. team_health_checkin_compile runs tonight at 19:59
-- and would have failed outright. Strip the call everywhere.
...
BEGIN
  ...
  LOOP
    v_status := CASE
      ...
      ELSE v_row.hits || '/' || v_target || ' ' || (ARRAY['😴','🥱'])[1 + ((v_lazy_start + v_behind_n) % 2)]
    END;
    IF v_row.hits < v_target AND v_row.hits < v_on_time_threshold THEN
  ...
  -- LOCKED TEMPLATE (Peter 2026-09-10). Do not change the header or line format
  ...
  -- LOCKED (Peter 2026-09-10): the health summary takes down the health
  -- reminder, and any nag, before it posts.
  PERFORM public.team_checkin_delete_message(
    p_agency_id, v_chat_id, 'health_eve', 'reminder', v_today);
  PERFORM public.team_checkin_delete_message(
    p_agency_id, v_chat_id, 'health_eve', 'nag', v_today);
...
-- The deactivated compile handler still referenced the dropped function too.
DROP FUNCTION IF EXISTS public.team_checkin_compile_results(uuid, uuid);