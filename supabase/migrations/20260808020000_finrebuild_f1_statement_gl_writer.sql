-- finrebuild_f1_statement_gl_writer
-- Phase 3.1: statement_gl_writer implements D3 (single ledger line per
-- statement transaction, no offset pair), D9 (entity-scoped account
-- resolution), D12 (_NO_JE marker), D13 (no category guessing), D15
-- (statement-native sign, direction from transaction_type only, never from
-- amount magnitude). Rule-matching mechanics (direct category match, then
-- rule match by ascending match_priority, entity-scoped resolution with
-- any-entity fallback, __SOURCE__ sentinel exclusion) carried over unchanged
-- from cc_gl_writer's proven algorithm, redirected from the old
-- matched-pair journal_entries/journal_lines write to a single ledger row.
...
BEGIN
  ...
  LOOP
    ...
    -- Idempotency: never write a second ledger row for a statement_id that
    -- already has one.
    ...
    -- D12: _NO_JE marker -> skip silently.
    IF v_category IS NOT NULL AND upper(trim(v_category)) LIKE '%\_NO\_JE' THEN
      v_count_skipped_no_je := v_count_skipped_no_je + 1;
      CONTINUE;
    END IF;

    -- D15: direction from transaction_type only, never from amount sign.
    -- Includes legacy vocabulary from the 1,542 merge-era rows (verified
    -- against real data, not guessed): bank 'credit'(amount>=0 23 rows) /
    -- 'debit'(amount<0 8 rows); credit 'credit'(18 rows) and 'payment'
    -- (57 rows) both credit-direction, matching old cc_gl_writer's mapping.
    v_direction := CASE
    ...
    -- b. Direct category match (D9 entity-scoped), never an income account.
    ...
    -- c. Rule match. ORDER BY match_priority ASC — lower number wins.
    ...
    -- d. No match -> entity's unclassified account (D13). No guessing.
    IF v_target_account_id IS NULL THEN
  ...
  -- Top 20 unclassified merchants by summed amount (dry-run reporting aid).
