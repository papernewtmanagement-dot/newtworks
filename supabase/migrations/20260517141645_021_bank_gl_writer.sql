-- =============================================================================
-- MIGRATION 021: bank_gl_writer (sibling to gl_entry_writer / payroll_gl_writer)
-- =============================================================================
-- Walks bank_transactions, posts post-cutover rows to journal_entries with:
--   inflow (+ amount):  DR bank_account / CR resolved_category_account
--   outflow (- amount): DR resolved_category_account / CR bank_account
-- Resolution waterfall for the "other side":
...
--   3. Fallback → COA-SUSP suspense account + create suspense task
-- Pre-cutover rows: posted_at set, journal_entry_id null, archive-only.
-- Idempotent: re-runs skip already-posted rows.
-- =============================================================================

-- Add posting tracking columns to bank_transactions
...
-- bank_transactions.journal_entry_id already exists per schema check

-- Suspense account verification: it lives at COA-SUSP
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'COA-SUSP suspense account not found in books_historical chart_namespace';
  END IF;
END $$;
...
CREATE OR REPLACE FUNCTION public.bank_gl_writer(
  p_agency_id uuid,
...
-- Runner-compat overload (matches run_internal_recipe signature)
CREATE OR REPLACE FUNCTION public.bank_gl_writer(
  p_agency_id uuid,
  p_recipe_id uuid
) RETURNS jsonb
...
AS $$
BEGIN
  RETURN public.bank_gl_writer(p_agency_id, FALSE);
END;
$$;

COMMENT ON FUNCTION public.bank_gl_writer(uuid, boolean) IS
'Posts post-cutover bank_transactions to journal_entries.
Resolution waterfall: 1) category exact match, 2) raw_split_label sub-account match, 3) gl_classification_rules priority match, 4) suspense fallback.
Bank side = bank_account_id from row. Pre-cutover archive-only. Idempotent.';
