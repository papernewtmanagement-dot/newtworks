...
BEGIN
  ...
  SELECT c.team_member_id, c.c_weekly_pay, c.c_base_advance, c.c_health_bonus, c.c_service_surge_share,
    -- TPB = sales-pool reconciliation. Subtract this week's BA + AP always. WP is
    -- subtracted ONLY when role_category='Sales' — Sales base salary is funded from
    -- the sales pool (advance against bonus). Retention base salary is funded from
    -- the retention budget (separate pool) and does not drain the sales pool.
    -- HSM_prior compensates for H+S+M dollars sitting inside qtd_paid that came
    -- from other pools (retention, scorecard, etc.), not the sales pool.
    CASE WHEN c.this_week_payroll_ytd IS NULL OR c.prior_qtr_end_payroll_ytd IS NULL THEN NULL
         ELSE GREATEST(0,
           ...
           - c.qtd_paid
           - CASE WHEN c.role_category = 'Sales' THEN c.c_weekly_pay ELSE 0 END
           - c.c_base_advance
...
SELECT public.write_weekly_pay('126794dd-25ff-47d2-a436-724499733365', '2026-06-20');
