-- Adds an optional p_statement_ids filter so a backfill can be aimed at a known
-- set of statements instead of every unposted statement in the account.
-- Needed 2026-08-13: 413 statements had never been posted at all, and a
-- whole-account run would have swept them in alongside the 119 suspense rows
-- being deliberately reclassified.
--
-- Old 5-argument signature is dropped and replaced by a 6-argument version whose
-- new argument defaults to NULL, so every existing 5-argument caller is unaffected.

DROP FUNCTION IF EXISTS public.statement_gl_writer(uuid, uuid, date, date, boolean);
