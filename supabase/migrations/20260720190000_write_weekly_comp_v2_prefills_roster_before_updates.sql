-- Write_weekly_comp_v2: prefill the roster before UPDATEs.
--
-- Prior behavior: write_weekly_comp_v2 only UPDATEd existing weekly_cpr_team_detail
-- rows. If a teammate had zero check-ins for the week (e.g. PTO all week, sick), no
-- snapshot row existed and they were silently unpaid.
--
-- Directive: on the roster = gets paid. Archived = doesn't. No third case.
--
-- Fix: call prefill_weekly_cpr_form at the top. Prefill uses get_expected_teammates
-- with purpose='compensation' (every active agency non-owner, not archived, not
-- admin-backoffice, not test user) and inserts missing rows via ON CONFLICT DO NOTHING.
-- Idempotent + also creates the weekly_cpr_reports row if missing.
...
BEGIN
  ...
  BEGIN
    ...
    last_week_delta AS (
      ...
      -- No "most recent non-null" fallback: NULL last week -> 0 SP contribution.
      -- Same-quarter QTD-delta subtraction preserved for mid-quarter weeks.
