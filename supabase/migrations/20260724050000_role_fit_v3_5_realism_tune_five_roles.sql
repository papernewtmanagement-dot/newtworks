...
-- 2026-07-24
--
-- Follow-up to the v3.4 realism tune pass. Six of seven role_fits reviewed
-- against day-to-day role reality; five had adjustments worth shipping,
-- aspirant is being held for Peter confirmation before rebuild.
--
-- Changes shipped in this migration:
--
-- 1. sales_outbound: rapid_rapport_warm -0.02 -> +0.05 (cold calls need warmth
--    to survive the first 10 seconds; the "over-empathize with no" risk is
--    already captured by handles_rejection). Trim tail positives by 0.01 each
--    (has_entrepreneurial_spirit, receives_coaching, prospects_in_community,
--    positively_influences_team). Deepen retention_watchfulness -0.02->-0.04
--    and routing_judgment -0.03->-0.04 to reinforce anti-retention-shape signal.
--
-- 2. sales_in_book: competes_for_recognition -0.03 -> +0.03 (in-book cross-sell
--    is quota work with recognition pay; recognition drive helps, not hurts).
--    Trim handles_rejection 0.05->0.03, positively_influences_team 0.04->0.03,
--    maintains_high_activity 0.04->0.03. Deepen is_fast_start and
--    prospects_in_community negatives by 0.01 each.
--
-- 3. sales_inbound: add composure_under_load (+0.08), works_without_close_
--    supervision (+0.04), retention_watchfulness (+0.04) — inbound seats
--    live these daily. Add dials_cold_calls (-0.04), prospects_in_community
--    (-0.04), has_entrepreneurial_spirit (-0.03) — inbound is explicitly
--    NOT outbound-hunter work.
--
-- 4. retention_support: add listens_discovers_needs (+0.04), proactive_touch_
--    discipline (+0.03), retention_watchfulness (+0.03) — support does touch
--    customers, just less warmly than reception. Trim top four (attention_to_
--    detail, manages_time_effectively, queue_throughput_discipline, works_
--    without_close_supervision) by 0.02 each. Ease rapid_rapport_warm
--    penalty -0.03->-0.02.
--
-- 5. retention_escalation: rapid_rapport_warm 0.04 -> 0.07 (upset customers
--    need warmth to be defused). Remove queue_throughput_discipline negative
--    (escalation IS a queue, just slower). Trim listens/attention/decisions/
--    routing/receives_coaching by 0.01 each to compensate.
--
-- All five functions preserve the v6 invariant (positive_sum + negative_sum
-- = 1.00 exactly). Model tag: role_fit_v3_5_realism_tune_2026_07_24.
