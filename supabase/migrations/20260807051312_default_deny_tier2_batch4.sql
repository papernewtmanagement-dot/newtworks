...
-- Batch 4 of 8. service_role policies (hlcw_service_all, hiregauge_rules_service_role_all)
-- are NOT touched anywhere below.

-- hiregauge_competency_weights: WATCH ITEM confirmed. Two duplicate wide-open
-- ALL policies under misleading anon/authenticated names, both role=authenticated.
...
-- hiregauge_rules: split the authenticated ALL policy, fix the separate open read too.
...
-- hiring_candidates: split the authenticated ALL policy, fix the separate
-- staff-read-all policy too (this closes the open_question about candidate
-- records being readable by all staff).
