-- PostgREST is returning 400 on calls to get_weekly_cpr_requirements even though
-- the function runs cleanly via direct SQL as anon. The TEMP TABLE inside the
-- function is the suspect — PostgREST is wary of functions that create persistent
-- objects. Rewrite the same logic using a jsonb state accumulator: no temp table,
-- no DDL inside the function, pure read-only plpgsql.
--
-- Logic is identical to the prior version (cycle-recursive, carryover from
-- cycle_start, snapshot policy of team-at-week-start). Tests against the
-- 2026-06-20 week should return the same 5 rows.
...
DECLARE
  ...
  v_state             jsonb;   -- map: team_member_id_text -> state_jsonb
BEGIN
  ...
  -- Seed state with all eligible members (team-Monday-morning snapshot +
  -- anyone who has a detail row for the target week, per the snapshot policy).
  ...
  WHILE v_loop_week <= p_week_ending_date LOOP
    -- Compute one week of state from the prior week's carryover_input values
    -- stored in v_state. Allocation logic + owed/net_quotes formulas are
    -- identical to the prior temp-table implementation.
    WITH
    members AS (
      SELECT
        (key)::uuid                              AS tm_id,
    ...
    SELECT COALESCE(
      jsonb_object_agg(
        a.tm_id::text,
        jsonb_build_object(
          -- carryover_input for NEXT week = this week's owed
          'carryover_input',  (a.total + a.modified - a.paid),
          ...
          'paid',             a.paid,
          'owed',             (a.total + a.modified - a.paid),
          'net_quotes',       (a.quotes_discussed - a.total - a.modified)
        )
      ),
      '{}'::jsonb
    )
