...
-- This migration was applied live on 2026-07-29 17:14:01 UTC (ledger name: reclassify_pending_je_v4_reject_income_on_liability_je) but was never
-- committed to this repo at the time. Content below is copied verbatim
-- from supabase_migrations.schema_migrations.statements for version 20260729171401.
-- No live change is made by adding this file — it only brings the repo
-- mirror into sync with what is already running in production, so a
-- fresh `supabase db reset` from this repo reproduces live state.
-- Reclassifier: same structural rule. If the JE's other side is a card (liability),
-- reject income/equity targets. If it's a bank (asset), income is fair game (deposits).
...
BEGIN
  ...
  LOOP
    ...
    WHERE jl.journal_entry_id = v_je.id AND jl.account_id = v_susp_account_id
    LIMIT 1;
    ...
    SELECT jl.account_id, coa.account_code, coa.account_type
    INTO v_other_line
    FROM public.journal_lines jl
    ...
    -- Structural: if the JE's other side is a liability (a credit card),
    -- income and equity are never valid targets. Cards don't produce income.
    SELECT r.id,
    ...
    WHERE r.agency_id = p_agency_id AND r.is_active = TRUE
      ...
      -- Structural guard: card-sourced JE cannot route to income/equity
      AND (v_source_account_type != 'liability'
           OR target_coa.account_type NOT IN ('income','equity'))
    ...
    IF NOT p_dry_run THEN
      ...
      SET classification_status = 'classified',
          suspense_reason = NULL, rule_id_used = v_rule_id,
