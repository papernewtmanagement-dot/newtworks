-- 1. Rename accounts per Peter's language
...
-- 2. Flatten personal P&L: when an account has no parent, section = account_type
--    (personal books stay flat; agency hierarchy still shows root_name).
...
AS $function$
  ...
  post_cutover AS (
    SELECT
      ...
      -- Flat personal books: if account has no parent, section = account_type;
      -- else use root_name (agency hierarchy).
