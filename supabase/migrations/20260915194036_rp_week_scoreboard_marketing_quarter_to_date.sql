-- Marketing on the Score tab showed this week only. Sales shows this week AND
-- the quarter so far, and marketing should read the same way (Peter 2026-09-15).
-- The marketing events are now priced once over the whole quarter and totalled
-- twice: filtered to this week for 'points' and 'items', and across the quarter
-- for 'qtd_points'. Weeks before the live cutover were reported rather than
-- computed, so the quarter total picks those up from marketing_points.
...
BEGIN
    ...
    roster AS (
      ...
      WHERE t.agency_id = p_agency_id
        ...
        AND (
          ...
          OR t.id IN (SELECT tm FROM sp WHERE ROUND(COALESCE(qtd, 0) - COALESCE(prev, 0), 2) <> 0)
        )
    ),
    conv AS (SELECT * FROM public.rp_week_rollup(v_week_end, NULL)),
    people AS (
      SELECT r.id, r.first_name, r.role_category,
        jsonb_build_object('points', COALESCE(m.points, 0), 'qtd_points', COALESCE(m.qtd, 0), 'items', '[]'::jsonb) AS marketing,
        jsonb_build_object(
  ...
  WITH contributors AS (
    ...
    -- they just no longer put someone on it.
  ...
  m_rev AS (
    ...
    WHERE l.agency_id = p_agency_id AND l.activity_key = 'google_review' AND l.status = 'credited'
      AND l.week_end_date BETWEEN v_live_from AND v_week_end
  ),
  m_rq AS (
    ...
    WHERE q.agency_id = p_agency_id AND q.status = 'active' AND q.marketing_source = 'referral'
      AND q.week_end_date BETWEEN v_live_from AND v_week_end
  ),
  m_rs AS (
    ...
    WHERE s.agency_id = p_agency_id AND s.status = 'active' AND s.marketing_source = 'referral'
      ...
      AND s.week_end_date BETWEEN v_live_from AND v_week_end
  ),
  m_ak AS (
    ...
    WHERE ap.agency_id = p_agency_id AND ap.status = 'active'
      ...
      AND ap.kept_on IS NOT NULL AND public.rp_week_end(ap.kept_on) BETWEEN v_live_from AND v_week_end
  ),
  m_as AS (
    ...
    WHERE ap.agency_id = p_agency_id AND ap.status = 'active'
      AND ap.escalated_to_team_member_id IS NOT NULL
      AND ap.escalated_to_team_member_id <> ap.team_member_id
      AND ap.sold_on IS NOT NULL AND public.rp_week_end(ap.sold_on) BETWEEN v_live_from AND v_week_end
  ),
  ...
  m_agg AS (
    SELECT tm,
           ...
           jsonb_agg(jsonb_build_object('id', id, 'kind', event_key, 'label', label, 'on_date', on_date, 'customer', customer, 'nth', prior + 1, 'points', points)
                     ORDER BY on_date DESC, customer) FILTER (WHERE wk = v_week_end) AS items
  ...
  people AS (
    SELECT r.id, r.first_name, r.role_category,
      jsonb_build_object('points', COALESCE(m.points, 0), 'qtd_points', COALESCE(mq.qtd, 0), 'items', COALESCE(m.items, '[]'::jsonb)) AS marketing,
