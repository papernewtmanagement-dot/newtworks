-- Fix write_weekly_comp_v2 goals_bonus_detail JSONB label bug.
-- Prior RETURNING clause returned wctd.id and the outer SELECT labeled it
-- 'team_member_id'. Result: diagnostic output showed weekly_cpr_team_detail
-- row IDs where actual team member IDs should have been. Cosmetic (function
-- output is diagnostic, no downstream consumer relies on it for pay), but
-- misleading.
--
-- Fix: RETURNING now includes w.team_member_id (from with_dollars CTE) and
-- wctd.id as row_id. Outer SELECT labels each correctly.
