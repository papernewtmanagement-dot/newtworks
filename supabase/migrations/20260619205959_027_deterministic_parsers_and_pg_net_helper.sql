...
AS $$
  SELECT
    ...
    -- by treating "no row yet AND old enough" as not-yet-done (NULL pair),
    -- and present timed_out=false on actual responses. The runner only flips
    -- timed_out=true when the upstream HTTP layer reports a timeout, which
    -- shows up as a populated error_msg here.
    false AS timed_out,
    r.created
