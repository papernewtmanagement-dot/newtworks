-- One bonus model on every surface (completes the 2026-08-28 sequence).
-- Migration pay_scale_bonus_mechanical_projection put the mechanical
-- seasoned-book bonus (projected_team_bonus) behind the sales pay scale,
-- but compute_role_earnings_projection — the year-by-year grid, the
-- retention curve, and the assumptions note — was still running the
-- earlier linear pool-grows model. The two surfaces disagreed. This
-- migration makes the projection call projected_team_bonus everywhere a
-- sales-point-producing seat gets a bonus, sources every live input from
-- pay_scale_bonus_inputs (one copy), and rewrites the assumptions note
-- for the mechanical model. Life Specialist keeps today's retention-hours
-- slice, unchanged: their production enters as premium, not points
-- (design recorded in pay_scale_bonus_mechanical_projection).
-- Inputs always reflect the latest settled week — same anchor the
-- pay_scale table reseeds from — so chart, table, and tier cards agree.
  ...
  RETURN jsonb_build_object(
    ...
    'assumptions', jsonb_build_object(
      ...
      'note', 'One sales point equals one dollar of commission. Team bonus is a '
           ...
           || 'about seventy percent of that by year three. The goals bonus is '
