-- pnl_drill_transactions was filtering by INITCAP(REPLACE(account_subtype))
-- but the P&L displays sections from get_pnl_history_for_entity which uses
-- section_label_override AND the budget_category=growth transaction_tags tag.
-- Result: drill panel found zero transactions for any account whose display
-- section didn't happen to match its raw subtype (97 of 99 COA rows).
--
-- Rewrite:
--   1. Descendants recursion added (drill from parent entity now covers children).
--   2. Section derivation now COALESCE(section_label_override, INITCAP(...)),
--      with the growth-tag CASE, matching get_pnl_history_for_entity byte-for-byte.
--   3. WHERE-filter on derived section moved outside the CTE so both journal
--      and prior_year_pl sides share the same section-match test.
...
RETURNS TABLE(
  source text, je_id uuid, line_id uuid, pyp_id uuid, entry_date date,
...
AS $function$
  ...
  journal_side AS (
    SELECT
      ...
      je.document_id,
      je.created_at,
      -- Section derivation mirrors get_pnl_history_for_entity exactly.
      CASE
  ...
  pyp_side AS (
    SELECT
      ...
      COALESCE(py.section, 'Uncategorized') AS derived_section
