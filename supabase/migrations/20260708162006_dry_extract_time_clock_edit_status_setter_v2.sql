-- Pair 2 (Tier 2): extract set_time_clock_edit_status helper.
-- (v2: DROP+CREATE the wrappers because existing signatures have defaults that
-- CREATE OR REPLACE can't reshape.)
...
-- DROP old wrappers (had defaults we can't change via CREATE OR REPLACE)
...
BEGIN
  ...
  -- Status transition via canonical helper (replaces old inline UPDATE tail)
