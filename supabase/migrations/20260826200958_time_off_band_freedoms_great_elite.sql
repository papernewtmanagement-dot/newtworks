-- Adds the two band freedoms locked 2026-08-25 to the time-off eligibility answer.
-- Great: may schedule inside the first four weeks of a quarter without the agent's
-- special permission. Elite: may schedule longer than one week at a time without it.
-- Both are returned as flags rather than decisions, because this function is given a
-- person and not a date range — the caller still compares them to the requested dates.
-- Owner is unrestricted, so both flags are true for the Owner seat.
