...
CREATE VIEW public.v_card_balances AS
 ...
 SELECT a.agency_id,
  ...
  WHERE a.agency_id = '126794dd-25ff-47d2-a436-724499733365'::uuid AND a.account_kind = 'credit'::text
...
CREATE VIEW public.v_bank_balances AS
 WITH latest_stmt AS (
         SELECT DISTINCT ON (statement_balances.account_code) statement_balances.account_code,
            ...
            statement_balances.closing_balance
           FROM statement_balances
          WHERE statement_balances.agency_id = '126794dd-25ff-47d2-a436-724499733365'::uuid AND statement_balances.account_kind = 'bank'::text
          ORDER BY statement_balances.account_code, statement_balances.statement_period_end DESC
        )
 SELECT a.agency_id,
    a.business_entity_id,
   ...
   FROM accounts a
     LEFT JOIN chart_of_accounts coa ON coa.id = a.chart_account_id
     LEFT JOIN latest_stmt ls ON ls.account_code = coa.account_code
  WHERE a.agency_id = '126794dd-25ff-47d2-a436-724499733365'::uuid AND a.account_kind = 'bank'::text
  ORDER BY a.account_name;
