-- One compile function for both cases: a team member, or a candidate who is
-- still at offer. Steps owned by someone other than the new hire (Alvi's
-- physical setup, for example) also land on that person's task list.
...
BEGIN
  ...
  INSERT INTO public.team_onboarding_steps (
    ...
    substeps, substeps_done, owner_kind, assigned_to
  )
  SELECT
  ...
  -- Steps that belong to a named teammate become tasks for that person.
  FOR r IN
  ...
  LOOP
    ...
    INSERT INTO public.tasks (
      ...
      COALESCE(r.description, '') || CASE WHEN r.phase_name IS NULL THEN '' ELSE E'\n\nPhase: ' || r.phase_name END,
      r.assigned_to, 'admin', 'task',
...
-- Old entry point keeps working; the frontend can migrate at its own pace.
CREATE OR REPLACE FUNCTION public.create_onboarding_plan_from_templates(
 ...
 SET search_path TO 'public'
AS $function$
  SELECT public.create_onboarding_plan(
...
-- When the candidate is hired, point their plan at the new team row.
CREATE OR REPLACE FUNCTION public.attach_onboarding_plan_to_team_member(
  p_plan_id uuid,
  p_team_member_id uuid
) RETURNS void
 LANGUAGE plpgsql
...
BEGIN
  ...
  SET team_member_id = p_team_member_id,
      attached_at = now(),
      role_snapshot = COALESCE(role_snapshot, (SELECT role FROM public.team WHERE id = p_team_member_id)),
      role_category_snapshot = COALESCE(role_category_snapshot, (SELECT role_category FROM public.team WHERE id = p_team_member_id)),
      role_level_snapshot = COALESCE(role_level_snapshot, (SELECT role_level FROM public.team WHERE id = p_team_member_id))
  WHERE id = p_plan_id;
END;
$function$;

-- Ticking the step off closes its task, and re-opening the step re-opens it.
...
DROP TRIGGER IF EXISTS trg_sync_onboarding_step_task ON public.team_onboarding_steps;
CREATE TRIGGER trg_sync_onboarding_step_task
AFTER UPDATE OF completed_at ON public.team_onboarding_steps
FOR EACH ROW EXECUTE FUNCTION public.sync_onboarding_step_task();
