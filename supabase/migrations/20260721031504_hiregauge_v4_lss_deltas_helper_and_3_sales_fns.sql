-- Migration 20260721031504 (mirror of applied migration)
-- Adds _cts_lss_delta_v4 helper + updates 3 sales fns to emit _lss_deltas alongside the flat competency ints.
-- delta = 15 * (acc_wt * acc_signal + spd_wt * spd_signal) / 2  when has_lss, else 0.
-- Shape: existing flat int keys unchanged, new '_lss_deltas' jsonb block sibling to '_meta'.
...
AS $$
  SELECT CASE
    WHEN p_has_lss THEN ROUND(15.0 * (COALESCE(p_acc_wt, 0) * p_acc_signal + COALESCE(p_spd_wt, 0) * p_spd_signal) / 2.0, 2)
    ELSE 0
  END;
$$;

COMMENT ON FUNCTION public._cts_lss_delta_v4 IS
  'Returns the raw LSS delta (rounded to 2dp numeric) that would be added to a base score for a competency with the given LSS weights and candidate signals. Zero when no LSS. Used for exposing per-competency LSS influence to the frontend for shading.';
...
RETURNS jsonb LANGUAGE sql STABLE AS $function$
  ...
  SELECT jsonb_build_object(
    ...
    '_meta', jsonb_build_object(
      ...
      'role', 'sales_outbound', 'model', 'sensitivity_weighted_v4_asymmetric'
    )
  )
  FROM adj;
$function$;
...
RETURNS jsonb LANGUAGE sql STABLE AS $function$
  ...
  SELECT jsonb_build_object(
    ...
    'cadence_compliance', public._cts_lss_apply_v4((22.000000) + (0.250000)*dm + (0.150000)*an + (0.150000)*rd + (0.050000)*op + (-0.050000)*is_val, (w->'cadence_compliance'->>'a')::numeric, (w->'cadence_compliance'->>'s')::numeric, acc_signal, spd_signal, rel_factor, has_lss),
    '_lss_deltas', jsonb_build_object(
      'maintains_high_activity',    public._cts_lss_delta_v4((w->'maintains_high_activity'->>'a')::numeric,    (w->'maintains_high_activity'->>'s')::numeric,    acc_signal, spd_signal, has_lss),
      'handles_rejection',          public._cts_lss_delta_v4((w->'handles_rejection'->>'a')::numeric,          (w->'handles_rejection'->>'s')::numeric,          acc_signal, spd_signal, has_lss),
      'listens_discovers_needs',    public._cts_lss_delta_v4((w->'listens_discovers_needs'->>'a')::numeric,    (w->'listens_discovers_needs'->>'s')::numeric,    acc_signal, spd_signal, has_lss),
      ...
      'cadence_compliance',         public._cts_lss_delta_v4((w->'cadence_compliance'->>'a')::numeric,         (w->'cadence_compliance'->>'s')::numeric,         acc_signal, spd_signal, has_lss)
    ),
    '_meta', jsonb_build_object(
      'has_lss', has_lss, 'acc_flags', acc_flags_int, 'spd_flags', spd_flags_int,
      'reliability', rel, 'distortion', dist,
...
RETURNS jsonb LANGUAGE sql STABLE AS $function$
  ...
  SELECT jsonb_build_object(
    ...
    'receives_coaching', public._cts_lss_apply_v4((33.550072) + (-0.005371)*dm + (0.109720)*rd + (0.113558)*ass + (-0.109892)*is_val + (-0.112440)*an + (0.217015)*com + (-0.113273)*sp + (0.113147)*bo + (0.110904)*op, (w->'receives_coaching'->>'a')::numeric, (w->'receives_coaching'->>'s')::numeric, acc_signal, spd_signal, rel_factor, has_lss),
    'positively_influences_team', public._cts_lss_apply_v4(op::numeric, (w->'positively_influences_team'->>'a')::numeric, (w->'positively_influences_team'->>'s')::numeric, acc_signal, spd_signal, rel_factor, has_lss),
    'cross_sell_instinct', public._cts_lss_apply_v4((10.000000) + (0.200000)*com + (0.200000)*an + (0.150000)*sp + (0.100000)*bo + (0.100000)*rd + (0.050000)*dm + (0.050000)*ass, (w->'cross_sell_instinct'->>'a')::numeric, (w->'cross_sell_instinct'->>'s')::numeric, acc_signal, spd_signal, rel_factor, has_lss),
    ...
    '_lss_deltas', jsonb_build_object(
      ...
      'receives_coaching',          public._cts_lss_delta_v4((w->'receives_coaching'->>'a')::numeric,          (w->'receives_coaching'->>'s')::numeric,          acc_signal, spd_signal, has_lss),
      'positively_influences_team', public._cts_lss_delta_v4((w->'positively_influences_team'->>'a')::numeric, (w->'positively_influences_team'->>'s')::numeric, acc_signal, spd_signal, has_lss),
      'cross_sell_instinct',        public._cts_lss_delta_v4((w->'cross_sell_instinct'->>'a')::numeric,        (w->'cross_sell_instinct'->>'s')::numeric,        acc_signal, spd_signal, has_lss),
