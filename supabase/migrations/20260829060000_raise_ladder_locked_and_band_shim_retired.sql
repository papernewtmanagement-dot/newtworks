...
-- Retire the sales_points_band_config shim for good. The time-off gate only
-- ever wanted one number out of it: the weekly Sales Points where the Good
-- band starts. That is a single pay_scale row, so the lookup goes straight
-- there and no view is needed.
DO $mig$
