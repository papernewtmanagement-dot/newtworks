...
BEGIN
  ...
  -- Single shared calculation (get_team_checkin_totals) — was previously a duplicated
  -- inline query filtered to checkin_type IN ('midday','eod'), which silently dropped
  -- anyone whose latest submission that week was a health_eve check-in. Fixed 2026-08-12.
  ...
  SELECT * INTO v_totals FROM public.get_team_checkin_totals(p_agency_id, v_wtw_cycle.cycle_start, v_wtw_cycle.week_ending_saturday);
  v_tts := v_totals.total_sales_points;
