-- Residual-pool comp Phase 1 function #3: orchestrator
-- Per-person weekly + annual comp view. Consumes basis+envelope function + person-commissions function + retention 5-factor formula.
-- Distribution per operational_rule: person_share = 0.65 * SP_share + 0.35 * retention_hours_share
-- Group health subtracts at share level (weekly_health_benefit_agency_paid * 52), floor at $0.
...
CREATE OR REPLACE FUNCTION public.compute_weekly_comp_residual_pool(
  ...
  annual_bonus_gross             numeric,
  annual_health_subtracted       numeric,
  annual_bonus_net               numeric,
  weekly_bonus_net               numeric,
...
BEGIN
  ...
  bonus_pool AS (
    SELECT
      ...
      (tt.total_base + tt.total_comm) * v_burden_multiplier AS burden,
      v_wc_annual AS wc,
      GREATEST(0, v_annual_envelope
                  - tt.total_base
                  - tt.total_comm
                  - ((tt.total_base + tt.total_comm) * v_burden_multiplier)
                  - v_wc_annual) AS annual_bonus_pool
    FROM team_totals tt
  ),
  ...
  SELECT
    ...
    jsonb_build_object(
      ...
      'team_total_burden',    (SELECT (total_base + total_comm) * v_burden_multiplier FROM team_totals),
...
COMMENT ON FUNCTION public.compute_weekly_comp_residual_pool(uuid,date) IS
  'Residual-pool comp Phase 1 orchestrator (per operational_rule "Team comp — residual-pool design (7/11/2026 rollout)"). Per-person annual + weekly base/commission/bonus. Bonus distributed by 0.65*SP_share + 0.35*retention_hours_share. Group health subtracts at share level, floor $0. Annualization: sum realized quarters + avg-of-realized for unrealized. Sales points: quarterly MAX + same annualization.';