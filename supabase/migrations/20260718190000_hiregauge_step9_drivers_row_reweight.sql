-- HireGauge Step 9: Drivers construct row weight reweight
-- Assessment 0.25 → 0.15; Reference 0.20 → 0.30. R (0.10) and I (0.45) unchanged. Sum stays 1.00.
--
-- Only change in verdict fn: v_drivers_a_w declaration line + v_drivers_ref_w declaration line.
-- Everything else byte-for-byte identical to Step 8.
--
-- Rationale: reference data (past employers) is stronger signal of drivers/motivation than
-- assessment self-report on the same dimension.
--
-- Impact on 2026-07-18 cohort (32 candidates):
--   - 1 verdict flip: Richard Casias consider (6.044) → decline (5.896) — crossed 6.0 threshold
--   - 1 ranking flip: Braden (7.137) now edges Priscilla (7.127) in top 3
--   - All others: same verdict, ±0.15 score drift
--   - Muted impact because all cohort candidates have ref_* columns NULL. Full reweight lands
--     when reference data is captured.
