-- Half-day calendar cut moves to 1:00 PM. Morning half-day now 8:30 AM - 1:00 PM
-- (4.5h, lunch hour falls inside it). Afternoon half-day unchanged at 1:00 PM - 5:30 PM (4.5h).
-- Full day unchanged at 8:30 AM - 5:30 PM (9h). Morning + Afternoon = Full Day.
...
BEGIN
  ...
  LOOP
    ...
    -- Times: half-days cut at 1 PM. Morning includes lunch hour, afternoon starts post-lunch.
    IF v_req.partial_day = 'morning' THEN
      v_start_ts := (v_req.start_date::text || ' 08:30:00')::timestamp AT TIME ZONE 'America/Chicago';
      v_end_ts   := (v_req.start_date::text || ' 13:00:00')::timestamp AT TIME ZONE 'America/Chicago';
    ...
    ELSE
      -- Full day(s): 8:30 AM start_date → 5:30 PM end_date
      v_start_ts := (v_req.start_date::text || ' 08:30:00')::timestamp AT TIME ZONE 'America/Chicago';
  ...
  -- Pass 2: event_id capture (unchanged)
  FOR v_req IN
