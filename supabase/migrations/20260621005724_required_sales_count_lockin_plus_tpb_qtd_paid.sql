-- ──────────────────────────────────────────────────────────────────────────
-- 1. Lock-in column on weekly_cpr_reports
-- ──────────────────────────────────────────────────────────────────────────
ALTER TABLE public.weekly_cpr_reports
  ADD COLUMN IF NOT EXISTS required_sales_members_count int;

COMMENT ON COLUMN public.weekly_cpr_reports.required_sales_members_count IS
'Count of role_category=Sales team members on the team during this week, stamped at row creation. Locks the historical truth so retroactive team changes do not shift prior-week pay math. compute_weekly_pay averages this column across the current cycle for the Manager Base Advance divisor.';

-- ──────────────────────────────────────────────────────────────────────────
-- 2. Trigger function — auto-stamp on INSERT only
-- Snapshot policy: member counted if start_date <= week_ending AND
-- (not archived OR archived after week_start).
-- ──────────────────────────────────────────────────────────────────────────
...
DROP TRIGGER IF EXISTS trg_weekly_cpr_stamp_required_count ON public.weekly_cpr_reports;
CREATE TRIGGER trg_weekly_cpr_stamp_required_count
BEFORE INSERT ON public.weekly_cpr_reports
FOR EACH ROW EXECUTE FUNCTION public.weekly_cpr_stamp_required_count();

-- ──────────────────────────────────────────────────────────────────────────
-- 3. Backfill existing rows. Idempotent: only fills NULL.
-- ──────────────────────────────────────────────────────────────────────────
...
WHERE required_sales_members_count IS NULL;

-- ──────────────────────────────────────────────────────────────────────────
-- 4. compute_weekly_pay — switch TPB to qtd_paid + use AVG(required_count)
-- ──────────────────────────────────────────────────────────────────────────
...
DECLARE
  ...
  v_required_count_avg numeric;     -- now an AVG across the cycle, not a one-week count
  v_weeks_elapsed      int;
...
BEGIN
  ...
  -- Manager BA divisor: average of required_sales_members_count across all
  -- weekly_cpr_reports rows in this cycle through this week (inclusive).
  -- Locked at row creation by trg_weekly_cpr_stamp_required_count.
  ...
  SELECT c.team_member_id, c.c_weekly_pay, c.c_base_advance, c.c_health_bonus, c.c_service_surge_share,
    -- TPB now subtracts qtd_paid (this quarter only), not payroll_ytd_paid (full YTD).
    -- NULL if either YTD anchor is missing.
    ...
    jsonb_build_object(
      ...
      'sales_pool_plus_hsm', c.this_week_qtd_sp + c.hsm_prior_total,
