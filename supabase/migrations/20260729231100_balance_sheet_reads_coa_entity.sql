-- 2026-07-29: Rewrite v_balance_sheet_anchored to read business_entity_id from
-- chart_of_accounts (via journal_lines.account_id) instead of directly from
-- journal_entries. Same architectural pattern Thread B used for P&L functions.
-- Preps for dropping the redundant journal_entries.business_entity_id column.
...
WITH agency AS (
  ...
  GROUP BY coa.account_code, coa.account_type, coa.business_entity_id
), post_net_income AS (
  SELECT
    coa.business_entity_id,
    round(sum(
  ...
  GROUP BY coa.business_entity_id
), codes AS (
