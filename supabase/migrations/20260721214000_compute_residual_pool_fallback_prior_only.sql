-- 2026-07-21: gate wctd.commission / wctd.manager_bonus fallback to prior weeks only.
--
-- SECOND LEAK (found after applying the pay_date filter migration): even with the
-- pay_date filter on payroll_detail, the comm_by_week and mgr_bonus_by_week CTEs
-- fell back to wctd.commission / wctd.manager_bonus when the payroll subquery
-- returned NULL. wctd.commission for the current week gets written by v2's own
-- stomp using the current-week payroll — so the fallback picked up EXACTLY the
-- value the pay_date filter was trying to exclude. Circular self-reference.
--
-- Fix: gate the wctd fallback in comm_by_week and mgr_bonus_by_week on
-- cw.week_end_date < p_week_end_date. For the current week, if payroll isn't yet
-- paid (pay_date > week_end), the commission and manager bonus subtractions
-- default to 0 — no money-out has happened.
--
-- Applies on top of 20260721213000_compute_residual_pool_pay_date_boundary.sql.
  ...
  comm_by_week AS (
    SELECT r.id AS tm_id, cw.week_end_date,
      COALESCE(
        ...
        CASE WHEN cw.week_end_date < p_week_end_date
             THEN (SELECT wctd.commission
  ...
  mgr_bonus_by_week AS (
    SELECT r.id AS tm_id, cw.week_end_date,
      COALESCE(
        ...
        CASE WHEN cw.week_end_date < p_week_end_date
             THEN (SELECT wctd.manager_bonus
                   FROM public.weekly_cpr_team_detail wctd
                   JOIN public.weekly_cpr_reports wr ON wr.id = wctd.weekly_cpr_report_id
                   WHERE wr.agency_id = p_agency_id AND wctd.team_member_id = r.id AND wr.week_ending_date = cw.week_end_date
                   LIMIT 1)
