...
-- This migration was applied live on 2026-07-11 06:32:39 UTC (ledger name: residual_pool_fix_role_category_ambig_2026_07_11) but was never
-- committed to this repo at the time. Content below is copied verbatim
-- from supabase_migrations.schema_migrations.statements for version 20260711063239.
-- No live change is made by adding this file — it only brings the repo
-- mirror into sync with what is already running in production, so a
-- fresh `supabase db reset` from this repo reproduces live state.
-- Fix: qualify role_category in team_totals CTE (conflicts with RETURNS TABLE output param)
...
CREATE OR REPLACE FUNCTION public.compute_weekly_comp_residual_pool(p_agency_id uuid, p_week_end_date date)
 RETURNS TABLE(
   team_member_id uuid, full_name text, out_role text, out_role_category text, out_role_level text,
