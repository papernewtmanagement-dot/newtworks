...
--   (1) Drop the pre-cutover branch entirely. Everything gets posted, no archive.
--   (2) Unmapped rows land on P&L catchall accounts (COA-SUB-SF-UNCL-INC / COA-SUB-SF-UNCL-DED)
--       instead of balance-sheet Suspense (COA-SUSP). Every SF dollar hits P&L.
--   (3) Keep classification_status='pending_review' so unmapped rows still surface in UI for reclassification.
...
DECLARE
  ...
  v_cash_acct_id uuid;
  v_sf_parent_id uuid;
  v_catchall_inc_id uuid;
  v_catchall_ded_id uuid;
  v_id uuid;
  v_period_year int;
  v_period_month int;
  v_period_day int;
...
BEGIN
  ...
  IF v_sf_parent_name IS NULL THEN v_sf_parent_name := 'State Farm'; END IF;
  ...
  SELECT id INTO v_sf_parent_id FROM chart_of_accounts
    WHERE agency_id = p_agency_id AND account_name = v_sf_parent_name AND parent_account_id IS NULL LIMIT 1;
  ...
  SELECT id INTO v_catchall_inc_id FROM chart_of_accounts
    WHERE agency_id = p_agency_id AND account_code = 'COA-SUB-SF-UNCL-INC' LIMIT 1;
  ...
  SELECT id INTO v_catchall_ded_id FROM chart_of_accounts
    WHERE agency_id = p_agency_id AND account_code = 'COA-SUB-SF-UNCL-DED' LIMIT 1;
  ...
  LOOP
    ...
    IF v_target_account_id IS NULL THEN
      -- CHANGED: catchall goes to P&L, not balance-sheet suspense
      IF v_is_deduction THEN
        v_target_account_id := v_catchall_ded_id;
        v_target_account_name := '99 - SF Unclassified Deduction';
      ELSE
        v_target_account_id := v_catchall_inc_id;
        v_target_account_name := '09 - SF Unclassified Income';
      END IF;
      ...
      SET posted_at = NOW(),
          ...
          notes = COALESCE(notes, '') || ' [posted by gl_entry_writer ' || NOW()::text ||
                  CASE WHEN v_classification_status = 'pending_review'
                       THEN '; pending_review: ' || COALESCE(v_suspense_reason, '') ELSE '' END || ']'
      WHERE id = v_id;
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      v_errors := v_errors || jsonb_build_object(
        'comp_recap_id', v_id, 'error', SQLERRM, 'sqlstate', SQLSTATE
      );
    END;
  END LOOP;
