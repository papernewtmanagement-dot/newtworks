-- 2026-07-21: compute_weekly_comp_residual_pool — payroll pay_date boundary filter.
--
-- BUG: for a compute of week X, the CTEs that read payroll_detail (base_by_week,
-- comm_by_week, bonus_paid_by_week, mgr_bonus_by_week, actual_base_this_week) only
-- matched pay_period_end = cw.week_end_date. They did NOT filter on pay_date. This
-- meant that a payroll whose pay_period_end fell inside the cycle but whose check
-- date was AFTER the compute week's end got pulled in as if it were already paid.
--
-- Concrete case: pay_period_end 2026-07-18 payroll had pay_date 2026-07-24. When
-- compute ran against week 07-18, it saw this payroll's 1Comm ($445), 5Manage
-- ($11.48), and REGULAR/SALARY ($2,950.56 of base) rows and treated them as
-- already-paid QTD subtractions — but the check hadn't actually been written yet
-- at the moment week 07-18 closed. Result: pool math moved as new payrolls got
-- ingested even for weeks that had already been sent.
--
-- FIX (Peter directive 2026-07-21): only pull payroll where pr.pay_date <=
-- p_week_end_date. A payroll counts against the pool only after the money has
-- actually left the agency. Same filter added to every payroll_detail JOIN inside
-- the function.
