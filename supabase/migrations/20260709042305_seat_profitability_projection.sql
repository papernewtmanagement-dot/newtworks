...
-- Assumptions:
--   * new-business pace = current trailing-quarter × 4 (annualized), constant going forward
...
DECLARE
  ...
  v_profit_hit           int;
BEGIN
  -- Pull baseline from compute_warning_trigger
  SELECT
  ...
  -- Check if already green at baseline
  ...
  IF v_coverage_hit IS NULL OR v_profit_hit IS NULL THEN
    FOR m IN 1..p_max_months LOOP
      ...
      -- Existing cohorts at v_future_date with fixed lapse (self-consistent projection)
      SELECT COALESCE(SUM(
        pp.premium_issued *
      ...
      -- Future cohorts (closed form). Only contribute once years_out >= 1.
...
-- Convenience wrapper: project all seats at once
