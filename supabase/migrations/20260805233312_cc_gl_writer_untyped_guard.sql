-- 2026-08-05: cc_gl_writer untyped-transaction guard.
-- Root cause of the journal sign corruption cleaned up by gl_sign_audit_20260805:
-- an untyped AMEX re-ingest batch stored charges as NEGATIVE amounts with NULL
-- transaction_type. The writer's amount-sign fallback (amount>0 => charge) posted
-- those charges reversed (credit expense / debit card liability) and re-posted
-- hundreds of duplicates the same-source-excluded dup check could not catch.
-- Fix: rows with NULL/blank transaction_type are never posted by sign-guessing;
-- they skip with error detail 'untyped_transaction_needs_type' until typed.
