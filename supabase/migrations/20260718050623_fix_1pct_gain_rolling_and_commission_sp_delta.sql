-- Fix 1: 1% gain target = prior 13 weeks of new-SP deltas ÷ 13 × 1.01 (was: prior-Q total ÷ 13)
-- Fix 2: commission = this week's new SP delta with Q-start floor (was: reads wctd.commission back from itself)
...
BEGIN
  ...
  -- FIX 2: commission = curr_qtd_sp - prior_sat_qtd_sp within same quarter, floor 0 at Q start
  comm_calc AS (
    SELECT
      curr.team_member_id,
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE r2.agency_id = p_agency_id
        AND d2.team_member_id = curr.team_member_id
  ...
  -- GOALS BONUS: 5 buckets x $10 each
  BEGIN
    WITH curr AS (
    ...
    -- FIX 1: rolling last-13-weeks of new-SP deltas ÷ 13 (excludes current week)
    prior_13_weeks AS (
      SELECT r.week_ending_date, d.team_member_id, d.sales_points AS qtd_sp,
    ...
    prior_13_deltas AS (
      SELECT p.team_member_id, p.week_ending_date,
             GREATEST(0,
               p.qtd_sp - COALESCE(
                 LAG(CASE WHEN p2.q_start = p.q_start THEN p2.qtd_sp END)
                   OVER (PARTITION BY p.team_member_id ORDER BY p.week_ending_date),
                 0
               )
             )::numeric AS new_sp_delta,
             row_number() OVER (PARTITION BY p.team_member_id ORDER BY p.week_ending_date DESC) AS rn
      FROM prior_13_weeks p
      LEFT JOIN prior_13_weeks p2
        ON p2.team_member_id = p.team_member_id
       AND p2.week_ending_date = p.week_ending_date - INTERVAL '7 days'
    ),
    prior_avg AS (
      SELECT team_member_id,
             (SUM(new_sp_delta) / 13.0)::numeric AS avg_new_sp,
             MAX(week_ending_date) AS last_included_week,
             SUM(new_sp_delta) AS sum_13
      FROM prior_13_deltas
      WHERE rn <= 13
    ...
    per_person AS (
      SELECT
        ...
        COALESCE(a.sum_13, 0)::numeric     AS prior_13wk_total,
        a.last_included_week               AS prior_13wk_last_week,
        ...
        COALESCE(lc.n, 0)::int   AS leaderboard_hits,
      ...
      SET goals_bonus = w.dollars,
          residual_pool_diag = COALESCE(wctd.residual_pool_diag, '{}'::jsonb) || jsonb_build_object(
            'goals_detail', jsonb_build_object(
              ...
              'prior_13wk_total',    ROUND(w.prior_13wk_total, 2),
              'prior_13wk_last_week', w.prior_13wk_last_week,
              'target_1pct',         ROUND(w.target_1pct, 2),
              'dollars',             w.dollars,
              'formula',             '$10 win-the-week (team) + $10 1% gain (prior 13 wk new-SP avg × 1.01) + $10 per All-Star + $10 per Leaderboard + $10 per Trailblazer'
            )
          ),
          updated_at = now()
      FROM with_dollars w
