-- Wrap every dollar output of compute_weekly_pay in ROUND(.., 2).
-- Storage columns are numeric(10,2) and would coerce on write, but explicit ROUND
-- ensures downstream readers (composer, CPRDetail) see the same 2-decimal values
-- and avoids rounding artifacts when components are summed and re-summed.
...
BEGIN
  ...
  computed AS (SELECT
      ...
      ROUND(
        ...
        END
      , 2) AS c_base_advance,
      ROUND(CASE WHEN w.weekday_hits>=5 THEN 25.00 ELSE 0 END, 2) AS c_health_bonus,
      ROUND(
