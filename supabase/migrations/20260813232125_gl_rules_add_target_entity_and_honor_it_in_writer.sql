-- Peter directive 2026-08-13: a ledger rule must be able to state which entity
-- it books to, instead of that being an accident of which card paid.
--
-- Adds gl_classification_rules.target_business_entity_id. When set, the writer
-- resolves the rule's account_code inside that entity. When null, behaviour is
-- unchanged (paying entity first, then any entity holding the code).

ALTER TABLE gl_classification_rules
  ADD COLUMN IF NOT EXISTS target_business_entity_id uuid REFERENCES business_entities(id);

COMMENT ON COLUMN gl_classification_rules.target_business_entity_id IS
  'Forces the rule to book into this entity. Null = follow the paying statement''s entity, then fall back to any entity holding the account code.';

CREATE INDEX IF NOT EXISTS idx_gl_rules_target_entity
  ON gl_classification_rules(target_business_entity_id)
  WHERE target_business_entity_id IS NOT NULL;

-- Anthropic / Claude are agency software, paid on the PaperNewt AMEX.
UPDATE gl_classification_rules
SET target_business_entity_id = 'b2222222-2222-2222-2222-222222222222',
    debit_account_code = '6310',
    sub_category_label = 'Anthropic / Claude — agency software',
    updated_at = NOW(),
    override_reason = 'Peter directive 2026-08-13: books to the Agency regardless of the paying card.'
WHERE agency_id = '126794dd-25ff-47d2-a436-724499733365'
  AND rule_name = 'Claude.ai subscription';

-- Broken rule scoped to a card id ('COA-024') that names no live account.
-- Peter directive 2026-08-13: get rid of it. The fee patterns it matched are
-- already covered by "MINED: NSF / Overdraft" (6940), "Historical Import #43/#44
-- US BANK FEES" (6940), and the personal "Service charge / Maintenance fee"
-- rules (9620), so nothing is left uncovered by removing it.
DELETE FROM gl_classification_rules
WHERE agency_id = '126794dd-25ff-47d2-a436-724499733365'
  AND rule_name = 'Bank fees & charges'
  AND match_source_account = 'COA-024';

-- ---------- writer: honour the new column ----------

CREATE OR REPLACE FUNCTION public.statement_gl_writer(p_agency_id uuid, p_account_id uuid DEFAULT NULL::uuid, p_from date DEFAULT NULL::date, p_to date DEFAULT NULL::date, p_dry_run boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
