-- End-of-day summary of every edit and every removal on the Production records.
-- Reads public.change_log. One alert a day; a re-run updates that same alert in place.
  ...
  FOR rec IN
    WITH raw AS (
      SELECT c.*,
             ...
             CASE
               WHEN c.action = 'DELETE' THEN 'removed'
               WHEN c.action = 'UPDATE' AND COALESCE(c.new_row->>'status','') IN ('void','voided')
       ...
       WHERE c.agency_id = p_agency_id
         ...
         AND c.action IN ('UPDATE', 'DELETE')
         AND COALESCE(c.via, '') <> 'automation'
    ), grouped AS (
      SELECT DISTINCT ON (r.txid, r.rank = 1)
             r.txid, r.changed_at, r.changed_by_label, r.what, r.item, r.subject, r.changed_fields, r.rank
        FROM raw r
       ORDER BY r.txid, r.rank = 1 DESC, r.rank, r.changed_at
    ), top AS (
      SELECT DISTINCT ON (g.txid) g.* FROM grouped g ORDER BY g.txid, g.rank, g.changed_at
    )
