-- Kill the last remnant of cutover: v_balance_sheet_anchored's dependency on gl_anchor_date setting.
-- Balance sheet is inherently a snapshot as-of a date; hardcode 2026-06-30 (the historical anchor
-- from Peter's 6/30 statements). If Peter wants a different snapshot date later, edit view directly.

CREATE OR REPLACE VIEW public.v_balance_sheet_anchored AS
WITH agency AS (
  SELECT '126794dd-25ff-47d2-a436-724499733365'::uuid AS id
), cfg AS (
  -- Balance sheet snapshot as-of date. Hardcoded; no more settings dependency.
...
-- Delete the last cutover-era setting
