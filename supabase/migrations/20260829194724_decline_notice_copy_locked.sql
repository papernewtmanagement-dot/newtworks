-- 2026-08-29 — Peter approved both letters and said "lock them in".
-- Body text unchanged from migration 20260829... (warmer copy). This migration
-- adds ONLY the lock notice at the top of the function, so anyone who pulls the
-- definition to edit it sees the constraint before they start typing.
...
-- great. Lock them in."
--
-- The two message bodies below are NOT to be reworded, tightened, shortened,
...
COMMENT ON FUNCTION public.send_one_candidate_decline_notice(uuid, uuid) IS
  'Sends the one decline letter a candidate ever gets. MESSAGE COPY IS LOCKED (Peter-approved 2026-08-29) — do not reword without explicit direction from Peter. See the header block in the function body and persistent_memory.operational_rule "Candidate decline letters — LOCKED Peter-approved copy 2026-08-29".';
