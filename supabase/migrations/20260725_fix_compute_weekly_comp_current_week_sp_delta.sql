-- Fix compute_weekly_comp_residual_pool: current-week commission derived live from
-- SP-delta instead of wctd.commission fallback. Eliminates one-cycle lag caused by
-- team check-ins updating sales_points before commission column gets rewritten.
-- Peter reported 2026-07-24: bonus pool inflated because commission subtracted was
-- from previous write cycle. Fix makes get_current_bonus_pool + write_weekly_comp_v2
-- both self-consistent in a single pass.
...
BEGIN
  ...
  comm_by_week AS (
    SELECT r.id AS tm_id, cw.week_end_date,
      COALESCE(
        -- Prior weeks that have been paid: actual payroll_detail Comm columns.
        ...
        -- Current week (not yet paid): compute LIVE SP-delta from wctd.sales_points.
        -- Reading wctd.commission here would lag one write cycle behind team check-ins.
        CASE WHEN cw.week_end_date = p_week_end_date THEN
          GREATEST(0,
            COALESCE((SELECT wctd.sales_points FROM public.weekly_cpr_team_detail wctd JOIN public.weekly_cpr_reports wr ON wr.id = wctd.weekly_cpr_report_id WHERE wr.agency_id = p_agency_id AND wctd.team_member_id = r.id AND wr.week_ending_date = cw.week_end_date LIMIT 1), 0)
            -
            COALESCE((SELECT wctd.sales_points FROM public.weekly_cpr_team_detail wctd JOIN public.weekly_cpr_reports wr ON wr.id = wctd.weekly_cpr_report_id WHERE wr.agency_id = p_agency_id AND wctd.team_member_id = r.id AND wr.week_ending_date < cw.week_end_date AND wr.week_ending_date >= v_cycle_start ORDER BY wr.week_ending_date DESC LIMIT 1), 0)
          )
        END,
        -- Prior weeks without payroll_detail (rare edge case): fall back to stored commission.
  ...
  actuals_through_current AS (
    SELECT r.id AS tm_id,
      ...
      -- current_week_comm derived live from SP-delta (this-week QTD SP minus most recent prior-week QTD SP within cycle)
      -- so display/annualization stays consistent with pool math. Formerly read wctd.commission which lags one write cycle.
      GREATEST(0,
        COALESCE(MAX(CASE WHEN wr.week_ending_date = p_week_end_date THEN wctd.sales_points END), 0)
        -
        COALESCE(MAX(CASE WHEN wr.week_ending_date < p_week_end_date THEN wctd.sales_points END), 0)
