-- 2026-07-21: subtract marketing bonus from residual pool.
--
-- Peter identified gap: marketing bonus is paid out via payroll (3Market line item)
-- but was never being subtracted from the residual pool. Pool was overstated by
-- the marketing amount every week.
--
-- Q3 through 07-18 example (before this fix):
--   Marketing paid via 3Market: $146.00 (07-11) + $289.80 (07-18) = $435.80
--   This money went out of the agency but pool didn't know.
--
-- Fix: add marketing_by_week CTE that mirrors comm_by_week — sum payroll_detail keys
-- ILIKE %Market% per cycle_week where pay_date <= week_end, fallback to
-- wctd.marketing_pool_earned_weekly for current week (SP-projected). Aggregate to
-- qtd_marketing_paid_total, subtract in pool_calc alongside commission and mgr.
--
-- Frozen weeks (07-11, 07-18) NOT recomputed. Their wctd holds the Sunday-snapshot
-- values with the pre-fix pool (overstated by marketing). Freeze guard blocks any
-- attempt to rewrite them. This fix affects 07-25 and forward.
...
BEGIN
      ...
      COALESCE(
        (SELECT SUM((kv.value->>'period')::numeric)
         ...
         WHERE pd.agency_id = p_agency_id
           ...
           AND pr.pay_period_end = cw.week_end_date
           AND pr.pay_date <= p_week_end_date
           AND kv.key ILIKE '%Market%'),
        (SELECT wctd.marketing_pool_earned_weekly
         FROM public.weekly_cpr_team_detail wctd
         JOIN public.weekly_cpr_reports wr ON wr.id = wctd.weekly_cpr_report_id
         WHERE wr.agency_id = p_agency_id AND wctd.team_member_id = r.id AND wr.week_ending_date = cw.week_end_date
         LIMIT 1),
        0
      ) AS week_marketing_paid
    FROM roster r CROSS JOIN cycle_weeks cw
  ),
  marketing_qtd AS (SELECT tm_id, SUM(week_marketing_paid) AS qtd_marketing_paid FROM marketing_by_week GROUP BY tm_id),
  ...
  SELECT s.tm_id, (s.first_name || ' ' || s.last_name)::text, s.r_role::text, s.r_role_category::text, s.r_role_level::text,
    ...
    jsonb_build_object(
      ...
      'marketing_bonus_semantic', 'qtd_marketing from payroll_detail keys ILIKE %Market% where pay_date <= week_end (prior weeks actually paid), with wctd.marketing_pool_earned_weekly fallback for current week (marketing-points-based projection). Subtracted from pool.',
      'base_semantic', 'qtd_actual_base_paid from payroll_detail SALARY+REGULAR+HOURLY+PTO where pay_date <= week_end (prior weeks actually paid), with time-clock or design-rate fallback for current week (not yet paid). PTO included as base-equivalent per Peter directive.',
      'design_note', '2026-07-21: added marketing bonus subtraction. marketing_by_week mirrors comm_by_week — payroll_detail %Market% with pay_date filter + wctd.marketing_pool_earned_weekly fallback. Subtracts qtd_marketing_total in pool_calc. Frozen weeks 07-11 and 07-18 not recomputed (freeze guard blocks); this fix applies to 07-25 and forward.',
      ...
      'qtd_subtractions', jsonb_build_object(
        ...
        'qtd_marketing_bonus_actual', ROUND(s.qtd_marketing_total, 2),
        'qtd_marketing_bonus_source', 'payroll_detail keys ILIKE %Market% (pay_date <= week_end); wctd.marketing_pool_earned_weekly fallback for current week',
        ...
        'qtd_actual_base_source', 'payroll_detail SALARY+REGULAR+HOURLY+PTO (pay_date <= week_end); time-clock or design-rate fallback for current week',
        ...
        'qtd_burden', ROUND((s.qtd_base_in_pool_total + s.qtd_comm_total + s.qtd_mgr_total + s.qtd_marketing_total + v_qtd_hdb_max + s.qtd_prize_cart + s.qtd_wtq_trip + s.qtd_goals_total + s.qtd_bonus_paid_prior_total + s.qtd_bonus_pool) * v_burden_mult, 2)
      ),
      ...
      'team_totals', jsonb_build_object(
        ...
        'qtd_actual_commission', ROUND(s.qtd_comm_total, 2),
        'qtd_actual_marketing', ROUND(s.qtd_marketing_total, 2),
        'qtd_bonus_paid_prior', ROUND(s.qtd_bonus_paid_prior_total, 2),
