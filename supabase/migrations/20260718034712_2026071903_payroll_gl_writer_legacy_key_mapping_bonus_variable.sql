-- payroll_gl_writer: legacy earnings-key mapping + BONUS moved from fixed to variable.
-- Legacy SurePayroll keys mapped per Peter direction 2026-07-19:
--   REGULAR -> HOURLY (fixed, growth ramp)
--   0Advnce/2Serve/3True/4Manage -> variable (no growth ramp)
--   1Health/5Goals -> fixed (growth ramp)
--   LIFE* -> fixed (growth ramp) [Claude call, patterned on 1Health]
--   PTO -> HOURLY (fixed) [Claude call, standard payroll semantics]
--   - O/TIME -> OT (fixed) [legacy prefix variant]
--   REIMB. -> REIMBURSEMENTS [legacy dot variant]
--   blank3 -> skip [structural placeholder, always $0]
-- BONUS reclassified from fixed_bundle to variable per Peter direction 2026-07-19.
-- Signature unchanged. Recipe wrapper unaffected.
...
BEGIN
  ...
  SELECT id INTO v_team_acct FROM chart_of_accounts
    WHERE agency_id=p_agency_id AND account_code='COA-SUB-087';
  ...
  LOOP
    ...
    LOOP
      ...
      -- Reset buckets per person
      v_salary := 0; v_hourly := 0; v_ot := 0; v_bonus := 0;
      v_commission := 0; v_other := 0; v_reimb := 0;

      -- Classify each earnings item into buckets. Modern + legacy keys both handled here.
      -- Unknown keys fall through to gap-catcher (v_gross - v_recognized) which goes to Team.
      FOR v_key IN SELECT jsonb_object_keys(v_items) LOOP
        ...
        CASE v_key
          -- Fixed pay (subject to growth ramp)
          WHEN 'SALARY' THEN v_salary := v_salary + v_val;
          WHEN 'HOURLY' THEN v_hourly := v_hourly + v_val;
          WHEN 'REGULAR' THEN v_hourly := v_hourly + v_val;             -- legacy: base hourly pay
          WHEN 'PTO' THEN v_hourly := v_hourly + v_val;                 -- paid time off; fixed pay for time not worked
          WHEN 'OT' THEN v_ot := v_ot + v_val;
          WHEN '- O/TIME' THEN v_ot := v_ot + v_val;                    -- legacy overtime prefix
          WHEN '1Health' THEN v_hourly := v_hourly + v_val;             -- fringe stipend per Peter direction
          WHEN '5Goals' THEN v_hourly := v_hourly + v_val;              -- goal-based fixed pay per Peter direction
          WHEN 'LIFE *' THEN v_hourly := v_hourly + v_val;              -- fringe (imputed GTL) treated as fixed
          -- Variable pay (no growth ramp)
          ...
          WHEN '0Advnce' THEN v_other := v_other + v_val;               -- legacy advance/other variable
          WHEN '2Serve' THEN v_other := v_other + v_val;                -- legacy service variable
          WHEN '3True' THEN v_other := v_other + v_val;                 -- legacy true-up/commission
          WHEN '4Manage' THEN v_other := v_other + v_val;               -- legacy manager override
          -- Reimbursements (own bucket)
          WHEN 'REIMBURSEMENTS' THEN v_reimb := v_reimb + v_val;
          WHEN 'REIMB.' THEN v_reimb := v_reimb + v_val;                -- legacy dot variant
          -- Skip
          WHEN 'blank3' THEN NULL;
          ELSE NULL;  -- unknown keys fall to gap-catcher
        END CASE;
      END LOOP;
      ...
      ELSE
        -- Growth ramp applies to SALARY + HOURLY + OT only (fixed pay).
        -- BONUS moved to variable per Peter direction 2026-07-19.
    ...
    -- PaperNewt-direct JE (Owner/admin/PN-entity)
    ...
    -- PaperNewt-side intercompany reconciliation
    IF v_ic_credit > 0 THEN
      INSERT INTO journal_entries (
        agency_id, entry_date, description, source, reference_number,
        classification_status, created_at, business_entity_id
