-- 2026-08-07: gl_entry_writer was using settings.gl_default_cash_account_code
-- (resolving to 1012) as the offset for every comp_revenue/comp_deduction line.
-- But bank_gl_writer already posts the real State Farm deposit as Dr 1012 / Cr 0005,
-- so the same cash was being counted twice in 1012 and 0005 never cleared.
-- Revenue/expense amounts and all mapping/suspense logic are correct and untouched;
-- only the offset account resolution changes, from a cash account (1012) to the
-- comp clearing account (0005). Local variables renamed v_cash_acct_* ->
-- v_clearing_acct_* since this side is no longer standing in for cash.
...
BEGIN
  ...
  SELECT id, account_name INTO v_clearing_acct_id, v_clearing_acct_name FROM chart_of_accounts
    WHERE agency_id = p_agency_id
      AND business_entity_id = v_pss_entity
      AND account_code = v_clearing_acct_code
  ...
  LOOP
    ...
    BEGIN
      IF v_is_deduction THEN
        ...
        v_dr_name := v_target_account_name; v_cr_name := v_clearing_acct_name;
      ELSE
        IF v_amount > 0 THEN
          v_dr_account_id := v_clearing_acct_id; v_cr_account_id := v_target_account_id;
          v_dr_name := v_clearing_acct_name; v_cr_name := v_target_account_name;
        ELSE
          v_dr_account_id := v_target_account_id; v_cr_account_id := v_clearing_acct_id;
          v_dr_name := v_target_account_name; v_cr_name := v_clearing_acct_name;
        END IF;
