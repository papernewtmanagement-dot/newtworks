...
-- This migration was applied live on 2026-07-29 04:02:00 UTC (ledger name: reclassify_pending_je_v2_skip_susp_rules) but was never
-- committed to this repo at the time. Content below is copied verbatim
-- from supabase_migrations.schema_migrations.statements for version 20260729040200.
-- No live change is made by adding this file — it only brings the repo
-- mirror into sync with what is already running in production, so a
-- fresh `supabase db reset` from this repo reproduces live state.
-- Patch: reclassifier must skip catch-all rules that target SUSP itself
-- (would produce no-op reclassification and mark the JE 'classified' while still on SUSP).
...
BEGIN
  ...
  LOOP
    ...
    WHERE r.agency_id = p_agency_id AND r.is_active = TRUE
      -- Skip catch-all rules that target SUSP (no-op for reclassify)
      AND NOT (COALESCE(r.debit_account_code, '') = 'COA-SUSP'
               AND COALESCE(r.credit_account_code, '') = 'COA-SUSP')
      -- Skip rules with no matching content (would false-fire on everything)
    ...
    IF v_rule_id IS NULL OR v_target_code IS NULL OR v_target_code = '__SOURCE__'
       OR v_target_code = 'COA-SUSP' THEN
      v_count_still_pending := v_count_still_pending + 1;
      CONTINUE;
    END IF;
