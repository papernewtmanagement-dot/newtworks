-- Plan A payroll_gl_writer rewrite
-- Per person: PaperNewt entity → direct to PaperNewt payroll expense.
--             Agency entity → split fixed_bundle (SALARY+HOURLY+OT+BONUS) by ramp
--             fraction between Growth Budget and Team Budget accounts.
--             Commission + Other + employer taxes → all to Team Budget.
--             Reimbursements → Reimbursements-pending account.
-- Two JEs per pay run: one agency-side, one PaperNewt-side (either can be zero).

-- Drop old overloads first (op-rule: function-signature changes)
DROP FUNCTION IF EXISTS public.payroll_gl_writer(uuid, boolean);
DROP FUNCTION IF EXISTS public.payroll_gl_writer(uuid, uuid);

-- New main: 3-arg with optional pay_period_end filter for targeted dry-runs
CREATE OR REPLACE FUNCTION public.payroll_gl_writer(
  ...
  p_only_pay_period_end date DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_growth_acct IS NULL OR v_team_acct IS NULL OR v_reimb_pending_acct IS NULL
     OR v_intercompany_acct IS NULL OR v_pn_expense_acct IS NULL OR v_pn_cash_acct IS NULL THEN
  ...
  LOOP
    ...
    -- Already has JE — must be reversed first before rewrite (protects historical)
    ...
    LOOP
      ...
      SELECT first_name, last_name, business_entity_id, start_date
        INTO v_tm_first, v_tm_last, v_tm_entity, v_tm_start
      ...
      IF v_tm_entity = v_papernewt_entity THEN
        -- PaperNewt direct path
      ...
      ELSE
        -- Agency-split path
        v_fixed := v_salary + v_hourly + v_ot + v_bonus;
        ...
        v_team_share := ROUND((v_fixed - v_grow_share) + v_commission + v_other + COALESCE(v_er_taxes, 0), 2);
    ...
    -- LIVE: agency-side JE (skip if nothing agency-side)
    IF v_ic_credit > 0 THEN
      ...
      IF v_team_total > 0 THEN
        ...
        VALUES (v_agency_je, p_agency_id, v_team_acct, v_team_total, 0,
                'Team Budget: post-ramp fixed pay + commission + other + ER taxes', v_agency_entity);
      END IF;
    ...
    -- LIVE: PaperNewt-side JE (skip if nothing PN-side)
    IF v_pn_total > 0 THEN
      INSERT INTO journal_entries (
        agency_id, entry_date, description, source, reference_number,
        classification_status, created_at, business_entity_id
      ) VALUES (
        p_agency_id, v_pay_date, v_desc || ' [PaperNewt-side, owner/spouse]',
      ...
      VALUES (v_pn_je, p_agency_id, v_pn_expense_acct, v_pn_total, 0,
              'PaperNewt payroll expense (owner/spouse W-2 wages + ER taxes)', v_papernewt_entity);
      INSERT INTO journal_lines (journal_entry_id, agency_id, account_id, debit, credit, description, business_entity_id)
      VALUES (v_pn_je, p_agency_id, v_pn_cash_acct, 0, v_pn_total,
              'PaperNewt cash paid to owner/spouse', v_papernewt_entity);
    END IF;

    -- Stamp run. journal_entry_id points to agency-side (or PN if agency zero).
    UPDATE payroll_runs
    SET posted_at=NOW(),
        journal_entry_id=COALESCE(v_agency_je, v_pn_je),
        notes=COALESCE(notes,'') || ' [posted by payroll_gl_writer plan_a ' || NOW()::text ||
              CASE WHEN v_agency_je IS NOT NULL AND v_pn_je IS NOT NULL
                   THEN '; pn_je=' || v_pn_je::text ELSE '' END || ']'
    WHERE id=v_run_id;
...
-- Recipe wrapper (unchanged behavior, calls new 3-arg main)
...
AS $function$
BEGIN
  -- Recipe-driven: always live, no pay_period_end filter
