-- 2026-07-21: include PTO in base pay sum from payroll_detail.
--
-- Peter directive: PTO is base-equivalent time paid. It belongs in the base pay
-- subtraction. Previously base_by_week and actual_base_this_week summed only
-- SALARY + REGULAR + HOURLY. PTO is a separate line item in SurePayroll raw_earnings
-- (e.g. items.PTO.period).
--
-- Confirmed reconciliation for week ending 07-18 (paid 07-24):
--   John:   SALARY $700  + PTO $300 = $1,000 = design rate
--   Thomas: SALARY $832  + PTO $208 = $1,040 = design rate
--
-- Effect on frozen weeks 07-11 and 07-18: none (07-11 payroll has no PTO; 07-18
-- payroll is excluded from 07-18 compute by pay_date filter — falls to design
-- fallback which already reflects the full expected amount).
--
-- Effect on next week's 07-25 compute: prior-week 07-18 base now correctly includes
-- $508 of PTO (John $300 + Thomas $208), matching actual paid amounts.
--
-- Only two payroll subqueries touched: base_by_week and actual_base_this_week.
-- Everything else unchanged. Freeze guard still holds.
...
BEGIN
  ...
  SELECT s.tm_id, (s.first_name || ' ' || s.last_name)::text, s.r_role::text, s.r_role_category::text, s.r_role_level::text,
    ...
    jsonb_build_object(
      ...
      'design_note', '2026-07-21: PTO added to base sum in base_by_week and actual_base_this_week. Reconciled: John 07-18 SALARY $700 + PTO $300 = $1000 design; Thomas 07-18 SALARY $832 + PTO $208 = $1040 design. pay_date <= p_week_end_date on payroll joins keeps week-anchored snapshot. Freeze guard on write_weekly_comp_v2 prevents post-send drift.',
