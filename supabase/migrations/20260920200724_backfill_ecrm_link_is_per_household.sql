-- Peter 2026-09-20: one ECRM link covers the whole household, the same way the
-- phone and the marketing source already do. Typed once, it lands on every sale
-- under that name that has none. Nothing already linked is overwritten.
--
-- This is the backfill only. rp_log_sale and rp_edit_sale still require the link
-- on the sale itself; that has not changed.
