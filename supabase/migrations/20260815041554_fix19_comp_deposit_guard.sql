-- Round 8: FIX 19 — commission-deposit guard in cash_register_gl_writer, plus a
-- one-time cleanup of the bad row it retroactively fixes.
--
-- The comp writer already books commission-statement revenue net of employee
-- deductions. The cash register was then posting the matching bank deposit a
-- second time as unclassified income -- same money counted twice. This guard
-- catches that: any bank-account credit whose amount matches a comp period's
-- net total (within $1.00), with the comp period landing within -2/+3 days of
-- the deposit date, gets suppressed as a possible transfer instead of posted.
--
-- Confirming case: 2026-08-14 deposit of $19,017.83 against the 8/15 comp
-- period (revenue $20,289.97 less $1,272.14 in deductions = $19,017.83 net).

CREATE OR REPLACE FUNCTION public.cash_register_gl_writer(p_agency_id uuid, p_dry_run boolean DEFAULT true, p_from date DEFAULT '2026-08-01'::date, p_settle_minutes integer DEFAULT 90)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_count_skipped_payroll_funding int:=0;
  v_count_skipped_comp_deposit int:=0;
  ...
  v_payroll_funding_match boolean;
  v_comp_deposit_match boolean;
...
BEGIN
  ...
  LOOP
    ...
    v_payroll_funding_match := FALSE; v_comp_deposit_match := FALSE;
    ...
    -- FIX 19: commission-deposit guard. Runs before the payroll-funding and
    -- card-payment guards. Applies only to bank-account credits (money coming
    -- in). The comp writer already books commission-statement revenue net of
    -- deductions; the bank deposit of that same net amount would otherwise get
    -- posted a second time as unclassified income. Match against a comp
    -- period's net total (period_year/month/day grouped sum), within $1.00,
    -- with the comp period landing within -2/+3 days of the deposit date.
    IF v_direction = 'credit' AND v_account_kind = 'bank' THEN
      SELECT EXISTS (
        SELECT 1 FROM (
          SELECT period_year, period_month, period_day, sum(amount) AS period_total
          FROM comp_recap
          WHERE agency_id = p_agency_id
          GROUP BY period_year, period_month, period_day
        ) cr
        WHERE abs(v_amount - cr.period_total) <= 1.00
          AND make_date(cr.period_year, cr.period_month, cr.period_day)
              BETWEEN v_txn_date - 2 AND v_txn_date + 3
      ) INTO v_comp_deposit_match;

      IF v_comp_deposit_match THEN
        v_count_skipped_comp_deposit := v_count_skipped_comp_deposit + 1;
        IF NOT p_dry_run THEN
          UPDATE cash_register_preliminary
          SET status = 'possible_transfer',
              coding_question = 'Commission deposit — the revenue is already booked from the commission statement, so this deposit is held out of the profit and loss to avoid counting it twice.',
              updated_at = now()
          WHERE id = v_reg_id;
        END IF;
        CONTINUE;
    ...
    IF v_direction = 'debit' AND v_account_kind = 'bank'
      ...
      SELECT EXISTS (
        ...
        WHERE pr.agency_id = p_agency_id
          AND pr.pay_date BETWEEN v_txn_date AND v_txn_date + 4
          AND abs(v_amount - (pr.total_cash_requirement - COALESCE(pr.total_employee_deductions,0))) <= 1.00
      ) INTO v_payroll_funding_match;

      IF v_payroll_funding_match THEN
        v_count_skipped_payroll_funding := v_count_skipped_payroll_funding + 1;
        IF NOT p_dry_run THEN
          UPDATE cash_register_preliminary
          SET status = 'possible_transfer',
              coding_question = 'Payroll funding draft — the wage and tax cost is already booked from the SurePayroll import, so this money movement is held out of the profit and loss.',
              updated_at = now()
          WHERE id = v_reg_id;
        END IF;
        CONTINUE;
  ...
  RETURN jsonb_build_object(
    ...
    'skipped_payroll_funding', v_count_skipped_payroll_funding,
    'skipped_comp_deposit', v_count_skipped_comp_deposit,
...
-- One-time cleanup: remove the pre-guard duplicate. This was the 2026-08-14
-- $19,017.83 credit posted by cash_register_gl_writer against the 8/15
-- commission statement's already-booked net revenue.
DELETE FROM ledger WHERE id = 'cd7ee611-c4d1-4aa1-b099-63215ad4ffa3';

UPDATE cash_register_preliminary
SET status = 'possible_transfer',
    coding_question = 'Commission deposit — the revenue is already booked from the commission statement, so this deposit is held out of the profit and loss to avoid counting it twice.',
    updated_at = NOW()
WHERE id = '8c0790a0-d50c-48de-aee2-afaf6f688530';
