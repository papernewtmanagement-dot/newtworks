-- 20260722_verdict_rpc_enforces_lss_autopass
--
-- hiregauge_three_construct_verdict now calls _hiregauge_lss_autopass with the
-- 4 structured res_* columns (licenses, education, prior similar role, plus
-- languages hooked up in case any future exception uses it). When the helper
-- returns any auto_pass_* status, verdict is overridden to 'decline_lss'.
--
-- Precedence order in verdict:
--   1. character_floor_failed → 'decline_character'
--   2. score_0_10 IS NULL → 'insufficient_data'
--   3. lss_autopass.status IN (auto_pass_no_exceptions, auto_pass_heavy_evidence_missing, auto_pass) → 'decline_lss'
--   4. framework verdict (hire/consider/decline)
--
-- meta gains an 'lss_autopass' key containing the helper's full return.
-- Signature unchanged — CREATE OR REPLACE only touches body.
...
AS $function$
DECLARE
  v_ta record; v_team_id uuid; v_is_active boolean; v_is_archived boolean;
  ...
  v_lss_acc numeric; v_dims_scored int := 0;
...
BEGIN
  ...
  END IF;
  v_lss_acc := v_ta.lss_total_accuracy;
  ...
  -- Compute LSS auto-pass status from structured qualification data
  v_lss_autopass := public._hiregauge_lss_autopass(
    ...
    v_ta.res_licenses,
    v_ta.res_education,
    v_ta.res_prior_similar_role
  );
  v_lss_autopass_status := v_lss_autopass->>'status';

  verdict := CASE
    WHEN character_floor_status = 'floor_failed' THEN 'decline_character'
    WHEN score_0_10 IS NULL THEN 'insufficient_data'
    WHEN v_lss_autopass_status IN ('auto_pass_no_exceptions','auto_pass_heavy_evidence_missing','auto_pass') THEN 'decline_lss'
    ELSE (CASE public._hiregauge_layer_verdict('framework', score_0_10)
            WHEN 'pass' THEN 'hire' WHEN 'consider' THEN 'consider' ELSE 'decline' END)
  END;
  ...
  v_calibration := CASE
    ...
    WHEN v_retro_verdict = 'fail_confirmed' AND verdict IN ('decline','decline_character','decline_lss') THEN 'framework_agrees_negative'
    WHEN v_retro_verdict = 'pass' AND verdict IN ('decline','decline_character','decline_lss') THEN 'framework_missed_positive'
    WHEN v_retro_verdict = 'fail_confirmed' AND verdict IN ('hire','consider') THEN 'framework_missed_negative'
    WHEN v_retro_verdict = 'flag' THEN 'partial' ELSE 'no_retrospective' END;
