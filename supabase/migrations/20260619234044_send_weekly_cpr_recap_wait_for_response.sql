-- Fix silent-failure bug: synchronously wait for Composio's HTTP response,
-- only stamp sent_to_team_at on 2xx, surface actual status/body on failure.
...
DECLARE
  ...
  v_response                record;
  v_attempts                int := 0;
  v_max_attempts            int := 60;   -- 60 * 500ms = 30s ceiling
BEGIN
  ...
  -- *** NEW: synchronously wait for response, up to 30 seconds ***
  LOOP
  ...
  -- ONLY now do we stamp sent_to_team_at — request confirmed 2xx
