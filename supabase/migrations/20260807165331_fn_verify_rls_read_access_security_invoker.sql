...
-- This migration was applied live on 2026-08-07 16:53:31 UTC (ledger name: fn_verify_rls_read_access_security_invoker) but was never
-- committed to this repo at the time. Content below is copied verbatim
-- from supabase_migrations.schema_migrations.statements for version 20260807165331.
...
DECLARE
  ...
  v_roles text[];
BEGIN
  -- This function must be called by a role that can both bypass RLS
  -- (table owner / BYPASSRLS, e.g. postgres or service_role) and SET ROLE
  -- to anon/authenticated. That's how Claude/Peter reach the DB via the
  -- Supabase service connection, so this is the expected caller.
  ...
  FOREACH t IN ARRAY p_tables LOOP
    ...
    -- true count: run as the calling (owner-privileged) role, bypasses RLS
