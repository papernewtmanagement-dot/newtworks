-- 2026-07-21: revert marketing bonus subtraction (migration 235500).
--
-- Marketing bonus is funded from its own budget line (marketing_pool basis, tracked
-- via marketing_pool_earned_weekly column) and is NOT part of the residual pool.
-- Subtracting it from the residual pool was wrong.
--
-- Restores compute to post-PTO state. Function subtracts base+comm+mgr+HDB+
-- prize_cart+wtq_trip+goals+bonus_paid_prior from envelope. Marketing lives outside.
--
-- Pool values restored: 07-11 back to $969.97, 07-18 back to $746.86.
...
BEGIN
  ...
  SELECT s.tm_id, (s.first_name || ' ' || s.last_name)::text, s.r_role::text, s.r_role_category::text, s.r_role_level::text,
    ...
    jsonb_build_object(
      ...
      'design_note', '2026-07-21: reverted marketing bonus subtraction. Marketing bonus is funded from its own budget line (marketing_pool basis) and is NOT part of the residual pool. This function subtracts base+comm+mgr+HDB+prize_cart+wtq_trip+goals+bonus_paid_prior from envelope. Marketing lives outside.',
