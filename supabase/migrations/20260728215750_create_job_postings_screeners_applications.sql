...
-- 3. Raw applications landing zone (audit + knockout gate before hiring_candidates)
...
-- 4. Extend hiring_candidates with source + posting linkage
...
-- 6. RLS — agency-scoped, admin write, public read on active postings only
ALTER TABLE public.job_screener_questions ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.job_postings ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.job_applications ENABLE ROW LEVEL SECURITY;

-- Screener questions: admin-only (agency-scoped)
...
-- Job postings: authenticated agency users can read/write all; anon can read only ACTIVE + published_to_careers_page
...
-- Job applications: agency-scoped admin write; anon INSERT allowed (careers-page apply form)
