...
DECLARE
  ...
  v_weeks_in_quarter   constant int := 13;
...
BEGIN
  ...
  -- Whole-team QTD sales-points total for this week.
  ...
  -- Count of team members "required to get sales points for WtW" — Account Managers.
  -- Counted from this week's detail rows so the snapshot policy is automatically honored.
  SELECT COUNT(*) INTO v_required_count
   ...
   WHERE r.agency_id = p_agency_id
     AND r.week_ending_date = p_week_ending_date
     AND t.category = 'agency'
     AND t.role_level = 'Account Manager';

  -- Manager baseline = team_total / required_count / weeks_in_quarter.
  -- Used by managers above Account Manager (Unit/Team/Section/Office). 10% of this is Base Advance.
  v_manager_baseline := CASE WHEN v_required_count > 0
    THEN v_team_total_sp / v_required_count::numeric / v_weeks_in_quarter::numeric
    ELSE 0
  END;
  ...
  WITH base AS (
    SELECT wctd.team_member_id,
      ...
      -- HSM = SUM(H + S + M) across detail rows in this cycle BEFORE this week.
      -- Excludes the prior-quarter-end anchor row (which is before cycle_start).
      COALESCE((
  ...
  computed AS (SELECT
      ...
      -- Base Advance: 10% of the relevant SP figure. For Unit Manager (and higher), uses the
      -- new manager baseline = team_total_sp / required_AM_count / 13. For other Sales roles,
      -- uses individual WoW SP delta.
      CASE
  ...
  SELECT c.team_member_id, c.c_weekly_pay, c.c_base_advance, c.c_health_bonus, c.c_service_surge_share,
    ...
    jsonb_build_object(
      ...
      'required_am_count', v_required_count,
