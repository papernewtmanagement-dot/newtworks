DROP VIEW IF EXISTS public.v_agency_rates;
DROP VIEW IF EXISTS public.v_producer_roi_inputs;

CREATE VIEW public.v_agency_rates AS
SELECT
  a.id,
  a.name,
  a.pc_base_rate,
  a.smvc_rate_pc,
  a.pc_base_rate + a.smvc_rate_pc AS effective_pc_rate,
  a.smvc_rate_pc_prior_year,
  a.smvc_rate_pc_2_years_prior,
  a.blended_rate_other,
  (SELECT annualized_rate FROM public.compute_lapse_rate(a.id) WHERE line = 'blended') AS lapse_rate_annual,
  a.aipp_rate,
  a.payroll_burden_multiplier,
...
WITH cfg AS (
  SELECT
    ...
    COALESCE(
      (SELECT annualized_rate FROM public.compute_lapse_rate(a.id) WHERE line = 'blended'),
      0.11
    ) AS lapse_rate_annual,
...
recent AS (
  ...
  WHERE pp.premium_type = 'new'::text
    AND make_date(pp.period_year, pp.period_month, 1) >=
        (date_trunc('month'::text, CURRENT_DATE::timestamp with time zone) - '3 mons'::interval)
...
SELECT
  ...
  COALESCE(
    round(r.new_premium_monthly_avg * 12::numeric * cfg.smvc_rate_pc * (1::numeric - cfg.lapse_rate_annual), 2),
    0::numeric
...
ALTER TABLE public.agency DROP COLUMN IF EXISTS lapse_rate_annual;
