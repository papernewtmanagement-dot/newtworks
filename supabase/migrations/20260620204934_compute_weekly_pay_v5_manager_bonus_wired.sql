-- compute_weekly_pay v5: Manager Bonus now wired off compute_scorecard_bonus()
--   role_level Unit Manager  → 0.001 × scorecard_annual / 52
--   role_level Team Manager  → 0.002 × scorecard_annual / 52
--   role_level Office Manager→ 0.003 × scorecard_annual / 52
--   else → 0
-- Scorecard call pins p_as_of_date = p_week_ending_date for stability.
...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      CASE WHEN t.role = 'Reception' THEN COALESCE((
        ...
        ), 0) ELSE NULL END                                              AS reception_hours,
      COALESCE((SELECT SUM(
        ...
        WHERE tor.agency_id=p_agency_id AND tor.requester_team_id=t.id
          AND tor.status='approved' AND tor.start_date<=p_week_ending_date AND tor.end_date>=v_week_start_date
          AND tor.is_paid=true), 0)                                      AS paid_off_hours,
      COALESCE((SELECT SUM(
          CASE WHEN tor.partial_day IN ('morning','afternoon') THEN 4 ELSE 8 END
          * (SELECT COUNT(*)::int FROM generate_series(GREATEST(tor.start_date, v_week_start_date),
               LEAST(tor.end_date, p_week_ending_date), '1 day'::interval) d
        ...
        WHERE tor.agency_id=p_agency_id AND tor.requester_team_id=t.id
          AND tor.status='approved' AND tor.start_date<=p_week_ending_date AND tor.end_date>=v_week_start_date
          AND tor.is_paid=false), 0)                                     AS unpaid_off_hours,
      COALESCE((SELECT COUNT(*)::int FROM public.team_health_checkins thc
        WHERE thc.agency_id=p_agency_id AND thc.team_id=t.id
          ...
          AND thc.hit_today=true AND EXTRACT(DOW FROM thc.log_date) BETWEEN 1 AND 5
        ), 0)                                                            AS weekday_hits,
      COALESCE((SELECT wctd_last.sales_points
  ...
  weighted AS (SELECT h.*,
      ...
      LEAST(1.00, 0.50 + CASE WHEN h.license_pc THEN 0.35 ELSE 0 END + CASE WHEN h.license_lh THEN 0.10 ELSE 0 END + CASE WHEN h.license_ips THEN 0.05 ELSE 0 END) AS license_w,
  ...
  computed AS (SELECT
      ...
      w.this_week_qtd_sp, w.last_week_qtd_sp, w.paid_qtd, w.weekday_hits,
      w.surge_eligible, w.weighted_hours, w.hours_for_surge, w.hourly_hours,
      w.salaried_paid_fraction, w.paid_off_hours, w.unpaid_off_hours,
      p.surge_pool, p.total_weighted_hours, p.reception_wages_this_week,
      CASE WHEN w.pay_type='HOURLY' THEN w.pay_rate * w.hourly_hours
           ELSE w.pay_rate * w.salaried_paid_fraction END                AS c_weekly_pay,
      CASE WHEN w.role_category='Sales'
           THEN 0.10 * GREATEST(0, w.this_week_qtd_sp - w.last_week_qtd_sp)
           ELSE 0 END                                                    AS c_base_advance,
      ...
      -- Manager Bonus: percentage of annual on-time Scorecard, paid weekly = /52
      CASE w.role_level
        WHEN 'Unit Manager'   THEN COALESCE(v_scorecard_annual, 0) * 0.001 / 52.0
        WHEN 'Team Manager'   THEN COALESCE(v_scorecard_annual, 0) * 0.002 / 52.0
        WHEN 'Section Manager'THEN COALESCE(v_scorecard_annual, 0) * 0.002 / 52.0
        WHEN 'Office Manager' THEN COALESCE(v_scorecard_annual, 0) * 0.003 / 52.0
        ELSE 0::numeric
      END                                                                AS c_manager_bonus,
  ...
  SELECT c.team_member_id, c.c_weekly_pay, c.c_base_advance, c.c_health_bonus, c.c_service_surge_share,
    CASE WHEN c.paid_qtd IS NULL THEN NULL
         ELSE GREATEST(0, c.this_week_qtd_sp - (c.paid_qtd + c.c_weekly_pay + c.c_base_advance + c.c_health_bonus + c.c_service_surge_share + c.c_manager_bonus + c.c_agency_profit_share)) END AS true_pay_bonus,
    ...
    jsonb_build_object(
      ...
      'pay_paid_to_date_qtd', c.paid_qtd, 'weekday_hits', c.weekday_hits,
