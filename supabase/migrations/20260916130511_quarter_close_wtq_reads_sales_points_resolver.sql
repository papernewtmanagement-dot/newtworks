-- quarter_close_wtq picked the quarter MVP by ADDING UP the sales points on every
-- weekly report row in the quarter. Those rows each hold a running quarter-to-date
-- total, not that week's earnings, so the sum was inflated and the MVP could be the
-- wrong person. The quarter figure is the quarter-to-date number at the close week,
-- which is what sales_points_qtd_for returns. Nothing else about the payout changes.
CREATE OR REPLACE FUNCTION public.quarter_close_wtq(p_agency_id uuid, p_as_of date DEFAULT NULL::date)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_report_id IS NULL THEN
    RETURN jsonb_build_object('ok', false,
      'reason', 'no CPR report for the final week of the quarter',
  ...
  IF v_wtq IS NULL THEN
    RETURN jsonb_build_object('ok', false,
      'reason', 'compute_pool_carveouts returned no wtq_trip block',
      'cycle_end', v_cycle_end, 'rows_written', 0);
  END IF;
  ...
  SELECT et.team_id,
         ...
         COALESCE((SELECT f.sales_points
                   FROM public.sales_points_qtd_for(p_agency_id, v_cycle_end, et.team_id) f), 0)
  ...
  IF v_halted OR v_pot <= 0 OR v_eligible = 0 THEN
    ...
    RETURN jsonb_build_object('ok', true, 'trip_awarded', false,
  ...
  SELECT jsonb_agg(jsonb_build_object(
           'name', r.display_name, 'quarter_sales_points', r.quarter_points,
  ...
  RETURN jsonb_build_object('ok', true, 'trip_awarded', true,
    'quarter', v_cycle.quarter_label, 'cycle_end', v_cycle_end,
    'pot_dollars', v_pot, 'mvp', v_mvp_name, 'mvp_dollars', v_mvp_dollars,
    'mvp_quarter_sales_points', v_mvp_points,
    'rest_count', v_eligible - 1, 'rest_each_dollars', v_rest_each,
    'eligible_count', v_eligible, 'rows_written', v_rows, 'who', v_detail);
END;
$function$;
