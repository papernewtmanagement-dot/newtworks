-- Peter 2026-09-11: "Treat marketing points the way sales points are currently treated."
-- How sales points actually behave, verified: team_checkins_sync_to_cpr_detail has NO
-- roster filter of any kind. It writes whoever reports, and COALESCE never blanks an
-- existing value. get_cpr_detail_sales_points_qtd reads the latest non-null figure per
-- person across the cycle, with no roster filter either. John Kostov (end_date
-- 2026-09-01) kept getting sales_points written for the week AFTER he left, and his
-- value still counts in the quarter total. Points belong to the person, not to their
-- employment status.
-- My previous filter (20260912040858) still carried an employment rule: a leaver was
-- kept only while end_date >= quarter start, so they would drop at the next quarter
-- boundary. Sales has no such boundary. Removed.
-- Roster is now: anyone active, plus anyone holding marketing points in this quarter,
-- whenever they left. Nothing about employment gates the points.
...
BEGIN
  ...
  -- (finrebuild_d3_drop_journal_entries_wipe_rename_ledger). ledger is self-contained:
  -- agency_id/account_id/debit/credit/entry_date all live on the one row now.
  ...
  WITH person_points AS (
    SELECT team_member_id, SUM(points) AS points_qtd,
  ...
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'left_mid_quarter',    (t.end_date IS NOT NULL AND t.end_date <= v_week_end),
    -- 2026-08-28: breakout columns dropped (never populated). Keys retained at 0 for
    -- frontend compatibility; they always read 0 anyway.
    'reviews_qtd',         0,
    'quoted_qtd',          0,
    'sold_qtd',            0,
  ...
  WHERE t.agency_id = p_agency_id
    ...
    -- Peter 2026-09-11: same treatment as sales points. Employment status does not
    -- gate the points. Active roster, plus anyone holding points this quarter.
