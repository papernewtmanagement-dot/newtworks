...
BEGIN
  ...
  RAISE NOTICE 'BEFORE: 2026 Jan-Jun agency 6005 Payroll Costs total = %', v_check_before;
  ...
  RAISE NOTICE 'AFTER: Growth=%, Team=%, Reimb(licensing)=%, TOTAL=% (BEFORE=%)',
    v_check_after_growth, v_check_after_team, v_check_after_reimb, v_check_after_total, v_check_before;

  IF ABS(v_check_after_total - v_check_before) > 0.10 THEN
    RAISE EXCEPTION 'Reconciliation FAILED: Growth+Team+Reimb=% vs Before=% (diff=%)',
      v_check_after_total, v_check_before, v_check_after_total - v_check_before;
  END IF;
  RAISE NOTICE 'Reconciled within tolerance.';
END $mig$;
