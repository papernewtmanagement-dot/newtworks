...
-- 1. Revert SafeRingZ (I never should have classified it)
...
BEGIN
  ...
  -- 1. Rename accounts to plain English
  ...
  -- 2. Revert SP SaferingZ -> suspense (I shouldn't have classified it)
  UPDATE public.journal_lines
     SET account_id = v_susp_out_id
   WHERE journal_entry_id IN (
     SELECT id FROM public.journal_entries
      WHERE source='pf4_personal_backfill' AND business_entity_id = v_pers_id
        AND description ILIKE '%SP SAFERINGZ%'
   )
     AND account_id = v_safety_pers_id;

  UPDATE public.journal_entries
     SET classification_status='pending_review', classified_by=NULL, classified_at=NULL
   WHERE source='pf4_personal_backfill' AND business_entity_id = v_pers_id
     AND description ILIKE '%SP SAFERINGZ%';

  -- 3. Create Home Office Security on PaperNewt
  ...
  -- 4. Buck & Doe: repoint personal-side DR line + create PaperNewt mirror
  FOR v_txn IN
    SELECT ct.id AS ct_id, ct.transaction_date, ct.description, ct.amount,
           ct.journal_entry_id AS pers_je_id
    FROM public.credit_transactions ct
    WHERE ct.business_entity_id = v_pers_id
      AND ct.description ILIKE '%BUCK AND DOE%'
  LOOP
    v_amt := ABS(v_txn.amount);

    UPDATE public.journal_lines
       SET account_id = v_reimb_pers_id
    ...
    UPDATE public.journal_entries
       SET memo = COALESCE(memo || ' | ', '') || 'Personal CC used for PaperNewt Home Office Security',
           classified_by = 'pf4f_buck_and_doe',
           classified_at = NOW()
  ...
  -- 5. Deactivate PERSONAL-9130 on personal (biz security lives on PaperNewt now)
  UPDATE public.chart_of_accounts
     SET is_active = false,
         account_name = 'Home Security & Safety [DEPRECATED — moved to PaperNewt]'
   WHERE id = v_safety_pers_id;

END $pf4f$;

-- 6. Fix the get_pnl_history_own_only display: fall back to account_subtype when root=account_name
...
AS $function$
  ...
  post_cutover AS (
    SELECT
      ...
      -- Section: if the COA has a parent above it, use root name.
      -- Else, fall back to a friendly version of subtype so we don't render account_name twice.
      COALESCE(
        NULLIF(r.root_name, coa.account_name),
