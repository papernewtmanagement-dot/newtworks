...
-- consonant" forever) and the wheel can never be spun again. This is the bug
-- Peter reported: "It doesn't allow them to keep spinning."
...
-- quiz_phrase_solve preserves pending_spin (it doesn't consume it), but for
-- the same reason -- so the client never has to guess -- include it in the
-- return too instead of relying on the frontend's fallback-to-previous-value.
