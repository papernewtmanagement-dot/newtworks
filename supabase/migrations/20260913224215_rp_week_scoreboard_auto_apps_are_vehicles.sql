-- Auto apps into compute_sp_from_production must be VEHICLES, not households.
-- Every auto product row carries policy_count = 1 (one household), but State Farm
-- writes one auto policy per vehicle and the incentive builder counts one app per
-- vehicle. The auto tier steps every 6 apps and lifts the rate on ALL property
-- premium, so counting households undercharged every property dollar.
-- Fire and life stay on policy_count. Display fields (policies, vehicles) unchanged.
