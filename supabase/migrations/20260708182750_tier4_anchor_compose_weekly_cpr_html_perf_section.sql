...
-- Dashboard P&C growth widget I fixed earlier today (2026-07-08).
--
...
-- The COALESCE fallback keeps v_days_elapsed sane if v_snap.id IS NULL (no
-- snapshot found) even though the perf CTE only fires inside IF v_snap.id IS NOT NULL.
--
-- Payroll on-time (payroll_with_ota CTE) is separately anchored to
-- days_employed_year which uses p_week_ending_date -- that stays as-is because
-- payroll YTD is stream data (payroll runs are events, not snapshots), and
-- p_week_ending_date is the correct endpoint anchor per the stream-vs-snapshot
-- distinction in the anchor rule.
...
BEGIN
  ...
  -- Sanity: expect exactly one occurrence of the old line
