...
DECLARE
  ...
  v_account_id uuid; v_business_entity_id uuid; v_source_account_code text;
...
BEGIN
  ...
  FOR v_reg_id, v_txn_date, v_direction, v_amount, v_account_last4, v_merchant,
      v_account_label, v_coding_status, v_peter_debit, v_peter_credit, v_sugg_debit, v_sugg_credit IN
    SELECT c.id, c.txn_date, c.direction, c.amount, c.account_last4, c.merchant,
           ...
           c.suggested_debit_account, c.suggested_credit_account
    FROM cash_register_preliminary c
    WHERE c.agency_id = p_agency_id
      AND c.txn_date >= p_from
      AND c.created_at < now() - (p_settle_minutes || ' minutes')::interval
  ...
  LOOP
    ...
    v_suspense_reason := NULL; v_account_id := NULL; v_business_entity_id := NULL; v_code := NULL; v_description := NULL;
    v_source_account_code := NULL; v_rule_id := NULL; v_rule_debit_code := NULL; v_rule_credit_code := NULL;
    v_rule_target_entity := NULL; v_rule_resolved_code := NULL;
    ...
    -- D4: resolve the register row's account (and, for FIX 7, its chart-of-accounts code)
    SELECT a.id, a.business_entity_id, coa.account_code
    INTO v_account_id, v_business_entity_id, v_source_account_code
    FROM accounts a
    ...
    IF v_code IS NULL AND v_merchant IS NOT NULL AND length(trim(v_merchant)) > 0 THEN
      ...
      IF v_rule_id IS NOT NULL THEN
        IF v_rule_debit_code = '__SKIP__' AND v_rule_credit_code = '__SKIP__' THEN
          -- Doesn't belong in the P&L at all. Leave the register row untouched
          -- for the statement pass to handle normally later — no post, no
        ...
        IF v_rule_resolved_code IS NOT NULL AND v_rule_resolved_code <> '__SOURCE__' THEN
          ...
          IF v_target_account_id IS NOT NULL THEN
            ...
            IF v_target_account_type NOT IN ('income','expense') THEN
              -- Resolves to a real account, but not one that belongs in the
              -- P&L (e.g. a liability or asset code). Leave the register row
              -- untouched, same as the not-P&L rule case above.
