-- Add Retention Quality Multiplier (RQM) to compute_warning_trigger.
-- Retention pool share now scales with book health: high lapse → lower credit.
-- Formula: RQM = LEAST(1.0, benchmark_lapse / GREATEST(actual_lapse, 0.001))
-- Where benchmark_lapse = 0.12 (industry-standard for well-retained SF book).
-- Sales roles unaffected — their own_renewal_stack already uses actual lapse in decay formula.
...
RETURNS TABLE(
  ...
  own_new_business_annualized numeric,
  renewal_stack_annual numeric,
  own_renewal_stack_credited numeric,
  retention_pool_share_annual numeric,
  retention_quality_multiplier numeric,
  attributed_revenue_annual numeric,
  coverage_bar numeric,
  coverage_pct numeric,
  coverage_status text,
  profitability_bar numeric,
  profitability_pct numeric,
  profitability_status text,
  lapse_rate_used numeric,
  lapse_status text,
...
BEGIN
  ...
  -- Retention Quality Multiplier: proportional to how well book is retained vs benchmark
  -- At benchmark (12%): RQM = 1.0. At 20%: 0.60. At 27%: 0.44. Capped at 1.0 (no bonus).
  ...
  compose AS (
    SELECT
      b.id, b.full_name, b.role, b.role_category,
  ...
  computed AS (
    SELECT
      c.*,
      c.q_agency_comm_stripped * 4.0 AS own_new_annualized,
      CASE WHEN c.c_tenure_mult >= 1.0
           THEN c.renewal_stack_raw * v_stack_producer_pct
           ELSE 0 END AS own_stack_credited,
      -- Retention pool share NOW scaled by Retention Quality Multiplier
  ...
  final AS (
    SELECT
      f.*,
      (f.own_new_annualized + f.own_stack_credited + f.retention_pool_share) AS attributed_revenue,
  ...
  SELECT
    ...
    ROUND(f.c_fully_loaded, 2),
    v_trailing_q,
    ROUND(f.pc_prem, 2),
    ROUND(f.lh_prem, 2),
    ROUND(f.q_agency_comm_stripped, 2),
    ROUND(f.own_new_annualized, 2),
    ROUND(f.renewal_stack_raw, 2),
    ROUND(f.own_stack_credited, 2),
    ROUND(f.retention_pool_share, 2),
    -- retention_quality_multiplier reported for all rows (agency-level value)
    ROUND(v_retention_quality, 4),
    ROUND(f.attributed_revenue, 2),
    ROUND(f.coverage_bar_val, 2),
    CASE WHEN f.coverage_bar_val > 0
         THEN ROUND((f.attributed_revenue / f.coverage_bar_val) * 100, 2)
         ELSE NULL END,
    CASE
      WHEN f.coverage_bar_val <= 0 THEN 'na'
    ...
    CASE WHEN f.profitability_bar_val > 0
         THEN ROUND((f.attributed_revenue / f.profitability_bar_val) * 100, 2)
         ELSE NULL END,
    CASE
      WHEN f.profitability_bar_val <= 0 THEN 'na'
    ...
    jsonb_build_object(
      ...
      'formulas', jsonb_build_object(
        ...
        'retention_pool_share', 'agency_renewal_ttm × 0.35 × hours_share × retention_quality_multiplier',
...
-- Add retention_quality_multiplier column to weekly_cpr_team_detail
ALTER TABLE public.weekly_cpr_team_detail
  ADD COLUMN IF NOT EXISTS retention_quality_multiplier numeric;

-- Update write_weekly_comp_v2 to persist the new column
...
'Per-person weekly seat assessment: Coverage + Profitability. '
'Attribution: own_new × 4 + own_stack × 0.65 (Wk52+) + retention_pool_share × RQM (Retention only). '
'RQM (Retention Quality Multiplier) = LEAST(1.0, 0.12 / actual_lapse). Retention gets full credit only when book is retained at benchmark. '
'Coverage bar = fully-loaded; Profitability bar = fully-loaded × 2.5. Green ≥100%, Yellow ≥80%. '
'lapse_rate_used and lapse_status retained in output for reference but no longer surfaced as a separate column.';
