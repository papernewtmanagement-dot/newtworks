-- Agency Performance monthly grain — one RPC returning 12-month series for
-- every row on the section 10 table. Called once per section render;
-- frontend caches in state and renders per row on expand.
--
-- Semantic per metric family:
--   Gain rows (auto/fire/life new/lost/gain, life_npf_count/premium):
--     Monthly INCREMENT within the calendar month.
--     Computed as (end_of_month YTD) - (end_of_prior_month YTD) from the
--     latest weekly snapshot on-or-before that month-end that has YTD cols
--     populated. Null when either side unavailable (pre-Q2 2026 weekly
--     ingestion did not populate YTD cols).
--     January resets to 0 baseline (YTD counter starts fresh each year).
--
--   Premium rows (auto/fire/life $):
--     End-of-month book value from agency_snapshot cadence='monthly'.
--     Preferred: row dated the 1st of the FOLLOWING month (= end-of-month
--     snapshot). Fallback: row dated the 1st of this month.
--
--   Rate rows (smvc %, smvc $, scorecard):
--     Computed at month-end via compute_agency_on_time_smvc and
--     compute_scorecard_bonus. SMVC $ = smvc_pct × pc_book_premium at
--     month-end (both from the same compute output).
...
DECLARE
  ...
  v_sc_val       numeric;
BEGIN
  -- Iterate calendar months: 11 months back through the current month of p_week_ending_date
  FOR v_i IN 0..11 LOOP
    ...
    -- Baseline: prior month-end YTD. Jan resets to 0.
    ...
    -- Deltas — NULL when either side missing
    ...
    -- Premium book: prefer next-month-1st snapshot (= end of this month); fallback this-month-1st.
    ...
    -- Rate rows: SMVC pct + pc_book_premium at month-end; SMVC $ = pct * basis; Scorecard bonus_projected.
    ...
    v_smvc_dol  := v_smvc_dol  || CASE WHEN v_smvc_pct_val IS NULL OR v_smvc_pc_prem IS NULL THEN NULL::numeric ELSE v_smvc_pct_val * v_smvc_pc_prem END;
