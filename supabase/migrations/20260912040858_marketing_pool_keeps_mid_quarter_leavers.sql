-- Peter 2026-09-11: "John's marketing points should remain relevant to the marketing
-- pool, as should anyone who leaves mid quarter."
-- The pool denominator (v_total_points_qtd) already sums every marketing_points row
-- for the quarter, leavers included. The per-person list did not: it required
-- t.is_active = true AND t.archived_at IS NULL, so a mid-quarter leaver silently
-- dropped out of 'people' and write_weekly_marketing_bonus then had no row to write
-- for them. Their points still inflated the denominator while their own share went
-- unwritten. Roster now keeps anyone whose end_date falls inside the quarter.
-- Scope: marketing pool only. Requirements, Hours and Payroll still drop pre-week
-- leavers, unchanged.
...
BEGIN
  ...
  WHERE t.agency_id = p_agency_id
    ...
    -- Peter 2026-09-11: mid-quarter leavers stay in the marketing pool for the
    -- rest of the quarter their points were earned in.
    AND (t.is_active = true OR (t.end_date IS NOT NULL AND t.end_date >= v_quarter_start));
