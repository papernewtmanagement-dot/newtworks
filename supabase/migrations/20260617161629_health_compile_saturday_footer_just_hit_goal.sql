-- Saturday health compile footer collapses to "X/Y hit goal" — drop the
-- "at goal or on time" + "reporting" decomposition and the "Week's in the books"
-- consolation tail. The whole-team-at-goal celebration line stays.
...
BEGIN
  ...
  -- Footer: weekday shows "at goal or on time" + reporting count;
  --         Saturday collapses to just "X/Y hit goal".
  IF v_is_saturday THEN
  ...
  -- Closing tone: keep the whole-team-at-goal celebration; drop the Saturday
  -- consolation tail; keep Friday's "one day left" nudge.
