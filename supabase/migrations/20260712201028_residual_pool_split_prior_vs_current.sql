-- Fix: prior_qtd_paid should EXCLUDE current week (we're computing that settlement).
-- qtd_mgr, qtd_hdb, qtd_commission still INCLUDE current week (they're the ACTUAL running totals).
...
RETURNS TABLE (
  ...
  sales_points_share_pct        numeric,
  ...
  weekly_sales_pool_share       numeric,
  weekly_retention_pool_share   numeric,
...
BEGIN
  ...
  -- QTD actual running totals (INCLUDE current week — these are running QTD, not "prior")
  ...
  -- Prior-week paid totals (EXCLUDE current week — used for settlement anchor)
  prior_paid AS (
    SELECT r.id AS tm_id,
    ...
    LEFT JOIN public.weekly_cpr_reports wr
      ON wr.agency_id = p_agency_id
     AND wr.week_ending_date >= v_cycle_start
     AND wr.week_ending_date <  p_week_end_date   -- STRICT < : exclude current week
    LEFT JOIN public.weekly_cpr_team_detail wctd
      ON wctd.weekly_cpr_report_id = wr.id AND wctd.team_member_id = r.id
    GROUP BY r.id
  ),
  ...
  SELECT
    ...
    jsonb_build_object(
      ...
      'qtd_subtractions', jsonb_build_object(
        ...
        'qtd_burden',                   ROUND((s.qtd_base_in_pool_total + s.qtd_comm_total + s.qtd_bonus_pool) * v_burden_mult, 2)
      ),
      ...
      'carveouts_outside_pool', jsonb_build_object(
        ...
        'note',              'Agency-funded team benefits outside the residual pool.',
