-- Morning kickoff: store the message with a {{commits}} marker and send the
-- composed version. The stored text is what every later refresh rebuilds from,
-- which is what keeps the random quote frozen across edits.
