...
BEGIN
  ...
  -- date_trunc('week') is ISO Monday-anchored
  v_week_start := date_trunc('week', v_today)::date;
  -- 1=Mon ... 7=Sun
  v_day_of_week := extract(isodow FROM v_today)::int;
  v_days_so_far := v_day_of_week;  -- how many days have elapsed including today

  SELECT setting_value::bigint INTO v_chat_id
  FROM public.settings
  WHERE agency_id = p_agency_id AND setting_key = 'telegram_team_group_chat_id';

  -- Expected respondents (same rule as work checkins)
  ...
  v_text := E'💪 Health Goal Update — Week of ' || to_char(v_week_start, 'Mon DD')
    || E'\n(Day ' || v_days_so_far || ' of 7)\n\n';
  ...
  FOR v_row IN
    ...
    days_responded AS (
      SELECT team_id, COUNT(DISTINCT log_date) AS days
      FROM week_data
      GROUP BY team_id
    )
    SELECT
      ...
      lo.week_total_override IS NOT NULL AS had_override
    FROM expected e
  ...
  LOOP
    -- Progress bar: filled blocks for hits up to target, empty for remaining
    v_progress_bar := '';
    ...
    -- Compose the line with warm tone
    IF v_row.days_responded = 0 THEN
      v_line := '• ' || v_row.first_name || ': no check-ins yet this week — start strong tonight';
    ELSIF v_row.hits >= v_target THEN
    ...
    ELSIF v_row.hits >= v_target - 1 AND v_days_so_far >= v_target THEN
      v_line := '• ' || v_row.first_name || ': ' || v_row.hits || '/' || v_target
        || ' ' || v_progress_bar || '  one more day';
    ELSIF v_row.hits = 0 THEN
      v_line := '• ' || v_row.first_name || ': ' || v_row.hits || '/' || v_target
        || ' ' || v_progress_bar || '  rest week so far — the couch is undefeated';
    ELSE
      v_line := '• ' || v_row.first_name || ': ' || v_row.hits || '/' || v_target
  ...
  ELSIF v_day_of_week >= 5 THEN
    v_text := v_text || E'\n\nWeekend''s here. Plenty of room to close the gap.';
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'output_summary', format('health_eve compile: %s/%s reporting, %s at goal, %s total workouts',
      v_responded_count, v_expected_count, v_at_or_above_target, v_total_hits)
  );
END;
$function$;
