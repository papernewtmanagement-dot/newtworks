-- One day of production changes, grouped the way the day was actually worked: every row that
-- came from a single click on the Log tab shares a transaction id and collapses to one line.
-- The daily alert, the Telegram count and the Changes tab all read THIS function, so the three
-- can never drift apart. Not SECURITY DEFINER on purpose: the admin-only row policy on
-- change_log is what keeps the tab admin-only, exactly as change_log_recent works today.
CREATE OR REPLACE FUNCTION public.production_changes_for_day(
  p_agency_id uuid, p_day date DEFAULT NULL::date)
...
AS $function$
  ...
  SELECT g.txid, g.changed_at, COALESCE(g.changed_by_label, 'Unknown') AS who, g.what, g.item,
    ...
    FROM grouped g
   ORDER BY g.changed_at;
$function$;

GRANT EXECUTE ON FUNCTION public.production_changes_for_day(uuid, date) TO anon, authenticated;
