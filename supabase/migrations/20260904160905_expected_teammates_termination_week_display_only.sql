-- Follow-up to 20260904160758. The termination-week grace was applied to every
-- purpose, which put an archived teammate back on the reminder/nag rosters
-- (health_checkin, work_checkin). Peter's directive was about the team status
-- DISPLAY only: someone terminated mid-week still shows on that week's status
-- block. Nobody who has left should be pinged for a check-in.
-- Grace now applies to p_purpose = 'work_display' only; every other purpose keeps
-- the original exact-date cutoff.
