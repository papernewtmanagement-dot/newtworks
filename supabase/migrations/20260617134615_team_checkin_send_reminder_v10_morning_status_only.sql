...
BEGIN
  ...
  IF v_checkin_type = 'morning' THEN
    -- v10: Morning is a STATUS RECAP, not a numbers ask.
    -- Nothing changes overnight; surface yesterday's EOD per-person + team total.
    -- First real numbers ask resumes at midday (12 PM).
    ...
    IF FOUND THEN
      ...
      WHERE tc.agency_id = p_agency_id
        AND tc.checkin_date = v_yesterday.checkin_date
        AND tc.checkin_type = 'eod';

      v_text := v_text || format('📊 Where we stood at yesterday''s EOD (%s):',
        to_char(v_yesterday.checkin_date, 'Mon DD')) || E'\n';
    ...
    -- Morning intentionally does NOT ask for fresh numbers. Midday at 12 PM is the next ask.
  ...
  -- Friday EOD adds the weekly wrapup tail (unchanged from v9)
