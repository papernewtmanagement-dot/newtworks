-- Round 7: FIX 16 — tighten the payroll-funding guard from a $100 tolerance to
-- the exact formula: bank draft = total_cash_requirement - total_employee_deductions
-- (employee withholdings never leave the bank as a separate item — they're
-- already subtracted before the draft is pulled). $1.00 tolerance only for
-- report rounding, not a fudge factor. Verified to the penny against 7/10,
-- 7/17, 7/24, 7/31 2026 payroll runs. Full function body below.

-- Round 6: FIX 13 — payroll-funding-draft guard in cash_register_gl_writer.
-- Recognizes a bank draft that funds a payroll run (amount within $100 of that
-- run's total_cash_requirement, landing within 4 days of the pay date) and
-- suppresses it as a possible_transfer, same pattern as the FIX 10 card-payment
-- guard, since the wage/tax expense is already booked from the SurePayroll
-- import and this draft is money movement, not a P&L line. Full function body
-- below (byte-for-byte from pg_get_functiondef after apply).
...
BEGIN
  ...
  LOOP
    ...
    -- FIX 13: payroll-funding-draft guard. Runs before the FIX 10 card-payment
    -- guard, same eligibility gate (bank-account debit, no merchant). Bank
    -- alerts for these carry only an amount and last-4 — no payee text — so
    -- the only way to recognize them is by matching against a known payroll
    -- run's total cash requirement, within a $100 tolerance (covers the
    -- provider's fee component riding along with the draft, without being
    -- wide enough to catch real expenses) and a 4-day landing window after
    -- the pay date. The wage/tax expense itself is already booked from the
    -- SurePayroll import, so this draft is money movement, not a P&L line.
