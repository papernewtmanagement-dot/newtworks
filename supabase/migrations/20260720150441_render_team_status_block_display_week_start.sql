-- Fix reset-logic week frame in render_team_status_block. Two week frames now
-- distinguished:
--   v_wtw_week_start   — from v_wtw_date (viewer's today). Drives WtW panel
--                        math (v_ttq / v_tts / v_wtw).
--   v_display_week_start — from p_as_of_date (the check-in target date's
--                        week). Drives is_prior_week reset in the person
--                        listing, so a "last week close" block shows last
--                        week's numbers without spurious zeroing.
-- Also switches roster lookup from purpose='work_checkin' to 'work_display'
-- so approved-full-day PTO teammates stay visible.
