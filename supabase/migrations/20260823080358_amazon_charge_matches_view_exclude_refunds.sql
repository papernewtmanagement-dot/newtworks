...
with_pref AS (SELECT *, min(date_priority) OVER (PARTITION BY ledger_id) AS best_priority FROM paired),
filtered AS (SELECT * FROM with_pref WHERE date_priority = best_priority),
with_min AS (SELECT *, min(day_diff) OVER (PARTITION BY ledger_id) AS min_day_diff FROM filtered),
nearest AS (
  SELECT *,
