-- Peter 2026-08-28: two changes to the Sales curve.
--
-- 1. Elite runs to the end of the band pattern instead of to 1000. The band
--    widths are an arithmetic run off the Danger width D: Danger D, Caution
--    2D, Good 3D, Great 4D, so Elite is 5D. At D = 50 that puts the top of
--    the chart at 500 + 250 = 750. Derived from sales_points_band_config so
--    it follows the Danger width if that ever moves. The pay_scale table
--    still holds all 101 rows to 1000; only the chart stops earlier.
--
-- 2. The raise ladder is returned with the role so the page can publish it
--    under the chart. It is read back off pay_scale, which is the one copy
--    of the ladder.
...
BEGIN
  ...
  SELECT b.min_threshold INTO v_top_start
    FROM public.sales_points_band_config b
   WHERE b.agency_id = p_agency_id ORDER BY b.display_order DESC LIMIT 1;
  ...
  LOOP
    ...
    BEGIN
      ...
      IF r_role.role_key = 'sales' THEN
        ...
        -- The published raise ladder, read back off the one copy of it.
        SELECT jsonb_agg(jsonb_build_object(
