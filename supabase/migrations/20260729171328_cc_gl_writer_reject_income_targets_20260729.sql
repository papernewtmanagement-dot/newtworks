-- STRUCTURAL RULE: Credit card transactions cannot route to income accounts.
-- A card is a liability. Charges = expense up, liability up. Refunds = expense down,
-- liability down. Payments = bank down, liability down. NEVER income.
--
-- Any rule whose target for the current direction is an income-typed account is
-- structurally invalid for CC-sourced JEs. Reject at rule-selection time so lower-
-- priority (correctly-shaped) rules can fire, and fall to SUSP if none match.
  ...
  LOOP
    ...
    IF v_category IS NOT NULL AND length(trim(v_category)) > 0 THEN
      SELECT id INTO v_other_side_account_id
        ...
        WHERE agency_id = p_agency_id
          ...
          AND is_active = TRUE
          AND account_type NOT IN ('income','equity')  -- structural: CC never routes to income/equity
        LIMIT 1;
    ...
    IF v_other_side_account_id IS NULL THEN
      ...
      BEGIN
        ...
        -- STRUCTURAL: reject rules whose target for this direction is an income or equity account.
        -- Cards don't produce income. A rule that would route a card charge to income is
        -- necessarily wrong (it was authored for a bank-statement context).
        SELECT r.id,
        ...
        WHERE r.agency_id = p_agency_id AND r.is_active = TRUE
          ...
          -- Target must exist, be a real account (not __SOURCE__, not COA-SUSP), and NOT be income/equity
          AND (CASE WHEN v_rule_direction = 'credit' THEN r.credit_account_code ELSE r.debit_account_code END)
              IS NOT NULL
          AND (CASE WHEN v_rule_direction = 'credit' THEN r.credit_account_code ELSE r.debit_account_code END)
              NOT IN ('__SOURCE__', 'COA-SUSP')
          AND target_coa.account_type NOT IN ('income','equity')
