-- Fix the dry-run preview JSON to show correct DR/CR account names
-- The function logic is correct; only the preview output had wrong labels.
...
BEGIN
  ...
  IF v_cash_acct_id IS NULL THEN
    RETURN jsonb_build_object('ok', FALSE, 'error', 'cash_account_not_found', 'searched_for', v_cash_acct_name);
  END IF;
  ...
  LOOP
    ...
    BEGIN
      IF v_is_deduction THEN
        ...
        v_dr_name := v_target_account_name;
        v_cr_name := v_cash_acct_name;
      ELSE
        IF v_amount > 0 THEN
          v_dr_account_id := v_cash_acct_id;
          v_cr_account_id := v_target_account_id;
          v_dr_name := v_cash_acct_name;
          v_cr_name := v_target_account_name;
        ELSE
          v_dr_account_id := v_target_account_id;
          v_cr_account_id := v_cash_acct_id;
          v_dr_name := v_target_account_name;
      ...
      IF p_dry_run THEN
        v_posted_runs := v_posted_runs || jsonb_build_object(
          ...
          'dr_account', v_dr_name,
          'cr_account', v_cr_name,
