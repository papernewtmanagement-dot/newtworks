-- ============================================================================
-- team_checkin_compile_results: Per-person section still iterates active
-- members only, but the "Team:" total now mirrors the canonical writer —
-- DISTINCT ON team_checkins across the full quarter, including archived
-- members' last reported quarterly value. Prior-week earned points no longer
-- vanish from the live Telegram message when a teammate is archived.
-- ============================================================================
...
DECLARE
  ...
  v_message_id bigint;
  v_row record;
  v_display_quotes numeric := 0;       -- sum across per-person display rows (active only)
  v_display_sales numeric := 0;
  v_team_total_quotes numeric := 0;    -- canonical Team total (incl archived QTD)
...
BEGIN
  ...
  -- Canonical Team total: DISTINCT ON team_id across the quarter, including
  -- archived members. Mirrors weekly_cpr_compute_outcome so live Telegram
  -- numbers and Saturday-writer numbers agree.
  -- Quotes are weekly counters; sales_points_quarter is cumulative-to-date.
  SELECT
    COALESCE(SUM(latest_q),  0),
    COALESCE(SUM(latest_sp), 0)
  INTO v_team_total_quotes, v_team_total_sales
  FROM (
  ...
  RETURN jsonb_build_object(
    ...
    'output_summary', format('%s compile: %s/%s reporting; team %s/%s; WtW Q:%s/%s SP:%s/%s; cpr_id=%s',
      v_checkin_type, v_fresh_count, v_expected_count, v_team_total_quotes, v_team_total_sales,
