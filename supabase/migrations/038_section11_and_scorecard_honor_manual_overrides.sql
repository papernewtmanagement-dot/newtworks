-- Migration 038:
-- Make get_cpr_section_11 + compute_scorecard_bonus honor manual override columns
-- on weekly_cpr_reports. Resolved-value pattern: override IF NOT NULL ELSE snapshot ELSE 0.
-- Overrides covered: auto/fire × new+lost YTD, life paid_for count + premium YTD.
-- ips_new_money_ytd has no override column yet — still snapshot-only.
-- Existing "latest snapshot with auto_new_ytd IS NOT NULL" fallback retained;
-- manual overrides on the target week's report row win when set.
--
-- Per Peter, 2026-06-27: the Scorecard projection wasn't reflecting his manual
-- corrections to partial snapshot rows. After this migration, the override
-- fields built into the CPR Detail UI flow through to Section 11 + the email
-- recap + budget computations.

CREATE OR REPLACE FUNCTION public.compute_scorecard_bonus(
  p_agency_id uuid,
...
BEGIN
  v_days_elapsed := LEAST(365, EXTRACT(DOY FROM p_as_of_date)::int);
  ...
  v_hc_tier_points := CASE v_career_tier
    WHEN 'honor' THEN 85 WHEN 'bronze' THEN 95 WHEN 'silver' THEN 105
    WHEN 'gold' THEN 115 WHEN 'crystal' THEN 125 ELSE 0
  END;

  SELECT min_target, max_target INTO v_auto_prod_min, v_auto_prod_max FROM public.sf_program_targets
  WHERE agency_id=p_agency_id AND program='scorecard' AND bucket_name='auto_pif_production' AND program_year=v_year;
  SELECT min_target, max_target INTO v_auto_gain_min, v_auto_gain_max FROM public.sf_program_targets
  WHERE agency_id=p_agency_id AND program='scorecard' AND bucket_name='auto_pif_gain' AND program_year=v_year;
  SELECT min_target, max_target INTO v_fire_prod_min, v_fire_prod_max FROM public.sf_program_targets
  WHERE agency_id=p_agency_id AND program='scorecard' AND bucket_name='fire_pif_production' AND program_year=v_year;
  SELECT min_target, max_target INTO v_fire_gain_min, v_fire_gain_max FROM public.sf_program_targets
  WHERE agency_id=p_agency_id AND program='scorecard' AND bucket_name='fire_pif_gain' AND program_year=v_year;
  SELECT min_target, max_target INTO v_fs_credits_min, v_fs_credits_max FROM public.sf_program_targets
  WHERE agency_id=p_agency_id AND program='scorecard' AND bucket_name='fs_credits' AND program_year=v_year;
  SELECT min_target INTO v_hc_npf_gate FROM public.sf_program_targets
  WHERE agency_id=p_agency_id AND program='honor_club' AND bucket_name='honor_club_life_policies_gate' AND program_year=v_year;
  SELECT min_target INTO v_hc_credits_gate FROM public.sf_program_targets
  WHERE agency_id=p_agency_id AND program='honor_club' AND bucket_name='honor_club_life_credits_gate' AND program_year=v_year;

  SELECT * INTO v_snap FROM public.agency_snapshot
  WHERE agency_id = p_agency_id AND snapshot_date <= p_as_of_date AND auto_new_ytd IS NOT NULL
  ...
  WHERE agency_id = p_agency_id AND week_ending_date = p_as_of_date;
  ...
  IF v_snap.snapshot_date IS NULL AND NOT v_has_overrides THEN
    RETURN jsonb_build_object(
      ...
      'as_of_date', p_as_of_date, 'last_year_bonus', v_last_year_bonus);
  END IF;
  ...
  v_auto_prod_pts := GREATEST(0, LEAST(125, CASE WHEN v_auto_prod_max > v_auto_prod_min
    THEN ((v_auto_prod_proj - v_auto_prod_min) / (v_auto_prod_max - v_auto_prod_min)) * 125 ELSE 0 END));
  v_auto_gain_pts := GREATEST(0, LEAST(200, CASE WHEN v_auto_gain_max > v_auto_gain_min
    THEN ((v_auto_gain_proj - v_auto_gain_min) / (v_auto_gain_max - v_auto_gain_min)) * 200 ELSE 0 END));
  v_auto_best_pts := GREATEST(v_auto_prod_pts, v_auto_gain_pts);
  v_fire_prod_pts := GREATEST(0, LEAST(100, CASE WHEN v_fire_prod_max > v_fire_prod_min
    THEN ((v_fire_prod_proj - v_fire_prod_min) / (v_fire_prod_max - v_fire_prod_min)) * 100 ELSE 0 END));
  v_fire_gain_pts := GREATEST(0, LEAST(100, CASE WHEN v_fire_gain_max > v_fire_gain_min
    THEN ((v_fire_gain_proj - v_fire_gain_min) / (v_fire_gain_max - v_fire_gain_min)) * 100 ELSE 0 END));
  v_fire_best_pts := GREATEST(v_fire_prod_pts, v_fire_gain_pts);
  v_fs_pts := GREATEST(0, LEAST(225, CASE WHEN v_fs_credits_max > v_fs_credits_min
    THEN ((v_fs_credits_proj - v_fs_credits_min) / (v_fs_credits_max - v_fs_credits_min)) * 225 ELSE 0 END));
  ...
  WHERE agency_id = p_agency_id AND period_year = v_year AND is_scorecard_eligible = true;
  ...
  RETURN jsonb_build_object(
    'as_of_date', p_as_of_date, 'snapshot_date', v_snap.snapshot_date,
    'has_manual_overrides', v_has_overrides,
    'program_year', v_year, 'days_elapsed', v_days_elapsed,
    ...
    'points_breakdown', jsonb_build_object(
      'honor_club', v_hc_pts, 'auto_best', round(v_auto_best_pts::numeric, 1),
      'fire_best', round(v_fire_best_pts::numeric, 1), 'fs_credits', round(v_fs_pts::numeric, 1)),
    'bucket_detail', jsonb_build_object(
      ...
      'hc_points', v_hc_pts, 'hc_qualified_projected', v_hc_qualified,
      'hc_career_tier', v_career_tier, 'hc_tier_points', v_hc_tier_points),
    'modifiers', jsonb_build_object(
      'auto_gain_mod', v_auto_gain_mod, 'fire_gain_mod', v_fire_gain_mod,
    ...
    'inputs', jsonb_build_object(
      'auto_prod_ytd', v_auto_prod_ytd, 'auto_prod_proj', round(v_auto_prod_proj::numeric, 1),
      'auto_gain_ytd', v_auto_gain_ytd, 'auto_gain_proj', round(v_auto_gain_proj::numeric, 1),
      'fire_prod_ytd', v_fire_prod_ytd, 'fire_prod_proj', round(v_fire_prod_proj::numeric, 1),
      'fire_gain_ytd', v_fire_gain_ytd, 'fire_gain_proj', round(v_fire_gain_proj::numeric, 1),
      'life_npf_ytd', v_life_npf_ytd, 'life_npf_proj', round(v_life_npf_proj::numeric, 1),
      'life_credits_ytd', v_life_credits_ytd, 'life_credits_proj', round(v_life_credits_proj::numeric, 2),
      'fs_credits_ytd', v_fs_credits_ytd, 'fs_credits_proj', round(v_fs_credits_proj::numeric, 2)),
    'computed_at', now());
END;
$function$;
...
BEGIN
  ...
  IF v_snap.snapshot_date IS NOT NULL OR v_has_overrides THEN
    ...
    v_fire_gain     := COALESCE(v_report.fire_new_ytd_manual,  v_snap.fire_new_ytd,  0)
                     - COALESCE(v_report.fire_lost_ytd_manual, v_snap.fire_lost_ytd, 0);
    v_fs_credits    := COALESCE(v_report.life_paid_for_premium_ytd_manual, v_snap.life_paid_for_premium_ytd, 0);
