...
-- Card accounts are liabilities: a debit (a charge -- money spent) raises
-- the balance owed, a credit (payment or refund) lowers it. Same anchor
-- logic as v_bank_balances -- only activity dated after the latest
-- statement's close counts, since anything on or before that date is
-- already inside ls.closing_balance.
pending AS (
  SELECT
    b.account_id,
    SUM(CASE WHEN c.direction = 'debit' THEN c.amount
              WHEN c.direction = 'credit' THEN -c.amount
...
SELECT
  ...
  b.minimum_payment,
  b.payment_due_day,
  b.credit_limit - (CASE WHEN b.last_statement_period_end IS NULL THEN NULL
                         ELSE b.last_statement_closing_balance + COALESCE(p.pending_delta, 0) END) AS available_credit,
  b.chart_account_id,
  b.account_type,
  b.interest_rate,
  b.account_number_last4 IS NULL AS needs_last4,
  (CASE WHEN b.last_statement_period_end IS NULL THEN NULL
