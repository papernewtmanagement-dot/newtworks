-- Prevent the same CareerPlug application being written twice.
create unique index if not exists uq_hiring_candidates_careerplug_app_id
  on public.hiring_candidates (agency_id, ((ingestion_metadata -> 'careerplug' ->> 'careerplug_app_id')))
  where (ingestion_metadata -> 'careerplug' ->> 'careerplug_app_id') is not null;
  ...
  loop
    begin
      ...
      if v_existing is not null then
        -- Enrich only. Never touch status or anything already populated.
        ...
        set first_name  = coalesce(hc.first_name, v_app ->> 'firstname'),
            ...
            phone       = coalesce(hc.phone,      v_app ->> 'phone'),
