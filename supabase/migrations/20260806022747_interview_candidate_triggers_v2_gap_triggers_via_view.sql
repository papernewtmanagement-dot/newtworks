-- v2: implements T_GAP_COMMITMENT / T_GAP_CHARACTER.
-- The layer composites (res_commitment, assessment_commitment, res_character,
-- assessment_character) live on v_hiring_candidates, not the base table.
-- Gap threshold 25 is provisional pending calibration at n>=30 v2-scored candidates.
...
BEGIN
  ...
  -- Layer composites live on v_hiring_candidates (res_* from resume scoring,
  -- assessment_* dual-path). Fire only when BOTH sides are present.
  -- Threshold 25 provisional pending n>=30 calibration.
