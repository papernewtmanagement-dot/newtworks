-- Corrective mig: restore Peter's cell-function view shape after my step 4a clobbered it.
-- My step 4a DROP+CREATE VIEW rebuilt v_hiring_candidates with inline math (byte-identical
-- output but wrong architecture). This restores the cell-function-calling shape from
-- Peter's 2026-07-23 refactor + drops the _legacy_resume_analysis_prose passthrough
-- (dead weight — no consumer reads legacy prose from the view).
