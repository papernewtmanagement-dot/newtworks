ALTER TABLE public.tasks ADD COLUMN IF NOT EXISTS scheduled_day date;
COMMENT ON COLUMN public.tasks.scheduled_day IS
  'The day inside the focus week this task is planned for. Separate from due_date, which stays the real deadline. Naming the day roughly doubles follow-through versus naming only the week (Gollwitzer & Sheeran 2006 meta-analysis, implementation intentions).';
...
BEGIN
  ...
  IF NOT p_dry_run THEN
    -- Anything that sat in last week and is still open has been carried.
    ...
    -- Carried too many times without finishing. Park it; it stops competing for slots.
    ...
    -- Never scheduled, no deadline, untouched for months. Park it too.
    ...
    -- Clear the board for the new week.
  ...
  WITH budget AS (
    ...
    FROM users u
    WHERE u.role IN ('owner','manager') AND u.is_active
  ),
  pool AS (
    ...
    WHERE t.agency_id=p_agency_id AND t.status='open'
      ...
      AND NOT EXISTS (SELECT 1 FROM tasks c
                      WHERE c.parent_task_id=t.id AND c.status='open')
  ),
  ranked AS (
    SELECT b.full_name, p.id, p.title, p.priority, p.estimated_hours,
           ROW_NUMBER() OVER (PARTITION BY p.assigned_to ORDER BY p.score DESC, p.estimated_hours ASC) AS rn,
  ...
  picked AS (
    ...
    FROM ranked r
    WHERE r.rn <= v_cap AND r.run_hours <= r.hours_budget
  ),
  applied AS (
    UPDATE tasks t
       SET in_weekly_focus = true, week_of = v_week, scheduled_day = pk.day_slot
      FROM picked pk
     WHERE t.id = pk.id AND NOT p_dry_run
    RETURNING t.id)
  SELECT pk.full_name::text, pk.rn::int, pk.day_slot, pk.priority,
         pk.estimated_hours, ROUND(pk.run_hours,2), pk.title::text, pk.id
  FROM picked pk
  ORDER BY pk.full_name, pk.rn;
END;
$fn$;

COMMENT ON FUNCTION public.build_weekly_focus IS
  'Builds one focus week. Runs dry by default. Fills each person to their hour budget and item cap in score order, skips epics and any parent that still has open children, assigns a weekday to each item, carries and parks anything that keeps rolling over. All parameters live in task_scoring_rules, not in this code.';