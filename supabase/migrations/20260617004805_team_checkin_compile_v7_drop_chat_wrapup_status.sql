-- v7: wrapups now go via email. Drop the chat-based wrapup ✅/❌ logic.
-- Will be restored with Gmail-backed detection in a follow-up.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'output_summary', format('%s compile: %s fresh + %s carried + %s zero of %s; totals %s/%s',
