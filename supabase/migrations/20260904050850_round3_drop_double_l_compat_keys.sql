-- Drop the two-L fallback payload keys. Nothing has ever sent them (every
-- Production table is empty and the page is being rewritten in the same
-- change), so keeping them would only preserve the old spelling in code.
