-- ============================================================================
-- BUG FIX 2026-08-22 (same class as smvc_fs_bucket_anchors_to_comp_statement_
-- coverage_date, swept in the same session).
--
-- compute_scorecard_bonus projects eligible commissions forward using
-- pace_factor, which is derived from the WEEKLY SNAPSHOT date. But eligible
-- commissions are summed from comp_recap - processed commission statements -
-- which cover through an earlier date. Dividing statement data by days-through-
-- today understates the projected bonus, and the understatement grows every
-- week until the next statement is processed.
--
-- Production inputs (auto/fire PIF, life policy counts, life premium, ips) all
-- come from agency_snapshot and correctly keep the snapshot-based pace factor.
-- Only the commission projection moves onto the statement coverage date.
--
-- Same principle as migration 20260702191843: each data source annualizes
-- against its own latest-data date.
-- ============================================================================
...
DECLARE
  ...
  v_comp_as_of        date;
  v_comp_days_elapsed int;
  v_comp_pace_factor  numeric;
  v_career_tier  text;
...
BEGIN
  ...
  FROM public.sf_program_targets WHERE agency_id=p_agency_id AND program='scorecard'
  ...
  FROM public.sf_program_targets WHERE agency_id=p_agency_id AND program='honor_club'
  ...
  WHERE agency_id = p_agency_id AND snapshot_date <= p_as_of_date AND cadence = 'weekly'
  ORDER BY snapshot_date DESC LIMIT 1;

  IF v_snap.snapshot_date IS NULL THEN
    RETURN jsonb_build_object(
      'error', 'No agency_snapshot row for the target week',
      'as_of_date', p_as_of_date, 'last_year_bonus', v_last_year_bonus);
  END IF;

  -- Snapshot-sourced production inputs annualize on the snapshot date.
  ...
  -- Statement-sourced eligible commissions annualize on the date the processed
  -- commission statements actually cover. Falls back to the snapshot anchor.
  v_comp_as_of := COALESCE(
    public.get_comp_recap_anchor_date(p_agency_id, p_as_of_date),
    v_effective_as_of
  );
  v_comp_days_elapsed := LEAST(365, EXTRACT(DOY FROM v_comp_as_of)::int);
  v_comp_pace_factor := 365.0 / GREATEST(1, v_comp_days_elapsed);

  v_auto_prod_ytd     := COALESCE(v_snap.auto_new_ytd,  0);
  v_auto_gain_ytd     := v_auto_prod_ytd - COALESCE(v_snap.auto_lost_ytd, 0);
  v_fire_prod_ytd     := COALESCE(v_snap.fire_new_ytd, 0);
  v_fire_gain_ytd     := v_fire_prod_ytd - COALESCE(v_snap.fire_lost_ytd, 0);
  v_life_npf_ytd      := COALESCE(v_snap.life_paid_for_count_ytd, 0);
  v_life_credits_ytd  := COALESCE(v_snap.life_paid_for_premium_ytd, 0);
  ...
  -- statement-sourced -> statement pace factor
  v_total_eligible_proj := v_total_eligible * v_comp_pace_factor;
  ...
  RETURN jsonb_build_object(
    ...
    'comp_effective_as_of_date', v_comp_as_of,
    'snapshot_date',      v_snap.snapshot_date,
    'program_year',       v_year,
    'days_elapsed',       v_days_elapsed,
    'pace_factor',        round(v_pace_factor::numeric, 4),
    'comp_days_elapsed',  v_comp_days_elapsed,
    'comp_pace_factor',   round(v_comp_pace_factor::numeric, 4),
