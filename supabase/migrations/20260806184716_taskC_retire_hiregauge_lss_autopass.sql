-- TASK C (build-instructions 2026-08-06): retire the LSS "autopass" gate.
-- Correction to the record: it was never a leniency/auto-pass. The only
-- verdict-level effect was a veto (status='decline_lss' forced
-- verdict='decline_lss'); 'exception_applies' and 'flag_lss_manual' changed
-- nothing beyond meta.lss_autopass. Removing it does not loosen the
-- framework -- it lets the composite score stand on its own. See
-- build-instructions doc for full citation-backed rationale (Kuncel et al.
-- 2013; Dawes 1979; Berry, Gruys & Sackett 2006) for why this is retired
-- rather than ported to a GMA-based equivalent in this task.
...
DROP FUNCTION IF EXISTS public._hiregauge_lss_autopass(numeric, text, numeric, text, jsonb, jsonb, jsonb);
