-- The document processor rewrites ingestion_metadata wholesale, wiping anything
-- the webhook writer put there. Keep CareerPlug identifiers in real columns instead.
alter table public.hiring_candidates
  add column if not exists careerplug_app_id text,
  add column if not exists careerplug_source_platform text;

drop index if exists public.uq_hiring_candidates_careerplug_app_id;

create unique index if not exists uq_hiring_candidates_careerplug_app_id
  on public.hiring_candidates (agency_id, careerplug_app_id)
  where careerplug_app_id is not null;

-- Restore what was overwritten, from the webhook events themselves.
update public.hiring_candidates hc
set careerplug_app_id = e.app_id,
    careerplug_source_platform = (e.payload -> 'data' ->> 'source_name'),
    updated_at = now()
from public.careerplug_webhook_events e
where e.agency_id = hc.agency_id
  and lower(e.payload -> 'data' -> 'applicant' ->> 'email') = lower(hc.email)
  and hc.careerplug_app_id is null;
