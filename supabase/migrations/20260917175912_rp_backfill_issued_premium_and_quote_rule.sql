-- Peter 2026-09-17, third pass on the backfill form:
--  * the issued premium is part of the same form, one box per issued policy,
--    prefilled on screen with the premium the policy was submitted at
--  * a quote only asks for a phone when the household has no sale at all. When
--    there is a sale, the phone from the sale fills the quote, so asking twice
--    for the same household is wasted work.
...
BEGIN
  ...
  WITH gaps AS (
    ...
    SELECT 'quote', q.id, q.customer_label, q.quote_date,
           ...
           -- a quote only asks for a phone when this household has no sale to take it from
           (q.phone_last4 IS NULL AND NOT EXISTS (
              SELECT 1 FROM public.sales_log s2
               WHERE s2.agency_id = q.agency_id AND s2.status = 'active' AND s2.customer_label = q.customer_label)),
           false,
...
DECLARE
  ...
  v_prem numeric; v_touched boolean;
...
BEGIN
  ...
  FOR rec IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
    ...
    -- Issued premium, one policy at a time, on the sale this row belongs to.
    IF v_kind = 'sale' AND jsonb_typeof(rec->'policies') = 'array' THEN
      FOR pol IN SELECT * FROM jsonb_array_elements(rec->'policies') LOOP
        v_prem := NULLIF(btrim(COALESCE(pol->>'issued_premium','')), '')::numeric;
        CONTINUE WHEN v_prem IS NULL;
        IF v_prem < 0 THEN RAISE EXCEPTION 'an issued premium cannot be negative'; END IF;
        IF v_prem > 1000000 THEN RAISE EXCEPTION 'that issued premium looks too large. Double-check it.'; END IF;
        UPDATE public.sales_log_products
           SET issued_premium = v_prem
         WHERE id = (pol->>'id')::uuid AND sales_log_id = v_id AND agency_id = a.agency_id;
        GET DIAGNOSTICS n = ROW_COUNT;
        IF n > 0 THEN v_issued := v_issued + n; v_touched := true; END IF;
      END LOOP;
    END IF;
