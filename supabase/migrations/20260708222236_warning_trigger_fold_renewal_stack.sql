...
--   * Renewal stack effective = CASE WHEN tenure_multiplier >= 1.0 THEN stack ELSE 0 (Peter's post-Week 52 gate)
...
-- 2) Rewrite compute_warning_trigger (drop first because RETURNS TABLE signature changes)
...
BEGIN
  ...
  final AS (
    SELECT
      f.*,
      -- Post-Week 52 gate: renewal stack applies only when tenure_multiplier fully unlocks
...
-- 3) Update write_weekly_comp_v2 to write the new renewal_stack_annual column
