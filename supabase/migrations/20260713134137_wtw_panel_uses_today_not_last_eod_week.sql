...
BEGIN
  -- WtW panel + team totals compute against this date's week
  -- Per-person historical block still uses p_as_of_date
  ...
  -- Team totals: sum within the WtW week (v_wtw_date's week), NOT p_as_of_date's week
...
-- Morning reminder now passes v_today as p_wtw_as_of_date, so the WtW panel + team
-- totals reflect THIS week regardless of when the last EOD was recorded.
-- Also: when last EOD is in a prior week, label the historical block to make the
-- new-week context explicit.
...
BEGIN
  ...
  IF v_checkin_type = 'morning' THEN
    ...
    IF v_last_eod_date IS NOT NULL THEN
      -- Determine if last EOD belongs to a prior week (Sun-Sat convention)
      ...
      IF v_last_eod_week_end < v_today_week_end THEN
        -- New week — label the historical block, WtW panel resets to this week
      ...
      SELECT * INTO v_block FROM public.render_team_status_block(
        ...
        v_header_label,
        v_today  -- WtW panel + team totals against THIS week
      );
