-- Migration 038:
-- Make get_cpr_section_11 and compute_scorecard_bonus honor manual override columns
-- on weekly_cpr_reports. Resolved-value pattern:
--   override IF NOT NULL ELSE snapshot ELSE 0
-- Override columns checked: auto/fire × new+lost YTD, life paid_for count + premium YTD.
-- ips_new_money_ytd has no override yet — still snapshot-only.
-- Original "latest snapshot with auto_new_ytd not null" lookup retained as the fallback
-- source; manual overrides on the target week's report row win when set.
...
BEGIN
  ...
  -- Snapshot fallback (same logic as before)
  SELECT * INTO v_snap
  ...
  -- Manual override row for the target week (if it exists)
  SELECT
  ...
  -- If neither snapshot nor overrides have anything, can't compute.
  ...
  -- Resolved YTD values: manual override > snapshot > 0
  ...
  v_ips_activity_ytd  := COALESCE(v_snap.ips_new_money_ytd, 0);  -- no manual override column yet
