ALTER TABLE public.automation_recipes ADD COLUMN IF NOT EXISTS internal_handler TEXT;
ALTER TABLE public.comp_recap ADD COLUMN IF NOT EXISTS posted_at TIMESTAMPTZ;
...
BEGIN
  ...
  WHERE agency_id = p_agency_id AND account_code = v_cash_code LIMIT 1;
  ...
  FOR v_unposted IN
    ...
    ORDER BY period_year, period_month, id LIMIT 500
  LOOP
    ...
    FROM public.chart_of_accounts
    WHERE agency_id = p_agency_id AND account_code = v_revenue_code LIMIT 1;
    IF v_revenue_acct_id IS NULL THEN CONTINUE; END IF;

    INSERT INTO public.journal_entries (agency_id, entry_date, entry_type, source, description, created_by, created_at)
    VALUES (p_agency_id, MAKE_DATE(v_unposted.period_year, v_unposted.period_month, 1),
            'comp_revenue', 'gl_entry_writer',
            COALESCE(v_unposted.description, COALESCE(v_unposted.comp_type, '') || ' ' || COALESCE(v_unposted.comp_category, '')),
            'gl_entry_writer', v_now)
    RETURNING id INTO v_entry_id;

    UPDATE public.journal_entries SET reference_number = 'comp_recap:' || v_unposted.id::text WHERE id = v_entry_id;

    INSERT INTO public.journal_lines (journal_entry_id, agency_id, account_id, debit, credit, description, created_at) VALUES
  ...
  RETURN jsonb_build_object('records_processed', v_count, 'output_summary', v_count || ' journal entries written from comp_recap');
END;
$func$;
...
BEGIN
  ...
  LOOP
    ...
    WHERE agency_id = p_agency_id AND staff_id = v_producer.id
...
DECLARE
  ...
  v_query TEXT;
BEGIN
  SELECT id, agency_id, recipe_name, internal_handler INTO v_recipe
  FROM public.automation_recipes WHERE id = p_recipe_id;

  IF NOT FOUND THEN RAISE EXCEPTION 'Recipe % not found', p_recipe_id; END IF;
  IF v_recipe.agency_id IS NULL THEN RAISE EXCEPTION 'Recipe % has no agency_id', p_recipe_id; END IF;
  IF v_recipe.internal_handler IS NULL OR length(trim(v_recipe.internal_handler)) = 0 THEN
    RAISE EXCEPTION 'Recipe % has composio_action=INTERNAL but no internal_handler set', v_recipe.recipe_name;
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON p.pronamespace = n.oid
                 WHERE n.nspname = 'public' AND p.proname = v_recipe.internal_handler) THEN
    RAISE EXCEPTION 'Recipe % references internal_handler "%" but function does not exist', v_recipe.recipe_name, v_recipe.internal_handler;
  END IF;
