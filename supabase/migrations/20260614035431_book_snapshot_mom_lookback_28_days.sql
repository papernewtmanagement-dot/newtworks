-- Change MoM lookback window from 25 days to 28 days per Peter (2026-06-13).
...
LEFT JOIN LATERAL (
  ...
  FROM public.book_snapshot bs2
  WHERE bs2.agency_id = b.agency_id
    AND bs2.snapshot_date <= b.snapshot_date - INTERVAL '28 days'
  ORDER BY bs2.snapshot_date DESC LIMIT 1
) mom ON true
LEFT JOIN LATERAL (
  SELECT bs2.auto_premium, bs2.fire_premium, bs2.life_premium, bs2.health_premium,
...
COMMENT ON VIEW public.v_book_snapshot_with_changes IS 'book_snapshot with date-based lookback. WoW=5 days, MoM=28 days, YoY=350 days. PIF totals across LOBs intentionally omitted.';

-- Rebuild growth summary on top of the updated changes view
