-- ============================================================================
-- Team total: quotes_week is a WEEKLY counter (resets each Sunday), so
-- DISTINCT-ON across the full quarter incorrectly inherits prior-week values
-- from archived members. Scope quotes to the current week only. SP stays
-- quarter-wide (cumulative metric).
-- ============================================================================
...
BEGIN
  ...
  -- Team QUOTES total: scoped to CURRENT WEEK only (quotes_week resets each
  -- Sunday — a value from prior week from an archived member would wrongly
  -- bleed into this week if pulled across the full quarter).
  ...
  -- Team SP total: CUMULATIVE metric, scope across full quarter incl archived.
  -- Preserves prior-week earned points from members archived mid-quarter.
