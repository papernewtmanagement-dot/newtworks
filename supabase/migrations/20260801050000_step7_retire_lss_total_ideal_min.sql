-- Step 7: retire hiring_candidates.lss_total_ideal_min.
-- Superseded by hiregauge_role_ideal_ranges (role-specific, live) as of
-- Step 6. Column is legacy: only 15/148 rows populated, nothing writes to
-- it anymore (no edge function, no RPC), and no active hiregauge_rules row
-- references it via trait_signature. Storing a "current" ideal-min here
-- would also violate the compensation-data-freshness pattern (never store
