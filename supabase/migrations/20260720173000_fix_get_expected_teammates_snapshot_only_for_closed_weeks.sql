-- Fix: get_expected_teammates snapshot branch only for closed weeks
--
-- Bug: mid-week the weekly_cpr_team_detail table is populated lazily by the
-- team_checkins sync trigger (one row per teammate on first check-in), and a
-- second trigger fills role_level on that row. Once ANY row exists with
-- role_level not null, get_expected_teammates flipped to the snapshot branch,
-- returning only teammates who had already checked in and silently dropping
-- everyone else (e.g. John missing from the 2026-07-20 midday summary).
--
-- Fix: only consult the snapshot when the week's Saturday is already in the
-- past. Mid-week + current-week calls fall through to live public.team.
-- Historical reads (last Saturday and earlier) still use the snapshot as
-- intended by the "canonical roster/computation routing" op-rule.
...
BEGIN
  ...
  IF p_as_of_date IS NOT NULL THEN
    ...
    -- Snapshot branch only for weeks that have already closed (Saturday in the past).
    -- Mid-week, weekly_cpr_team_detail is populated lazily by the team_checkins sync
    -- trigger; using the partial snapshot would silently exclude teammates who
    -- haven't checked in yet. Full roster is written on Saturday close.
  ...
  v_time_off_date := COALESCE(p_as_of_date, v_today_ct);
