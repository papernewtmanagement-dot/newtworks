...
-- Pattern mirrors residual pool: YTD-accumulating envelope + underspend halved into pool + person share = points_ytd / total_points_ytd
...
DECLARE
  ...
  v_mktg_root_id UUID;
BEGIN
  -- Resolve week end (default: current or upcoming Saturday)
  v_week_end := COALESCE(
    p_week_end_date,
  ...
  -- 1) Basis from residual-pool source of truth
  ...
  -- 2) Envelope (10% of basis-ex-scorecard, annual + prorated YTD)
  ...
  -- 3) Marketing spend YTD from 0003 MARKETING envelope + all descendants
  SELECT id INTO v_mktg_root_id
  FROM public.chart_of_accounts
  WHERE agency_id = p_agency_id AND account_code = 'QBO-021';
  ...
  -- 4) Underspend + pool (50% of underspend, floor 0)
  ...
  -- 6) Per-person breakdown (active agency team, non-admin, non-owner)
  ...
  v_result := jsonb_build_object(
    ...
    'spend', jsonb_build_object(
      'ytd',    v_spend_ytd,
      'source', '0003 MARKETING envelope (chart_of_accounts.account_code = QBO-021) + all descendants'
    ),
