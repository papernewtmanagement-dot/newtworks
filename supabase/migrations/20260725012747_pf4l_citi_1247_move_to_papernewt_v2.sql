...
BEGIN
  ...
  -- Null the FK from credit_transactions BEFORE deleting the JEs
  UPDATE public.credit_transactions SET journal_entry_id = NULL WHERE credit_account_id = v_citi_ca_id;

  -- Delete old pf4k JEs (all 4 CC-side; there were personal-side + PN-side mirror pairs)
  ...
  -- Post fresh JEs for CC charges only (payments handled via bank-side flip below)
  FOR v_txn IN
  ...
  -- Flip 3 bank-side Citi payment JEs: DR was Internal Transfers -> becomes Owner Contribution
  -- Then create matching PN-side JE per payment: DR PN Citi liability, CR PN Owner Contribution
  FOR v_ba IN
  ...
  -- Deactivate old personal-side Citi COA
...
UPDATE public.gl_classification_rules
   SET debit_account_code = 'COA-PN-COGS-PRINT',
       credit_account_code = 'COA-PN-CC-1247',
