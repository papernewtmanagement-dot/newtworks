...
-- This migration was applied live on 2026-07-31 03:49:51 UTC (ledger name: 20260731000004_v_hiring_candidates_add_assessment_source_and_reword_lie_item) but was never
-- committed to this repo at the time. Content below is copied verbatim
-- from supabase_migrations.schema_migrations.statements for version 20260731034951.
...
-- Peter noted "had to" implies he was forced to lie, so respondents who lie but
-- weren't compelled would honestly answer no and register as impression-managing.
-- Classic Paulhus BIDR wording is "sometimes told" — restore the intent.
...
-- Verify both landed
SELECT 'view_column' AS check_type,
       (SELECT count(*) FROM information_schema.columns
         WHERE table_schema='public' AND table_name='v_hiring_candidates'
           AND column_name='assessment_source')::text AS result
UNION ALL
SELECT 'item_reworded' AS check_type,
       (SELECT item_text FROM public.hiregauge_instrument_items
         WHERE id='62cbd9af-24b5-490d-80e0-d82ab5016b56') AS result;
