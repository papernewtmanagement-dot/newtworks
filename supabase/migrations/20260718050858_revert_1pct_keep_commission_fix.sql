-- Revert 1% gain formula back to prior-Q total / 13 (weekly detail rows aren't there to support rolling 13wk)
-- Keep commission fix (SP delta with Q-start floor) - that one works and doesn't need historical rows
...
BEGIN
  ...
  -- GOALS BONUS (1% gain = prior completed quarter total / 13 × 1.01 — same as pre-fix)
  BEGIN
