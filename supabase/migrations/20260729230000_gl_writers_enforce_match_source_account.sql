-- Enforce match_source_account in both GL writers.
-- Before this migration, rules with match_source_account set were treated as unscoped,
-- causing e.g. the Amazon-on-Discover rule (source=COA-PERSONAL-CC-3208, target=Tithe)
-- to fire on Amazon charges from every card, misrouting $4,523.91 of agency AMEX Amazon
-- to personal Tithe & Charitable.
-- Fix: add v_source_code lookup + rule WHERE filter to both bank_gl_writer and cc_gl_writer.

-- ============================================================================
-- BANK GL WRITER (dry_run signature)
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Resolve the source account code so match_source_account scoping can be enforced below
    SELECT account_code INTO v_bank_source_code
    FROM chart_of_accounts
    ...
    BEGIN
      ...
      IF p_dry_run THEN
        v_posted_runs := v_posted_runs || jsonb_build_object(
          ...
          'classification_status', v_classification_status, 'rule_id', v_rule_id,
...
-- ============================================================================
-- CC GL WRITER (dry_run signature)
-- ============================================================================
