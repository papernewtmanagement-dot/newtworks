-- Only change here: header now uppercases EOD (was "Eod" via initcap)
...
BEGIN
  ...
  FOR v_row IN
    ...
    wrapups AS (
      ...
      WHERE agency_id = p_agency_id AND week_start_date = v_week_start
    )
    SELECT
  ...
  IF v_is_friday_eod THEN
    ...
    IF v_wrapup_in_count < v_expected_count THEN
      v_text := v_text || ' — responses still open';
    END IF;
  END IF;
