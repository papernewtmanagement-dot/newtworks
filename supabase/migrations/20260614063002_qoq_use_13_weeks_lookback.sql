-- Change QoQ lookback from INTERVAL '3 months' to INTERVAL '13 weeks' per Peter.
-- 13 weeks = consistent 91-day operational quarter window (vs calendar months which vary 89-92 days).
...
LEFT JOIN LATERAL (
  ...
  FROM public.book_snapshot bs2
  WHERE bs2.agency_id = b.agency_id
    AND bs2.snapshot_date <= (b.snapshot_date - INTERVAL '13 weeks')::date
  ORDER BY bs2.snapshot_date DESC LIMIT 1
) qoq ON true
LEFT JOIN LATERAL (
  SELECT bs2.auto_premium, bs2.fire_premium, bs2.life_premium, bs2.health_premium,
...
COMMENT ON VIEW public.v_book_snapshot_with_changes IS 'book_snapshot with calendar-interval lookback: WoW=1 week, MoM=1 month, QoQ=13 weeks, YoY=1 year. WoW only computed for cadence=weekly.';
