...
BEGIN
  ...
  -- Team-average WoW sales-points delta — used by Unit Manager (and higher) Base Advance.
  -- Average is taken across ALL agency teammates with a detail row this week and a
  -- detail row last week (excludes Owner). Members without prior-week data don't contribute.
  SELECT AVG(GREATEST(0, COALESCE(wctd.sales_points,0) - COALESCE(prev.sales_points,0)))
    INTO v_team_avg_sp_delta
    ...
    LEFT JOIN public.weekly_cpr_team_detail prev ON prev.team_member_id = t.id
      AND prev.weekly_cpr_report_id = (SELECT id FROM public.weekly_cpr_reports
        WHERE agency_id = p_agency_id AND week_ending_date = p_week_ending_date - 7)
   WHERE r.agency_id = p_agency_id
     AND r.week_ending_date = p_week_ending_date
     AND t.category = 'agency'
     AND COALESCE(t.role_level,'') <> 'Owner';
  v_team_avg_sp_delta := COALESCE(v_team_avg_sp_delta, 0);
  ...
  WITH base AS (
    SELECT wctd.team_member_id,
      ...
      COALESCE(wctd.hsm_prior_total, 0) AS hsm_prior_total,
  ...
  computed AS (SELECT
      ...
      -- Base Advance: 10% × WoW SP increase. For Unit Manager (and higher), uses the TEAM
      -- AVERAGE WoW SP increase instead of the individual's. Non-Sales role_category → 0.
      CASE
        WHEN w.role_category <> 'Sales' THEN 0
        WHEN w.role_level IN ('Unit Manager', 'Team Manager', 'Section Manager', 'Office Manager')
          THEN 0.10 * v_team_avg_sp_delta
  ...
  SELECT c.team_member_id, c.c_weekly_pay, c.c_base_advance, c.c_health_bonus, c.c_service_surge_share,
    -- True Pay Bonus: residual that brings payroll_ytd_with_this_week to (sales_points + HSM).
    --   TPB = GREATEST(0, (SP + HSM) − payroll_ytd_paid − (WP + BA + H + S + M + AP))
    -- NULL when YTD anchors are missing (so QTD context isn't fully defined).
    ...
    jsonb_build_object(
      ...
      'team_avg_sp_delta', v_team_avg_sp_delta,
