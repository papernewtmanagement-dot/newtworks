-- Rolling averages use wctd.commission (weekly-earned SP $, populated post-7/11).
-- NULLs / missing weeks = 0. Denominator always 13 or 4.
...
BEGIN
  ...
  cpr_actuals_qtd AS (
    SELECT r.id AS tm_id,
      ...
      MAX(CASE WHEN wr.week_ending_date = p_week_end_date THEN wctd.commission END)   AS current_week_commission
    FROM roster r
  ...
  -- Rolling SP averages: 13 Saturdays ending at p_week_end_date, using wctd.commission as weekly-earned $.
  -- NULL / missing rows = 0. Pre-hire weeks = 0 (Peter directive).
  weeks_series AS (
  ...
  combined AS (
    SELECT r.id AS tm_id, r.first_name, r.last_name,
      ...
      COALESCE(a.current_week_commission, 0)   AS c_current_week_comm,
  ...
  SELECT
    ...
    ROUND(s.c_current_week_comm, 2)       AS weekly_commission,
    ...
    jsonb_build_object(
      'design_note',
        ...
        || 'Rolling SP avg source = wctd.commission per week (NULLs / missing weeks / pre-hire weeks = 0). '
        || 'Denominator always 13 or 4.',
