...
--
-- Verified before revoking: all already hold an explicit service_role grant
...
-- directly.
--
-- NOTE on detection: the correct test for a PUBLIC grant is
--   EXISTS (SELECT 1 FROM aclexplode(proacl) a WHERE a.grantee = 0
--           AND a.privilege_type = 'EXECUTE')
-- A text match on proacl for '=X/' is WRONG -- it also matches
-- 'authenticated=X/postgres' and 'service_role=X/postgres', which overstates
-- the exposed set badly. Use aclexplode.
