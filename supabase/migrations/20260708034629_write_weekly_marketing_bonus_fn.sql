...
BEGIN
  ...
  -- Person-level upsert into weekly_cpr_team_detail
  WITH people AS (
    SELECT
  ...
  prior AS (
    -- prior week's earned_ytd for delta
...
COMMENT ON FUNCTION public.write_weekly_marketing_bonus(UUID, DATE) IS
  'Snapshots compute_weekly_marketing_bonus into weekly_cpr_team_detail per person. '
  'Also computes marketing_pool_earned_weekly (delta from prior weeks earned_ytd). '
  'Pattern mirrors write_weekly_comp_v2.';
