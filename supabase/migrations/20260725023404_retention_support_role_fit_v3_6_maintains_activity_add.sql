...
BEGIN
  ...
  fit := (s->>'attention_to_detail')::numeric             * 0.20
       ...
       + (s->>'has_entrepreneurial_spirit')::numeric      * (-0.08)
  ...
  RETURN jsonb_build_object(
    ...
    'weights', jsonb_build_object(
      ...
      'works_without_close_supervision', 0.11,
      'cadence_compliance', 0.08, 'receives_coaching', 0.08, 'analytical', 0.08,
      'makes_decisions_quickly', 0.07, 'listens_discovers_needs', 0.06,
      'retention_watchfulness', 0.05, 'positively_influences_team', 0.04,
      'proactive_touch_discipline', 0.03, 'routing_judgment', 0.03
    ),
    'negative_weights', jsonb_build_object(
      'competes_for_recognition', -0.08, 'has_entrepreneurial_spirit', -0.08,
      'dials_cold_calls', -0.04, 'prospects_in_community', -0.04,
      'is_fast_start_oriented', -0.02
    ),
    'meta', jsonb_build_object(
      'positive_weight_sum', 1.26, 'negative_weight_sum', -0.26, 'net_weight_sum', 1.00,
      'model', 'role_fit_v3_6_maintains_activity_add_2026_07_24',
