...
BEGIN
  ...
  -- MANAGER BONUS
  -- Weekly: 0.1/0.2/0.3% × annual OT Scorecard per manager
  -- Annual: × 52 for annualized carve-out
  SELECT
    COALESCE(SUM(
      CASE t.role_level
  ...
  -- Quarterly: 1% × (Q_OT_SMVC + Q_OT_Scorecard) × prior_qtr_wins/13
  -- Annualized (× 4 quarters, quarterly basis is annual/4): 1% × annual_basis × prior_wins/13
  v_annual_mvp := 0.01 * v_annual_ot_basis * (v_prior_qtr_wins::numeric / 13.0);

  -- WtQ TRIP (annualized) with floor-impossible halt
  -- Formula: 10% × annual_ot_basis × curr_qtr_wins/13
