-- Peter 2026-09-14: he needs to reach the historical backfill sales to edit them.
-- They are up to a year old, so the rolling day window can never surface them.
-- A search box does: type a customer name or the last four digits of a phone number and
-- the window is ignored, all time is searched.
-- The list also now says where a row came from, so a historical row is visibly historical
-- until the moment it is edited.
DROP FUNCTION IF EXISTS public.rp_recent_entries(integer, uuid, integer);
...
DECLARE a RECORD; v_from date; v_week date := public.rp_week_end(public.rp_today_central());
        ...
        v_q text; v_digits text;
BEGIN
  SELECT * INTO a FROM public.rp_resolve_actor(NULL);
  v_q := NULLIF(btrim(COALESCE(p_search, '')), '');
  v_digits := NULLIF(regexp_replace(COALESCE(v_q, ''), '\D', '', 'g'), '');

  -- A search reaches all the way back. No search keeps the rolling window.
  v_from := CASE WHEN v_q IS NULL
                 THEN public.rp_today_central() - GREATEST(1, COALESCE(p_days, 14))
...
GRANT EXECUTE ON FUNCTION public.rp_recent_entries(integer, uuid, integer, text) TO anon, authenticated;