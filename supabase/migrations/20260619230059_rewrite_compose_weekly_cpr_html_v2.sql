-- Rewritten to use the new data model:
--   - Team checklist booleans now read from weekly_cpr_reports (not aggregated from detail).
--   - Hours/location read from get_weekly_cpr_hours() (runtime view).
--   - Requirements (carryover/missed/cost/total/paid/owed) read from get_weekly_cpr_requirements() (runtime).
--   - Net Quotes read from get_weekly_cpr_requirements().net_quotes (runtime).
-- Stored fields still read directly: code_reds, code_yellows, personal checklist booleans,
-- quotes_discussed, sales_points, pay components, claims/non-pays, WtW columns.
...
BEGIN
  ...
  -- TEAM CHECKLIST: now read directly from v_report
  ...
  SELECT string_agg(name, ' • ' ORDER BY ord) INTO v_team_misses_list FROM (
    ...
    UNION ALL SELECT 6,  'Cases'         WHERE NOT COALESCE(v_report.cases_done, true)
    UNION ALL SELECT 7,  'No Onboarding' WHERE NOT COALESCE(v_report.no_onboarding_done, true)
    UNION ALL SELECT 8,  'No FU Task'    WHERE NOT COALESCE(v_report.no_fu_task_done, true)
    UNION ALL SELECT 9,  'New Opps'      WHERE NOT COALESCE(v_report.new_opps_done, true)
    UNION ALL SELECT 10, 'No Phone'      WHERE NOT COALESCE(v_report.no_phone_done, true)
    UNION ALL SELECT 11, 'Bad Data'      WHERE NOT COALESCE(v_report.bad_data_done, true)
  ) miss;

  v_team_quote_owe := (v_team_checklist_total - v_team_checklist_hit) * v_team_size;

  -- PERSONAL CHECKLIST: still per-person on detail
  ...
  -- CODE REDS / YELLOWS: still per-person on detail
  SELECT
  ...
  -- REQUIREMENTS: read from get_weekly_cpr_requirements() runtime function
  SELECT string_agg(
    '<tr>'
  ...
  -- HOURS: read from get_weekly_cpr_hours() runtime function, pivoted to one row per teammate
  WITH h_pivot AS (
    SELECT
      team_member_id,
  ...
  -- ACTIVITY: quotes_discussed + sales_points from detail, net_quotes from requirements function
  SELECT string_agg(
    '<tr>'
    || '<td style="padding:6px 10px;border-bottom:1px solid #e5e7eb;font-weight:600;color:#1e293b">' || COALESCE(NULLIF(t.nickname,''), t.first_name) || '</td>'
  ...
  -- PAYROLL: pay components from detail (unchanged)
  SELECT string_agg(
    '<tr>'
    || '<td style="padding:6px 10px;border-bottom:1px solid #e5e7eb;font-weight:600;color:#1e293b">' || COALESCE(NULLIF(t.nickname,''), t.first_name) || '</td>'
  ...
  -- ASSEMBLE THE HTML (unchanged from prior version)
  v_html :=
