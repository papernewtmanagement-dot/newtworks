-- write_weekly_comp_v2 was computing each person's weekly sales-points delta from the raw
-- weekly_cpr_team_detail.sales_points column. That column is only ever written by the team
-- check-in sync (a self-reported quarter figure typed into Telegram), which stopped carrying
-- that field on 2026-09-11. From week ending 2026-09-19 the column is NULL for everyone, so
-- commission came out 0, no MVP row was written, and the sales half of the bonus pool collapsed.
--
-- The canonical source is public.sales_points_week_delta(), which sits on
-- public.get_sales_points_qtd() (frozen -> production log -> CPR override -> self-reported).
-- compute_weekly_comp_residual_pool already calls it. This makes the comp writer call it too,
-- for commission, for the goals/gain deltas, and for MVP detection. One function, every caller.
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.weekly_pool_lock wl
