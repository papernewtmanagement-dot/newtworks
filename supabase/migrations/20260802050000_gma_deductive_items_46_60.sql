...
--
-- Item type: two premises about abstract categories, candidate judges
-- whether a stated conclusion must be true / cannot be true / can't be
-- determined. Abstract (nonsense-word) category labels are deliberate --
-- standard device in reasoning research to isolate logical-form ability
-- from "belief bias" (judging validity by whether a conclusion sounds
-- true rather than whether it follows). Evans, Newstead & Byrne (1993),
...
-- in /home/claude/gma_ded/generate_deductive.py, not the author's own
-- logical intuition. Classical syllogism form background: Copi & Cohen,
-- Introduction to Logic (11th ed.) -- but the checker is the actual
-- authority for each item, not the textbook.
--
-- Answer distribution deliberately balanced 5/5/5 across the three response
-- options (must be true / cannot be true / cannot be determined) so the
-- test cannot be gamed by always picking one answer -- of 64 possible
-- premise-pair combinations, only 4 each yield a determinate must/cannot
-- verdict (the rest are genuinely indeterminate), so item 4 and item 5
-- share an underlying logical form with different labels -- normal test-
-- construction practice, not a defect.
--
