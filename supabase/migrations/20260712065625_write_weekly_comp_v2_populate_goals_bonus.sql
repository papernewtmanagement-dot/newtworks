-- Extend write_weekly_comp_v2 to compute + persist goals_bonus per person AFTER the
-- audit_weekly_leaderboard_crossings call (which populates all_star_crossings + trailblazer_crossings
-- for the week). Also stashes a goals_detail block on residual_pool_diag so the frontend can render
-- the Team Activity "1% vs 13-wk" column and the payroll Goals row breakdown.
...
BEGIN
  ...
  -- ── GOALS BONUS (2026-07-12) ────────────────────────────────────
  -- Per-person weekly $ = $10 × all_star_crossings row(s) this week
  --                     + $10 × trailblazer_crossings row(s) this week
  --                     + $10 if this-week new SP >= 1.01 × avg weekly new SP over prior 13 weeks
  -- Runs AFTER audit so all_star_crossings + trailblazer_crossings rows for this week are already inserted.
  BEGIN
    WITH curr AS (
    ...
    -- Weekly new-SP deltas over prior 13 weeks (bounded to same-quarter LAG so QTD reset shows correctly)
    prior_13_weekly AS (
      SELECT d.team_member_id, r.week_ending_date, d.sales_points AS qtd,
             LAG(d.sales_points) OVER (
               PARTITION BY d.team_member_id, date_trunc('quarter', r.week_ending_date::timestamp)
               ORDER BY r.week_ending_date
             ) AS prior_qtd_same_q
      FROM public.weekly_cpr_team_detail d
      JOIN public.weekly_cpr_reports r ON r.id = d.weekly_cpr_report_id
      WHERE r.agency_id = p_agency_id
        ...
        AND r.week_ending_date < p_week_end_date
        AND r.week_ending_date >= (p_week_end_date - INTERVAL '13 weeks')::date
    ),
    prior_13_deltas AS (
      SELECT team_member_id,
             GREATEST(0, qtd - COALESCE(prior_qtd_same_q, 0))::numeric AS new_sp
      FROM prior_13_weekly
    ),
    prior_13_avg AS (
      SELECT team_member_id,
             AVG(new_sp) AS avg_new_sp,
             COUNT(*) AS n_weeks
    ...
    per_person AS (
      SELECT
        ...
        COALESCE(a.n_weeks, 0)::int AS n_prior_weeks,
      ...
      FROM this_new_sp t
      LEFT JOIN prior_13_avg a ON a.team_member_id = t.team_member_id
      ...
      SET goals_bonus = w.dollars,
          residual_pool_diag = COALESCE(wctd.residual_pool_diag, '{}'::jsonb) || jsonb_build_object(
            'goals_detail', jsonb_build_object(
              ...
              'n_prior_weeks',    w.n_prior_weeks,
              'target_1pct',      ROUND(w.target_1pct, 2),
              'gain_hit',         w.gain_hit,
              'as_hits',          w.as_hits,
              'tb_hits',          w.tb_hits,
              'dollars',          w.dollars,
              'formula',          '$10 per All-Star crossing + $10 per Trailblazer crossing + $10 if this-week new SP >= 1.01 * avg new SP prior 13wk'
            )
          ),
          updated_at = now()
      FROM with_dollars w
