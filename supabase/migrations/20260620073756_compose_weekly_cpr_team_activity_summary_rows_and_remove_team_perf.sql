...
BEGIN
  ...
  -- Team Total / Goal / Result rows. All values pulled from weekly_cpr_reports
  -- columns so the email mirrors what CPRDetail.jsx renders (single source of
  -- truth at the DB layer; both surfaces compute from the same row).
  ...
  v_team_net_quotes := COALESCE(v_report.quotes_total_net, 0);
  v_team_sales_pts := COALESCE(v_report.quarterly_sales_points_qtd, 0);
  v_wtw_hit := v_report.won_the_week IS TRUE;
  v_quotes_owed_next := COALESCE(v_report.quotes_owed_next_week, 0);
  ...
  v_activity_summary_rows :=
    ...
    || '<td style="padding:6px 10px;text-align:right;font-weight:700;color:#1e293b">'
       || CASE WHEN v_team_net_quotes > 0 THEN '+' WHEN v_team_net_quotes < 0 THEN '-' ELSE '' END
    ...
    -- Result
    || '<tr>'
    || '<td style="padding:6px 10px;font-weight:700;color:#334155">Result</td>'
    || '<td style="padding:6px 10px;text-align:right;color:#334155"></td>'
    || '<td style="padding:6px 10px;text-align:right;font-weight:700;color:' || CASE WHEN v_wtw_hit THEN '#15803d' ELSE '#b91c1c' END || '">'
       || CASE WHEN v_wtw_hit THEN '✓ Won' ELSE '✗ ' || v_quotes_owed_next::text || ' owed next wk' END
       || '</td>'
    || '<td style="padding:6px 10px;text-align:right;color:#334155"></td>'
    || '</tr>';
  ...
  -- Team Activity table now includes per-person rows + Team Total / Goal / Result summary
  v_html := v_html
    || '<div style="margin:14px 0">'
  ...
  -- Team Performance gray box removed 2026-06-20: data now lives in Team Activity
  -- summary rows above + other surfaces. Removed per Peter.
  ...
  v_html := v_html
    || '<hr style="border:none;border-top:1px solid #e2e8f0;margin:20px 0">'
    || '<div style="font-size:11px;color:#94a3b8;font-style:italic;margin:8px 0">Coming in upcoming iterations: Sales Points History (rolling averages), Leaderboards &amp; All-Stars, Prize Cart. Full data is on the <a href="' || v_cpr_url || '" style="color:#64748b">CPR detail page</a>.</div>';
