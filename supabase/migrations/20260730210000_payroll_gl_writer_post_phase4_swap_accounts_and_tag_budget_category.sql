-- Migration: rewire payroll_gl_writer to post-Phase-4 account codes + budget_category tags.
--
-- Phase 4 deactivated / removed the account codes the writer targeted:
--   COA-SUB-078 (team payroll)              → 6010 Staff Wages (agency) + tag budget_category=team
--   COA-SUB-086 (growth payroll)            → 6010 Staff Wages (agency) + tag budget_category=growth
--   COA-SUB-088 (reimbursements pending)    → 0003 *Unclassified (agency)  [Peter walks these later]
--   COA-IC-001  (agency intercompany)       → 2902 Due to PaperNewt LLC (agency)
--   COA-PN-001  (PN payroll expense)        → 6020 Owner W-2 Wages (PaperNewt)
--   COA-PN-002  (PN cash — unmapped)        → 1050 US Bank Business Checking (3977) (PaperNewt)
--   COA-IC-002  (PN intercompany asset)     → 1901 Due from Peter Story State Farm (PaperNewt)
--
-- Growth + team both land on the same account (6010). Semantic split is carried by
-- (a) journal_line.description, (b) transaction_tags.budget_category = team|growth.
-- Historical lines were tagged in Phase 4b with created_by='phase4_coa_reorg'.
-- Forward-write lines are tagged here with created_by='payroll_gl_writer'.
--
-- Companion (uuid, uuid) recipe wrapper is unchanged and forwards to this overload.
...
DECLARE
  ...
  -- Post-Phase-4 targets. Growth + team share the same wages account; the
  -- difference is carried in the transaction_tags budget_category tag.
  v_wages_acct uuid;
...
BEGIN
  ...
  -- Agency-side targets (business_entity_id = Peter Story State Farm)
  ...
  IF v_wages_acct IS NULL OR v_reimb_pending_acct IS NULL
    ...
    RETURN jsonb_build_object('ok', false, 'error', 'coa_lookup_failed',
      'wages', v_wages_acct, 'reimb_pending', v_reimb_pending_acct,
  ...
  LOOP
    ...
    IF v_ic_credit > 0 THEN
      ...
      IF v_grow_total > 0 THEN
        ...
        VALUES (v_agency_je, p_agency_id, v_wages_acct, v_grow_total, 0,
                'Growth Budget share of team fixed pay')
        RETURNING id INTO v_grow_line_id;

        INSERT INTO transaction_tags (agency_id, journal_line_id, tag_key, tag_value, created_by)
        VALUES (p_agency_id, v_grow_line_id, 'budget_category', 'growth', 'payroll_gl_writer');
      END IF;
      IF v_team_total > 0 THEN
        INSERT INTO journal_lines (journal_entry_id, agency_id, account_id, debit, credit, description)
        VALUES (v_agency_je, p_agency_id, v_wages_acct, v_team_total, 0,
                'Team Budget: post-ramp fixed pay + bonus/commission/other + ER taxes + unclassified legacy items')
        RETURNING id INTO v_team_line_id;

        INSERT INTO transaction_tags (agency_id, journal_line_id, tag_key, tag_value, created_by)
        VALUES (p_agency_id, v_team_line_id, 'budget_category', 'team', 'payroll_gl_writer');
      END IF;
    ...
    SET posted_at=NOW(),
        ...
        notes=COALESCE(notes,'') || ' [posted by payroll_gl_writer post_phase4 ' || NOW()::text ||
