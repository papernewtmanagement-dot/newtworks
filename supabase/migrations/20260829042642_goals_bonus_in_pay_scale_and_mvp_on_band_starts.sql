...
--    Legend a week, halved through year one. Every pay_scale row knows its
--    band, and the band carries the tier, so the annual figure can be
--    worked out per row instead of only inside the projection.
--
...
-- Prize-cart draws line up with the band starts.
...
-- Spread goals across each band and carry both columns through a reseed.
DO $mig$
...
BEGIN
  ...
  -- Every row takes the goals rate of the band it sits in.
  v_new := replace(v_new,
...
-- The Sales curve now carries the goals money like the other two do.
DO $mig2$
DECLARE v_def text; v_new text;
BEGIN
  SELECT pg_get_functiondef(p.oid) INTO v_def FROM pg_proc p
...
$b$                 'bonus', round(COALESCE(p.expected_team_bonus_annual,0)
  ...
  v_new := replace(v_new,
    $a$           || 'tier, and a raise never steps back down. The goals bonus is '$a$,
    $b$           || 'tier, and a raise never steps back down. The goals bonus is '$b$);
