-- Reverse the 3 pre-Plan-A payroll JEs (2026-06-27, 2026-07-04, 2026-07-11).
-- These booked everything to COA-SUB-078 lump vs new Plan A structure.
-- Approach: post opposite-sign reversal JEs referencing originals, then NULL out
-- payroll_runs.journal_entry_id + posted_at so payroll_gl_writer can re-post fresh.
...
BEGIN
  FOREACH v_wrong_je IN ARRAY ARRAY[
    ...
    -- Mirror each original line with debit<->credit swapped
    FOR v_line IN
    ...
    -- Mark the ORIGINAL wrong JE as voided in its description so it's obvious in reports
  ...
  -- Clear payroll_runs pointer + posted_at so writer will re-post fresh
  UPDATE payroll_runs
