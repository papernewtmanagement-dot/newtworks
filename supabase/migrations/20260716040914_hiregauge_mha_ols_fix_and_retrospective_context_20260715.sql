-- ============================================================
-- Part 1: _hiregauge_get_trait_value — MHA fix (naive avg → OLS canonical)
-- Previous formula: (DM + RD + ASS) / 3 — silently underestimated MHA
-- for high-Analytical / low-Compassion profiles. Tommy: 51.7 → 55 (real).
-- Fix: extract from cts_sales_competencies (canonical OLS regression).
-- NULL-safe: returns NULL if any raw trait is NULL (no COALESCE-to-0).
-- ============================================================

CREATE OR REPLACE FUNCTION public._hiregauge_get_trait_value(p_ta public.team_assessments, p_trait text)
 RETURNS numeric
 LANGUAGE sql
 IMMUTABLE
AS $function$
...
-- ============================================================
-- Part 2: cts_all_competencies wrapper — single RPC returning
-- competencies for all four role fits, keyed by role.
-- Enables the frontend Role Fit & Competencies section with
...
-- ============================================================
-- Part 3: hiregauge_composite_recommendation — add retrospective_context field.
-- When assessment.team_member_id links to an active, non-archived team row,
-- verdict becomes 'retrospective_read'. Framework signals still surface for
-- calibration but not framed as a hiring decision.
-- Adds new output column, requires DROP+CREATE (RETURNS TABLE shape change).
-- ============================================================
...
BEGIN
  ...
  -- Retrospective context override: currently-hired + active team member.
  -- Framework signals valuable for calibration; verdict framed accordingly.
  ...
  -- Original verdict logic (candidates + former team)
