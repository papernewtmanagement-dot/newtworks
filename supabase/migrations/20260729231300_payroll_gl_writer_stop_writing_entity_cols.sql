-- 2026-07-29: payroll_gl_writer + get_payroll_run_drilldown — stop writing to
-- and reading from redundant journal_entries.business_entity_id /
-- journal_lines.business_entity_id. Entity flows through account_id in each
-- journal_line (chart_of_accounts.business_entity_id is source of truth).
-- Reads from team.business_entity_id (routing decision), payroll_runs.business_entity_id
-- (that table's own column) are unchanged. Display-side entity per JE is now
-- derived via correlated subquery over the JE's journal_lines.
    ...
    IF v_ic_credit > 0 THEN
      ...
      IF v_reimb_total > 0 THEN
        ...
        VALUES (v_agency_je, p_agency_id, v_reimb_pending_acct, v_reimb_total, 0,
                'Reimbursements (pending categorization)');
      END IF;
      INSERT INTO journal_lines (journal_entry_id, agency_id, account_id, debit, credit, description)
...
BEGIN
  ...
  -- Derive each JE's entity from the DISTINCT chart_of_accounts.business_entity_id
  -- of its journal_lines. Single-entity JE → that entity. Mixed → NULL. This
  -- replaces the direct je.business_entity_id read.
  SELECT COALESCE(jsonb_agg(
           jsonb_build_object(
             ...
             'business_entity_id', (
               SELECT CASE WHEN COUNT(DISTINCT coa.business_entity_id) = 1
                           THEN MIN(coa.business_entity_id)
                           ELSE NULL::uuid
                      END
