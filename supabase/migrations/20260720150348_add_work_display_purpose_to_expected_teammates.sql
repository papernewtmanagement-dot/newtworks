-- Add a new purpose 'work_display' to get_expected_teammates. Same shape as
-- 'work_checkin' but WITHOUT the PTO exclusion and WITHOUT the
-- tag_in_team_reminders gate — because those flags govern NAGGING (who gets
-- reminded / @-mentioned), not who appears in a status listing.
-- Peter directive 2026-07-20: "if somebody is off, we don't nag them. We still
-- show them in the listing."
-- Fully-unlicensed filter is retained for now (separate question outstanding
-- for Cassandra).
