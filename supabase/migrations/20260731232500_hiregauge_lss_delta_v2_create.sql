...
AS $function$
-- =========================================================================
-- Purpose: emit the raw signal the new Intelligence-composite scoring
-- stack consumes. Replaces v1's per-subtest per-role ideal-range model.
--
-- Return shape:
--   {
--     "path": "per_subtest" | "per_subtest_acc_only" | "no_lss_data",
--     "instrument_source": "v1" | "cts",
--     "intelligence_composite": 0-100 numeric (or NULL if no_lss_data),
--     "accuracy":   { "verbal": 0-100, "math": 0-100, "problem_solving": 0-100 },
--     "efficiency": { "verbal": 0-100, "math": 0-100, "problem_solving": 0-100 },
--     "n_valid_subtests": 0..3
--   }
--
-- accuracy[k]   = 100 * raw_score / total_items       (proportion correct)
-- efficiency[k] = accuracy[k] * speed_multiplier      (bounded [0, 100])
--   where speed_multiplier = exp(-k * (ln(winsorize(t_actual, p2.5, p97.5)) - ln(baseline)))
--
--   Guessing guards (applied to efficiency, not accuracy):
--     accuracy_prop <  chance_rate  -> efficiency = 0
--     accuracy_prop <= chance_rate  -> speed multiplier pinned to 1.0
--
-- intelligence_composite:
--   path='per_subtest'          -> unit-weighted mean of the efficiencies present
--   path='per_subtest_acc_only' -> unit-weighted mean of the accuracies present
--   path='no_lss_data'          -> NULL
--
-- Consumers (competency fns, role-fit fns) apply their own curves against
-- the composite:
--   comp-side monotonic floor:  mult = 1 if intel >= floor else exp(-3 * (floor-intel)/floor)
--   fit-side asymmetric:        under-floor exponential w=1.0;
--                               over-ceiling quadratic w=0.4
--
-- Research grounding (cite in consumer function docstrings, not here):
--   Ree/Earles/Teachout 1994    g plus almost nothing
--   Schmidt & Hunter 1998       g monotonic for performance, r=.51
--   Wainer 1976                 unit-weighted composites within measurement error of optimal
--   Sheppard & Vernon 2008      RT-g correlation, k=0.30 log-RT
--   Ratcliff 1993               RT outlier winsorization at empirical p2.5/p97.5
--   van Zandt 2000              RT distributions log-normal (rationale for log-RT form)
--   Jensen 1998; Salthouse 1996 mental chronometry (speed at same accuracy = ability signal)
-- =========================================================================
DECLARE
  v_source text;
  v_calib jsonb;
  ...
  -- Per-subtest computed values
  va_prop numeric; ma_prop numeric; pa_prop numeric;
  acc_v numeric; acc_m numeric; acc_p numeric;   -- 0-100 (or NULL if that subtest missing)
  eff_v numeric; eff_m numeric; eff_p numeric;   -- 0-100 (or NULL)
...
BEGIN
  ...
  IF v_src IS NULL THEN
    -- Unknown assessment_source; fall back to CTS calibration
    v_src := v_calib->'cts';
    v_source := 'cts';
  END IF;
  ...
  -- Per-subtest accuracy (proportion correct * 100, clamped)
  ...
  IF v_all_speed THEN
    ...
    IF va_raw IS NOT NULL THEN
      ...
      ELSIF va_prop <= v_chance THEN
        eff_v := ROUND(LEAST(100, GREATEST(0, va_prop * 100)), 2);  -- speed pinned to 1.0
      ELSE
        t_v    := LEAST(v_p975, GREATEST(v_p25, vs_sec::numeric));   -- Ratcliff winsorization
    ...
    -- Composite: unit-weighted mean of the efficiencies of subtests present
    v_composite := ROUND(
  ...
  ELSE
    ...
    -- No complete speed data; composite = unit-weighted mean of accuracies present
    v_composite := ROUND(
