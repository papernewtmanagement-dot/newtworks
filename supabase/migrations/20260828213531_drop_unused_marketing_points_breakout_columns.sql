-- 2026-08-28: points_reviews / points_referrals_quoted / points_referrals_sold were never
-- populated (all zero across every row since inception). The real number always went into
-- marketing_points.points via the CPR inline input. Dropping the three dead columns.
-- compute_weekly_marketing_bonus summed them, so it is rebuilt below with the sums removed.
-- The three output keys (reviews_qtd / quoted_qtd / sold_qtd) are RETAINED emitting 0 so any
-- frontend reading them keeps working exactly as it does today (they already always read 0).

ALTER TABLE public.marketing_points DROP COLUMN IF EXISTS points_reviews;
ALTER TABLE public.marketing_points DROP COLUMN IF EXISTS points_referrals_quoted;
ALTER TABLE public.marketing_points DROP COLUMN IF EXISTS points_referrals_sold;
