...
DECLARE
  ...
  v_reference_number text; v_chart_account_code text; v_stmt_account_id uuid;
  ...
  -- cash-register claim state
  v_claim_direction text; v_claim_ledger_id uuid; v_claim_old_status text; v_claim_old_account_id uuid;
  v_claim_register_id uuid;
  v_count_claimed int := 0; v_total_claimed numeric := 0;
  v_dry_claimed uuid[] := ARRAY[]::uuid[];
  v_count_claim_reversed_verification int := 0;
  v_count_claim_reversed_balance_sheet int := 0;
BEGIN
  FOR v_stmt_id, v_business_entity_id, v_txn_date, v_description, v_amount,
      v_transaction_type, v_category, v_reference_number, v_chart_account_code, v_stmt_account_id IN
    SELECT s.id, s.business_entity_id, s.transaction_date, s.description, s.amount,
           s.transaction_type, s.category, s.reference_number, coa.account_code, s.account_id
    FROM statements s
  ...
  LOOP
    ...
    v_claim_direction := NULL; v_claim_ledger_id := NULL; v_claim_old_status := NULL; v_claim_old_account_id := NULL;
    v_claim_register_id := NULL;
    ...
    -- ===== CASH-REGISTER CLAIM STEP =====
    -- A register-sourced ledger row that matches this statement line is claimed
    -- instead of letting the normal insert path create a second row.
    IF v_stmt_account_id IS NOT NULL THEN
      v_claim_direction := CASE
      ...
      IF v_claim_direction IS NOT NULL THEN
        IF p_dry_run THEN
          -- FIX 2: exclude ids already "claimed" earlier in this same dry run so
          -- two identical statement lines don't both match the same register row.
          SELECT l2.id, l2.classification_status, l2.account_id, c.id
          INTO v_claim_ledger_id, v_claim_old_status, v_claim_old_account_id, v_claim_register_id
          FROM ledger l2
          JOIN cash_register_preliminary c ON c.id = l2.cash_register_id
          JOIN accounts ra ON (ra.account_number_last4 = c.account_last4 OR c.account_last4 = ANY(ra.alternate_last4s))
          WHERE l2.cash_register_id IS NOT NULL
            AND l2.statement_id IS NULL
            AND ra.id = v_stmt_account_id
            AND ra.is_active = TRUE
            AND l2.id <> ALL(v_dry_claimed)
            AND ((v_claim_direction = 'debit' AND l2.debit > 0 AND round(l2.debit,2) = round(abs(v_amount),2))
              OR (v_claim_direction = 'credit' AND l2.credit > 0 AND round(l2.credit,2) = round(abs(v_amount),2)))
            AND abs(l2.entry_date - v_txn_date) <= 4
          ORDER BY abs(l2.entry_date - v_txn_date) ASC, l2.created_at ASC
          LIMIT 1;

          IF v_claim_ledger_id IS NOT NULL THEN
            v_dry_claimed := array_append(v_dry_claimed, v_claim_ledger_id);
          END IF;
        ELSE
          WITH candidate AS (
            SELECT l2.id
          ...
          UPDATE ledger l
          SET statement_id = v_stmt_id,
              entry_date = v_txn_date,
              description = v_description,
              reference_number = v_reference_number
          FROM candidate
          WHERE l.id = candidate.id
          RETURNING l.id, l.classification_status, l.account_id, l.cash_register_id
          INTO v_claim_ledger_id, v_claim_old_status, v_claim_old_account_id, v_claim_register_id;

          IF v_claim_ledger_id IS NOT NULL THEN
            UPDATE cash_register_preliminary cr
            SET coding_status = 'statement_confirmed',
                reconciled_at = NOW(),
                reconciled_journal_entry_id = v_claim_ledger_id,
                updated_at = NOW()
            WHERE cr.id = v_claim_register_id;
          END IF;
        END IF;
      END IF;
    END IF;
    ...
    IF v_skip_rule_id IS NOT NULL THEN
      ...
      -- FIX 1: a claim made above must not survive this skip — this statement
      -- line just told us the transaction doesn't belong in income/expense at
      -- all, so the register-sourced row that was stamped "confirmed" gets
      -- removed from the ledger and the register row is marked reconciled
      -- with no journal entry attached.
      IF v_claim_ledger_id IS NOT NULL THEN
        v_count_claim_reversed_verification := v_count_claim_reversed_verification + 1;
        IF NOT p_dry_run THEN
          ...
          SET coding_status = 'statement_confirmed',
              ...
              coding_question = 'The statement confirmed this transaction but it does not belong in income or expense, so it was removed from the ledger.',
              updated_at = NOW()
    ...
    IF v_target_account_id IS NOT NULL THEN
      ...
      IF v_target_account_type NOT IN ('income', 'expense') THEN
        ...
        -- FIX 1: same reversal, for the balance-sheet-target skip path.
        IF v_claim_ledger_id IS NOT NULL THEN
          v_count_claim_reversed_balance_sheet := v_count_claim_reversed_balance_sheet + 1;
          IF NOT p_dry_run THEN
            DELETE FROM ledger WHERE id = v_claim_ledger_id;
            UPDATE cash_register_preliminary
            SET coding_status = 'statement_confirmed',
    ...
    IF v_claim_ledger_id IS NOT NULL THEN
      -- claimed a cash-register-sourced row: never insert a second row for it
      v_count_claimed := v_count_claimed + 1;
      v_total_claimed := v_total_claimed + abs(v_amount);
      IF v_claim_old_status = 'unclassified' AND v_classification_status = 'classified' AND NOT p_dry_run THEN
        UPDATE ledger
        SET account_id = v_target_account_id,
            original_account_id = v_claim_old_account_id,
            original_account_code = (SELECT account_code FROM chart_of_accounts WHERE id = v_claim_old_account_id),
            original_account_name = (SELECT account_name FROM chart_of_accounts WHERE id = v_claim_old_account_id),
            classification_status = 'classified',
            classified_by = CASE WHEN v_rule_id IS NOT NULL THEN 'rule:' || v_rule_id::text ELSE 'category_match' END,
            classified_at = NOW(),
            rule_id_used = v_rule_id,
            suspense_reason = NULL
        WHERE id = v_claim_ledger_id;
      END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'claimed_from_cash_register', v_count_claimed, 'claimed_from_cash_register_amount', v_total_claimed,
    'claim_reversed_verification_rule', v_count_claim_reversed_verification,
    'claim_reversed_balance_sheet', v_count_claim_reversed_balance_sheet,
