-- =========================================================================
-- Migration: step1d_classify_and_post_bank_txns_entity_scoped
-- Applied: 2026-07-31 via Supabase MCP (session: pipeline_repair_step1)
-- =========================================================================
-- Rewrite classify_and_post_bank_txns (external bank-reparse flow) to
-- derive entity from source-account code and use entity-scoped unclassified as
-- suspense fallback.
-- =========================================================================

-- Manual-invocation function used to (re)ingest bank transactions from an
-- externally-parsed statement. Fix: derive the business entity from the
-- source-account code and use entity-scoped unclassified as suspense.
...
DECLARE
  ...
  v_no_entity_count int := 0;
BEGIN
  -- Resolve the source account once (all txns are on the same source in this batch)
  SELECT id, business_entity_id
    INTO v_source_account_id, v_source_entity_id
  FROM public.chart_of_accounts
  WHERE agency_id = p_agency_id AND account_code = p_source_account_code AND is_active = TRUE
  LIMIT 1;

  IF v_source_account_id IS NULL THEN
    RETURN jsonb_build_object('ok', FALSE, 'error', 'source_account_not_found',
      'account_code', p_source_account_code);
  END IF;

  IF v_source_entity_id IS NULL THEN
    RETURN jsonb_build_object('ok', FALSE, 'error', 'source_account_missing_business_entity',
      'account_code', p_source_account_code);
  END IF;

  FOR v_row IN
    SELECT (ord - 1) AS idx, elem
    FROM jsonb_array_elements(p_txns) WITH ORDINALITY AS t(elem, ord)
  LOOP
    v_amt := (v_row.elem->>'amount')::numeric;
    IF v_amt IS NULL OR v_amt = 0 THEN
      v_skipped_count := v_skipped_count + 1;
      CONTINUE;
    END IF;
    v_payee := COALESCE(v_row.elem->>'payee', '');
    v_memo := COALESCE(v_row.elem->>'memo', '');
    v_date := (v_row.elem->>'date')::date;
    ...
    IF v_rule.id IS NOT NULL THEN
      ...
      v_is_suspense := FALSE;
      v_desc := v_payee || CASE WHEN v_rule.sub_category_label IS NOT NULL AND v_rule.sub_category_label != ''
                                THEN ' — ' || v_rule.sub_category_label ELSE '' END;

      -- Same-entity preference on target account lookup
      SELECT id INTO v_debit_id FROM public.chart_of_accounts
        WHERE agency_id = p_agency_id AND account_code = v_debit_code AND is_active = TRUE
      ...
      SELECT id INTO v_credit_id FROM public.chart_of_accounts
        WHERE agency_id = p_agency_id AND account_code = v_credit_code AND is_active = TRUE
          AND (business_entity_id = v_source_entity_id OR business_entity_id IS NULL)
        ORDER BY (business_entity_id = v_source_entity_id) DESC NULLS LAST
        LIMIT 1;
      ...
      END IF;
    ELSE
      -- No rule matched → suspense (entity-scoped)
      v_suspense_id := public.get_entity_unclassified_account(
        p_agency_id, v_source_entity_id,
        CASE WHEN v_amt > 0 THEN 'income' ELSE 'expense' END
      );
      IF v_suspense_id IS NULL THEN
        v_no_entity_count := v_no_entity_count + 1;
        CONTINUE;
      END IF;

      IF v_amt < 0 THEN
        v_debit_id := v_suspense_id;
        v_credit_id := v_source_account_id;
      ELSE
        v_debit_id := v_source_account_id;
        v_credit_id := v_suspense_id;
      END IF;
      v_is_suspense := TRUE;
      v_desc := v_payee;
    END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'no_entity_count', v_no_entity_count
  );
END; $function$;
