...
BEGIN
  ...
  -- SNAPSHOT v_team_size — writer-consistent (archived after week start still counts)
  ...
  -- REQUIREMENTS rows (snapshot policy comes from get_weekly_cpr_requirements; uses archived_at)
  SELECT string_agg(
    '<tr>'
  ...
  -- HOURS rows — writer-consistent snapshot
  WITH h_pivot AS (
    SELECT
      team_member_id,
  ...
  SELECT string_agg(
    '<tr>'
    || '<td style="padding:6px 10px;border-bottom:1px solid #e5e7eb;font-weight:600;color:#1e293b">' || COALESCE(NULLIF(t.nickname,''), t.first_name) || '</td>'
    || '<td style="padding:6px 10px;text-align:right;border-bottom:1px solid #e5e7eb;color:#334155">' || COALESCE(hp.mon_h::text, '0') || ' ' || CASE WHEN hp.mon_loc='remote' THEN '🟣' WHEN hp.mon_loc='in_office' THEN '🟢' ELSE '' END || '</td>'
    || '<td style="padding:6px 10px;text-align:right;border-bottom:1px solid #e5e7eb;color:#334155">' || COALESCE(hp.tue_h::text, '0') || ' ' || CASE WHEN hp.tue_loc='remote' THEN '🟣' WHEN hp.tue_loc='in_office' THEN '🟢' ELSE '' END || '</td>'
    || '<td style="padding:6px 10px;text-align:right;border-bottom:1px solid #e5e7eb;color:#334155">' || COALESCE(hp.wed_h::text, '0') || ' ' || CASE WHEN hp.wed_loc='remote' THEN '🟣' WHEN hp.wed_loc='in_office' THEN '🟢' ELSE '' END || '</td>'
    || '<td style="padding:6px 10px;text-align:right;border-bottom:1px solid #e5e7eb;color:#334155">' || COALESCE(hp.thu_h::text, '0') || ' ' || CASE WHEN hp.thu_loc='remote' THEN '🟣' WHEN hp.thu_loc='in_office' THEN '🟢' ELSE '' END || '</td>'
    || '<td style="padding:6px 10px;text-align:right;border-bottom:1px solid #e5e7eb;color:#334155">' || COALESCE(hp.fri_h::text, '0') || ' ' || CASE WHEN hp.fri_loc='remote' THEN '🟣' WHEN hp.fri_loc='in_office' THEN '🟢' ELSE '' END || '</td>'
  ...
  -- AGENCY PERFORMANCE: 5 rows. Year-end "On Time" projection = YTD × 365 / days_elapsed.
  SELECT * INTO v_snap
  ...
  IF v_snap.id IS NOT NULL THEN
    WITH lines AS (
      SELECT 1 AS ord, 'Auto Gain' AS label,
             COALESCE(v_snap.auto_production_ytd,0) - COALESCE(v_snap.auto_lapse_ytd,0) AS ytd,
             (SELECT goal_value FROM public.book_performance_goals WHERE agency_id = p_agency_id AND lob = 'auto' AND metric = 'gain' AND year = EXTRACT(YEAR FROM p_week_ending_date)::int LIMIT 1) AS goal,
             false AS is_money
      UNION ALL SELECT 2, 'Fire Gain',
             COALESCE(v_snap.fire_production_ytd,0) - COALESCE(v_snap.fire_lapse_ytd,0),
             (SELECT goal_value FROM public.book_performance_goals WHERE agency_id = p_agency_id AND lob = 'fire' AND metric = 'gain' AND year = EXTRACT(YEAR FROM p_week_ending_date)::int LIMIT 1),
             false
      UNION ALL SELECT 3, 'Life Gain',
             COALESCE(v_snap.life_production_ytd,0) - COALESCE(v_snap.life_loss_ytd,0),
             (SELECT goal_value FROM public.book_performance_goals WHERE agency_id = p_agency_id AND lob = 'life' AND metric = 'gain' AND year = EXTRACT(YEAR FROM p_week_ending_date)::int LIMIT 1),
             false
      UNION ALL SELECT 4, 'Life Paid #',
             COALESCE(v_snap.life_paid_count_ytd,0),
             (SELECT goal_value FROM public.book_performance_goals WHERE agency_id = p_agency_id AND lob = 'life' AND metric = 'net_paid_for' AND year = EXTRACT(YEAR FROM p_week_ending_date)::int LIMIT 1),
             false
      UNION ALL SELECT 5, 'Life Premium',
             COALESCE(v_snap.life_premium_credits_ytd,0)::numeric,
             (SELECT goal_value FROM public.book_performance_goals WHERE agency_id = p_agency_id AND lob = 'life' AND metric = 'premium' AND year = EXTRACT(YEAR FROM p_week_ending_date)::int LIMIT 1),
             true
    )
    SELECT string_agg(
      '<tr>'
  ...
  -- AGENCY PERFORMANCE TABLE (Section 10 per canon)
  ...
  v_html := v_html
    || '<hr style="border:none;border-top:1px solid #e2e8f0;margin:20px 0">'
    || '<div style="font-size:11px;color:#94a3b8;font-style:italic;margin:8px 0">Coming in upcoming iterations: SMVC &amp; Scorecard, Sales Points History (rolling averages), Leaderboards &amp; All-Stars, Prize Cart. Full data is on the <a href="' || v_cpr_url || '" style="color:#64748b">CPR detail page</a>.</div>';
