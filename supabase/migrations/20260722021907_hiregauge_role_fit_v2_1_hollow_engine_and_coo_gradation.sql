...
-- This migration was applied live on 2026-07-22 02:19:07 UTC (ledger name: hiregauge_role_fit_v2_1_hollow_engine_and_coo_gradation) but was never
-- committed to this repo at the time. Content below is copied verbatim
-- from supabase_migrations.schema_migrations.statements for version 20260722021907.
-- No live change is made by adding this file — it only brings the repo
-- mirror into sync with what is already running in production, so a
-- fresh `supabase db reset` from this repo reproduces live state.
-- v2.1: refine hollow_broadcast to require engine collapse; gradate CoO caps by warmth requirement
...
AS $function$
  SELECT jsonb_build_object(
    ...
    -- v2.1: hollow_broadcast requires engine collapse — cold-warm hybrids with intact engines
    -- (John, Tommy) work in transactional-firm roles despite Compassion floor + Optimism ceiling.
    -- Only genuinely engine-collapsed candidates (Randy: DM 9 + IS 15) are truly hollow.
...
BEGIN
  ...
  IF (gates->>'hollow_broadcast')::boolean THEN
    ...
    v_adjustments := v_adjustments || jsonb_build_array(jsonb_build_object(
      ...
      'reason','Optimism ceiling + Compassion floor + engine collapse (DM < 40 OR IS < 40)'
    ));
  END IF;

  v_after_comp_cap := LEAST(v_dampened, p_floor_comp_cap);

  -- v2.1: CoO caps gradated by warmth requirement (Reception/Aspirant = warm-required,
  -- Escalation/In-Book = warm-tolerable, Support = warm-optional)
  IF p_role = 'sales_outbound' THEN
    v_coo_cap := 55; v_hwe_cap := 50; v_pit_cap := 55;
  ELSIF p_role = 'sales_inbound' THEN
    v_coo_cap := 50; v_hwe_cap := 50; v_pit_cap := 55;
  ...
  ELSIF p_role = 'retention_escalation' THEN
    v_coo_cap := 55; v_hwe_cap := 55; v_pit_cap := 55;
  ...
  RETURN jsonb_build_object(
    ...
    'gates', gates,
    'model', 'competency_fit_v2_1_2026_07_21'
  );
END;
$function$;
