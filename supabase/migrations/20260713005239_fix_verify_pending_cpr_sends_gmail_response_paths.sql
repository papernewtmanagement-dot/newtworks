...
-- Fix: Composio GMAIL_FETCH_MESSAGE_BY_MESSAGE_ID response shape is
--   {data:{labelIds, messageId, messageTimestamp, threadId, ...}, successful, error}
-- NOT {data:{response_data:{...}}} as previously assumed (session_note 2026-07-05).
-- Old paths returned NULL on every fetch → every phase-2 verify hit reset_no_sent_label
-- → send_dispatched_at cleared → 90-min duplicate-send guard defeated.
--
-- Changes vs prior version:
--   1. v_label_ids: path {data,response_data,labelIds} -> {data,labelIds}
--   2. Timestamp: {data,response_data,internalDate}::bigint (epoch ms) replaced with
--      {data,messageTimestamp}::timestamptz (ISO 8601). Renamed var v_gmail_dt_epoch -> v_gmail_ts.
--   3. Wrapped ISO parse in BEGIN/EXCEPTION so a malformed timestamp doesn't blow up the loop.
--   4. Detail log key gmail_internal_date -> gmail_message_timestamp.
  ...
  LOOP
    ...
    ELSE
      ...
      ELSIF v_verify_resp.status_code BETWEEN 200 AND 299 THEN
        ...
        -- FIXED: actual Composio response is {data:{labelIds, messageTimestamp, ...}}
