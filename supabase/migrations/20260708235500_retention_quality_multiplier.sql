...
-- Also drops the standalone Lapse column from the frontend view (values retained in output for reference).
  ...
  SELECT
    ...
    jsonb_build_object(
      ...
      'formulas', jsonb_build_object(
        'attribution', 'own_new x 4 + (own_stack x 0.65 IF Wk52+) + retention_pool_share',
        'coverage_bar', 'annual_base x tenure_mult x 1.08',
...
'Attribution: own_new x 4 + own_stack x 0.65 (Wk52+) + retention_pool_share x RQM (Retention only). '
...
'Coverage bar = fully-loaded; Profitability bar = fully-loaded x 2.5. Green >=100%, Yellow >=80%. '
