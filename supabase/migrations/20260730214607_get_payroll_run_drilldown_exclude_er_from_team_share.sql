-- Migration: get_payroll_run_drilldown now excludes ER taxes from per-person team_share.
-- Mirrors post-2026-07-30 writer where ER taxes post to 6030 separately from wages on 6010.
-- Per-person employer_taxes field is unchanged (still SurePayroll-reported total per person).
-- Only change: v_team_share formula drops the "+ COALESCE(v_p_er_taxes, 0)" term.
