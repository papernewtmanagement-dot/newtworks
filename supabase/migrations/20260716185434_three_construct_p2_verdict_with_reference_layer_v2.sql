...
CREATE OR REPLACE FUNCTION public.hiregauge_three_construct_verdict(p_assessment_id uuid)
 RETURNS TABLE(
    ...
    character_floor_status text, character_floor_failed text[],
    retrospective_verdict text, retrospective_notes text, retrospective_context text,
    calibration_status text,
    dimensions_scored integer, confidence text,
    meta jsonb
 )
 LANGUAGE plpgsql
 STABLE
...
BEGIN
  SELECT * INTO v_ta FROM public.hiring_assessments WHERE id = p_assessment_id;
  ...
  -- INTERVIEW layer, Nurture cell + character floor
  ...
  BEGIN
    ...
    v_wsum := 0; v_sum := 0;
    IF v_nref  IS NOT NULL THEN v_sum := v_sum + v_nref  * v_nature_w;  v_wsum := v_wsum + v_nature_w;  END IF;
    IF v_nuref IS NOT NULL THEN v_sum := v_sum + v_nuref * v_nurture_w; v_wsum := v_wsum + v_nurture_w; END IF;
    IF v_dref  IS NOT NULL THEN v_sum := v_sum + v_dref  * v_drivers_w; v_wsum := v_wsum + v_drivers_w; END IF;
...
'4x3 matrix: layers Resume/Assessment/Interview/Reference × constructs Nature/Nurture/Drivers. Layer weights within construct: Nature R5/A75/I15/Ref5; Nurture R10/A15/I45/Ref30; Drivers R10/A25/I45/Ref20. Construct weights 35/30/35. Reference is 4th pre-hire scored layer (ref_nature/ref_nurture/ref_drivers). Retrospective observation stays weightless — post-hire calibration companion only.';
