...
RETURNS TABLE(
  verdict text,                    -- 'decline' | 'consider' | 'hire'
  primary_reason text,             -- one-line summary
  character_floors_failed text[],  -- e.g. ['CF-Concern','CF-HWE']
  decline_signals text[],          -- archetype/exit_mode/filter_rule short labels
  consider_signals text[],         -- coaching_variant/strategic_seat/diagnostic_tool short labels
  hire_signals text[],             -- positive archetype short labels
  ...
  LOOP
    ...
    ELSIF v_row.out_pass = true THEN
      ...
      CASE v_row.out_rule_type
        WHEN 'archetype' THEN
          -- Warm Non-Starter / Exit-mode-style archetypes lean decline; others hire
        ...
        WHEN 'diagnostic_tool' THEN
          v_consider := v_consider || COALESCE(v_row.out_short_label, v_row.out_rule_name);
        ELSE
          -- reader_vulnerability, money_motivator, drive_test, behavioral_tell,
          -- resume_screen_signal, recommendation_logic, validity_rule, framework_principle
          -- These are informational — don't push verdict on their own
          NULL;
      END CASE;
    END IF;
  END LOOP;
