-- Peter 2026-09-17: filling the gaps on older production records. The imported
-- history has no phone last four, no ECRM link, and on some rows no marketing
-- source. Phone is the household key, so it is filled per household and lands
-- on every record under that name at once. ECRM and marketing source are per
-- record. Nothing here touches products, entry_source, or credits.
...
BEGIN
  ...
  CREATE TEMP TABLE IF NOT EXISTS _x (x int);  -- placeholder never used; kept out of RPC path
  ...
  FOR rec IN SELECT * FROM jsonb_array_elements(CASE WHEN jsonb_typeof(p->'ecrm') = 'array' THEN p->'ecrm' ELSE '[]'::jsonb END) LOOP
    ...
    IF (rec->>'kind') = 'sale' THEN
      UPDATE public.sales_log SET ecrm_opportunity_url = v_url, updated_at = now()
       WHERE id = (rec->>'id')::uuid AND agency_id = a.agency_id;
      GET DIAGNOSTICS n = ROW_COUNT; v_ecrm_rows := v_ecrm_rows + n;
    ELSIF (rec->>'kind') = 'quote' THEN
      UPDATE public.quote_log SET ecrm_opportunity_url = v_url, updated_at = now()
       WHERE id = (rec->>'id')::uuid AND agency_id = a.agency_id;
      GET DIAGNOSTICS n = ROW_COUNT; v_ecrm_rows := v_ecrm_rows + n;
    END IF;
  END LOOP;
...
GRANT EXECUTE ON FUNCTION public.rp_backfill_queue(integer, integer) TO authenticated;
