...
-- Architectural fix: source assessment cells (v_na, v_nua, v_da) from
-- v_hiring_candidates view so RPC assessment_score locks to view.assessment_composite,
-- matching the Matrix Assessment ROW cells (already reading view.assessment_* per step 4).
-- Nature falls back to best_fit_role's OS when assessment_target_role IS NULL (35 of 60
-- candidates). v_nua and v_da are role-invariant on the view. For _by_role, per-role
-- OS drives v_na; v_nua/v_da role-invariant.
...
'Per-role variant. Each role loop uses role OS as v_na. v_nua and v_da role-invariant from view. All inputs on 0-100 scale. Interview cells from iv_*.';
