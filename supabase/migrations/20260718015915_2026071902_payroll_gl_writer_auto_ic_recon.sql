-- Extend payroll_gl_writer to auto-post PN-side intercompany reconciliation leg.
-- After each -AGENCY JE (agency owes PN), post a -PAPERNEWT-IC-RECON JE on PN
-- (Dr COA-IC-002 "Due from Story Agency" / Cr COA-PN-002 "Payroll Cash — unmapped")
-- so intercompany balances cross-entity from the writer forward.
-- Signature unchanged (uuid, boolean, date). No new overload; 2-arg recipe wrapper unaffected.
  ...
  LOOP
    ...
    LOOP
      ...
      ELSE
        ...
        v_variable := v_commission + v_other;
    ...
    -- Agency-side JE: Dr Growth/Team/Reimb, Cr IC-001 (Due to PaperNewt)
    IF v_ic_credit > 0 THEN
      ...
      IF v_team_total > 0 THEN
        ...
        VALUES (v_agency_je, p_agency_id, v_team_acct, v_team_total, 0,
                'Team Budget: post-ramp fixed pay + commission/variable + ER taxes + unclassified legacy items', v_agency_entity);
      END IF;
    ...
    -- PaperNewt-direct JE (Owner/admin/PN-entity people): Dr COA-PN-001, Cr COA-PN-002
    ...
    -- PaperNewt-side intercompany reconciliation for the agency-split portion.
    -- Mirrors the -AGENCY JE's Cr to IC-001 on the PN books: Dr IC-002 / Cr PN-002.
    -- Economic reality: PaperNewt wrote one check covering the whole payroll (both
    -- Owner-direct and passed-through-to-team); this leg records the pass-through
    -- cash-out as an intercompany receivable so agency's IC-001 payable balances.
    IF v_ic_credit > 0 THEN
      INSERT INTO journal_entries (
        agency_id, entry_date, description, source, reference_number,
        classification_status, created_at, business_entity_id
