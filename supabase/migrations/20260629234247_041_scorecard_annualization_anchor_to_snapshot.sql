-- Fix: compute_scorecard_bonus pace_factor was using p_as_of_date for annualization
-- even when YTD inputs were from a stale agency_snapshot. Stretching N-day data
-- across M>N calendar days understated pace.
--
...
BEGIN
  ...
  -- Manual override row for the target week
  SELECT
  ...
  -- Effective as-of date: anchor annualization to the freshest data we actually have.
  -- When overrides exist for this week, override-week date is most current. Otherwise
  -- the snapshot date is the data anchor. This prevents understating pace when a stale
  -- snapshot is being stretched across more calendar days.
