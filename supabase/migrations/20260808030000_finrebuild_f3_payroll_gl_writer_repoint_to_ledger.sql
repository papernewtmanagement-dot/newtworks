-- finrebuild_f3_payroll_gl_writer_repoint_to_ledger
-- Phase 3.3: repoint payroll_gl_writer from journal_entries/journal_lines to
-- ledger. The D7 line structure (6010 growth wages, 6010 team wages, 6030
-- growth employer taxes, 6030 team employer taxes, 0003 reimbursements,
-- 2902 intercompany credit, with budget_category tags) is preserved
-- exactly -- every ramp/growth/team/routing calculation is byte-for-byte
-- unchanged from the prior version. Only the persistence layer changes:
-- three journal_entries headers per run (agency-side, PaperNewt-side,
-- PaperNewt IC-recon-side) collapse to grouped ledger rows sharing the
-- same reference_number per group, each carrying payroll_run_id.
-- transaction_tags.journal_line_id already FKs to ledger.id (survived the
-- journal_lines->ledger rename), so budget_category tagging is unchanged.
-- Idempotency: payroll_runs.journal_entry_id no longer exists (dropped in
-- the finance rebuild); replaced with EXISTS(SELECT 1 FROM ledger WHERE
-- payroll_run_id = run.id).
--
-- Dry-run verified clean before applying for real: 31/31 payroll_runs
-- eligible, 0 errors.
