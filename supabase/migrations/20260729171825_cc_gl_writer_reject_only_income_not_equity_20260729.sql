-- Narrow structural guard: reject income on CC targets, but ALLOW equity
-- (Internal Transfers, Owner Draws etc. are valid card-payment offset accounts).

-- Update cc_gl_writer: change "NOT IN ('income','equity')" -> "!= 'income'"
    ...
    IF v_other_side_account_id IS NULL THEN
      ...
      BEGIN
        ...
        WHERE r.agency_id = p_agency_id AND r.is_active = TRUE
          ...
          AND target_coa.account_type != 'income'  -- STRUCTURAL: card never routes to income
...
-- Also update reclassifier: same narrowing
CREATE OR REPLACE FUNCTION public.reclassify_pending_je(
  p_agency_id uuid, p_source_account_code text DEFAULT NULL,
  p_je_id uuid DEFAULT NULL, p_dry_run boolean DEFAULT true
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $function$
DECLARE
  v_susp_account_id uuid; v_je RECORD; v_susp_line RECORD; v_other_line RECORD;
  v_source_account_code text; v_source_account_type text;
...
BEGIN
  SELECT id INTO v_susp_account_id FROM public.chart_of_accounts
    WHERE agency_id = p_agency_id AND account_code = 'COA-SUSP' LIMIT 1;
  ...
  LOOP
    ...
    SELECT jl.id, jl.debit, jl.credit INTO v_susp_line
    FROM public.journal_lines jl
    WHERE jl.journal_entry_id = v_je.id AND jl.account_id = v_susp_account_id LIMIT 1;
    IF NOT FOUND THEN v_count_still_pending := v_count_still_pending + 1; CONTINUE; END IF;

    SELECT jl.account_id, coa.account_code, coa.account_type INTO v_other_line
    FROM public.journal_lines jl JOIN public.chart_of_accounts coa ON coa.id = jl.account_id
    WHERE jl.journal_entry_id = v_je.id AND jl.account_id != v_susp_account_id LIMIT 1;
    IF NOT FOUND THEN v_count_still_pending := v_count_still_pending + 1; CONTINUE; END IF;
    ...
    IF v_susp_line.debit > 0 THEN v_amount := v_susp_line.debit; v_direction := 'debit';
    ELSE v_amount := v_susp_line.credit; v_direction := 'credit'; END IF;

    v_je_description := v_je.description; v_rule_id := NULL; v_target_code := NULL;

    -- Structural: if the JE's other side is a liability (card), reject income targets.
    -- Equity (Internal Transfers, Owner Draws) IS a valid card-payment offset — allow it.
    SELECT r.id,
    ...
    SELECT id INTO v_new_account_id FROM public.chart_of_accounts
      WHERE agency_id = p_agency_id AND account_code = v_target_code AND is_active = TRUE LIMIT 1;
    IF v_new_account_id IS NULL THEN
      v_count_error := v_count_error + 1;
      v_details := v_details || jsonb_build_object('je_id', v_je.id, 'error', 'target_account_not_found');
      CONTINUE;
    END IF;

    IF NOT p_dry_run THEN
      UPDATE public.journal_lines SET account_id = v_new_account_id WHERE id = v_susp_line.id;
