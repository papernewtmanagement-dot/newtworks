-- Tighten Telegram team check-in message copy per Peter's 2026-07-09 spec:
--   • "Morning meeting in 5 minutes" → "Kickoff in 5"
--   • "Last EOD (date)" → "EOD date"
--   • individual "(carried from date type)" → "(Type date)"
--   • drop "n of N reporting" from team total line
--   • "Win the Week — Week n of 13 (ends Sat date)" → "Week n date"
--   • drop "✅ cleared" annotation when a goal is hit
--   • "of" → "/" everywhere it read as a fraction
--   • trim phones block: header + drop "of X ringing" → "(X ringing)"
--   • trim "(carryover: N from prior week)" → "(+N carry)"
...
CREATE OR REPLACE FUNCTION public.render_team_status_block(p_agency_id uuid, p_as_of_date date, p_fresh_type text, p_header_label text)
 RETURNS TABLE(block_text text, team_total_quotes numeric, team_total_sales numeric, fresh_count integer, carried_count integer, no_data_count integer, expected_count integer)
 LANGUAGE plpgsql
AS $function$
DECLARE
  v_cycle record;
...
BEGIN
  ...
  -- Expected count via canonical
  SELECT count(*) INTO v_expected
  FROM public.get_expected_teammates(p_agency_id, 'work_checkin', p_as_of_date);

  -- Line items: fresh (submitted p_as_of_date as p_fresh_type) vs carried (latest prior)
  FOR v_row IN
    WITH expected AS (
  ...
  -- Team totals: separate aggregation, no archive filter (departed contributions count silently)
...
BEGIN
  ...
  IF v_checkin_type = 'morning' THEN
    ...
    -- (typically yesterday; falls back gracefully if no data present).
    v_calls_block := public.render_daily_calls_block(p_agency_id, v_today - 1);
    IF v_calls_block IS NULL OR v_calls_block = '' THEN
      -- Try one day earlier in case the report hasn't arrived yet
...
BEGIN
  -- Team members with activity (excludes Not Applicable bucket)
  FOR v_row IN
    SELECT
      t.first_name,
  ...
  -- Not Applicable bucket (missed at agency level)
  SELECT
  ...
  IF v_na_abandoned + v_na_voicemail > 0 THEN
    v_out := v_out
      || format(
        ...
        v_na_abandoned,
        v_na_voicemail,
        CASE WHEN v_na_in > 0 THEN format(' (%s ringing)', v_na_in) ELSE '' END
      );
  END IF;
