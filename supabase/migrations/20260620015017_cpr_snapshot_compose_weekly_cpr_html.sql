...
BEGIN
  ...
  -- SNAPSHOT v_team_size: currently-active agency members + any archived who have
  -- a detail row for this report (they were on the team during the week).
  ...
  WHERE t.agency_id = p_agency_id
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE dd.weekly_cpr_report_id = v_report.id
  ...
  -- REQUIREMENTS: snapshot policy from get_weekly_cpr_requirements + Modified column
  SELECT string_agg(
    '<tr>'
  ...
  -- HOURS: snapshot policy for team filter
  WITH h_pivot AS (
    SELECT
      team_member_id,
  ...
  WHERE t.agency_id = p_agency_id AND t.category = 'agency'
    ...
    AND (
      (t.is_active = true AND t.archived_at IS NULL)
      OR EXISTS (SELECT 1 FROM public.weekly_cpr_team_detail dd WHERE dd.weekly_cpr_report_id = v_report.id AND dd.team_member_id = t.id)
    );

  -- ACTIVITY: snapshot policy for team filter
  SELECT string_agg(
    '<tr>'
    || '<td style="padding:6px 10px;border-bottom:1px solid #e5e7eb;font-weight:600;color:#1e293b">' || COALESCE(NULLIF(t.nickname,''), t.first_name) || '</td>'
  ...
  -- PAYROLL: snapshot policy for team filter
  SELECT string_agg(
    '<tr>'
    || '<td style="padding:6px 10px;border-bottom:1px solid #e5e7eb;font-weight:600;color:#1e293b">' || COALESCE(NULLIF(t.nickname,''), t.first_name) || '</td>'
