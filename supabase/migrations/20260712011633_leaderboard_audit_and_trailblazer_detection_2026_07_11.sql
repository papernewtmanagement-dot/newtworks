...
CREATE TABLE IF NOT EXISTS public.all_star_crossings (
  id               uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  agency_id        uuid NOT NULL,
  team_member_id   uuid NOT NULL,
  category         text NOT NULL,     -- quarter_sp | week_sp | week_quotes
  week_ending      date NOT NULL,     -- for quarter_sp this is quarter-end Saturday
  value_at_crossing numeric NOT NULL,
  floor_at_crossing numeric NOT NULL,
  created_at       timestamptz NOT NULL DEFAULT now(),
  UNIQUE (agency_id, team_member_id, category, week_ending)
);

CREATE INDEX IF NOT EXISTS idx_all_star_crossings_agency_cat
  ON public.all_star_crossings (agency_id, category, week_ending DESC);

ALTER TABLE public.all_star_crossings ENABLE ROW LEVEL SECURITY;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='all_star_crossings' AND policyname='asc_read') THEN
    CREATE POLICY asc_read ON public.all_star_crossings FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='all_star_crossings' AND policyname='asc_write_service') THEN
    CREATE POLICY asc_write_service ON public.all_star_crossings FOR ALL USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');
  END IF;
END $$;

-- =============================================================
-- Audit function: called weekly (write_weekly_comp_v2) + at quarter close
-- =============================================================
CREATE OR REPLACE FUNCTION public.audit_weekly_leaderboard_crossings(
  p_agency_id uuid,
...
DECLARE
  ...
  period_lbl           text;
BEGIN
  -- Determine if this week_end is also quarter-end (for quarter_sp category)
  ...
  -- Get weekly_cpr_reports id for this week
  ...
  -- ── Loop each category (skip quarter_sp unless quarter close) ───
  FOR cfg IN
  ...
  LOOP
    ...
    -- Pull current bronze + gold record values for this category
    ...
    -- Compute floor + trailblazer threshold using same math as frontend
    ...
    -- Iterate active Sales-side teammates and get their value for this category
    FOR r IN
      SELECT
        t.id AS team_member_id,
        t.first_name,
        CASE cfg.category
          WHEN 'week_quotes' THEN COALESCE(d.quotes_modified, 0)::numeric
          WHEN 'week_sp'     THEN COALESCE(d.sales_points, 0)::numeric
    ...
    LOOP
      ...
      -- Period label: week ending date for weekly cats, "Q<n> YYYY" for quarter_sp
      IF cfg.category = 'quarter_sp' THEN
        period_lbl := 'Q' || EXTRACT(quarter FROM v_cycle_end)::text || ' ' || EXTRACT(year FROM v_cycle_end)::text;
      ELSE
        period_lbl := to_char(p_week_end_date, 'YYYY-MM-DD');
      END IF;

      -- 1. All-star crossing (idempotent via unique key)
      IF crossed THEN
        WITH ins AS (
        ...
        -- If a new event was recorded, increment all_star_counts
        IF EXISTS (
      ...
      -- 2. Trailblazer threshold crossing (idempotent)
      ...
      -- 3. leaderboard bump — if value beats current bronze, insert + demote
      IF r.the_value > COALESCE(bronze_val, 0) THEN
        -- Avoid duplicate leaderboard entry for same (person, period)
        IF NOT EXISTS (
          ...
          -- Assemble new top-3: existing + candidate, sort desc by value, take top 3
          WITH combined AS (
          ...
          -- Wipe + rewrite this category's leaderboard (3 rows)
          , wiped AS (
...
GRANT EXECUTE ON FUNCTION public.audit_weekly_leaderboard_crossings(uuid, date) TO service_role, authenticated;
