-- send_weekly_cpr_recap: extend polling to 90s + add late-landing recovery sweep
-- Root cause (2026-06-20): Composio Gmail returned at 30.6s; polling loop bailed at 30.0s
-- → false-negative timeout error → Telegram alert despite successful send.
-- Fix: lift timeout_milliseconds to 90000 and v_max_attempts to 180 (90s poll ceiling),
-- and if the loop still exits without a response, do a final 15-second recovery sleep
-- and re-check net._http_response before declaring timeout. Treat a late 2xx as success.
...
BEGIN
  ...
  IF v_response.status_code IS NULL THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Composio request timed out after ~105s (90s poll + 15s recovery). Email may still send. Check net._http_response id=' || v_request_id::text || ' before retrying.',
      'request_id', v_request_id
    );
  END IF;
