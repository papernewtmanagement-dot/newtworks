-- 20260724022121_hiregauge_lss_delta_primitive.sql
-- Pure math primitive for per-competency LSS delta calculation.
-- Takes candidate row + weights jsonb + optional thresholds override.
-- Returns jsonb {delta, path, signals|acc_flags/spd_flags, lin_signal}.
-- NOT a shared alter-the-competency helper — each competency owns its own
-- weights and interpretation; this only reads inputs and returns math.
-- NOTE: The initial v1 body carried an aggregate fast-when-wrong flip that
-- was corrected in 20260724022557 to skip when speed weight <= 0. This file
-- records the final (v1.1) body so 20260724022557 is a no-op replacement,
