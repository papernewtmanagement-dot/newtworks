-- Rewrite compose_weekly_cpr_html: replace the per-teammate wrap-up loop with a
-- single rendered block reading weekly_cpr_reports.wrapup_summary_text. Same
-- position (between opener and WEEKLY PAY). Same escape + newline-to-br pattern
-- as opener/looking-ahead. Everything else in the function unchanged.
