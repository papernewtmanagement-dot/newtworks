...
-- This migration was applied live on 2026-07-12 06:57:57 UTC (ledger name: residual_pool_quarterly_actuals_carveouts_outside) but was never
-- committed to this repo at the time. Content below is copied verbatim
-- from supabase_migrations.schema_migrations.statements for version 20260712065757.
...
BEGIN
  ...
  per_person_qtd_base AS (
    SELECT tm_id, SUM(week_base_paid) AS qtd_base_paid
  ...
  combined AS (
    SELECT b.id AS tm_id, b.first_name, b.last_name,
      ...
      COALESCE(ppb.qtd_base_paid,   0) AS c_qtd_base,
  ...
  team_totals AS (
    SELECT
      SUM(c.c_qtd_base)   AS qtd_base_total,
  ...
  pool_split AS (
    SELECT pc.*,
      ...
      (pc.qtd_base_total + pc.qtd_actual_commission + pc.qtd_bonus_pool) * v_burden_multiplier
                                                                             AS qtd_burden
    FROM pool_calc pc
  ),
  distributed AS (
  ...
  SELECT
    ...
    jsonb_build_object(
      ...
      'design_note',
        'Quarterly envelope accreted from pool schedule ramp. Subtractions are QTD actuals '
        || '(payroll_detail SALARY+REGULAR w/ configured-rate fallback; health weekly × weeks). '
        || 'Carveouts moved OUTSIDE pool (still agency-funded, no longer suppress bonus).',
      ...
      'qtd_subtractions', jsonb_build_object(
        'qtd_actual_base',              ROUND(s.qtd_base_total, 2),
        'qtd_actual_base_source',
        ...
        'qtd_burden_note',
          '0.08 × (base + commission + bonus); implicit in envelope math (envelope − health − wc)/1.08 − base'
      ),
      ...
      'person_qtd', jsonb_build_object(
        'qtd_actual_base',                ROUND(s.c_qtd_base, 2),
