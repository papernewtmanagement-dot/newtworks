-- Adds one new purpose, agency_calendar_invite. Identical to agency_active_all
-- except it also includes someone who has not started yet but whose start date
-- falls in the next seven days, so a new teammate is on the Daily Kickoff
-- invite before their first morning rather than on it (Peter, 2026-09-18).
-- Deliberately a separate purpose: agency_active_all also drives who gets the
-- weekly CPR email, and a hire who has not started should not be sent that.
-- Every other purpose is byte-for-byte unchanged.
  ...
  WHERE r.is_test_user IS NOT TRUE
    ...
    AND (
      ...
      (p_purpose = 'agency_active_all'
        ...
        AND r.is_active = true)
      OR
      -- Calendar invites only. Everyone on agency_active_all, plus a teammate
      -- whose start date is today or inside the next seven days, so they are on
      -- the invite before their first morning.
      (p_purpose = 'agency_calendar_invite'
        AND r.category = 'agency'
        AND (r.is_active = true
             OR (r.start_date IS NOT NULL
                 AND r.start_date >= v_today_ct
