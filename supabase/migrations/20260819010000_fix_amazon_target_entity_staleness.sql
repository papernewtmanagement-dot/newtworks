-- Root cause: amazon_apply_charge_categories() picks the target entity from
-- amazon_orders.target_business_entity_id, which is set once at capture time and
-- never updated when a ledger line gets reclassified to a different entity later
-- (like the Personal -> PaperNewt moves earlier today). That let it silently move
-- 88 lines back to Personal (Kids/Medical/Clothing/Groceries), undoing the fix.

-- 1. Update the stale field for the two cards just reclassified, so it matches reality.
UPDATE amazon_orders o
SET target_business_entity_id = 'b1111111-1111-1111-1111-111111111111'
WHERE o.payment_card_last4 IN ('7435','3208')
  AND o.target_business_entity_id <> 'b1111111-1111-1111-1111-111111111111';

-- 2. Make the view pull entity from the ledger row's CURRENT account, not the
--    stale amazon_orders field, so this class of bug can't recur for any future
--    entity reassignment. This is the authoritative source of truth going forward.
-- (Also includes the earlier same-day fix: nearest-date resolution for "ambiguous"
--  matches, so charges that share a dollar amount with a recurring subscription
--  aren't flagged ambiguous just because of the coincidence.)
...
WITH amz_ledger AS (
  SELECT l.id AS ledger_id, l.agency_id, l.entry_date, l.description,
    ...
    coa.business_entity_id AS current_entity_id,
...
order_shape AS (
  SELECT o.order_id, o.agency_id, o.order_date::date AS order_date, o.grand_total,
    o.payment_card_last4, o.matched_ledger_id,
    count(*) AS item_count,
  ...
  GROUP BY o.order_id, o.agency_id, o.order_date, o.grand_total, o.payment_card_last4, o.matched_ledger_id
),
paired AS (
  SELECT al.ledger_id, al.agency_id, al.entry_date, al.description, al.charge_amount, al.source,
    ...
    os.order_id, os.order_date, os.grand_total, al.current_entity_id AS target_business_entity_id, os.item_count,
