-- 6) compose_weekly_cpr_html — 4 places filter team. Each gets is_admin_backoffice = false appended.
...
BEGIN
  ...
  -- 2026-07-01: exclude is_admin_backoffice from team size
  ...
  WHERE t.agency_id = p_agency_id AND t.category = 'agency'
    AND COALESCE(t.role_level,'') <> 'Owner'
    AND t.is_admin_backoffice = false  -- 2026-07-01
    AND (t.archived_at IS NULL OR t.archived_at > v_week_start::timestamptz);
