-- Refines the guard added in pay_projection_admin_guard_and_year_one_path.
-- The strict is_agency_admin() check also blocked service-side callers
-- (maintenance connections and the service key), which have no app user row
-- and so can never be "admin" — that broke reseed_pay_scale from the
-- toolchain. Guard now blocks browser callers only: a request arriving with
-- the anon or authenticated role must be an owner/manager, while a
-- service-role or direct database connection passes. Staff and the anon key
-- are still shut out of the pay ladder, which is the point of the check.
