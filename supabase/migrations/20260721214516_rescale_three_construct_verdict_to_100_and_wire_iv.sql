-- Step 5: rescale hiregauge_three_construct_verdict + _by_role to 0-100 internally
-- and wire interview construct inputs (v_ni/v_nui/v_di) from v_hiring_candidates.iv_*.
--
-- Context (see session_note "2026-07-21 — Interview layer rewire steps 1-4 shipped + UI polish"):
--   Steps 1-4 already shipped view rebuild (iv_* computed from interview_answers on 0-100),
--   Priscilla migration, expander UI, matrix Interview row swap to view directly.
--   RPC was last-mile: still derived interview inputs from scorecard rp_/char_/mot_ cols
--   (all NULL for Priscilla) → interview inputs missing from construct rollups.
--
-- This migration:
--   1. Wires v_ni <- v_ta.iv_nature, v_nui <- v_ta.iv_nurture, v_di <- v_ta.iv_drivers
--      (values already on 0-100, no scaling needed).
--   2. Deletes stale rp_/char_/mot_ interview scoring paths.
--   3. Rescales entire RPC internal math to 0-100 for consistency with view scale.
--      Previously the resume layer (v_nr/v_nur/v_dr) already delivered 0-100 (from the
--      2026-07-19 res-aggregates-to-view refactor) while the rest of the math still
--      assumed 0-10 → produced garbage composite for anyone with resume data (Priscilla
--      score_0_10 was 22.46 → wrongly said "hire").
--   4. Character floor logic unchanged — still reads raw char_* cols. For candidates
--      whose interview was scored via answers (like Priscilla), char_* are NULL and
--      floor status is 'not_scored'. iv_verdict='decline' from structural_mismatch
--      override is honored by frontend on the Interview cell (step 4), not here.
--
-- Verdict thresholds rescaled proportionally:
--   framework: 7.5→75 (hire), 6.0→60 (consider)
--   resume layer: 7.0→70 (pass), 5.0→50 (consider)
--   other layers: 7.5→75 (pass), 6.0→60 (consider)
--   score_hire_at_70/75/80: pass 70/75/80, consider 55/60/65
--
-- Return column name score_0_10 preserved to avoid frontend rename cascade — value is
-- now on 0-100 scale. Frontend updated in same commit.
--
-- Consumer audit (grepped repo before ship):
--   - hiregauge_three_construct_verdict: sole caller = src/components/CandidateDetail.jsx
--     (Results matrix + composite footer). Matrix Interview and Assessment cells + layer
--     total column read view directly (step 4), so RPC interview_score/assessment_score
--     are dead paths for the matrix — still returned in output shape for consistency.
--     Frontend updated in same commit to: strip "/ 10" suffix, round to int display,
--     rescale color thresholds to 75/60, rescale @7.0/7.5/8.0 labels to @70/75/80,
--     add reference to is100 layer list.
--   - hiregauge_three_construct_verdict_by_role: zero frontend callers. Same rewire
--     applied for future consistency.
...
DECLARE
  ...
  -- All matrix inputs on 0-100 scale (per 2026-07-21 rescale).
...
BEGIN
  ...
  -- Resume layer inputs — res_nature/nurture/drivers already on 0-100 (view). resume_quality
  -- fallback is smallint 1-10 → scale × 10.
  ...
  -- Assessment layer (nature construct) — build on 0-10 then × 10 for 0-100 output.
  IF v_ta.deadline_motivation IS NOT NULL THEN
    ...
    SELECT AVG((val)::numeric) INTO v_sales_comp_avg
      FROM jsonb_each_text(v_role_comp_json) e(key, val) WHERE LEFT(e.key, 1) <> '_';

    v_lss_acc := COALESCE(v_ta.lss_total_accuracy, 0);
    v_lss_score := LEAST(10.0, GREATEST(0.0, v_lss_acc / 3.5));

    v_na := 10 * (COALESCE(v_best_fit_os / 10.0, 0) * v_asub_os_w
                + COALESCE(v_sales_comp_avg / 10.0, 0) * v_asub_comp_w
                + v_lss_score * v_asub_lss_w);

    -- Drivers assessment cell — cts_drivers_assessment_cell returns 0-10 → × 10.
    v_da := 10 * public.cts_drivers_assessment_cell(p_assessment_id, v_best_fit_role);

    v_dims_scored := v_dims_scored + 1;
  END IF;

  -- Assessment layer (nurture construct) — reliability × distortion rules × 10.
  IF v_ta.reliability IS NOT NULL THEN
    v_nua := CASE
      WHEN v_ta.reliability IN ('high','very_high') AND v_ta.response_distortion = 'low' THEN 70
      WHEN v_ta.reliability = 'moderate' AND v_ta.response_distortion IN ('low','moderate') THEN 50
      WHEN v_ta.response_distortion = 'high' THEN 20
      ELSE 40
    END;
    v_dims_scored := v_dims_scored + 1;
  END IF;

  -- Interview layer — sourced from v_hiring_candidates.iv_* (0-100, from interview_answers
  -- via view). Replaces prior rp_/char_/mot_ scorecard derivation. Character floor still
  -- reads raw char_* cols below for candidates scored via scorecard path.
  ...
  -- Character floor — raw char_* cols (1-10). Threshold < 7 unchanged.
  -- For candidates scored via interview_answers (like Priscilla), char_* are NULL and
  -- floor status is 'not_scored'; iv_verdict handles structural override on frontend.
  ...
  -- Reference layer — ref_* smallint 1-10 → × 10 for 0-100 parity.
  ...
  -- Construct rollups — weighted avg across 4 layers per construct.
  ...
  -- Layer rollups — per-layer row totals via layer-weighted math.
  DECLARE v_wsum numeric; v_sum numeric;
  BEGIN
    v_wsum := 0; v_sum := 0;
  ...
  -- Overall composite — construct-weighted rollup. Now on 0-100 scale.
  ...
  -- Layer verdicts — 0-100 thresholds.
  resume_verdict := CASE
  ...
  -- Framework overall verdict — 0-100 thresholds.
  verdict := CASE
    WHEN character_floor_status = 'floor_failed' THEN 'decline_character'
  ...
  -- Threshold previews — 0-100 scale (70/75/80 pass, 55/60/65 consider).
...
'Suggs 4-layer × 3-construct verdict framework. All matrix inputs on 0-100 scale (rescaled 2026-07-21 from mixed 0-10/0-100). Interview inputs sourced from v_hiring_candidates.iv_nature/iv_nurture/iv_drivers (computed from interview_answers). Framework verdict thresholds: hire >=75, consider >=60. Layer verdict thresholds: resume pass >=70 / consider >=50, other layers pass >=75 / consider >=60.';
...
BEGIN
  ...
  -- Resume layer inputs — 0-100 (view) with 1-10 fallback × 10.
  IF v_ta.res_nature IS NOT NULL THEN v_nr := v_ta.res_nature::numeric; v_dims_scored := v_dims_scored + 1; END IF;
  IF v_ta.res_nurture IS NOT NULL THEN v_nur := v_ta.res_nurture::numeric; END IF;
  IF v_ta.res_drivers IS NOT NULL THEN v_dr := v_ta.res_drivers::numeric; END IF;
  IF v_nr IS NULL AND v_ta.resume_quality IS NOT NULL THEN
  ...
  -- Assessment nurture (reliability × distortion) × 10 — role-invariant, computed once.
  IF v_ta.reliability IS NOT NULL THEN
    v_nua := CASE
      WHEN v_ta.reliability IN ('high','very_high') AND v_ta.response_distortion = 'low' THEN 70
      WHEN v_ta.reliability = 'moderate' AND v_ta.response_distortion IN ('low','moderate') THEN 50
  ...
  -- Interview layer — sourced from v_hiring_candidates.iv_* (0-100).
  IF v_ta.iv_nature IS NOT NULL THEN v_ni := v_ta.iv_nature::numeric; v_dims_scored := v_dims_scored + 1; END IF;
  IF v_ta.iv_nurture IS NOT NULL THEN v_nui := v_ta.iv_nurture::numeric; v_dims_scored := v_dims_scored + 1; END IF;
  IF v_ta.iv_drivers IS NOT NULL THEN v_di := v_ta.iv_drivers::numeric; v_dims_scored := v_dims_scored + 1; END IF;

  -- Character floor — raw char_* cols unchanged.
  IF v_ta.char_honesty IS NOT NULL AND v_ta.char_honesty < 7 THEN v_char_floors_failed := array_append(v_char_floors_failed, 'char_honesty'); END IF;
  IF v_ta.char_hwe     IS NOT NULL AND v_ta.char_hwe     < 7 THEN v_char_floors_failed := array_append(v_char_floors_failed, 'char_hwe'); END IF;
  IF v_ta.char_persres IS NOT NULL AND v_ta.char_persres < 7 THEN v_char_floors_failed := array_append(v_char_floors_failed, 'char_persres'); END IF;
  IF v_ta.char_concern IS NOT NULL AND v_ta.char_concern < 7 THEN v_char_floors_failed := array_append(v_char_floors_failed, 'char_concern'); END IF;
  ...
  -- Reference layer — smallint 1-10 × 10.
  IF v_ta.ref_nature IS NOT NULL THEN v_nref := 10 * v_ta.ref_nature::numeric; v_dims_scored := v_dims_scored + 1; END IF;
  IF v_ta.ref_nurture IS NOT NULL THEN v_nuref := 10 * v_ta.ref_nurture::numeric; v_dims_scored := v_dims_scored + 1; END IF;
  IF v_ta.ref_drivers IS NOT NULL THEN v_dref := 10 * v_ta.ref_drivers::numeric; v_dims_scored := v_dims_scored + 1; END IF;
  ...
  LOOP
    IF v_ta.deadline_motivation IS NOT NULL THEN
      ...
      v_na := 10 * (COALESCE(v_role_os::numeric / 10.0, 0) * v_asub_os_w
                  + COALESCE(v_role_comp_avg / 10.0, 0) * v_asub_comp_w
                  + v_lss_score * v_asub_lss_w);

      v_da := 10 * public.cts_drivers_assessment_cell(p_assessment_id, v_role);
    ELSE
      v_na := NULL;
      v_da := NULL;
    END IF;

    -- Construct rollups per role.
    v_wsum := 0; v_sum := 0;
    IF v_nr   IS NOT NULL THEN v_sum := v_sum + v_nr   * v_nature_r_w;   v_wsum := v_wsum + v_nature_r_w;   END IF;
    IF v_na   IS NOT NULL THEN v_sum := v_sum + v_na   * v_nature_a_w;   v_wsum := v_wsum + v_nature_a_w;   END IF;
    IF v_ni   IS NOT NULL THEN v_sum := v_sum + v_ni   * v_nature_i_w;   v_wsum := v_wsum + v_nature_i_w;   END IF;
    ...
    -- Overall composite per role — 0-100.
    v_wsum := 0; v_sum := 0;
...
'Per-role variant of hiregauge_three_construct_verdict — emits one row per role. All matrix inputs on 0-100 scale (rescaled 2026-07-21). Interview inputs from v_hiring_candidates.iv_*. No current frontend consumers.';
