-- =========================================================================
-- Migration: step1b_bank_gl_writer_entity_scoped_suspense
-- Applied: 2026-07-31 via Supabase MCP (session: pipeline_repair_step1)
-- =========================================================================
-- Rewrite bank_gl_writer to use entity-scoped suspense via
-- get_entity_unclassified_account and prefer same-entity account lookups on
-- rule targets. Removes reference to legacy COA-SUSP account_code.
-- =========================================================================
...
BEGIN
  ...
  LOOP
    ...
    v_bank_source_code := NULL;
    v_bank_entity_id := NULL;
    ...
    -- Resolve the bank's source code AND its business entity in one lookup
    SELECT account_code, business_entity_id
      INTO v_bank_source_code, v_bank_entity_id
    ...
    IF v_bank_entity_id IS NULL THEN
      ...
      v_errors := v_errors || jsonb_build_object(
        'txn_id', v_txn_id, 'reason', 'bank_account_missing_business_entity',
    ...
    -- --- Classification tier 1: direct category match (entity-scoped) ---
    IF v_category IS NOT NULL AND length(trim(v_category)) > 0 THEN
      SELECT id INTO v_other_side_account_id
        ...
        WHERE agency_id = p_agency_id
          ...
          AND is_active = TRUE
          AND (business_entity_id = v_bank_entity_id OR business_entity_id IS NULL)
        ORDER BY (business_entity_id = v_bank_entity_id) DESC NULLS LAST
        LIMIT 1;
    ...
    -- --- Classification tier 2: split label match (entity-scoped) ---
    ...
    -- --- Classification tier 3: rule match ---
    IF v_other_side_account_id IS NULL THEN
      ...
      BEGIN
        ...
        WHERE r.agency_id = p_agency_id
          ...
          AND (r.match_source_account IS NULL OR r.match_source_account = v_bank_source_code)
        ...
        IF v_rule_id IS NOT NULL AND v_target_code IS NOT NULL AND v_target_code != '__SOURCE__' THEN
          -- Prefer same-entity account when the target code exists on multiple entities
          SELECT id INTO v_other_side_account_id
            FROM chart_of_accounts
            WHERE agency_id = p_agency_id
          ...
          -- If no same-entity match, allow ANY entity's copy as fallback
          IF v_other_side_account_id IS NULL THEN
            SELECT id INTO v_other_side_account_id
              FROM chart_of_accounts
              WHERE agency_id = p_agency_id
    ...
    -- --- Fallback: suspense (entity-scoped) ---
    IF v_other_side_account_id IS NULL THEN
      -- For a bank txn: positive amount = money in (income direction), negative = expense direction
      v_suspense_account_id := get_entity_unclassified_account(
        p_agency_id, v_bank_entity_id,
      ...
      IF v_suspense_account_id IS NULL THEN
        ...
        v_errors := v_errors || jsonb_build_object(
          ...
          'business_entity_id', v_bank_entity_id
        );
        CONTINUE;
      END IF;
    ...
    BEGIN
      ...
      IF p_dry_run THEN
        v_posted_runs := v_posted_runs || jsonb_build_object(
          'txn_id', v_txn_id, 'date', v_txn_date, 'amount', v_amount,
  ...
  RETURN jsonb_build_object(
    ...
    'posted_runs', v_posted_runs, 'error_details', v_errors
  );
END;
$function$;
