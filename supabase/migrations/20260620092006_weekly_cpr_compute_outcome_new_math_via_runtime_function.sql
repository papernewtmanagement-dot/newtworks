-- Saturday writer rewrite. All team-level numbers now flow from
-- get_weekly_cpr_requirements (the runtime function that walks the cycle from
-- cycle_start with Modified inside Total). This guarantees the writer's stored
-- snapshot matches the live runtime values the page/email render.
--
-- Stored column meanings AFTER this migration:
--   quotes_owed_carryover  = Σ per-person "Last Wk" (team's true carryover INTO this week)
--   quotes_fresh_needed    = 15*AM-Sales + 8*AM-Retention (composition baseline; informational)
--   quotes_total_net       = Σ per-person Net Quotes (= quotes_discussed − paid) — TRUE NET
--   quotes_owed_next_week  = Σ per-person "Next Wk" (team's true carryover OUT of this week)
--   won_the_week           = team_quotes_pool >= team_total_debt AND sales_points_qtd >= sp_target
--
-- The Win-the-Week comparison still uses GROSS pool vs total debt (correct).
-- Pool comes from Σ quotes_discussed in the function (same source the per-person Activity rows read).
...
BEGIN
  ...
  -- Pull team-level outcomes from the runtime CPR requirements function.
  -- This walks the carryover chain from cycle_start using the new math
  -- (Modified inside Total) so the writer's stored snapshot matches what the
  -- page and email render at view time.
  SELECT
  ...
  -- Win-the-Week: gross pool covers total debt (which already includes Modified).
  v_quotes_pass := v_team_quotes_pool >= v_team_total_debt;
  ...
  RETURN jsonb_build_object(
    ...
    'output_summary', format(
      'Week %s of 13 (ending %s): carryover=%s, debt=%s, pool=%s, paid=%s, owed_fwd=%s, net=%s, SP %s/%s, won=%s',
      v_cycle.week_of_cycle, v_week_end,
      v_team_carryover, v_team_total_debt, v_team_quotes_pool, v_team_paid,
      v_quotes_owed_next, v_team_net_quotes,
