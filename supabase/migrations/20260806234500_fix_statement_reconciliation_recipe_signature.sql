-- fix_statement_reconciliation_recipe_signature
--
...
-- expects jsonb back containing records_processed / output_summary. The
-- original zero-argument, integer-returning version could never have been
-- dispatched by the scheduler; it would have raised on every run.
--
-- Also note the companion data fix applied in the same session:
--   UPDATE automation_recipes SET composio_action = 'INTERNAL'
--   WHERE recipe_name IN ('Statement reconciliation check','Ledger Dup Candidate Pass');
-- automation-runner only takes the internal branch when composio_action = 'INTERNAL';
-- with NULL it falls through to the Composio branch and dies on a missing
-- composio_connection. 'Ledger Dup Candidate Pass' had been failing daily for
-- exactly this reason.
