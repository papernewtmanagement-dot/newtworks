...
BEGIN
  ...
  -- r.net_quotes already includes the individual WtW requirements buy-back (locked 2026-08-15,
  -- folded directly into get_weekly_cpr_requirements). Only the team-pool-funded remainder
  -- (wtw_requirements_adjustment_quotes — not attributable to one person) needs adding on top;
  -- adding wtw_quotes_restored here instead would double-count the individual portion.
  SELECT
  ...
  v_team_quotes := v_team_quotes + COALESCE(v_report.wtw_requirements_adjustment_quotes, 0);
