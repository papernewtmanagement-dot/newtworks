...
BEGIN
  ...
  -- (book_performance_goals_anon_select already exists — kept)

  -- ====== core_principles ======
  -- CRITICAL: this table governs Peter's doctrine. Anon must never write.
  ...
  -- (core_principles_select for anon+auth already exists — kept)
  ...
  -- (lsq_select_all for anon+auth already exists — kept)
  ...
  -- No replacement — RLS enforced, no policies = no client access.
  -- service_role bypasses RLS anyway (edge fns keep working).
  ...
  -- (prize_cart_anon_select already exists — kept)
  ...
  -- (sf_program_targets_anon_read already exists — kept)

  -- ====== team_assessments ======
  -- Personnel data. Anon should NEVER write, but read stays per single-tenant pattern.
  ...
  -- (staff_assessments_select already exists — kept)
  ...
  -- (behavioral_notes_select already exists — kept)
  ...
  -- (time_clock_edit_requests_anon_read + _authenticated_read stay)
  ...
  -- (time_clock_entries_anon_read + _authenticated_read stay)

  -- ====== user_preferences_history ======
  -- Peter's private drift log. Anon should have zero access. Even authenticated is agency-scoped.
