-- Hotfix: replace MIN(coa.business_entity_id) with (array_agg(DISTINCT ...))[1]
-- min(uuid) is not a native Postgres aggregate. This bug has been latent in the RPC
-- since it shipped 2026-07-18 (pg_stat_user_functions shows 0 successful calls).
-- Runs where journal_lines had mixed business_entity_ids didn't trigger it; runs where
-- all lines share one entity do. Post-Phase-4 the JEs are all single-entity, so this
-- now trips every call.
...
BEGIN
  ...
  LOOP
    ...
    ELSE
      ...
      -- Post-2026-07-30: ER taxes moved to 6030 by the writer; team_share is wages-only.
