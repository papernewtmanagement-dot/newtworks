-- LSS Step 5 kickoff: build hiregauge_lss_ceiling_penalty_v2 helper
--
-- Above-ceiling quadratic penalty multiplier for the 2d role-fit curve.
-- Symmetric architectural pair with hiregauge_lss_penalty_v2 (below-floor
-- exponential). Locked design per open_question "LSS per-subtest ideal range
-- scoring — build + rewire" section 2d: asymmetric fit curve, quadratic
-- above ceiling with coefficient 0.4.
--
-- Called only from the 7 role_fit fns (assessment_role_fit_*), never from
-- competency fns. Competency fns are floor-only per 2c comp-side curve.
