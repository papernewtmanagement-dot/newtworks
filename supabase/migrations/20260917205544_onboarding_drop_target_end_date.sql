-- Drop target_end_date from onboarding plans.
-- Peter ruling 2026-09-17: a target end date is meaningless because the length
-- of onboarding is decided as it runs. Removed from the create-plan popup and
-- the plan header in commit 4b98369e; this removes the column and the two
-- functions' parameters. Zero rows existed, so no data is lost.

DROP FUNCTION IF EXISTS public.create_onboarding_plan_from_templates(uuid, date, date, text);
DROP FUNCTION IF EXISTS public.create_onboarding_plan(uuid, uuid, text, text, text, date, date, text);
...
GRANT EXECUTE ON FUNCTION public.create_onboarding_plan(uuid, uuid, text, text, text, date, text) TO PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_onboarding_plan(uuid, uuid, text, text, text, date, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.create_onboarding_plan(uuid, uuid, text, text, text, date, text) TO service_role;
GRANT EXECUTE ON FUNCTION public.create_onboarding_plan_from_templates(uuid, date, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.create_onboarding_plan_from_templates(uuid, date, text) TO service_role;

ALTER TABLE public.team_onboarding_plans DROP COLUMN IF EXISTS target_end_date;
