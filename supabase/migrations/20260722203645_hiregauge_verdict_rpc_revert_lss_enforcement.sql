-- Revert the LSS auto-pass enforcement in the RPC. Back to the version that just
-- annotates lss_autopass in meta without overriding verdict. Data foundation
-- (structured license/language fields) needs to land first — see handoff notes.
...
-- Confirm Allan is back to consider
SELECT candidate_name, verdict FROM public.hiring_candidates hc
CROSS JOIN LATERAL public.hiregauge_three_construct_verdict(hc.id) v
WHERE hc.candidate_name='Allan Piedrabuena'
