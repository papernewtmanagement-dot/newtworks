-- hiregauge_three_construct_verdict — per-layer row totals now use LAYER-WEIGHTED math
-- (each row weights the three constructs by that row's own normalized contribution profile).
-- Resume row verdict uses 7.0/5.0 thresholds (matches resume rubric config); other layers
-- keep 7.5/6.0. Character floor override on Interview row unchanged. Overall framework
-- score still uses Suggs construct weights 35/30/35 — that math is unchanged.
...
DECLARE
  ...
  -- Column-side weights: how each layer contributes WITHIN a construct (unchanged)
  ...
  -- Row-side weights: each layer weights its three cells by its normalized contribution profile.
  -- Resume 5/10/10 -> 20/40/40 · Assessment 75/15/25 -> 65/13/22 · Interview 15/45/45 -> 14/43/43
  -- · Reference 5/30/20 -> 9/55/36.
...
BEGIN
  ...
  -- ASSESSMENT layer, Nature + Drivers cells
  ...
  -- INTERVIEW layer, Nature cell (role-play performance)
  ...
  -- INTERVIEW layer, Nurture cell (character floors)
  ...
  -- INTERVIEW layer, Drivers cell (motivation)
  ...
  -- Per-construct subtotals (column-side, unchanged mechanics; feed the overall framework score)
  ...
  -- Per-layer row totals — LAYER-WEIGHTED math per Option B.
  -- Each row weights its three cells by that row's normalized contribution profile.
  DECLARE v_wsum numeric; v_sum numeric;
  BEGIN
    v_wsum := 0; v_sum := 0;
  ...
  -- Overall framework score (unchanged — Suggs construct weighting)
  ...
  -- Per-layer verdicts. Resume uses 7.0/5.0 (matches resume rubric config). Other layers use 7.5/6.0.
  resume_verdict := CASE
