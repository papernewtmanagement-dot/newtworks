...
-- This migration was applied live on 2026-07-18 01:24:36 UTC (ledger name: plan_a_payroll_gl_dryrun_v1) but was never
-- committed to this repo at the time. Content below is copied verbatim
-- from supabase_migrations.schema_migrations.statements for version 20260718012436.
-- No live change is made by adding this file — it only brings the repo
-- mirror into sync with what is already running in production, so a
-- fresh `supabase db reset` from this repo reproduces live state.
-- Plan A payroll GL dry-run: shows projected per-person JE lines split by entity + growth/team
-- No CoA changes, no writes. Just returns the JE shape for review.
...
CREATE OR REPLACE FUNCTION public.payroll_gl_writer_plan_a_dryrun(
  ...
  p_pay_period_end date DEFAULT NULL  -- NULL = latest; otherwise specific
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_fixed_loaded numeric;  -- salary+hourly+ot+bonus (splittable)
  v_variable_team numeric; -- commission+other (always team)
  v_er_taxes numeric;
...
BEGIN
  ...
  ELSE
    SELECT id, pay_period_start, pay_period_end, pay_date
      INTO v_run_id, v_period_start, v_period_end, v_pay_date
    FROM payroll_runs
    WHERE agency_id = p_agency_id AND pay_period_end = p_pay_period_end
    LIMIT 1;
  END IF;

  IF v_run_id IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'error', 'no_run_found', 'pay_period_end', p_pay_period_end);
  END IF;

  -- Iterate people on this run
  FOR r IN
    SELECT pd.id AS detail_id, pd.gross_pay, pd.employer_taxes,
           pd.raw_earnings,
           pd.team_member_id,
  ...
  LOOP
    -- Extract buckets from raw_earnings.items
    ...
    -- Ramp math: same formula as v_growth_budget_ytd
    ...
    -- Route by entity
    IF r.employee_entity = v_papernewt_entity THEN
      -- Peter + Leslie: 100% PaperNewt, direct expense + cash out
      ...
      v_per_person := v_per_person || jsonb_build_object(
        ...
        'other', v_other,
        'reimb', v_reimb,
        'weeks_since_start', v_weeks_since_start,
        'growth_pct', NULL
      );
    ELSE
      -- Agency team members: growth/team split on fixed-loaded bundle
      ...
      IF v_growth_amt > 0 THEN
        v_lines_agency := v_lines_agency || jsonb_build_object(
          'person', r.first_name || ' ' || r.last_name,
          'DR_account', 'Payroll — Growth Budget',
          'DR_amount', v_growth_amt,
      ...
      IF v_team_amt > 0 THEN
        v_lines_agency := v_lines_agency || jsonb_build_object(
          'person', r.first_name || ' ' || r.last_name,
          'DR_account', 'Payroll — Team Budget',
          'DR_amount', v_team_amt,
          'note', 'team_pct=' || round((1-v_growth_pct) * 100, 1) || '% × fixed-loaded + commission $' || v_commission || ' + other $' || v_other
        );
      ...
      IF v_reimb > 0 THEN
        v_lines_agency := v_lines_agency || jsonb_build_object(
          'person', r.first_name || ' ' || r.last_name,
          'DR_account', 'Reimbursements — Pending Categorization',
          'DR_amount', v_reimb,
      ...
      -- Employer taxes: still hit agency payroll cost bucket (team side)
      IF v_er_taxes > 0 THEN
        v_lines_agency := v_lines_agency || jsonb_build_object(
          'person', r.first_name || ' ' || r.last_name,
          'DR_account', 'Payroll — Team Budget',
          'DR_amount', v_er_taxes,
          'note', 'employer taxes (FICA+FUTA+SUTA)'
        );
      ...
      v_per_person := v_per_person || jsonb_build_object(
        ...
        'growth_dollars', v_growth_amt,
        'team_dollars', v_team_amt
      );
    END IF;
  END LOOP;

  -- Add the CR intercompany line on agency side (single lump)
  IF v_dr_agency > 0 THEN
    v_lines_agency := v_lines_agency || jsonb_build_object(
      'person', '(all agency team members)',
