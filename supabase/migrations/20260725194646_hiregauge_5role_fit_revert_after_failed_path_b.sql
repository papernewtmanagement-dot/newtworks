-- Revert 5 role_fit functions to v3.5/v3.4 baseline after two failed Path B signal-integration attempts.
--
-- Attempt 1 (v3.6 with trims): 11/36 match count. Lost Jodi Sape (RR→RE) + Cheryl Hemphill (SI→RR).
--   Trims of core role-defining competencies (rapid_rapport_warm, listens_discovers_needs) hurt
--   correctly-shaped candidates most while signal additions did not compensate.
-- Attempt 2 (v3.7 no trims): 3/36 match count. Signal weights of 0.15-0.20 total added on top of
--   existing 1.00-1.12 positive weight sums pushed multiple functions above their prior levels
--   universally. RE became near-universal argmax; SI/SIB dominated broadly.
--
-- Root cause: signal weights I chose were too heavy relative to competency baselines. The SO/RS
-- pattern worked because it trimmed competencies that signals actually capture (MHA/HR/ATD/QTD
-- for hwe signal). For SI/SIB/RR/RE/ASP there is no analogous overlap — signals are additive to
-- an already-normalized weight profile.
--
-- Path B (extend signals to non-SO/RS seats) remains an open item pending different approach
-- (much smaller weight magnitudes, or per-seat targeted signals rather than universal).
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'meta', jsonb_build_object('model', 'role_fit_v3_5_realism_tune_2026_07_24_reverted_2026_07_25')
  );
END;
$function$;
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'meta', jsonb_build_object('model', 'role_fit_v3_4_realism_tune_2026_07_24_reverted_2026_07_25')
  );
END;
$function$;
