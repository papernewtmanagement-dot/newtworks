-- Step C of HireGauge accuracy audit fix sequence.
-- Bug-fix sweep across 5 _adjusted competency functions covering 8 hand-spec
-- competencies whose round-number coefficients had weak or missing rationale
-- (session note flagged cadence_compliance's -0.10*IS specifically; scan
-- surfaced systemic IS/SP over-penalization on flexibility/service competencies).
--
-- Not touched (defensible):
--   retention_watchfulness (-0.05*OP is a genuine vigilance signal)
--   composure_under_load  (-0.05*AN is a genuine "over-thinker under pressure" signal)
--
-- Base cts_*_competencies functions left with prior coefficients; only the
-- _adjusted variants feed the scoring pipeline (verdict + composite).
--
-- Changes per competency:
--   rapid_rapport_warm (inbound + reception): -0.100*AN → -0.050*AN
--   cadence_compliance (inbound):             -0.100*IS → -0.050*IS; drop +0.100*BO; drop -0.050*SP
--   cross_sell_instinct (in_book):            drop -0.050*IS
--   proactive_touch_discipline (escalation):  drop -0.050*IS; drop -0.050*SP
--   queue_throughput_discipline (support):    -0.100*SP → -0.050*SP
--   attention_to_detail (support):            drop -0.050*OP; drop -0.050*SP
--   routing_judgment (reception):             -0.100*IS → -0.050*IS; -0.100*SP → -0.050*SP
--   pivots_to_customer_need (reception):      drop -0.050*IS; drop -0.050*SP
...
RETURNS jsonb LANGUAGE sql STABLE AS $function$
  ...
  SELECT
    jsonb_build_object(
      ...
      -- CHANGE: rapid_rapport_warm -0.100*AN → -0.050*AN (analytical people are not "cold")
      'rapid_rapport_warm', public._cts_finalize_competency(GREATEST(0, LEAST(100, ROUND((20.000000) + (0.300000)*com + (0.200000)*op + (0.200000)*bo + (-0.050000)*an + (0.050000)*ass)::int)), lss_mod, rel),
      -- CHANGE: cadence_compliance -0.100*IS → -0.050*IS; drop +0.100*BO (weak); drop -0.050*SP (weak)
      'cadence_compliance', public._cts_finalize_competency(GREATEST(0, LEAST(100, ROUND((22.000000) + (0.250000)*dm + (0.150000)*an + (0.150000)*rd + (0.050000)*op + (-0.050000)*is_val)::int)), lss_mod, rel),
...
RETURNS jsonb LANGUAGE sql STABLE AS $function$
  ...
  SELECT
    jsonb_build_object(
      ...
      'receives_coaching', public._cts_finalize_competency(GREATEST(0, LEAST(100, ROUND((33.550072) + (-0.005371)*dm + (0.109720)*rd + (0.113558)*ass + (-0.109892)*is_val + (-0.112440)*an + (0.217015)*com + (-0.113273)*sp + (0.113147)*bo + (0.110904)*op)::int)), lss_mod, rel),
      'positively_influences_team', public._cts_finalize_competency(op, lss_mod, rel),
      -- CHANGE: cross_sell_instinct — drop -0.050*IS (independent people can absolutely cross-sell)
      'cross_sell_instinct', public._cts_finalize_competency(GREATEST(0, LEAST(100, ROUND((10.000000) + (0.200000)*com + (0.200000)*an + (0.150000)*sp + (0.100000)*bo + (0.100000)*rd + (0.050000)*dm + (0.050000)*ass)::int)), lss_mod, rel),
      -- retention_watchfulness — LEFT ALONE (-0.05*OP is a defensible vigilance signal)
...
RETURNS jsonb LANGUAGE sql STABLE AS $function$
  ...
  SELECT
    jsonb_build_object(
      ...
      'receives_coaching', public._cts_finalize_competency(GREATEST(0, LEAST(100, ROUND((33.550072) + (-0.005371)*dm + (0.109720)*rd + (0.113558)*ass + (-0.109892)*is_val + (-0.112440)*an + (0.217015)*com + (-0.113273)*sp + (0.113147)*bo + (0.110904)*op)::int)), lss_mod, rel),
      'positively_influences_team', public._cts_finalize_competency(op, lss_mod, rel),
      -- CHANGE: rapid_rapport_warm -0.100*AN → -0.050*AN
      'rapid_rapport_warm', public._cts_finalize_competency(GREATEST(0, LEAST(100, ROUND((20.000000) + (0.300000)*com + (0.200000)*op + (0.200000)*bo + (-0.050000)*an + (0.050000)*ass)::int)), lss_mod, rel),
      -- CHANGE: routing_judgment -0.100*IS → -0.050*IS; -0.100*SP → -0.050*SP
      'routing_judgment', public._cts_finalize_competency(GREATEST(0, LEAST(100, ROUND((22.000000) + (0.250000)*an + (0.200000)*bo + (0.150000)*com + (0.050000)*dm + (0.050000)*op + (0.050000)*ass + (-0.050000)*is_val + (-0.050000)*sp)::int)), lss_mod, rel),
      -- composure_under_load — LEFT ALONE (-0.05*AN is defensible "over-thinker under pressure" signal)
      ...
      -- CHANGE: pivots_to_customer_need — drop -0.050*IS; drop -0.050*SP
      'pivots_to_customer_need', public._cts_finalize_competency(GREATEST(0, LEAST(100, ROUND((12.000000) + (0.250000)*com + (0.200000)*an + (0.150000)*ass + (0.100000)*op + (0.100000)*bo + (0.050000)*rd)::int)), lss_mod, rel),
      '_meta', jsonb_build_object(
        'lss_modifier', lss_mod,
        'reliability_confidence', public._cts_reliability_confidence(rel),
        'distortion_severity', public._cts_distortion_severity(dist),
...
RETURNS jsonb LANGUAGE sql STABLE AS $function$
  ...
  SELECT
    jsonb_build_object(
      ...
      'receives_coaching', public._cts_finalize_competency(GREATEST(0, LEAST(100, ROUND((33.550072) + (-0.005371)*dm + (0.109720)*rd + (0.113558)*ass + (-0.109892)*is_val + (-0.112440)*an + (0.217015)*com + (-0.113273)*sp + (0.113147)*bo + (0.110904)*op)::int)), lss_mod, rel),
      'positively_influences_team', public._cts_finalize_competency(op, lss_mod, rel),
      -- retention_watchfulness — LEFT ALONE (defensible vigilance -OP)
      'retention_watchfulness', public._cts_finalize_competency(GREATEST(0, LEAST(100, ROUND((20.000000) + (0.250000)*com + (0.200000)*an + (0.100000)*bo + (0.050000)*ass + (0.050000)*dm + (-0.050000)*op)::int)), lss_mod, rel),
      -- CHANGE: proactive_touch_discipline — drop -0.050*IS; drop -0.050*SP
      'proactive_touch_discipline', public._cts_finalize_competency(GREATEST(0, LEAST(100, ROUND((20.000000) + (0.250000)*dm + (0.150000)*an + (0.150000)*com + (0.100000)*rd + (0.050000)*op)::int)), lss_mod, rel),
      '_meta', jsonb_build_object(
        'lss_modifier', lss_mod,
        'reliability_confidence', public._cts_reliability_confidence(rel),
        'distortion_severity', public._cts_distortion_severity(dist),
...
RETURNS jsonb LANGUAGE sql STABLE AS $function$
  ...
  SELECT
    jsonb_build_object(
      ...
      'receives_coaching', public._cts_finalize_competency(GREATEST(0, LEAST(100, ROUND((33.550072) + (-0.005371)*dm + (0.109720)*rd + (0.113558)*ass + (-0.109892)*is_val + (-0.112440)*an + (0.217015)*com + (-0.113273)*sp + (0.113147)*bo + (0.110904)*op)::int)), lss_mod, rel),
      'positively_influences_team', public._cts_finalize_competency(op, lss_mod, rel),
      -- CHANGE: queue_throughput_discipline -0.100*SP → -0.050*SP
      'queue_throughput_discipline', public._cts_finalize_competency(GREATEST(0, LEAST(100, ROUND((20.000000) + (0.250000)*dm + (0.150000)*an + (0.150000)*is_val + (0.100000)*rd + (0.050000)*op + (-0.050000)*sp)::int)), lss_mod, rel),
      -- CHANGE: attention_to_detail — drop -0.050*OP; drop -0.050*SP
      'attention_to_detail', public._cts_finalize_competency(GREATEST(0, LEAST(100, ROUND((20.000000) + (0.300000)*an + (0.150000)*dm + (0.100000)*com + (0.100000)*rd + (0.050000)*is_val)::int)), lss_mod, rel),
      '_meta', jsonb_build_object(
        'lss_modifier', lss_mod,
        'reliability_confidence', public._cts_reliability_confidence(rel),
        'distortion_severity', public._cts_distortion_severity(dist),
