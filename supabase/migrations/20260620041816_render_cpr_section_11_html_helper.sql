-- Helper function that returns the Section 11 (SMVC & Scorecard) HTML block.
-- Pulled into its own function so compose_weekly_cpr_html stays maintainable.

CREATE OR REPLACE FUNCTION public.render_cpr_section_11_html(
  p_agency_id uuid,
...
BEGIN
  ...
  v_current := NULLIF(v_smvc->>'current', '')::numeric;
  ...
  v_html := v_html
    ...
    || '</tbody></table>'
    -- Two budget lines
    || '<div style="font-size:12px;color:#475569;margin-top:8px;padding-left:14px">Next Quarter On-Time Prize Cart Budget: <span style="color:#94a3b8">—</span></div>'
    || '<div style="font-size:12px;color:#475569;margin-top:2px;padding-left:14px">Next Quarter On-Time WtQ Trip Budget: <span style="color:#94a3b8">—</span></div>'
