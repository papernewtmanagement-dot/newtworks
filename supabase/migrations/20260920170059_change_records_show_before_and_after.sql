-- Change records show the before and after values, not just which fields moved
-- (Peter 2026-09-19). The building blocks already existed -- change_field_label,
-- change_field_hidden, change_value_text, change_diff -- they were never wired
-- into the two functions the screens read. They are now, so the label map and
-- the value formatting live in exactly one place and the three surfaces
-- (Activity Log day view, Activity Log full history, CPR Log Changes box)
-- cannot drift apart.
...
AS $function$
  ...
  -- Every field that moved anywhere inside one click, in the order the rows
  -- were written. A sale and the policy line under it are one click, so both
  -- sets of values belong on the same line.
  diffs AS (
    SELECT r.txid,
           jsonb_agg(e.val ORDER BY r.rank, r.changed_at, e.ord) AS changes
      FROM raw r
...
RETURNS TABLE(txid bigint, changed_at timestamptz, who text, what text, item text, subject text,
              changed_fields text[], changes jsonb, row_count integer, line text)
LANGUAGE sql STABLE
SET search_path TO 'public', 'pg_temp'
AS $function$
...
RETURNS TABLE(id uuid, changed_at timestamptz, txid bigint, who text, via text, action text,
              item text, table_name text, row_id uuid, subject text, changed_fields text[],
              changes jsonb, old_row jsonb, new_row jsonb)
LANGUAGE sql STABLE
SET search_path TO 'public', 'pg_temp'
AS $function$
  SELECT c.id, c.changed_at, c.txid, c.changed_by_label, c.via, c.action,
         ...
         CASE WHEN lower(c.action) = 'update'
              THEN public.change_diff(c.agency_id, c.changed_fields, c.old_row, c.new_row)
              ELSE '[]'::jsonb END,
         c.old_row, c.new_row
...
GRANT EXECUTE ON FUNCTION public.change_log_recent(integer, uuid, integer) TO authenticated, service_role;
