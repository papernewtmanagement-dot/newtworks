...
-- 1a. Drop the per-person EUR count
ALTER TABLE public.weekly_cpr_team_detail DROP COLUMN IF EXISTS eur_count;

-- 1b. Add EUR text field on the weekly report
...
-- 2 + 3. Rewrite get_weekly_cpr_requirements:
--    - NULL on team or personal booleans now counts as a miss
--    - Personal misses skipped entirely when no detail row exists for the week
...
BEGIN
  ...
  WHILE v_loop_week <= p_week_ending_date LOOP
    ...
    this_report AS (
      SELECT
        id AS report_id,
        -- NULL now counts as a miss (was: NULL counted as done)
    ...
    per_person AS (
      SELECT
        ...
        -- No detail row for this week → no personal misses (member not active that week).
        -- Detail row present → NULL or FALSE both count as miss.
