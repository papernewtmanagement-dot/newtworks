-- New mirror function: computes 9 Sales competencies with per-competency LSS adjustment layered on top.
-- Preserves existing cts_sales_competencies untouched so we can compare and revert.
-- Returns richer jsonb (base + lss_delta + final per competency) to support two-segment UI bar.
...
DECLARE
  ...
  v_has_lss boolean;
BEGIN
  -- Base competencies (identical to cts_sales_competencies)
  ...
  IF v_has_lss THEN
    ...
    -- Centered [-1, +1] signals
    v_acc_signal := (v_acc_flags - 1.5) / 1.5;
    v_spd_signal := (v_spd_flags - 1.5) / 1.5;

    -- Per-competency swing: 15 * (w_acc*acc_signal + w_spd*spd_signal) / 2
