-- FIX: cc_gl_writer sign-convention bug.
--
-- Source data (credit_transactions) uses statement convention:
--   transaction_type='charge'  → amount POSITIVE (money spent)
--   transaction_type='payment' → amount NEGATIVE (money paid on card)
--   transaction_type='credit'  → amount NEGATIVE (refunds/rewards to card)
--
-- Correct JE for each:
--   CHARGE:  DR expense (goes up), CR card liability (goes up)
--   PAYMENT: DR card liability (goes down), CR bank/asset (goes down)
--   CREDIT:  DR card liability (goes down), CR expense/rewards contra (goes down)
--
-- Prior code had the two amount branches inverted:
--   amount > 0 (charge): DR card, CR other        ← WRONG (that's the payment shape)
--   amount < 0 (payment/credit): DR other, CR card ← WRONG (that's the charge shape)
--
-- Fix: swap the two branches. Also skip already-linked duplicates from claude_bank_reparse
-- on same card + date + amount so we don't double-book.
...
DECLARE
  ...
  v_count_skipped_duplicate int := 0;
...
BEGIN
  ...
  LOOP
    ...
    -- Duplicate check: does an existing JE from a different source (bank reparse, manual)
    -- already book the same card + date + abs(amount)?
    SELECT je.id INTO v_dup_je_id
    FROM journal_entries je
    JOIN journal_lines jl ON jl.journal_entry_id = je.id
    WHERE je.agency_id = p_agency_id
      AND je.entry_date = v_txn_date
    ...
    IF v_dup_je_id IS NOT NULL THEN
      ...
      IF NOT p_dry_run THEN
        UPDATE credit_transactions
        SET journal_entry_id = v_dup_je_id,
            posted_at = NOW(),
            notes = COALESCE(notes, '') || ' [linked to existing JE ' || v_dup_je_id::text ||
                    ' from other source; cc_gl_writer skipped duplicate]'
        WHERE id = v_txn_id;
      END IF;
      CONTINUE;
    END IF;
    ...
    --   'debit'  = expense goes on debit side (charges/normal spend)
    --   'credit' = expense goes on credit side (refunds/rewards)
    --   NULL/both = either
    IF v_other_side_account_id IS NULL THEN
      DECLARE
        ...
        v_target_code text;
      BEGIN
        -- Charge: expense debited (direction='debit'). Payment/credit: reverse (direction='credit').
        v_rule_direction := CASE
          WHEN v_txn_type = 'charge' THEN 'debit'
          WHEN v_txn_type IN ('payment','credit') THEN 'credit'
          WHEN v_amount > 0 THEN 'debit'   -- fallback for null type: positive = charge
          ELSE 'credit'                     -- fallback: negative = payment/credit
        END;
        ...
        WHERE r.agency_id = p_agency_id AND r.is_active = TRUE
          -- Skip catch-all "everything to SUSP" rule
        ...
        IF v_rule_id IS NOT NULL AND v_target_code IS NOT NULL
           AND v_target_code NOT IN ('__SOURCE__', 'COA-SUSP') THEN
          SELECT id INTO v_other_side_account_id
            FROM chart_of_accounts
            WHERE agency_id = p_agency_id
    ...
    -- Post the JE with CORRECT sign convention based on transaction_type.
    DECLARE
    ...
    BEGIN
      v_is_charge := CASE
        ...
        WHEN v_amount > 0 THEN TRUE   -- fallback: positive = charge
        ELSE FALSE                     -- fallback: negative = payment/credit
      END;
      ...
      ELSE
        -- PAYMENT/CREDIT: DR card liability (down), CR other (bank asset or rewards contra)
      ...
      IF p_dry_run THEN
        v_posted_runs := v_posted_runs || jsonb_build_object(
          'txn_id', v_txn_id, 'date', v_txn_date, 'amount', v_amount, 'type', v_txn_type,
          'card', v_card_name, 'description', left(v_description, 80),
...
GRANT EXECUTE ON FUNCTION public.cc_gl_writer(uuid, boolean) TO authenticated;
