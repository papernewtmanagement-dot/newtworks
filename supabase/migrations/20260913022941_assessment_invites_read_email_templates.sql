...
AS $function$
  -- 2026-09-12: the invite and reminder wording moved to
  -- public.hiring_email_templates (keys assessment_invite, assessment_reminder).
  -- Peter edits them under Team > Growth > Email Templates. Seeded verbatim
  -- from the copy that used to be built here, including the measured
  -- "about an hour" sitting length (migration invite_copy_measured_hour_and_fcq_min_n_20).
DECLARE
  v_first_sent int := 0;
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      FROM public.render_hiring_email(p_agency_id, 'assessment_invite', jsonb_build_object(
        'first_name',      COALESCE(v_cand.first_name, 'there'),
        'position',        v_position_display,
        'role_phrase',     v_role_phrase,
        'assessment_link', v_full_link
      )) r;
  ...
  LOOP
    BEGIN
      ...
      SELECT r.subject, r.body_html INTO v_subject, v_html
      FROM public.render_hiring_email(p_agency_id, 'assessment_reminder', jsonb_build_object(
        'first_name',      COALESCE(v_cand.first_name, 'there'),
        'position',        v_position_display,
        'role_phrase',     v_role_phrase,
        'assessment_link', v_full_link
