-- Extend send_v1_assessment_invitations to handle the full lifecycle:
--   Step 1a: sweep 'sent' -> 'completed' when v1_assessment_complete alert exists
--   Step 1b: sweep 'sent' -> 'declined' when hiring_candidates.status='declined'
--   Step 2a: attempt 1 sends (unchanged behavior; adds next_attempt_at=+1d)
--   Step 2b: reminder sends (attempts 2 & 3), 1 day apart per manual "resend daily"
--   Step 3:  sweep 'sent' -> 'no_response' after attempt 3 + 1 day grace
--
-- Single automation, one recipe row. Global rate cap = 10 sends/run across (new + reminders).
-- Status stays 'applied' throughout (manual: "don't decline"). Only outcome column tracks give-up.
-- Alert existence is authoritative completion signal; status flip is used for eligibility gate.
...
BEGIN
  ...
  -- 'sent' -> 'completed' when v1_assessment_complete alert exists
  -- ============================================
  WITH updated AS (
  ...
  -- 'sent' -> 'declined' when hiring_candidates.status='declined'
  -- ============================================
  WITH updated AS (
    UPDATE public.assessment_invitations ai
  ...
  LOOP
    BEGIN
      ...
      -- After attempt 2 -> schedule attempt 3 for +1 day
      -- After attempt 3 -> no next attempt; terminal sweep handles later
      v_next_attempt := CASE
  ...
  -- 'sent' -> 'no_response' for candidates whose attempt 3 was >24h ago
  -- and who still have no completion alert
  -- ============================================
  WITH exhausted AS (
