-- Personal checklist item 3: Scorecard replaces the retired CPR-reply box.
-- Peter directive 2026-08-05.
--
-- cpr_reply_done was retired from the recap page around 2026-07-19 and
-- replaced by scorecard_done, but get_weekly_cpr_requirements kept scoring
-- the dead column. Nothing has written it since 2026-07-18, so every
-- teammate carried a phantom personal miss for the weeks ending 07-25,
-- 08-01 and 08-08, and real Scorecard misses were never counted.
-- Personal items are now exactly the three the page shows: Wrap-up,
-- Inbox, Scorecard. Column dropped in the same migration -- no other
-- function, view or frontend file referenced it (verified).
...
ALTER TABLE public.weekly_cpr_team_detail DROP COLUMN IF EXISTS cpr_reply_done;
