-- 126 of the 143 conversation scores carry no customer first name, and
-- rp_customer_label RAISES on a blank name. rp_recent_entries called it unguarded,
-- so the whole Recent entries list threw instead of returning rows the moment a
-- nameless score fell inside the window. Guard the call: no name, no label.
...
BEGIN
  ...
  WITH rows AS (
    ...
    SELECT 'scorecard', f.id, f.scorecard_date, public.rp_week_end(f.scorecard_date), f.team_member_id,
           CASE WHEN NULLIF(btrim(COALESCE(f.customer_first_name, '')), '') IS NULL
                THEN NULL
                ELSE public.rp_customer_label(f.customer_first_name, NULL) END,
           f.phone_last4,
