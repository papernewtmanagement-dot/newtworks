-- 2026-07-21: FREEZE GUARD on write_weekly_comp_v2.
--
-- ROOT CAUSE addressed (Peter directive 2026-07-21):
-- write_weekly_comp_v2 reads live inputs (payroll_detail, agency_snapshot, team_checkins,
-- time_clock_entries, etc.). Any input drift after send causes a subsequent re-invocation
-- to produce different pool numbers for the same function on the same week. Week ending
-- 2026-07-18 case: pool moved between Sunday's send-time write and Monday's CPR-page
-- "Save & recompute" click because a new payroll (pay_period_end 2026-07-18) was ingested
-- into payroll_detail in between.
--
-- FIX: once weekly_cpr_reports.sent_to_team_at IS NOT NULL for the week, the writer
-- refuses. Raises so callers (UI, cron, manual RPC) see a hard failure rather than
-- silently succeeding with unchanged values.
--
-- Auto-send flow (try_send_weekly_cpr_recap) is unaffected: it early-exits when
-- sent_to_team_at IS NOT NULL BEFORE it ever calls the writer.
...
BEGIN
  -- FREEZE GUARD: once a CPR has been sent to the team, settlement numbers are locked.
  -- Any recompute call against a sent week is refused. To override (unusual), clear
  -- weekly_cpr_reports.sent_to_team_at manually first.
  ...
  IF v_sent_at IS NOT NULL THEN
    RAISE EXCEPTION 'CPR for week ending % was sent to the team at % — settlement numbers are frozen. Recompute refused.', p_week_end_date, v_sent_at
      USING ERRCODE = 'check_violation',
            HINT = 'To edit a sent CPR, clear weekly_cpr_reports.sent_to_team_at first. Overriding a locked snapshot means the team saw different numbers than payroll settles against.';
  END IF;

  -- Guarantee every active teammate has a snapshot row for this week BEFORE UPDATEs run.
  -- Rule: on the roster = gets paid. Archived = doesn't. No third case. Prefill uses
  -- get_expected_teammates(..., 'compensation', week_start) as the "who's on the roster"
  -- source of truth. Idempotent (ON CONFLICT DO NOTHING). Also creates the
  -- weekly_cpr_reports row if missing.
  ...
  IF v_report_id IS NULL THEN
    -- Should be unreachable now (prefill creates the report row) but keep as safety net.
  ...
  -- GOALS BONUS (1% gain = rolling 13-wk avg new SP x 1.01)
