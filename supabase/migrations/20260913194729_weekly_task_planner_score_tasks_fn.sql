...
BEGIN
  ...
  cat AS (
    SELECT b.id,
           COALESCE(
             ...
             (SELECT r.importance_value FROM task_scoring_rules r
               WHERE r.agency_id=p_agency_id AND r.rule_kind='category_weight'
                 AND r.is_active AND r.match_category IS NULL LIMIT 1),
             50
           )::numeric AS cat_imp
    FROM base b
  ),
  kw AS (
    ...
    LEFT JOIN task_scoring_rules r
      ON r.agency_id = p_agency_id AND r.rule_kind='keyword' AND r.is_active
     AND b.hay ~ r.match_pattern
    GROUP BY b.id
  ),
  struct AS (
    SELECT b.id,
           EXISTS (SELECT 1 FROM tasks c WHERE c.parent_task_id=b.id AND c.status='open') AS has_kids
    FROM base b
  ),
  typ AS (
    SELECT b.id,
           (SELECT r.hours_value FROM task_scoring_rules r
  ...
  SELECT b.id,
         b.task_type,
         b.priority_source,
         b.estimated_hours_source,
         GREATEST(0, LEAST(100,
           cat.cat_imp + kw.imp_bump + CASE WHEN struct.has_kids THEN 8 ELSE 0 END
         ))::smallint AS importance,
         GREATEST(0, LEAST(100,
           CASE
             WHEN b.due_date IS NULL THEN 25
             WHEN b.due_date <= CURRENT_DATE THEN 100
             WHEN b.due_date <= CURRENT_DATE + 3  THEN 90
             WHEN b.due_date <= CURRENT_DATE + 7  THEN 75
             WHEN b.due_date <= CURRENT_DATE + 14 THEN 60
             WHEN b.due_date <= CURRENT_DATE + 30 THEN 40
             ELSE 20
           END
           + kw.urg_bump
         ...
         CASE
           WHEN b.task_type = 'epic' THEN NULL
           ELSE GREATEST(0.25, ROUND((COALESCE(kw.kw_hours, typ.type_hours, 1.5) * pf_mult) * 4, 0) / 4)
         END::numeric(5,2) AS est_hours
  FROM base b
  JOIN cat ON cat.id=b.id
  JOIN kw ON kw.id=b.id
  JOIN struct ON struct.id=b.id
  JOIN typ ON typ.id=b.id;
  ...
  -- importance / urgency / scored_at always refresh. These are inputs, not Peter's fields.
  UPDATE tasks t
  ...
  -- priority only where nobody has overridden it
  WITH upd AS (
    UPDATE tasks t
       SET priority = CASE
             ...
             ELSE 'low' END
      FROM _sc s
     WHERE t.id = s.id AND s.priority_source = 'auto'
    RETURNING 1)
  SELECT count(*) INTO v_pri FROM upd;

  -- hours only where nobody has overridden them
  WITH upd2 AS (
    UPDATE tasks t
       SET estimated_hours = s.est_hours
      FROM _sc s
     WHERE t.id = s.id AND s.estimated_hours_source = 'auto' AND s.task_type <> 'epic'
    RETURNING 1)
...
COMMENT ON FUNCTION public.score_tasks IS
  'Sunday scoring pass. Scores importance and urgency separately, combines into the four priority bands, and estimates hours. Never touches a row where priority_source or estimated_hours_source is manual. Incremental by default: only rows changed since their last score. Pass p_force=true to rescore everything.';