-- Peter 2026-08-28: the chart gets base + commission as its own line, so
-- every curve point now carries the commission and the base-plus-commission
-- running total instead of leaving the frontend to infer them. The pool
-- percentage the projection used is surfaced in the model-inputs box so it
-- is visible rather than buried in a function.
...
-- by public.pay_scale_bonus_inputs at the 156-week horizon -- the bonus
-- pool percentage is the forward average off the dated schedule, not this
-- week's figure (migration pay_scale_bonus_inputs_forward_pool_schedule).
...
BEGIN
    ...
    BEGIN
      ...
      IF r_role.role_key = 'sales' THEN
        SELECT jsonb_agg(jsonb_build_object(
                 ...
                 'bonus', round(COALESCE(p.expected_team_bonus_annual,0), 0),
  ...
  RETURN jsonb_build_object(
    ...
    'assumptions', jsonb_build_object(
      ...
      'pool_pct_used', round(COALESCE((v_inputs->>'pool_pct_forward')::numeric,0) * 100, 2),
      'pool_pct_this_week', round(COALESCE((v_inputs->>'pool_pct_now')::numeric,0) * 100, 2),
      'pool_pct_horizon_weeks', (v_inputs->>'pool_horizon_weeks')::int,
      'pool_pct_window_end', v_inputs->>'pool_window_end',
      'note', 'One sales point equals one dollar of commission. Team bonus is a '
           ...
           || 'follow sales points, one third follows retention hours. The share '
           || 'of agency earnings that funds the pool is not held at this week''s '
           || 'figure — it follows the dated plan, and the chart uses the average '
           || 'of that plan over the next three years, which is the window this '
           || 'money is earned in. Numbers '
