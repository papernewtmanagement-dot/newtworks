-- Peter 2026-08-28, correcting the shape entered an hour earlier: six autos,
-- THREE fire, and TWO HUNDRED DOLLARS of life premium. Life is specified as
-- premium, not as a policy count, so it no longer runs through life premium
-- per policy at all.
--
-- For reference against the agency's own trailing six months: 244 auto and
-- 113 fire is six-to-2.8, so three fire per six autos is close to what is
-- actually written. Life premium actually ran about $470 per six autos, so
-- $200 is the more conservative assumption of the two.
...
-- assumed shape of six auto, three fire, two hundred dollars of life
-- premium.
DECLARE
  v_week   date;
  v_diag   jsonb;
...
-- Solves for how many SETS of business the seat writes -- Peter's assumed
-- shape of six auto, three fire, two hundred dollars of life premium --
-- such that the real SF builder rate curve (compute_sp_from_production,
-- the single copy of those rates) pays them p_x points a week. P&C premium
-- follows from the policy counts; life enters as premium directly.
...
DECLARE
  ...
  v_rf numeric := COALESCE((p_inputs->>'set_fire_apps')::numeric, 3);
  v_lp numeric := COALESCE((p_inputs->>'set_life_premium')::numeric, 200);
