-- 20260719060000_assessment_layer_3level_ordinals.sql
-- Peter directive 2026-07-19: reliability and distortion are 3-level ordinals in the vendor's
-- ratings — high/moderate/low. Simplify the mappings in v_hiring_candidates.assessment_nurture.
-- Symmetric 90/50/10 (wide separation, centered at 50, room to breathe).
-- Reliability + Distortion moved same direction: higher output = better (distortion inverted).
...
SELECT
  ...
  round(
    ...
    + aw.w_nur * (
        (hc.compassion + hc.belief_in_others
         + (CASE hc.reliability
              WHEN 'high' THEN 90 WHEN 'moderate' THEN 50 WHEN 'low' THEN 10 ELSE NULL
            END)
         + (CASE hc.response_distortion
              WHEN 'low' THEN 90 WHEN 'moderate' THEN 50 WHEN 'high' THEN 10 ELSE NULL
            END)
        ) / 4.0
      )
