...
-- Rewrite get_payroll_run_drilldown to derive JE entity from the accounts on its
-- journal_lines (via chart_of_accounts), matching Thread B's P&L pattern.
-- payroll_runs.business_entity_id is a separate concern (payroll_runs table's own
-- column) and is left alone here.
