-- Fix: assessment invite/reminder emails duplicated the role phrase when
-- hiring_candidates.position was null/blank ("Thanks for applying for the
-- the role role at Peter Story State Farm."). Root cause: v_position_display
-- fell back to the literal string 'the role', and the body template wrapped
-- it in its own "the ... role" phrase, doubling both words.
--
-- Found 2026-08-13 while investigating why Emily Rodriguez got reminder
-- emails after seeming to say she wasn't interested. Conclusion on that
-- question: she hadn't told us before — her only decline reply (8/14) came
-- after both reminders (8/12, 8/13), and no reminder went out after she
-- declined. Not a bug in the decline-handling path. This role-phrase
-- duplication is a separate, real bug hit by any candidate with no position
-- on file (147 candidates in the pipeline at time of fix).
--
-- Fix: body text now uses a dedicated v_role_phrase built once per
-- candidate — "the <strong>X</strong> role" when a position is on file,
-- "this role" when it isn't. Subject line logic is unchanged (it was never
-- affected). See public.send_v1_assessment_invitations for full definition.
...
BEGIN
  ...
  -- Fix (2026-08-11, Peter-approved, open_question 7e377007 part 2): reads
  -- public.resume_weighted_composite (Capability 20/Character 40/Commitment
  -- 40) instead of the plain unweighted 'avg' field, matching the number
  -- the published verdict bands were actually designed against.
  ...
  -- Fix (2026-08-11, part 2): resume_weighted_composite, not plain 'avg'.
  FOR v_cand IN
  ...
  LOOP
    BEGIN
      ...
      -- Fix (2026-08-13, Peter-reported via Emily Rodriguez case): when
      -- position is null/blank, v_position_display fell back to 'the role',
      -- and the body template wrapped it in its own "the ... role" phrase,
      -- producing "for the the role role at Peter Story State Farm."
      -- v_role_phrase is now built once per case so the body always reads
      -- naturally, whether or not a position is on file.
      v_role_phrase := CASE
