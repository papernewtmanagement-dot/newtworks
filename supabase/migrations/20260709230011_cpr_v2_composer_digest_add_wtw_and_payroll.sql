...
BEGIN
  ...
  -- Subject-line date format (cross-month-safe, matches sender)
  ...
  -- Opener + Looking-ahead (escape + \n → <br>; fallback if empty)
  ...
  -- Win the Week: sum team net_quotes + sales_points, compare to goal
  SELECT
  ...
  -- Weekly Pay snapshot — one row per active agency teammate (excl Owner + admin_backoffice)
