      ...
      SELECT
        ...
        CASE cfg.category
          WHEN 'week_quotes' THEN
            -- Net quotes = quotes_discussed - paid. Pulled from get_weekly_cpr_requirements.
            COALESCE(
              ...
              0)::numeric
          WHEN 'week_sp' THEN
            -- Weekly SP delta = curr QTD - prior Saturday QTD within same quarter (floor 0 at Q start)
            GREATEST(0,
...
-- Re-run for this week so John's 29 net quotes is scored against the leaderboard.
SELECT public.audit_weekly_leaderboard_crossings('126794dd-25ff-47d2-a436-724499733365'::uuid, '2026-07-11'::date);
