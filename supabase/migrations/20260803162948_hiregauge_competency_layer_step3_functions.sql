...
-- Consumes hiregauge_v2_normalized_inputs(candidate_id) (the already-shipped
-- Step 2) as the single normalized-input source.
...
CREATE OR REPLACE FUNCTION public._newtworks_competency_composite(
  p_values numeric[],
  p_labels text[],
  p_reliability text,
  p_distortion text
...
CREATE OR REPLACE FUNCTION public._newtworks_competency_role_context(
  p_agency_id uuid, p_role_category text, p_competency_name text
) RETURNS jsonb
LANGUAGE sql
STABLE
AS $fn$
...
CREATE OR REPLACE FUNCTION public.newtworks_competency_drive_work_intensity(p_candidate public.hiring_candidates, p_role_category text DEFAULT NULL)
...
CREATE OR REPLACE FUNCTION public.newtworks_competency_persuasive_influence(p_candidate public.hiring_candidates, p_role_category text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql STABLE AS $fn$
DECLARE v_norm jsonb; v_core jsonb; v_ctx jsonb := '{}'::jsonb;
BEGIN
  v_norm := public.hiregauge_v2_normalized_inputs(p_candidate.id);
...
-- 3. Rapport Building
CREATE OR REPLACE FUNCTION public.newtworks_competency_rapport_building(p_candidate public.hiring_candidates, p_role_category text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql STABLE AS $fn$
DECLARE v_norm jsonb; v_core jsonb; v_ctx jsonb := '{}'::jsonb;
BEGIN
  v_norm := public.hiregauge_v2_normalized_inputs(p_candidate.id);
...
CREATE OR REPLACE FUNCTION public.newtworks_competency_needs_discovery(p_candidate public.hiring_candidates, p_role_category text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql STABLE AS $fn$
DECLARE v_norm jsonb; v_core jsonb; v_ctx jsonb := '{}'::jsonb;
BEGIN
  v_norm := public.hiregauge_v2_normalized_inputs(p_candidate.id);
...
CREATE OR REPLACE FUNCTION public.newtworks_competency_resilience_under_rejection(p_candidate public.hiring_candidates, p_role_category text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql STABLE AS $fn$
DECLARE v_norm jsonb; v_core jsonb; v_ctx jsonb := '{}'::jsonb;
BEGIN
  v_norm := public.hiregauge_v2_normalized_inputs(p_candidate.id);
...
CREATE OR REPLACE FUNCTION public.newtworks_competency_composure_under_pressure(p_candidate public.hiring_candidates, p_role_category text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql STABLE AS $fn$
DECLARE v_norm jsonb; v_core jsonb; v_ctx jsonb := '{}'::jsonb;
BEGIN
  v_norm := public.hiregauge_v2_normalized_inputs(p_candidate.id);
...
CREATE OR REPLACE FUNCTION public.newtworks_competency_accuracy_procedural_discipline(p_candidate public.hiring_candidates, p_role_category text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql STABLE AS $fn$
DECLARE v_norm jsonb; v_core jsonb; v_ctx jsonb := '{}'::jsonb;
BEGIN
  v_norm := public.hiregauge_v2_normalized_inputs(p_candidate.id);
...
CREATE OR REPLACE FUNCTION public.newtworks_competency_rule_compliance_adherence(p_candidate public.hiring_candidates, p_role_category text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql STABLE AS $fn$
DECLARE v_norm jsonb; v_core jsonb; v_ctx jsonb := '{}'::jsonb;
BEGIN
  v_norm := public.hiregauge_v2_normalized_inputs(p_candidate.id);
...
-- 9. Integrity
CREATE OR REPLACE FUNCTION public.newtworks_competency_integrity(p_candidate public.hiring_candidates, p_role_category text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql STABLE AS $fn$
DECLARE v_norm jsonb; v_core jsonb; v_ctx jsonb := '{}'::jsonb;
BEGIN
  v_norm := public.hiregauge_v2_normalized_inputs(p_candidate.id);
...
CREATE OR REPLACE FUNCTION public.newtworks_competency_judgment_escalation(p_candidate public.hiring_candidates, p_role_category text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql STABLE AS $fn$
DECLARE v_norm jsonb; v_core jsonb; v_ctx jsonb := '{}'::jsonb;
BEGIN
  v_norm := public.hiregauge_v2_normalized_inputs(p_candidate.id);
...
CREATE OR REPLACE FUNCTION public.newtworks_competency_coachability_team_contribution(p_candidate public.hiring_candidates, p_role_category text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql STABLE AS $fn$
DECLARE v_norm jsonb; v_core jsonb; v_ctx jsonb := '{}'::jsonb;
BEGIN
  v_norm := public.hiregauge_v2_normalized_inputs(p_candidate.id);
...
CREATE OR REPLACE FUNCTION public.newtworks_competency_autonomy_ownership(p_candidate public.hiring_candidates, p_role_category text DEFAULT NULL)
RETURNS jsonb LANGUAGE plpgsql STABLE AS $fn$
DECLARE v_norm jsonb; v_core jsonb; v_ctx jsonb := '{}'::jsonb;
BEGIN
  v_norm := public.hiregauge_v2_normalized_inputs(p_candidate.id);
