-- Missed consumers of assessment_best_fit_role return record: assessment_nature +
-- verdict_overall read <role>_os / best_os fields off the record variable.
-- Those column names got renamed to _fit_score in the prior migration.
-- Fix now to unbreak v_hiring_candidates (chains through both).
-- Also rename verdict_overall meta jsonb key 'best_fit_os' -> 'best_fit_score'.
