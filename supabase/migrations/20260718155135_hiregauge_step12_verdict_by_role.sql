-- Step 12: hiregauge_three_construct_verdict_by_role
-- Returns 7 rows, one per role slug, each computed as if that role were the hire target.
-- Role-agnostic cells (Nurture-assessment, Resume, Interview-nature/nurture/motivation, Reference) computed once and reused.
-- Only Assessment Nature (v_na) and Assessment Drivers (v_da) recompute per role → downstream rollups differ.
-- Existing single-row hiregauge_three_construct_verdict left in place (non-destructive).
...
BEGIN
  ...
  -- Resume construct trio (role-agnostic)
  ...
  -- Assessment Nurture (reliability-based, role-agnostic)
  ...
  -- Interview Nature (role-play scores)
  ...
  -- Interview Nurture (character-based)
  ...
  -- LSS + retrospective (role-agnostic)
  ...
  -- The Assessment layer triggers only when deadline_motivation exists; count dim once here since
  -- v_na and v_da existence is gated by the same condition across all roles.
  ...
  -- Per-role loop: compute role-specific v_na, v_da, then all downstream rollups
