...
DECLARE
  ...
  v_count_skipped_excluded int := 0;
BEGIN
  FOR v_stmt_id, v_business_entity_id, v_txn_date, v_description, v_amount,
      v_transaction_type, v_category, v_reference_number, v_chart_account_code, v_stmt_account_id, v_account_kind IN
    SELECT s.id, s.business_entity_id, s.transaction_date, s.description, s.amount,
           s.transaction_type, s.category, s.reference_number, coa.account_code, s.account_id, s.account_kind
    FROM statements s
  ...
  LOOP
    ...
    IF v_target_account_id IS NULL THEN
      -- RULE (2026-08-16, Peter directive): a credit-card transaction is
      -- ALWAYS an expense-type account when unclassified, regardless of
      -- which direction the money moved. A purchase increases the expense
      -- (debit); a refund/return reduces that same expense bucket (credit).
      -- It is NEVER income -- a card refund is money back on something
      -- already spent, not new earnings. Only a genuine bank/checking
      -- account keeps the old direction-based fallback (deposit could be
      -- real income; withdrawal is real spend).
      v_target_account_id := get_entity_unclassified_account(
        p_agency_id, v_business_entity_id,
        CASE
          WHEN v_account_kind = 'credit' THEN 'expense'
          WHEN v_direction = 'credit' THEN 'income'
          ELSE 'expense'
        END
      );
      IF v_target_account_id IS NULL THEN
