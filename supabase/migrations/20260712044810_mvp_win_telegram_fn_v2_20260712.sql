...
-- This migration was applied live on 2026-07-12 04:48:10 UTC (ledger name: mvp_win_telegram_fn_v2_20260712) but was never
-- committed to this repo at the time. Content below is copied verbatim
-- from supabase_migrations.schema_migrations.statements for version 20260712044810.
-- No live change is made by adding this file — it only brings the repo
-- mirror into sync with what is already running in production, so a
-- fresh `supabase db reset` from this repo reproduces live state.
CREATE OR REPLACE FUNCTION public.send_mvp_prize_win_telegram(
  p_agency_id uuid,
  p_mvp_team_id uuid,
  p_prize_cart_id uuid
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_group_chat_id IS NULL OR v_mvp_name IS NULL OR v_prize_desc IS NULL THEN
    ...
    VALUES (p_agency_id, 'mvp_prize_draw', 'high',
