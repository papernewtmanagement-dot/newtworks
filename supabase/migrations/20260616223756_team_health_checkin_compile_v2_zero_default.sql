...
BEGIN
  ...
  v_text := E'💪 Health Goal Update — Week of ' || to_char(v_week_start, 'Mon DD') || E'\n\n';
  ...
  LOOP
    ...
    -- v2: every line shows the X/5 number; "no data" people default to 0/5
    -- Tone calibrated to whether week is far along enough to warrant nudges
    ...
    ELSIF v_row.hits = v_target - 1 AND v_day_of_week >= 4 THEN
      v_line := '• ' || v_row.first_name || ': ' || v_row.hits || '/' || v_target
        || ' ' || v_progress_bar || '  one more workout';
    ELSIF v_row.hits = 0 AND v_day_of_week >= 4 THEN
  ...
  ELSIF v_day_of_week = 5 THEN
    v_text := v_text || E'\n\nWeekend ahead — plenty of room to close the gap.';
  END IF;
