-- pnl_drill_transactions: add business_entity_id to the returned row set so
-- the P&L transaction drill/edit panel can know which entity each
-- transaction currently belongs to (needed to support entity-switching in
-- the edit form). Journal-sourced rows get it via chart_of_accounts
-- (entity lives on the account, not the ledger row). prior_year_pl rows
-- get it directly from prior_year_pl.business_entity_id. Peter directive
-- 2026-08-14: P&L transaction edit popup needs to allow switching entities.
DROP FUNCTION IF EXISTS public.pnl_drill_transactions(uuid, text, text, text, date, date);

CREATE FUNCTION public.pnl_drill_transactions(p_entity_id uuid, p_account_name text, p_section text, p_account_type text, p_from_date date, p_to_date date)
 RETURNS TABLE(source text, je_id uuid, line_id uuid, pyp_id uuid, entry_date date, amount numeric, description text, memo text, reference_number text, je_source text, classification_status text, account_id uuid, account_code text, account_name text, document_id uuid, created_at timestamp with time zone, confirmation text, cash_register_id uuid, business_entity_id uuid)
 LANGUAGE sql
 STABLE
AS $function$
