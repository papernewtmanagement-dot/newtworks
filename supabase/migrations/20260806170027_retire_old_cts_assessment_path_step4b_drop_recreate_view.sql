...
CREATE VIEW public.v_hiring_candidates AS
 WITH resume_w AS (
         SELECT max(CASE WHEN hiregauge_layer_composite_weights.construct = 'capability'::text THEN hiregauge_layer_composite_weights.weight ELSE NULL::numeric END) AS w_cap,
            max(CASE WHEN hiregauge_layer_composite_weights.construct = 'character'::text THEN hiregauge_layer_composite_weights.weight ELSE NULL::numeric END) AS w_chr,
            max(CASE WHEN hiregauge_layer_composite_weights.construct = 'commitment'::text THEN hiregauge_layer_composite_weights.weight ELSE NULL::numeric END) AS w_com
 ...
 SELECT hc.id,
    ...
    round(rw.w_cap * COALESCE(resume_capability(hc.id), 0::numeric) + rw.w_chr * COALESCE(resume_character(hc.id), 0::numeric) + rw.w_com * COALESCE(resume_commitment(hc.id), 0::numeric), 2) AS res_composite,
