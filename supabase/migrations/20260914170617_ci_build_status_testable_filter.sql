-- p_status_filter exists so the failure path can be exercised against a real
-- past failed run instead of being taken on faith. Runner never passes it.
...
BEGIN
  ...
  FROM extensions.http((
    ...
    'https://api.github.com/repos/papernewtmanagement-dot/newtworks/actions/runs?branch=main&per_page=1'
      || coalesce('&status=' || p_status_filter, ''),
    ARRAY[
...
REVOKE ALL ON FUNCTION public.check_ci_build_status(uuid, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.check_ci_build_status(uuid, uuid, text) TO service_role;
