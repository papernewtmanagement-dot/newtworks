-- Migration: split employer payroll taxes off wages (6010) onto 6030 Payroll Tax Expense - ER Share,
-- tagged by budget_category (growth vs team) per-person pro-rata.
--
-- Rationale: Wages and employer payroll taxes are conceptually different (wages = compensation to the
-- employee; ER taxes = government charge on the employment relationship). Standard chart-of-accounts
-- practice separates them; the residual-pool op-rule already treats burden as pro-rata across wage
-- buckets. Splitting to 6030 aligns the P&L with those conventions and makes "Payroll + Taxes / Gross"
-- vs "Team Payroll Only / Gross" ratio reads clean.
--
-- Part 1: rewrite payroll_gl_writer to compute per-person ER-tax split and post to 6030 with tags.
-- Part 2: retro-fix the 2 live post-cutover agency JEs (pay dates 2026-07-24 + 2026-07-31) by moving
--         each person's ER portion from the aggregate 6010 team line onto 2 new 6030 lines,
--         tagged growth/team per that JE's per-person breakdown.

-- =============================================================================
-- Part 1: payroll_gl_writer(uuid, boolean, date) with per-person ER split
-- =============================================================================
...
DECLARE
  ...
  v_weeks_in int;
  v_ramp numeric;
  v_wages_growth numeric;   -- per-person: ramp-portion of fixed pay
  v_wages_team numeric;     -- per-person: everything else on wage side (non-ER)
  v_wages_person_total numeric;
  v_er_growth numeric;      -- per-person: ER-tax portion allocated to growth
  v_er_team numeric;        -- per-person: ER-tax portion allocated to team
  ...
  LOOP
    ...
    LOOP
      ...
      ELSE
        ...
        -- Per-person ER-tax split: pro-rata across the same growth/team wage buckets.
        -- Fall back to 100% team when there is no wage denominator (defensive; shouldn't happen).
    ...
    IF v_ic_credit > 0 THEN
      ...
      -- 6010 Staff Wages: growth bucket
      IF v_wages_growth_total > 0 THEN
        INSERT INTO journal_lines (journal_entry_id, agency_id, account_id, debit, credit, description)
        VALUES (v_agency_je, p_agency_id, v_wages_acct, v_wages_growth_total, 0,
                'Growth Budget share of team fixed pay')
      ...
      -- 6010 Staff Wages: team bucket (post-ramp fixed + variable + gap; ER taxes now split off)
      IF v_wages_team_total > 0 THEN
        INSERT INTO journal_lines (journal_entry_id, agency_id, account_id, debit, credit, description)
        VALUES (v_agency_je, p_agency_id, v_wages_acct, v_wages_team_total, 0,
                'Team Budget: post-ramp fixed pay + bonus/commission/other + unclassified legacy items')
        RETURNING id INTO v_line_id;
        INSERT INTO transaction_tags (agency_id, journal_line_id, tag_key, tag_value, created_by)
        VALUES (p_agency_id, v_line_id, 'budget_category', 'team', 'payroll_gl_writer');
      END IF;

      -- 6030 Payroll Tax Expense - ER Share: growth bucket (pro-rata to growth wages)
      IF v_er_growth_total > 0 THEN
        INSERT INTO journal_lines (journal_entry_id, agency_id, account_id, debit, credit, description)
        VALUES (v_agency_je, p_agency_id, v_er_tax_acct, v_er_growth_total, 0,
                'Employer payroll taxes — growth bucket (pro-rata to growth wages)')
        RETURNING id INTO v_line_id;
        INSERT INTO transaction_tags (agency_id, journal_line_id, tag_key, tag_value, created_by)
        VALUES (p_agency_id, v_line_id, 'budget_category', 'growth', 'payroll_gl_writer');
      END IF;

      -- 6030 Payroll Tax Expense - ER Share: team bucket
      IF v_er_team_total > 0 THEN
        INSERT INTO journal_lines (journal_entry_id, agency_id, account_id, debit, credit, description)
        VALUES (v_agency_je, p_agency_id, v_er_tax_acct, v_er_team_total, 0,
                'Employer payroll taxes — team bucket (pro-rata to team wages)')
        RETURNING id INTO v_line_id;
        INSERT INTO transaction_tags (agency_id, journal_line_id, tag_key, tag_value, created_by)
        VALUES (p_agency_id, v_line_id, 'budget_category', 'team', 'payroll_gl_writer');
      END IF;

      -- 0003 Unclassified: reimbursements (Peter walks year-end)
      IF v_reimb_total > 0 THEN
        INSERT INTO journal_lines (journal_entry_id, agency_id, account_id, debit, credit, description)
        VALUES (v_agency_je, p_agency_id, v_reimb_pending_acct, v_reimb_total, 0,
                'Reimbursements (pending categorization — walk into real bucket at year-end)');
      END IF;

      -- 2902 Due to PaperNewt LLC (intercompany credit)
      INSERT INTO journal_lines (journal_entry_id, agency_id, account_id, debit, credit, description)
    ...
    IF v_pn_total > 0 THEN
      ...
      VALUES (v_pn_je, p_agency_id, v_pn_expense_acct, v_pn_total, 0,
              'PaperNewt payroll expense (owner/officer/PN-direct W-2 wages + ER taxes)');
      INSERT INTO journal_lines (journal_entry_id, agency_id, account_id, debit, credit, description)
      VALUES (v_pn_je, p_agency_id, v_pn_cash_acct, 0, v_pn_total,
              'PaperNewt cash paid (owner/officer/PN-direct)');
    END IF;
...
-- =============================================================================
-- Part 2: retro-fix the 2 live post-cutover agency JEs
--   pay dates: 2026-07-24, 2026-07-31
-- For each JE:
--   - compute per-person ER-tax split (er_growth, er_team) using same algorithm as writer
--   - UPDATE the aggregate 6010 team line to remove the ER-tax portion (subtract er_total)
--   - INSERT new 6030 lines for er_growth_total and er_team_total with budget_category tags
-- Total debits unchanged; IC credit line unchanged.
-- =============================================================================
...
DECLARE
  ...
  v_team_line_id uuid;
  v_team_line_debit numeric;
  v_new_team_line_debit numeric;

  v_line_id uuid;
  v_affected_count int := 0;
BEGIN
  SELECT id INTO v_wages_acct FROM chart_of_accounts
    WHERE agency_id=v_agency_id AND account_code='6010'
      AND business_entity_id=v_agency_entity AND is_active=true;
  SELECT id INTO v_er_tax_acct FROM chart_of_accounts
    WHERE agency_id=v_agency_id AND account_code='6030'
      AND business_entity_id=v_agency_entity AND is_active=true;

  IF v_wages_acct IS NULL OR v_er_tax_acct IS NULL THEN
    RAISE EXCEPTION 'retro aborted: could not resolve 6010 (%) or 6030 (%)', v_wages_acct, v_er_tax_acct;
  END IF;

  FOR v_je IN
    SELECT id, reference_number, entry_date,
           -- Parse run_id out of 'PAYROLL-<uuid>-AGENCY'
           SUBSTRING(reference_number FROM 'PAYROLL-([0-9a-f-]+)-AGENCY')::uuid AS run_id
    FROM journal_entries
    WHERE source='payroll_gl_writer'
      AND agency_id=v_agency_id
      AND entry_date IN ('2026-07-24','2026-07-31')
      AND reference_number LIKE 'PAYROLL-%-AGENCY'
    ORDER BY entry_date
  LOOP
    v_run_id := v_je.run_id;

    v_er_growth_total := 0;
    v_er_team_total := 0;

    FOR v_pd IN
      SELECT pd.id AS pd_id, pd.team_member_id, pd.gross_pay,
             pd.employer_taxes, pd.raw_earnings,
             pr.pay_period_end
      FROM payroll_detail pd
      JOIN payroll_runs pr ON pr.id=pd.payroll_run_id
      WHERE pd.payroll_run_id=v_run_id
    LOOP
      SELECT first_name, last_name, business_entity_id, start_date, role_level,
             COALESCE(is_admin_backoffice,false) AS is_admin_backoffice
        INTO v_tm
        FROM team WHERE id=v_pd.team_member_id;

      -- Skip PaperNewt-routed people (Owner / admin_backoffice / PN-entity)
      IF v_tm.role_level='Owner'
         OR v_tm.is_admin_backoffice=true
         OR v_tm.business_entity_id=v_papernewt_entity THEN
        CONTINUE;
      END IF;

      v_items := COALESCE(v_pd.raw_earnings->'items', '{}'::jsonb);
      v_salary := 0; v_hourly := 0; v_ot := 0; v_bonus := 0;
      v_commission := 0; v_other := 0; v_reimb := 0;
      ...
      v_variable := v_bonus + v_commission + v_other;
      v_recognized := v_fixed + v_variable + v_reimb;
      v_gap := GREATEST(0, v_pd.gross_pay - v_recognized);

      IF v_tm.start_date IS NULL THEN
        v_ramp := 0;
        v_weeks_in := NULL;
      ELSE
        v_weeks_in := GREATEST(0, FLOOR((v_pd.pay_period_end - v_tm.start_date) / 7.0)::int);
        v_ramp := 1.0 - LEAST(1.0, GREATEST(0::numeric, v_weeks_in / 52.0));
      END IF;
      ...
      IF v_wages_person_total > 0 AND COALESCE(v_pd.employer_taxes, 0) > 0 THEN
        v_er_growth := ROUND(COALESCE(v_pd.employer_taxes, 0) * v_wages_growth / v_wages_person_total, 2);
        v_er_team := ROUND(COALESCE(v_pd.employer_taxes, 0) - v_er_growth, 2);
      ELSE
        v_er_growth := 0;
        v_er_team := COALESCE(v_pd.employer_taxes, 0);
      END IF;
    ...
    v_er_total_check := v_er_growth_total + v_er_team_total;

    -- Locate the aggregate Team Budget line (the one currently including ER taxes)
    SELECT id, debit INTO v_team_line_id, v_team_line_debit
    FROM journal_lines
    WHERE journal_entry_id=v_je.id
      AND account_id=v_wages_acct
      AND description LIKE 'Team Budget%';

    IF v_team_line_id IS NULL THEN
      RAISE NOTICE 'skipping JE % (%): no Team Budget line found', v_je.reference_number, v_je.entry_date;
      CONTINUE;
    END IF;

    v_new_team_line_debit := ROUND(v_team_line_debit - v_er_total_check, 2);
    IF v_new_team_line_debit < 0 THEN
      RAISE EXCEPTION 'retro would negate Team Budget line on JE % (was %, subtracting %)',
        v_je.reference_number, v_team_line_debit, v_er_total_check;
    END IF;

    UPDATE journal_lines
    SET debit = v_new_team_line_debit,
        description = 'Team Budget: post-ramp fixed pay + bonus/commission/other + unclassified legacy items (ER taxes moved to 6030 on 2026-07-30)'
    WHERE id = v_team_line_id;

    -- Insert new 6030 growth line
    IF v_er_growth_total > 0 THEN
      INSERT INTO journal_lines (journal_entry_id, agency_id, account_id, debit, credit, description)
      VALUES (v_je.id, v_agency_id, v_er_tax_acct, v_er_growth_total, 0,
              'Employer payroll taxes — growth bucket (retro-split from 6010 on 2026-07-30)')
      RETURNING id INTO v_line_id;
      INSERT INTO transaction_tags (agency_id, journal_line_id, tag_key, tag_value, created_by)
      VALUES (v_agency_id, v_line_id, 'budget_category', 'growth', 'payroll_gl_writer_er_split_retro');
    END IF;

    -- Insert new 6030 team line
    IF v_er_team_total > 0 THEN
      INSERT INTO journal_lines (journal_entry_id, agency_id, account_id, debit, credit, description)
      VALUES (v_je.id, v_agency_id, v_er_tax_acct, v_er_team_total, 0,
              'Employer payroll taxes — team bucket (retro-split from 6010 on 2026-07-30)')
      RETURNING id INTO v_line_id;
      INSERT INTO transaction_tags (agency_id, journal_line_id, tag_key, tag_value, created_by)
      VALUES (v_agency_id, v_line_id, 'budget_category', 'team', 'payroll_gl_writer_er_split_retro');
    END IF;

    v_affected_count := v_affected_count + 1;
    RAISE NOTICE 'retro-fixed JE % (%): team line %→%, er_growth=%, er_team=%',
      v_je.reference_number, v_je.entry_date,
      v_team_line_debit, v_new_team_line_debit,
      v_er_growth_total, v_er_team_total;
  END LOOP;

  IF v_affected_count <> 2 THEN
    RAISE EXCEPTION 'expected to affect 2 JEs, affected % — aborting to force review', v_affected_count;
  END IF;
END $retro$;
