...
-- 1996 (Journal of Applied Psychology 81(6) 679-703 issue, pp. 660-679) found
-- social desirability acts as neither a predictor, a suppressor, nor a mediator
-- of the personality-to-performance relationship, and that correcting scores for
-- it does not improve validity -- it mostly removes real variance. So this is
...
-- these scales than research volunteers do, so a raw band drawn from research
-- norms would over-flag. Revisit after 30 completed assessments.
-- ============================================================================
...
CREATE OR REPLACE FUNCTION public.hiregauge_v2_impression_management(
  p_candidate_id uuid,
...
CREATE OR REPLACE FUNCTION public.apply_newtworks_v2_impression_management_to_candidate(
  p_candidate_id uuid,
  p_sitting integer DEFAULT 1
)
RETURNS jsonb
