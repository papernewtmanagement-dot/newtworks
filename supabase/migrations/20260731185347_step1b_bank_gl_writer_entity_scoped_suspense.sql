-- Step 1b of pipeline repair (2026-07-31): rewrite bank_gl_writer to use
-- entity-scoped suspense via get_entity_unclassified_account, and prefer
-- same-entity account lookups on rule targets. Removes reference to legacy
-- COA-SUSP account_code that no longer exists.
...
BEGIN
  ...
  LOOP
    ...
    -- Tier 2: split label match (entity-scoped)
