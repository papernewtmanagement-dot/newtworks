...
--
-- Peter directive 2026-07-13 pm: Stephanie promoted today; last week's
...
CREATE TABLE IF NOT EXISTS public.team_weekly_snapshot (
  ...
  last_name                 text,
  nickname                  text,
  -- Classification (drives WtW math + filter branches)
  role                      text,
  role_level                text,
  role_category             text,
  category                  text,
...
-- RLS: agency-scoped read (mirrors pattern from 22-table RLS hardening)
...
-- Trigger function: on weekly_cpr_reports INSERT, snapshot the roster.
...
BEGIN
  ...
  WHERE t.agency_id = NEW.agency_id
    -- Snapshot everyone who was active or was archived AFTER this week began
    -- (so termed-mid-week rows still appear correctly in historical CPRs)
