...
-- Coverage bar = fully-loaded × 1.0 (no tenure multiplier)
-- Profitability bar = fully-loaded × 2.5
-- Own-stack credit no longer gated on Wk52+; compute_renewal_stack's own 12mo cohort filter is sufficient.
-- tenure_multiplier still returned as a data field for display (shows how long they've been there)
-- but is no longer applied to the bar or the stack credit.
...
BEGIN
  ...
  base_calc AS (
    SELECT r.id, r.first_name || ' ' || r.last_name AS full_name,
      ...
      -- Tenure multiplier retained as a DATA field for display, but NOT applied to bar.
  ...
  compose AS (
    SELECT
      ...
      -- Fully-loaded is annual salary × payroll burden. NO tenure ramp.
  ...
  computed AS (
    SELECT
      ...
      -- No Wk52+ gate. compute_renewal_stack's own 12mo cohort filter is sufficient.
  ...
  SELECT
    ...
    jsonb_build_object(
      ...
      'formulas', jsonb_build_object(
        'attribution', 'own_new × 4 + own_stack × 0.65 + retention_pool_share',
        'coverage_bar', 'annual_base × 1.08 (fully-loaded, no ramp)',
...
'Attribution: own_new × 4 + own_stack × 0.65 + retention_pool_share × RQM (Retention only). '
'Coverage bar = fully-loaded (base × 1.08, no ramp); Profitability bar = fully-loaded × 2.5. '
