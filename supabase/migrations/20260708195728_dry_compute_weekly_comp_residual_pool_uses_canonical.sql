-- Tier-3 DRY (comp engine): compute_weekly_comp_residual_pool now uses canonical roster.
--
-- Single roster CTE (was hand-rolling: category='agency' + non-Owner + is_active +
-- non-admin, no test_user filter). Replaced with:
--   get_expected_teammates(p_agency_id, 'time_off_participant', p_week_end_date)
-- JOIN team for columns not in canonical return shape (pay_type, pay_rate,
-- work_location, license_pc/lh/ips, weekly_health_benefit_agency_paid).
--
-- Behavioral note: canonical adds is_test_user IS NOT TRUE (baseline) and switches
-- archived-check from `archived_at IS NULL` to `archived_at IS NULL OR archived_at
-- > p_week_end_date::timestamptz`. Zero drift verified against current agency
-- (compute_pool_carveouts sibling refactor produced byte-exact parity).
--
-- Rest of body UNCHANGED (base_calc, commission projection, SP projection,
-- weighted hours calc, bonus pool allocation, final SELECT). This is a
-- surgical roster-CTE swap ONLY.
...
RETURNS TABLE(
  ...
  weighted_hours_at_40 numeric, retention_hours_share_pct numeric, person_share_pct numeric,
  annual_bonus_gross numeric, annual_health_subtracted numeric, annual_bonus_net numeric,
  weekly_bonus_net numeric, annual_total_comp numeric, weekly_total_comp numeric,
  diagnostics jsonb
)
LANGUAGE plpgsql
AS $fn$
