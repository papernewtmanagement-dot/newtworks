-- HireGauge Step 8: role-dependent Drivers assessment cell
-- Extracts v_da (Drivers · Assessment) computation from hiregauge_three_construct_verdict
-- into a dedicated fn dispatched on best_fit_role.
--
-- Formulas (all return 0-10 numeric):
--   Sales - Outbound     : (DM + RD + IS) / 30            [kept from prior]
--   Sales - Inbound      : (DM + RD) / 20                 [IS dropped — seat supplies structure]
--   Sales - In-Book      : (DM*0.5 + RD + CO_adj) / 25    [de-weight deadline, reward relational engine]
--   Retention (all 3)    : (DM + RD + IS) / 30            [inherits seat-agnostic per spec]
--   Aspirant / NULL role : (DM + RD + IS) / 30            [inherits seat-agnostic per spec]
--
-- CO_adj uses _cts_dampen_trait_by_distortion(compassion, 'compassion', response_distortion) —
-- same dampening as adjusted competencies (socially-desirable ceiling on compassion).
--
-- Non-additive: verdict fn caller updated. New fn is pure add. Existing v_da value semantics preserved for
-- non-inbound/in-book roles (identical formula).
-- Consumer audit: only hiregauge_three_construct_verdict inlined v_da. Updated in same migration.

------------------------------------------------------------
-- 1. New fn: role-dependent Drivers · Assessment cell
------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.cts_drivers_assessment_cell(p_assessment_id uuid, p_best_fit_role text)
RETURNS numeric
LANGUAGE sql
STABLE
AS $function$
  SELECT CASE p_best_fit_role
    WHEN 'sales_inbound' THEN
      (COALESCE(ta.deadline_motivation, 0) + COALESCE(ta.recognition_drive, 0))::numeric / 20.0
    WHEN 'sales_in_book' THEN
      (COALESCE(ta.deadline_motivation, 0) * 0.5
       + COALESCE(ta.recognition_drive, 0)
       + COALESCE(public._cts_dampen_trait_by_distortion(ta.compassion, 'compassion', ta.response_distortion), 0)
      )::numeric / 25.0
    ELSE  -- sales_outbound, retention_*, aspirant, NULL → seat-agnostic
...
------------------------------------------------------------
-- 2. Update hiregauge_three_construct_verdict to call new fn
------------------------------------------------------------
...
BEGIN
  ...
  IF v_ta.deadline_motivation IS NOT NULL THEN
    ...
    -- Step 8: role-dependent Drivers assessment cell
