-- Sub-items on onboarding steps. A step like "HR paperwork" or "Workday
-- courses" is really several things; they belong on the checklist itself
-- instead of pointing at another page. substeps is a JSON array of strings on
-- the template; a plan snapshots that list and tracks which ones are ticked.
...
-- ── Agent-side setup that only existed at the bottom of the Tech Setup page ──
...
-- ── Compile function carries substeps into the plan ───────────────────
...
-- Backfill substeps onto steps in plans that already exist, matched by key.
