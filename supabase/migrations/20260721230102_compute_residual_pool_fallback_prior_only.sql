...
--
-- SECOND LEAK: even with the pay_date filter on payroll_detail, the comm_by_week and
-- mgr_bonus_by_week CTEs fall back to wctd.commission / wctd.manager_bonus when the
-- payroll subquery returns NULL. wctd.commission for the current week gets written by
-- v2's own stomp using the current-week payroll — so the fallback picks up EXACTLY the
-- value the pay_date filter was trying to exclude. Circular self-reference.
--
-- Fix: gate the wctd fallback on cw.week_end_date < p_week_end_date. For the current
-- week, if payroll isn't yet paid (pay_date > week_end), the commission and manager
-- bonus subtractions default to 0 — no money-out has happened.

-- This is a narrow patch to just the two CTEs. Rest of the function is unchanged from
-- the 21:30 migration.
