-- Migration: team_checkin_send_reminder — call-log walkback (up to 4 days)
-- Companion to render_team_status_block roster+pace fixes (prior migration).
-- Rationale: Mon morning fallback only tried today-1 (Sun) and today-2 (Sat),
-- both empty. Never reached Fri call log. Walk back up to 4 days from today-1
-- until a populated block is found; stops on first hit.
...
BEGIN
  ...
  IF v_checkin_type = 'morning' THEN
    ...
    -- Walk back up to 4 days looking for a populated call log. On Monday this
    -- reaches Friday; on other days it still picks up yesterday first when
    -- populated. Stops on the first non-empty block.
    v_calls_block := NULL;
