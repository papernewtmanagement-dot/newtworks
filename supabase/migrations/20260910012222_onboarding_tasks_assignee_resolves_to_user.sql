-- tasks.assigned_to points at users(id), but onboarding steps carry a team(id).
-- Translate on the way in, and never let a missing login block plan creation —
-- the task is still created, just unassigned.

-- Alvi's login existed but was not linked to her team row, which is why the
-- lookup came up empty.
UPDATE public.users
SET team_member_id = 'd7431075-d29f-4833-9503-430945894b04'
WHERE id = 'f2b584dd-97ac-4575-a600-945893d6a491'
...
BEGIN
  ...
  LOOP
    -- team(id) on the step, users(id) on the task
    SELECT u.id INTO v_owner_user_id
    FROM public.users u
    WHERE u.team_member_id = r.assigned_to
    LIMIT 1;
