-- 2026-08-07: cc_gl_writer had NO skip path for a deliberate "do not post" marker.
-- A credit-card line marked category='REWARDS_CYCLE_INTERNAL_NO_JE' (meaning: the real
-- cash event is booked from the bank side, do not book it again from the card side)
-- fell through Tier 1 (no chart account by that name), fell through Tier 2 (no rule),
-- and landed in the suspense fallback -- which POSTS an entry against an expense account.
-- Result: a phantom $493.01 expense on the personal profit and loss, plus a double
-- credit to the AMEX Personal card, for one real refund that was already booked
-- correctly from the US Bank side on 2026-06-30.
-- Fix: honor the marker explicitly, before any classification runs. Any category
-- ending in _NO_JE means "no journal entry, ever" -- mark the row handled and move on.
...
BEGIN
  ...
  LOOP
    ...
    -- 2026-08-07 guard: explicit "do not post" marker. A category ending in _NO_JE
    -- means the real economic event is booked from another source (usually the bank
    -- side) and this card-side line must NEVER generate a journal entry. Runs BEFORE
    -- every classification tier so the marker can never fall through to suspense.
    -- Without this, an unrecognized marker was treated as "category unresolved" and
    -- posted a phantom expense (the 2026-06-19 $493.01 AMEX credit balance refund).
    IF v_category IS NOT NULL AND upper(trim(v_category)) LIKE '%\_NO\_JE' THEN
      v_count_skipped_no_je_marker := v_count_skipped_no_je_marker + 1;
      IF NOT p_dry_run AND v_posted_at IS NULL THEN
        UPDATE credit_transactions
        SET posted_at = NOW(),
            notes = COALESCE(notes, '') || ' [no JE by marker ' || v_category || ']'
        WHERE id = v_txn_id;
      END IF;
      CONTINUE;
    END IF;
    ...
    -- 2026-08-05 guard: never derive debit/credit orientation from the sign of the
    -- amount. Sign conventions differ between statement parses; guessing produced
    -- reversed charges and mass duplicates (see gl_sign_audit_20260805). Untyped
    -- rows wait until transaction_type is set ('charge' | 'payment' | 'credit').
    IF v_txn_type IS NULL OR length(trim(v_txn_type)) = 0 THEN
      v_count_skipped_untyped := v_count_skipped_untyped + 1;
      v_errors := v_errors || jsonb_build_object('txn_id', v_txn_id, 'reason', 'untyped_transaction_needs_type');
      CONTINUE;
    END IF;
    ...
    -- Tier 1: Direct category match (entity-scoped)
    ...
    BEGIN
      -- Orientation comes ONLY from transaction_type (untyped rows never reach here).
      v_is_charge := (v_txn_type = 'charge');
  ...
  RETURN jsonb_build_object(
    ...
    'skipped_no_je_marker', v_count_skipped_no_je_marker,
    ...
    'skipped_untyped', v_count_skipped_untyped,
