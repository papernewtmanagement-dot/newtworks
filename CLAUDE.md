...
| 008_bridge_generator.sql | `newtworks_generate_bridges()` function | **YES — Path A only** | Skip |
...
WITH expected_tables(name) AS (VALUES
  ...
  ('persistent_memory'),('positions'),('settings'),('agency_snapshot'),('sf_program_targets'),
...
```sql
CREATE OR REPLACE VIEW newtworks_chart_of_accounts AS
SELECT
    id::text AS id,
    agency_id,
