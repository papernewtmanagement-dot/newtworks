...
//   Processing:         Groq (free LLM, no API key needed)
...
const MOCK_RECIPES = [
  ...
  },
  {
    ...
    recipe_description:"Monitors Gmail for financial documents — COMP_RECAP, payroll exports, bank statements. Saves to Google Drive, processes via Groq, routes data to correct Supabase tables.",
    trigger_type:"cron", cron_expression:"0 * * * *", cron_label:"Every hour",
    composio_action:"GMAIL_LIST_THREADS", composio_connection:"gmail",
  ...
  },
  {
    ...
    recipe_description:"Scans Gmail daily for incoming resumes. Auto-creates applicant records, scores candidates via Groq, generates One Page Interview Focus, fires new applicant alert.",
  ...
  return (
    <div>
      <div style={{ fontSize:13, color:T.slate500, marginBottom:14, lineHeight:1.6 }}>
        Recipe definitions live in your Supabase database. Each recipe defines what triggers it, which Composio connection executes it, whether Groq processes the output, and which table receives the results. Enable or disable any recipe at any time.
      </div>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:12, padding:"14px 16px" }}>
                <div style={{ flex:1, cursor:"pointer" }} onClick={() => setExpanded(isExpanded?null:recipe.id)}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4, flexWrap:"wrap" }}>
                    ...
                    {recipe.uses_groq && (
                      <span style={{ fontSize:10, fontWeight:600, padding:"2px 7px", borderRadius:20, background:T.tealLt, color:T.teal }}>Groq</span>
                    )}
                  </div>
    ...
    <Card>
      ...
      <div style={{ display:"flex", gap:0, marginBottom:16, overflow:"hidden", borderRadius:10, border:`1px solid ${T.slate200}` }}>
        {[
          ...
          { step:"3", label:"Groq classifies", sub:"free LLM processing" },
...
export default function Automations() {
  ...
  const toggleRecipe = async (id) => {
    ...
    // Optimistic UI
    setRecipes(prev => prev.map(r => r.id === id ? { ...r, is_active: newState } : r));
    ...
    if (error) {
      console.error("[Automations] toggleRecipe failed:", error);
      // Roll back optimistic update
      setRecipes(prev => prev.map(r => r.id === id ? { ...r, is_active: current?.is_active } : r));
    }
  };

  // ── Derived: Connections from settings table ─────────────────
  // The settings table has key/value pairs like composio_gmail_account_id,
  // composio_googledrive_account_id, etc. For each known integration we
  // check whether the corresponding key has a non-empty value, which means
  // OAuth was completed. Missing/empty = needs connection.
  ...
  const imports = useMemo(() => {
    return (liveDocs || []).map(d => {
      ...
      return {
        ...
        groq_type: d.groq_classification || d.file_type,
