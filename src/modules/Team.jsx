...
import { supabase, AGENCY_ID, BUSINESS_ENTITY_ID, PAYROLL_ENTITY_ID } from "../lib/supabase.js";
    ...
    async function load() {
      try {
        ...
        const [agencyRes, staffRes, prodRes, payrollDetailRes, payrollRunsRes, compRes, aippRes, aippTrackRes, lapseRes] = await Promise.all([
          ...
          supabase.from("payroll_detail").select("team_member_id, gross_pay, payroll_run_id").eq("business_entity_id", PAYROLL_ENTITY_ID),
          supabase.from("payroll_runs").select("id, pay_date, pay_period_start, pay_period_end").eq("business_entity_id", PAYROLL_ENTITY_ID).order("pay_date",{ascending:false}).limit(24),
