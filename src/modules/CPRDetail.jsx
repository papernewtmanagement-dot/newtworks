...
function useCPRData(weekDate) {
  ...
  const [state, setState] = useState({
    ...
    reportPrefills: {},  // report-level fields that open prefilled from the most recent prior week
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 760 }}>
            ...
            <tbody>
              {rows.map(r => {
                ...
                const shades =
                  r.rowKind === "premium" ? { row: "#FBFDF6", mid: "#F1F7DF", dark: "#E4EEC7" } :
                  r.rowKind === "rate"    ? { row: "#F8FAFD", mid: "#EBF1F9", dark: "#D8E4F2" } :
    ...
    <div>
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 680 }}>
            ...
            <tbody>
              {sorted.map(d => {
                ...
                // Two separate totals (Peter 2026-09-08): hours actually worked, and
                // PTO. Never summed into one figure — worked hours are what the
                // retention split and any overtime question read, PTO is neither.
                // PTO is PAID time off only (Peter 2026-09-09). Unpaid absence is
                // deliberately not shown anywhere: it is simply hours that never
                // appear. Do not add an unpaid figure or an unpaid marker.
                let totalWorked = 0;
                let totalPto = 0;
                DAYS.forEach(day => {
                  totalWorked += Number(tmHours[day]?.hours) || 0;
                  totalPto    += Number(tmHours[day]?.paid_off) || 0;
                });
                return (
                  <tr key={d.team_member_id}>
                    <Td style={{ paddingLeft: 14, color: T.slate700, fontWeight: 600 }}>{firstName(d.__name)}</Td>
                    {DAYS.map(day => {
                      ...
                      // 🟡 marks PTO. PTO is NOT hours worked and never counts toward
                      // the 40-hour overtime line (29 CFR 778.218). Unpaid absence is
                      // deliberately unmarked — it is just hours that do not appear.
                      // Nothing worked and no PTO renders blank, no icon.
                    ...
                    <Td align="right" style={{ fontWeight: 700 }}>{totalWorked > 0 ? totalWorked.toFixed(2) : "—"}</Td>
                    <Td align="right" style={{ fontWeight: 700, color: T.slate500 }}>{totalPto > 0 ? totalPto.toFixed(2) : "—"}</Td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <div style={{ padding: "8px 14px", borderTop: `1px solid ${T.slate200}`, fontSize: 12, color: T.slate500 }}>
          🟢 in office · 🟣 remote · 🟡 PTO. PTO is totalled separately and is not hours worked.
        </div>
      </Card>
    </div>
  );
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
        ...
        {canEdit && (
          <div style={{
            ...
            {editMode && saveError && (
              <span style={{ color: T.red, fontSize: 12, marginRight: "auto" }}>
                {saveError}
              </span>
            )}
            {editMode ? (
  ...
  return (
    <div style={{ padding: "10px 14px 16px", fontSize: 12 }}>
      ...
      <div style={{ color: T.slate500, fontSize: 11, marginBottom: 6, lineHeight: 1.5 }}>
        ...
        Base salaries (payroll actuals), commissions (payroll actuals), Manager Bonus (payroll actuals), prior-week team bonuses paid (payroll actuals), HDB, MVP Prize Cart, WtQ Trip, goals-bonus carveouts (WtW/Gain/Leaderboard/All-Star/Trailblazer), and this week's residual bonus pool ALL subtract INSIDE the wrap (they're wages → burdened). Subtractors labeled "payroll actuals" read from payroll_detail (real paycheck line items) with the CPR-computed value as fallback when payroll for that week hasn't transmitted yet.
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <tbody>
          ...
          {row("− Team base salaries (in pool)", -qtdBaseInPool, "source: payroll_detail SALARY+HOURLY+REGULAR lines (real paycheck actuals). Fallback: (hours worked + paid time off) × pay_rate for hourly, from the same shared hours function the Hours section reads, else config pay_rate × 40 hrs × the Mon–Fri workdays employed that week. × tenure_mult.")}
          {Number(sub.qtd_departure_recapture || 0) > 0 &&
            row("− Departure recapture QTD", -Number(sub.qtd_departure_recapture || 0), "a teammate left: the base that stopped being charged to the pool is held back instead of going to the rest of the team — 100% of the freed base in the week they leave, easing straight-line to 0 over 52 weeks (the new-hire growth budget in reverse). Only the part that was in the pool (× tenure_mult at departure). The held-back money is added back to the growth budget — see Financials → Growth Budget.")}
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <tbody>
          ...
          {(() => {
            ...
            if (factor === null || factor === undefined || !fd) {
              return (
                <tr>
            ...
            const line = (label, value, hint) => (
              <tr key={label}>
                ...
                <Td align="right" style={{ color: T.slate700, fontSize: 11 }}>{value}</Td>
                <Td style={{ color: T.slate500, fontSize: 11, paddingLeft: 10 }}>{hint}</Td>
              </tr>
            );
            return (
...
export default function CPRDetail({ weekDate, onClose = () => {}, onNavigateWeek = null, userRole = null, viewerTeamMemberId = null }) {
  ...
  function doStartEdit() {
    ...
    edit.begin(data.report, data.details, data.snapshot || {}, data.reportPrefills || {});
  }
