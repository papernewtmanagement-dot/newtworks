...
// 5 — Code Reds / Yellows
function CodeRedsYellowsSection({ details }) {
...
function TeamChecklistSection({ details, team }) {
  ...
  return (
    <div>
      <SectionHeader icon="✅" title="Team Checklist" hint="Daily ops + opportunity lists, 11 items per person" />
...
function PersonalChecklistSection({ details, team }) {
  if (!details || details.length === 0) {
    return (
      <div>
...
function RequirementsSection({ details, team }) {
  if (!details || details.length === 0) {
    return (
      <div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 600 }}>
            ...
            <tbody>
              {sorted.map(d => (
                <tr key={d.team_member_id}>
                  <Td style={{ paddingLeft: 14, color: T.slate700, fontWeight: 600 }}>{firstName(d.__name)}</Td>
                  <Td align="right">{fmtInt(d.carryover)}</Td>
                  <Td align="right">{fmtInt(d.missed)}</Td>
                  <Td align="right">{fmtInt(d.cost)}</Td>
                  <Td align="right">{fmtInt(d.total)}</Td>
                  <Td align="right">{fmtInt(d.paid)}</Td>
                  <Td align="right" style={{ fontWeight: 700, color: T.slate900 }}>{fmtInt(d.owed)}</Td>
                </tr>
              ))}
            </tbody>
          </table>
...
function HoursWorkedSection({ details, team }) {
  if (!details || details.length === 0) {
    return (
      <div>
            ...
            <tbody>
              {sorted.map(d => {
                const total = ["mon", "tue", "wed", "thu", "fri"]
                  .reduce((sum, day) => sum + (Number(d[`${day}_hours`]) || 0), 0);
                return (
                  <tr key={d.team_member_id}>
                    <Td style={{ paddingLeft: 14, color: T.slate700, fontWeight: 600 }}>{firstName(d.__name)}</Td>
...
// MAIN COMPONENT
// ─────────────────────────────────────────────────────────────
export default function CPRDetail({ weekDate, onClose = () => {} }) {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
      ...
      {/* 1. Opener */}
      <Section><OpenerSection weekDate={weekDate} report={data.report} /></Section>
      ...
      {/* 5. Code reds/yellows */}
      <Section><CodeRedsYellowsSection details={data.details} /></Section>

      {/* 6. Team checklist */}
      <Section><TeamChecklistSection details={data.details} team={data.team} /></Section>

      {/* 7. Personal checklist */}
      <Section><PersonalChecklistSection details={data.details} team={data.team} /></Section>

      {/* 8. Requirements */}
      <Section><RequirementsSection details={data.details} team={data.team} /></Section>
      ...
      {/* 16. Hours worked */}
      <Section><HoursWorkedSection details={data.details} team={data.team} /></Section>
