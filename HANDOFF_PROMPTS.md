...
**3.5. Ask [CLIENT-FIRST-NAME] for their AA05 SMVC commission rate** (most agents know this — it's their P&C commission per the State Farm AA05 agreement). Also confirm their blended commission rate for non-P&C lines (Life/Health/FS — typically 8-10%). Update the agency record:
```sql
UPDATE agency
...
**4. Install the bridge generator.** Run `supabase/migrations/008_bridge_generator.sql` in Studio. This installs the `newtworks_generate_bridges()` function — it doesn't create any views yet.
...
```
Then ask [CLIENT-FIRST-NAME] for their AA05 SMVC commission rate and blended rate for non-P&C lines, and update the agency record:
```sql
UPDATE agency
...
Then call:
```sql
SELECT * FROM newtworks_generate_bridges('<your-json-here>'::jsonb);
```
